#!/usr/bin/env python3
"""Protagine digest — bundles completed/failed job summaries and messages owner.
...
logger = logging.getLogger("protagine_digest")
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(name)s %(message)s")

PROTAGINE_URL = os.environ.get("PROTAGINE_URL", "http://127.0.0.1:7777")
API_TOKEN = os.environ.get("PROTAGINE_AGENT_API_TOKEN", "")
...
def main() -> int:
    try:
        resp = httpx.get(
            f"{PROTAGINE_URL}/v1/host/queue/digest",
            headers=_HEADERS,
            params={"hours": 6},
            timeout=10,
        )
