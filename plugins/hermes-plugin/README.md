# Governed Apsimo plugin for Hermes

This is Apsimo's generic sidecar integration through Hermes interfaces. It gives Hermes
transport-scoped Apsimo reads, mediator-only action intents, one exact turn
...
deployment-specific identity. Apsimo memory is supplied by the separate
`apsimo-memory` provider; execution belongs to an authenticated action plane.

An optional [`apsimo_task` tool](../../docs/NATIVE-TASK-CHANNELS.md) starts
...
plugins:
  apsimo:
    ...
    action_mediator_api_key: ${APSIMO_ACTION_MEDIATOR_API_KEY}
    ...
    enabled_action_tools:
      - apsimo_autonomy_disable
      - apsimo_autonomy_enable
      - apsimo_create_commitment
      - apsimo_resolve_commitment
    ...
    owner_message_mediator_api_key: ${APSIMO_OWNER_MESSAGE_MEDIATOR_API_KEY}
    owner_message_mediator_principal: hermes-owner-message
    enabled_message_tools: [apsimo_send_message]
...
must carry a sender and resolve to an exact Apsimo contact.

Set the coexistence latches documented in [SPEC.md](SPEC.md), then enable both
the `apsimo` general plugin and canonical `apsimo-memory` provider. If the
...
`apsimo_send_message` is a separate contact-message intent boundary. It is
available to an authenticated owner text turn and to an explicitly attested
local system turn, which lets Apsimo autonomy initiate and follow up without
...
The deployment's authenticated producer resolves exactly one non-owner Apsimo
...
return. The bundled `ApsimoClient` is stronger than that generic callback
