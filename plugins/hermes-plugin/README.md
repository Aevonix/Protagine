...
| `tools.py` | `protagine_self` (`state|log|why|rate|yes|no`: the only source for claims about the agent's own actions, the record shown only in the owner's own session (anyone else gets the switch state and a refusal for `log` and `why`); `state` carries `working_on` and the narrative, `log` filters by `since_hours`, `kind`, `recipient`, `why` on an unknown id answers "no intention `<id>` exists in the audit log"), `protagine_people` (`who|inspect|propose_link`; owner: `set_permission|set_cadence|merge`; with `mind.faculties.people: false` only `who|inspect|set_permission`), `protagine_memory_search`, `protagine_memory_forget` |
