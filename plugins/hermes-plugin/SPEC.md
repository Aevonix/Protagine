# Protagine Hermes general-plugin governance specification

## Scope

This plugin is a generic Protagine sidecar adapter for Hermes 0.18.2-compatible
...
3. Private legacy read tools require an owner/system authority lane. Guest
   context comes from the canonical Protagine memory provider.
...
handlers. `protagine_autonomy_status` is a bounded owner/system-only GET projection
...
`protagine_send_message` is registered only when its exact message mediator and
...
`GOVERNED_EVENT_TYPES` is empty until Protagine can provide an exact
...
versions, and Protagine alone cannot prove correction of already-streamed tokens.
...
their paths are made inert. Reverting the Protagine repository and restoring those
