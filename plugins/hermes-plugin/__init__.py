"""Governed Protagine sidecar integration for Hermes.
...
Protagine's memory provider supplies automatic recall.
...
it does not call Protagine, start a subscriber, or modify Hermes configuration.
"""
...
from .protagine_hostworker.catalog import (
...
# Governed action schemas come directly from protagine_hostworker.catalog, the
...
_LOCAL_TOOL_SCHEMAS: list[dict[str, Any]] = [
    ...
    {
        "name": "protagine_memory_annotate",
    ...
    {
        "name": "protagine_work_initiative",
    ...
    {
        "name": "protagine_accept_local_draft",
    ...
    {
        "name": "protagine_read_work_source",
    ...
    {
        "name": "protagine_commitment_work",
    ...
    {
        "name": "protagine_autonomy_status",
        "description": "Read bounded Protagine autonomy status in an attested private scope.",
        "parameters": _parameters({}),
    },
    {
        "name": "protagine_get_initiative",
        "description": "Read one Protagine initiative in an attested private viewer scope.",
        "parameters": _parameters({
            "initiative_id": _identifier_model_schema(),
        }, ("initiative_id",)),
    },
    {
        "name": "protagine_list_commitments",
    ...
    {
        "name": "protagine_list_initiatives",
...
_READ_TOOL_NAMES: tuple[str, ...] = (
    "protagine_autonomy_status",
    "protagine_get_initiative",
    "protagine_list_commitments",
    "protagine_list_goals",
    "protagine_list_initiatives",
    "protagine_memory_search",
    "protagine_query_entities",
    "protagine_queue_stats",
)
...
def governance_attestation() -> dict[str, Any]:
    ...
    return {
        "schema": "ProtagineHermesGeneralGovernanceAttestationV2",
        "version": 2,
        "source_ready": True,
        "runtime_ready": False,
        "live_ready": False,
        "runtime_attestation_schema": (
            "ProtagineHermesGeneralRuntimeAttestationV1"
        ),
        "posture": {
...
_REVIEW_PARENT_SCOPE: ContextVar[_TransportScope | None] = ContextVar(
    "protagine_review_parent_scope", default=None,
)
...
_TOOL_EXECUTION_CONTEXT: ContextVar[dict[str, str] | None] = ContextVar(
    "protagine_tool_execution_context", default=None,
)


def _current_participant(client: ProtagineClient, scope: _TransportScope) -> str | None:
...
class ActionMediator:
    """Separate effect boundary; it never falls back to the Protagine client."""
    ...
    def submit(self, intent: HermesToolActionIntentV1) -> dict[str, Any]:
        ...
        if self.principal:
            headers["X-Protagine-Principal"] = self.principal
...
def _validated_owner_message_admission(
    ...
    if (
        ...
        or state != "held"
        and not re.fullmatch(r"protagine-intent:[a-f0-9]{64}", intent_id)
    ):
        raise RuntimeError("owner message mediator intent binding is invalid")
    return {
        "schema": "ProtagineOwnerMessageAdmissionV1",
        "version": 1,
        "status": state,
        "effect_performed": False,
...
def _bounded_autonomy_status(value: Any) -> dict[str, Any]:
    ...
    for key in (
        ...
        counters[key] = item
    return {
        "schema": "ProtagineAutonomyStatusProjectionV1",
        "version": 1,
...
def _bounded_queue_stats(value: Any) -> dict[str, Any]:
    ...
    return {
        "schema": "ProtagineQueueStatsProjectionV1",
        "version": 1,
...
class _ToolDispatcher:
    ...
    def dispatch(self, name: str, args: Mapping[str, Any], **handler_kwargs: Any) -> str:
        try:
            ...
            logger.warning("governed Protagine tool %s failed closed: %s", name, error)
    ...
    def _dispatch(self, name: str, args: Mapping[str, Any], **handler_kwargs: Any) -> str:
        if name not in (
            ...
            return _canonical_json({"reason": "unknown Protagine tool", "status": "denied"})
        ...
        if name in _READ_TOOL_NAMES:
            ...
            if name == 'protagine_memory_search':
    ...
    def _read(self, name: str, args: dict[str, Any], scope: _TransportScope) -> str:
        try:
            ...
            elif name == "protagine_queue_stats":
...
def _guard_chat_mode() -> str:
    raw = os.environ.get("PROTAGINE_GUARD_CHAT_MODE", "").strip().lower()
    if raw in _GUARD_CHAT_MODES:
        return raw
    legacy = os.environ.get("PROTAGINE_GUARD_CHAT_SHADOW", "0").strip().lower()
...
def _prepare_runtime_boundary(config: Mapping[str, Any]) -> _RuntimeBoundary:
    ...
    if not isinstance(config, Mapping):
        raise RuntimeError("Protagine plugin runtime configuration must be an object")
    ...
    attestation = {
        "schema": "ProtagineHermesGeneralRuntimeAttestationV1",
        "version": 1,
        "source_schema": "ProtagineHermesGeneralGovernanceAttestationV2",
        "source_ready": True,
...
def _require_coexistence_latches() -> None:
    ...
    copied_provider = Path(__file__).resolve().parent.parent / "protagine-memory" / "provider.py"
    if not copied_provider.is_file():
        copied_provider = Path(__file__).resolve().parent.parent / "protagine-memory" / "provider.py"
    if copied_provider.is_file():
        import importlib.util
        spec = importlib.util.spec_from_file_location("_protagine_profile_provider", copied_provider)
        ...
        from protagine_memory.provider import _active_hermes_home, general_plugin_memory_ownership

    ownership = general_plugin_memory_ownership(_active_hermes_home())
    if ownership is True:
        return
    if ownership is False:
        raise RuntimeError("General Protagine plugin is explicitly deselected in this Hermes profile")
    ...
    values = {
        "PROTAGINE_GENERAL_PLUGIN_ACTIVE": os.environ.get("PROTAGINE_GENERAL_PLUGIN_ACTIVE", ""),
        "PROTAGINE_MEMORY_WORKER_TOOLS": os.environ.get("PROTAGINE_MEMORY_WORKER_TOOLS", ""),
        "PROTAGINE_MEMORY_TURN_WRITER": os.environ.get("PROTAGINE_MEMORY_TURN_WRITER", ""),
    }
    expected = {
    ...
    if values != expected:
        raise RuntimeError(
            "Protagine memory coexistence latches are unsafe; require "
...
def register(ctx: Any) -> None:
    ...
    config = _plugin_config(ctx)
    url = str(config.get("url") or os.environ.get("PROTAGINE_URL") or "http://127.0.0.1:7777")
    api_key = _resolve_env_placeholder(
        config.get("api_key") or os.environ.get("PROTAGINE_API_KEY") or ""
    )
    owner_contact_id = str(
        ...
        or os.environ.get("PROTAGINE_OWNER_CONTACT_ID")
        or ""
    ).strip()
    ...
    client = ProtagineClient(url=url, api_key=api_key)
    ...
    review_parent_memory = ContextVar('protagine_review_parent_memory', default=None)
    ...
    def pre_llm_call(**kwargs: Any) -> None:
        ...
        if scope is None and native_tasks is not None and kwargs.get('platform') == 'protagine_task':
            try:
            ...
            scope = _TransportScope(*key, platform='protagine_task',
        ...
        def observed(args):
            ...
            def dispatch(selected_args):
                ...
                if not is_direct_scope(scope):
                    value = without_discovery_tool(value, context, 'protagine_task')
    ...
    def reconcile_request(request, **kwargs):
        ...
        if native_drafts is not None and native_drafts.worker:
            result = {'request': describe(without_tool(without_tool(request,
                'protagine_task'), 'protagine_memory_retain_observation'))}
    ...
    def local_work_handler(name, args):
        context = _TOOL_EXECUTION_CONTEXT.get() or {}
        if name == 'protagine_read_work_source':
    ...
    for schema in _TOOL_SCHEMAS:
        name = schema["name"]
        if name == 'protagine_reminder' and not reminders_available:
            continue
        if name == 'protagine_task' and native_tasks is None:
            continue
        ...
        ctx.register_tool(
            name=name,
            toolset="protagine_local_work" if name == 'protagine_read_work_source' else "protagine",
            schema=schema,
            handler=(
                native_task_handler if name == 'protagine_task' else
                initiative_work_handler if name == 'protagine_work_initiative' else
                source_annotate_handler if name == 'protagine_memory_annotate' else
                observation_handler if name == 'protagine_memory_retain_observation' else
                source_read_handler if name == 'protagine_memory_read_source' else
                source_forget_handler if name == 'protagine_memory_forget' else
                judgment_handler if name == "protagine_judgments" else
                contact_handler if name == "protagine_contacts" else
                followup_handler if name == "protagine_followup" else
                reminder_handler if name == "protagine_reminder" else
                commitment_work_handler if name == "protagine_commitment_work" else
                (lambda args=None, _name=name, **kwargs: local_work_handler(_name, args))
                if name in {'protagine_accept_local_draft', 'protagine_read_work_source'} else
    ...
    if native_tasks is not None:
        ctx.register_platform(name='protagine_task', label='Protagine background tasks',
    ...
    if callable(register_command):
        for command_name, handler in SLASH_COMMANDS.items():
            register_command(
                f"protagine {command_name}",
                lambda args="", _handler=handler: _handler(args),
            )

    logger.info(
        "governed Protagine plugin registered "
