#!/usr/bin/env python3
"""PacoMind Doctor — generic health/validation for a Hermes+PacoMind integration.

Catches the plugin/hook regressions that silently break PacoMind on any agent host,
...
It runs TWO kinds of check:
  ...
  LIVE (HTTP only — no plugin execution):
    ...
    - PacoMind LLM is configured with a generative provider (not an embedding model)
...
PLUGINS_DIR = os.environ.get("PACOMIND_DOCTOR_PLUGINS_DIR", os.environ.get("PACOMIND_DOCTOR_PLUGINS_DIR", os.path.join(HOME, ".hermes", "plugins")))
STATE_FILE = os.path.join(HOME, ".hermes", ".pacomind_doctor_state.json")
SIDECAR_URL = os.environ.get("PACOMIND_URL", os.environ.get("PACOMIND_URL", "http://127.0.0.1:7777"))
...
def pacomind_key():
    configured = os.environ.get("PACOMIND_API_KEY", os.environ.get("PACOMIND_API_KEY", ""))
    if configured:
        return configured
    try:
        for ln in open(os.path.join(HOME, ".pacomind", ".env")):
...
def live_checks():
    ...
    # PacoMind LLM provider sane (generative, not an embedding model)
    cfg_path = os.path.join(os.environ.get("PACOMIND_STATE_DIR", os.environ.get("PACOMIND_STATE_DIR", os.path.join(HOME, ".pacomind", "data"))),
                            ".pacomind-llm-config.json")
    try:
        cfg = json.load(open(cfg_path))
        models = " ".join(str(v) for v in (cfg.get("models") or {}).values()).lower()
        prov = cfg.get("provider", "?")
        if "embed" in models:
            fail(f"PacoMind LLM uses an embedding model as a chat tier: {models}")
        else:
            ok(f"PacoMind LLM configured (provider={prov}, models ok)")
    except Exception:
        warn("no persisted PacoMind LLM config (.pacomind-llm-config.json) — using defaults")
...
def config_checks():
    ...
    safety net): SOUL doctrine, pacomind LLM, cron
    delivery routing, and the scheduled launchd jobs."""
    print("\n[config & durability]")
    import subprocess as _sp
    # SOUL.md pacomind doctrine
    try:
    ...
    # config.yaml: pacomind LLM
    try:
        from hermes_cli.config import load_config, cfg_get
        c = load_config()
        col = cfg_get(c, "plugins", "pacomind", default=None)
        if col is None:
            col = cfg_get(c, "plugins", "pacomind", default={}) or {}
        large = str(col.get("llm_large", "")).lower()
        if col.get("llm_provider") and "embed" not in large:
            ok(f"pacomind LLM configured (provider={col.get('llm_provider')}, model={col.get('llm_large')})")
        else:
            warn("plugins.pacomind LLM not explicitly configured (relies on auto-detect)")
    except Exception as e:
        warn(f"config.yaml check skipped: {e}")
    # cron: pacomind jobs should not deliver to the owner DM (origin)
    try:
        ...
        (warn if bad else ok)(f"cron delivery: {'these pacomind jobs still hit the owner DM -> ' + str(bad) if bad else 'pacomind jobs route off the owner DM'}")
    except Exception:
        pass
    # launchd scheduled jobs
    for label in ("ai.aevonix.pacomind-doctor", "ai.hermes.activity-monitor"):
        try:
