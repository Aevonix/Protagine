...
- `pacomind-doctor.py` performs static/read-only integration checks and reports a
  non-zero exit status on drift.
- `pacomind-doctor-cron.sh` writes the doctor result to a local log. It does not
  send a message.
...
- `pacomind-activity-monitor.py` and `hermes-gateway-restart-runner.sh` are inert
  compatibility targets. Their former direct notification/restart behavior
  bypassed the action plane.
- `pre-restart-summary.py` reads the local agent log and PacoMind timeline, then
