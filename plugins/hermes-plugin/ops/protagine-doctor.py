#!/usr/bin/env python3
"""Protagine Doctor — generic health/validation for a Hermes+Protagine integration.

Catches the plugin/hook regressions that silently break Protagine on any agent host,
...
It runs TWO kinds of check:
  ...
  LIVE (HTTP only — no plugin execution):
    ...
    - Protagine LLM is configured with a generative provider (not an embedding model)
...
def protagine_key():
    ...
    if configured:
        return configured
    try:
        for ln in open(os.path.join(HOME, ".protagine", ".env")):
            if ln.startswith("PROTAGINE_API_KEY="):
...
def live_checks():
    ...
    try:
        ...
        if "embed" in models:
            fail(f"Protagine LLM uses an embedding model as a chat tier: {models}")
        else:
            ok(f"Protagine LLM configured (provider={prov}, models ok)")
    except Exception:
        warn("no persisted Protagine LLM config (.protagine-llm-config.json) — using defaults")
...
def config_checks():
    ...
    safety net): SOUL doctrine, protagine LLM, cron
    delivery routing, and the scheduled launchd jobs."""
    print("\n[config & durability]")
    import subprocess as _sp
    # SOUL.md protagine doctrine
    try:
    ...
    # config.yaml: protagine LLM
    try:
        from hermes_cli.config import load_config, cfg_get
        c = load_config()
        col = cfg_get(c, "plugins", "protagine", default=None)
        if col is None:
            col = cfg_get(c, "plugins", "protagine", default={}) or {}
        large = str(col.get("llm_large", "")).lower()
        if col.get("llm_provider") and "embed" not in large:
            ok(f"protagine LLM configured (provider={col.get('llm_provider')}, model={col.get('llm_large')})")
        else:
            warn("plugins.protagine LLM not explicitly configured (relies on auto-detect)")
    except Exception as e:
        warn(f"config.yaml check skipped: {e}")
    # cron: protagine jobs should not deliver to the owner DM (origin)
    try:
        ...
        (warn if bad else ok)(f"cron delivery: {'these protagine jobs still hit the owner DM -> ' + str(bad) if bad else 'protagine jobs route off the owner DM'}")
    except Exception:
        pass
    # launchd scheduled jobs
    for label in ("ai.aevonix.protagine-doctor", "ai.hermes.activity-monitor"):
        try:
