#!/usr/bin/env python3
"""Pre-restart activity summary (generic — any Hermes+Protagine agent).

Run by the gateway restart runner BEFORE the gateway is stopped, while the log
and Protagine are still current. Captures what the agent was just doing and writes
...
No LLM call — pulls from the unified agent.log + Protagine's /v1/host/timeline.
"""
import json
import os
import re
...
def _protagine_key():
    for path in (os.path.join(HOME, ".protagine/.env"), os.path.join(HOME, ".hermes/.env")):
        try:
            for ln in open(path):
...
def _timeline_digest():
    try:
        req = urllib.request.Request(
            f"{PROTAGINE_URL}/v1/host/timeline?since=1h&limit=8",
            headers={"Authorization": "Bearer " + _protagine_key()})
...
def build():
    ...
    dg = _timeline_digest()
    if dg:
        parts.append("Recent activity (Protagine timeline):\n"
