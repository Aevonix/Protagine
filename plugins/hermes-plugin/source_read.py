...
def handle(args, scope, client, request_memory, context):
    ...
    allowed = {'source_id', 'source_version', 'view', 'claim_id', 'offset', 'read_revision', 'asset_hash'}
    if (not isinstance(args, dict) or set(args) - allowed
            ...
            or args.get('view', 'source') not in {'source', 'assertions', 'image'}
    ...
    if ((image_view and (not re.fullmatch('[0-9a-f]{64}', str(args.get('asset_hash', '')))
            ...
            or not image_view and args.get('asset_hash') is not None):
    ...
    try:
        response = client.post('/v1/host/memory/read', timeout=3, json={
            ...
            **({'asset_hash': args['asset_hash']} if image_view else {})})
