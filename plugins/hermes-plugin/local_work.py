...
ACTIVE = ContextVar('protagine_selected_local_work', default=None)
...
class Undertaking:
    ...
    def before_tool(self, context):
        try:
            if (not self.bound or self.error or context.get('task_id') != self.task_id
                    or context.get('tool_name') not in {'protagine_read_work_source', 'tool_search', 'tool_describe'}):
    ...
    def verify_holder(self):
        ...
        if coordinator.before_tool({**context, 'tool_name': 'protagine_read_work_source'}) is not None:
