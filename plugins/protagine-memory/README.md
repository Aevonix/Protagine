...
This provider mounts Protagine as a context sidecar without changing Hermes core.
It binds every real-channel turn to the transport sender resolved by Protagine and
...
`protagine-memory.json` and credentials through Hermes's private environment.
...
`PROTAGINE_AGENT_TIMEZONE` in its environment; absent either, local clock context
uses UTC and does not inspect another Protagine instance's state directory.
...
Editing a native memory file does not create another owner statement in Protagine.
...
matching `protagine` and `protagine-hermes` packages and select the Hermes profile.
