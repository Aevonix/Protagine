"""CLI commands for the Protagine memory provider.

Exposes:
  hermes protagine-memory status     → Health + capabilities
  hermes protagine-memory goals      → List active goals
  hermes protagine-memory context    → Fetch current context assembly
  hermes protagine-memory sync       → Force a turn sync
"""
...
app = typer.Typer(help="Protagine cognitive infrastructure commands")
...
def register_cli(subparser) -> None:
    ...
    """
    commands = subparser.add_subparsers(dest="protagine_command", required=True)
    health = commands.add_parser("status", help="Check Protagine sidecar health")
    health.add_argument("--url", "-u")
    health.set_defaults(func=lambda args: _native_call(status, url=args.url))

    goal_list = commands.add_parser("goals", help="List Protagine goals")
    ...
    recall = commands.add_parser("context", help="Fetch Protagine context")
    ...
    turn = commands.add_parser("sync", help="Sync one turn to Protagine")
...
@app.command()
def status(
    url: Optional[str] = typer.Option(None, "--url", "-u", help="Protagine sidecar URL"),
) -> None:
    """Check Protagine sidecar health and capabilities."""
    ...
    try:
        ...
        typer.echo(f"Protagine sidecar unreachable: {exc}", err=True)
...
def context(
    ...
    """Fetch Protagine cognitive context for a contact."""
...
def sync(
    ...
    """Force a turn sync to Protagine."""
    sidecar, cid, headers = _connection(url, contact_id)
