"""PacoMind memory provider for Hermes.

Implements Hermes's MemoryProvider ABC to inject PacoMind's cognitive context
(commitments, affect, facts, patterns, world model) into Hermes conversations
and sync turns back for extraction.

Config key: memory.provider = "pacomind-memory"
"""
...
def _profile_config_path(home: Path) -> Path:
    return home / "pacomind-memory.json"
...
def _profile_config(hermes_home: Path) -> dict[str, Any]:
    """Read the selected profile; provider setup values override native YAML."""
    import yaml
    ...
    try:
        ...
        raise ValueError("Selected profile has invalid PacoMind memory configuration") from None


def _profile_environment(name: str, hermes_home: Path) -> dict[str, str]:
    """Read the selected profile before process defaults, without mutation."""
...
def general_plugin_memory_ownership(
    ...
    try:
        ...
        if (enabled is not None and not isinstance(enabled, list)) or not isinstance(disabled, list):
            raise ValueError("Selected profile has invalid PacoMind ownership configuration")
        selected_names = {"pacomind"}
        if selected_names.intersection(disabled) or (enabled is not None and not selected_names.intersection(enabled)):
            return False
        if memory.get("provider") not in {"pacomind-memory"} or enabled is None or not selected_names.intersection(enabled):
            return None
        ...
        if writer not in {"auto", "disabled", "off", "false", "0"}:
            raise ValueError("General PacoMind plugin owns memory; provider turn_writer must be auto or disabled")
        return True
    except (OSError, UnicodeError, AttributeError, TypeError, yaml.YAMLError):
        raise ValueError("Selected profile has invalid PacoMind ownership configuration") from None
...
# ---------------------------------------------------------------------------
# PacoMind tool schemas — what the LLM sees
# ---------------------------------------------------------------------------
...
_QUEUE_MUTATION_TOOLS = frozenset({
    "pacomind_claim_task",
    "pacomind_complete_task",
    "pacomind_fail_task",
    "pacomind_heartbeat_task",
})
...
_PACOMIND_TOOL_SCHEMAS: List[Dict[str, Any]] = [
    {
        "name": "pacomind_check_commitments",
        "description": (
    ...
    {
        "name": "pacomind_resolve_commitment",
        "description": (
            ...
            "stale, or should be ignored. Get the id from pacomind_check_commitments."
        ),
        "parameters": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "pacomind_list_pending_tasks",
        "description": (
            "List pending AGENT_ACTION jobs in the PacoMind task queue. "
    ...
    {
        "name": "pacomind_claim_task",
        "description": (
            "Claim an AGENT_ACTION job from the PacoMind task queue. "
    ...
    {
        "name": "pacomind_complete_task",
        "description": (
            "Report a completed job to PacoMind. "
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "claim_attempt_id": {
                    "type": "string",
                    "description": "Exact server claim attempt returned by pacomind_claim_task",
                },
            },
    ...
    {
        "name": "pacomind_fail_task",
        "description": (
            "Report a failed job to PacoMind. "
    ...
    {
        "name": "pacomind_initiative_feedback",
        "description": (
...
GENERAL_PLUGIN_FORBIDDEN_TOOL_NAMES = tuple(sorted(
    {
        ...
        for schema in _PACOMIND_TOOL_SCHEMAS
        if str(schema.get("name") or "") not in _READ_CONTEXT_TOOLS
    }
    | {"pacomind_approve_initiative"}
))

_GENERAL_PLUGIN_SYSTEM_PROMPT = (
    "PacoMind cognitive context is active in read-only mode. This memory provider "
    "exposes pacomind_check_commitments, "
    "pacomind_get_affect, pacomind_get_facts, and pacomind_timeline. Refer to the current "
...
def catalog_attestation() -> Dict[str, Any]:
    ...
    visible_schemas = sorted(
        (
            _bound_read_schema(schema) for schema in _PACOMIND_TOOL_SCHEMAS
    ...
    return {
        "schema": "PacoMindMemoryCatalogAttestationV1",
        "version": 1,
        "posture": {
...
class PacoMindMemoryProvider(_MemoryProviderABC):
    """PacoMind memory provider for Hermes.

    Reads cognitive context from PacoMind's sidecar via /v1/host/context/assemble
    and injects it as prefetched memory. Syncs turns back to PacoMind for
    extraction of commitments, affect, and facts.

    Config: selected profile's pacomind-memory.json,
    with native memory.config
    supplying values not yet written by native setup. Explicit constructor
    configuration overrides both for embedded callers.
        url: PacoMind sidecar URL (default http://127.0.0.1:7777)
        api_key: PacoMind API key (or set PACOMIND_API_KEY env var)
        contact_id: Contact ID for context assembly (or set PACOMIND_MCP_CONTACT_ID)
    """
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        # Prefetch-cache bookkeeping (PACOMIND_PREFETCH_QUERY_CHECK=1): remember
        ...
        for binding_flag in (
            "PACOMIND_PREFETCH_TURN_CONTACT",
            "PACOMIND_PREFETCH_QUERY_CHECK",
        ):
    ...
    def _configure(self, config: dict[str, Any]) -> None:
        ...
        self.sidecar_url = config.get("url") or _profile_env("PACOMIND_URL", home) or "http://127.0.0.1:7777"
        try:
            ...
            valid = False
        if not valid:
            raise ValueError("PacoMind URL must be HTTP(S) with a valid port and no embedded credentials or query")
        self.sidecar_url = self.sidecar_url.rstrip("/")
        raw_key = config.get("api_key") or _profile_env("PACOMIND_API_KEY", home)
        # Resolve unexpanded env-var placeholders like ${PACOMIND_API_KEY}
        ...
        self._contact_id = config.get("contact_id") or _profile_env("PACOMIND_MCP_CONTACT_ID", home) or "default"
        self._timezone = config.get("timezone") or _profile_env("PACOMIND_AGENT_TIMEZONE", home)
        self._turn_writer_mode = str(config.get(
            "turn_writer", _profile_env("PACOMIND_MEMORY_TURN_WRITER", home) or "auto"
        ) or "auto").strip().lower()
    ...
    def _turn_writer_enabled(self) -> bool:
        ...
        The selected profile's enabled PacoMind plugin and memory provider own
    ...
    def _general_plugin_active(self) -> bool:
        ...
        return _env_true("PACOMIND_GENERAL_PLUGIN_ACTIVE")
    ...
    def _record_connection_failure(self) -> None:
        ...
        if self._connection_failures >= 3:
            ...
            logger.warning("PacoMind: circuit breaker opened for 60s after %d failures", self._connection_failures)

    def _record_connection_success(self) -> None:
        self._connection_status = "connected"
        if self._connection_failures > 0:
            logger.info("PacoMind: connection recovered, resetting failure count")
            self._connection_failures = 0
            self._circuit_open_until = None
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        return [
            {
                "key": "url",
                "description": "PacoMind sidecar URL",
                "default": "http://127.0.0.1:7777",
            },
            {
                "key": "api_key",
                "description": "PacoMind API key",
                "secret": True,
                "env_var": "PACOMIND_API_KEY",
            },
            {
                "key": "contact_id",
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        if home != self._hermes_home:
            if self._session_id:
                raise ValueError("Create a new PacoMind memory provider for another Hermes profile")
        ...
        if not self._api_key:
            logger.warning("PacoMind: PACOMIND_API_KEY not set — requests will fail if sidecar requires auth")
        logger.info("PacoMind memory provider initialized (session=%s, platform=%s, home=%s)",
                     session_id, self._platform, self._hermes_home)

    def system_prompt_block(self) -> str:
        """Return static context about PacoMind for the system prompt."""
    ...
    def _last_session_block(self) -> str:
        ...
        if not txt:
            return ""
        persistence = (
            "Let the canonical PacoMind turn writer retain ordinary conversation; "
    ...
    def resolve_contact(self, platform: str, user_id: str) -> None:
        """Resolve the real PacoMind contact from the message sender so per-contact
        ...
        if cid:
            logger.debug(
                "PacoMind resolved turn contact %s for %s:%s",
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        """Recall relevant PacoMind context for the upcoming turn.
        ...
        if not contact_id:
            logger.warning(
                "PacoMind prefetch withheld: current turn has no attested "
                "participant binding"
            )
            return ""
        try:
    ...
    def _prefetch_turn_contact_enabled() -> bool:
        """PACOMIND_PREFETCH_TURN_CONTACT=1 -> context prefetch resolves the
    ...
    def _default_contact_fallback_allowed(self, platform: str) -> bool:
        authority = _env(
            "PACOMIND_MEMORY_DEFAULT_CONTEXT_AUTHORITY", ""
        ).strip().lower()
        return bool(
    ...
    def _prefetch_contact(self, session_id: str = "") -> str:
        ...
        if sender or chat or not internal_lane:
            ...
            try:
                ...
                logger.debug("PacoMind per-turn prefetch contact failed: %s", exc)
                return ""
    ...
    def _projection_readiness_sync(self, contact_id: str) -> bool:
        ...
        try:
            ...
            logger.debug("PacoMind scoped projection preflight failed: %s", exc)
    ...
    def _fresh_temporal_block_sync(self, *, contact_id: Optional[str] = None):
        ...
        try:
            ...
            logger.debug("PacoMind temporal brief fetch failed: %s", exc)
        if not block:
    ...
    def _with_fresh_temporal_sync(
        ...
        marker = "[PacoMind Cognitive Context]\n"
    ...
    _RW_RECENT_HOURS = float(_env("PACOMIND_REPLY_WINDOW_RECENT_HOURS", "6"))
    _RW_MSGS = int(_env("PACOMIND_REPLY_WINDOW_MSGS", "5"))
    _RW_BUDGET = int(_env("PACOMIND_REPLY_WINDOW_BUDGET", "1200"))
    _RW_LOOKBACK = _env("PACOMIND_REPLY_WINDOW_LOOKBACK", "14d")
    _RW_STATE_MAX = 64
    ...
    def _prefetch_sync(
        ...
        *,
        ...
        if guest and not self._projection_readiness_sync(bound_contact):
            logger.warning(
                "PacoMind guest context withheld: scoped projection is not ready"
            )
            return ""
        try:
            ...
            if code in (401, 403):
                logger.warning("PacoMind prefetch auth failed (HTTP %d) — check PACOMIND_API_KEY", code)
            else:
                logger.debug("PacoMind prefetch failed: %s", exc)
            return ""
        ...
        if not attested:
            logger.warning(
                "PacoMind context withheld: response viewer attestation is invalid"
            )
            return ""
    ...
    def _prefetch_query_check_enabled() -> bool:
        """PACOMIND_PREFETCH_QUERY_CHECK=1 -> consume the prefetch cache only when
    ...
    def _resolve_handle(self, platform: str, sender: str) -> Optional[str]:
        """Resolve a gateway sender handle -> PacoMind contact_id (None if unknown),
        ...
        if not sender:
            return None
        try:
            from pacomind_hermes.native_task_platform import bound_task_contact
            task_contact = bound_task_contact(platform, sender, self._session_id)
        except ImportError:
            task_contact = None
        if task_contact is not None or platform == 'pacomind_task':
        ...
        try:
            ...
            logger.debug("PacoMind resolve_handle failed: %s", exc)
        with self._handle_cache_lock:
            if resolved:
    ...
    def sync_turn(
        ...
        *,
        ...
        """Persist a completed turn to PacoMind for extraction.
        ...
        if not self._turn_writer_enabled():
            if not self._turn_writer_skip_logged:
                logger.info(
                    "PacoMind memory provider is read/context-only; "
                    "the general PacoMind plugin owns turn ingestion"
                )
                self._turn_writer_skip_logged = True
            return
        try:
            from pacomind_hermes.evidence import native_work_capture_excluded
        except ImportError:
        ...
        if turn_sender and not contact_id:
            logger.warning(
                "PacoMind sync_turn withheld: real-channel sender did not "
        ...
        if contact_id:
            ...
            logger.warning(
                "PacoMind sync_turn withheld: channel has no exact sender binding"
            )
            return
            ...
            logger.debug("PacoMind sync_turn skipped: no participant + no conversation context (system/self turn)")
            return
        ...
        def _sync():
            if self._is_circuit_open():
                logger.warning("PacoMind turn sync skipped — circuit breaker open")
                return
            for attempt in range(3):
                try:
                    ...
                    if self._is_circuit_open():
                        logger.warning("PacoMind turn sync circuit opened after connection failure")
                        return
                    if attempt < 2:
                    ...
                    code = exc.response.status_code
                    if code in (401, 403):
                        logger.warning("PacoMind turn sync auth failed (HTTP %d)", code)
                    else:
                        logger.debug("PacoMind turn sync HTTP error: %s", exc)
                    return  # Don't retry or count toward breaker
                except Exception as exc:
                    self._last_sync_error = str(exc)
                    logger.debug("PacoMind turn sync unexpected error: %s", exc)
                    return  # Don't retry or count toward breaker
    ...
    def _tool_available(self, tool_name: str) -> bool:
        ...
        """
        if tool_name == "pacomind_approve_initiative":
            return False
        if tool_name == "pacomind_claim_task" and _env(
        ...
        if tool_name in _QUEUE_MUTATION_TOOLS:
            return _env_true("PACOMIND_MEMORY_WORKER_TOOLS")
        return True
    ...
    def _mutation_denial(self, tool_name: str) -> Optional[str]:
        ...
        if tool_name in _QUEUE_MUTATION_TOOLS and not self._tool_available(tool_name):
            return "pacomind worker tools are disabled"
        if (
            tool_name in _GENERAL_PLUGIN_MUTATION_TOOLS
            and not self._tool_available(tool_name)
        ):
            return "PacoMind memory provider is read/context-only"
        return None

    def get_tool_schemas(self) -> List[Dict[str, Any]]:
        """Return PacoMind tool schemas for the model."""
        schemas = []
        for schema in _PACOMIND_TOOL_SCHEMAS:
    ...
    def handle_tool_call(self, tool_name: str, args: Dict[str, Any], **kwargs) -> str:
        """Handle an PacoMind tool call from the agent."""
        if not self._tool_available(tool_name):
            return json.dumps({
                "error": "PacoMind tool is not available in this mode",
            })
        if tool_name in _READ_CONTEXT_TOOLS:
            ...
            if not bound_contact:
                return json.dumps({
                    "error": (
                        "PacoMind read context withheld: no attested turn "
                        "participant binding"
                    ),
                })
            supplied = str(
            ...
            if bound_contact != self._contact_id:
                return json.dumps({
                    "error": (
                        "PacoMind direct read tool withheld: guest-scoped tool "
        ...
        if handler is None:
            return json.dumps({"error": f"Unknown PacoMind tool: {tool_name}"})
        try:
            return handler(args)
        except Exception as exc:
            logger.warning("PacoMind tool %s failed: %s", tool_name, exc)
            return json.dumps({"error": f"Tool failed: {exc}"})

    # -- Tool handlers ---------------------------------------------------------

    def _tool_pacomind_check_commitments(self, args: dict) -> str:
    ...
    def _tool_pacomind_resolve_commitment(self, args: dict) -> str:
        denial = self._mutation_denial("pacomind_resolve_commitment")
        if denial:
    ...
    def _tool_pacomind_get_affect(self, args: dict) -> str:
        contact_id = args.get("contact_id", self._contact_id)
        try:
            with httpx.Client(timeout=5) as client:
                resp = client.get(
    ...
    def _tool_pacomind_get_facts(self, args: dict) -> str:
        contact_id = args.get("contact_id", self._contact_id)
    ...
    def _tool_pacomind_get_patterns(self, args: dict) -> str:
        contact_id = args.get("contact_id", self._contact_id)
        limit = args.get("limit", 10)
        try:
            with httpx.Client(timeout=5) as client:
    ...
    def _tool_pacomind_list_goals(self, args: dict) -> str:
    ...
    def _tool_pacomind_record_affect(self, args: dict) -> str:
        denial = self._mutation_denial("pacomind_record_affect")
        if denial:
            return json.dumps({"error": denial})
        try:
            with httpx.Client(timeout=5) as client:
    ...
    def _tool_pacomind_timeline(self, args: dict) -> str:
    ...
    def _tool_pacomind_list_pending_tasks(self, args: dict) -> str:
        try:
            with httpx.Client(timeout=5) as client:
                resp = client.get(
    ...
    def _tool_pacomind_claim_task(self, args: dict) -> str:
        denial = self._mutation_denial("pacomind_claim_task")
        if denial:
            return json.dumps({"error": denial})
        try:
            worker_id = (
                _env("PACOMIND_MEMORY_WORKER_NODE_ID", "").strip()
                or _env(
                    "PACOMIND_WORKER_NODE_ID", "pacomind-memory-worker"
                ).strip()
                or "pacomind-memory-worker"
            )
            capabilities = [
                item.strip()
                for item in _env(
                    "PACOMIND_MEMORY_WORKER_CAPABILITIES",
            ...
            if not (
                ...
                raise RuntimeError(
                    "PACOMIND_MEMORY_WORKER_CAPABILITIES must include a safe "
    ...
    def _tool_pacomind_complete_task(self, args: dict) -> str:
        denial = self._mutation_denial("pacomind_complete_task")
        if denial:
            return json.dumps({"error": denial})
        try:
            with httpx.Client(timeout=5) as client:
    ...
    def _tool_pacomind_fail_task(self, args: dict) -> str:
        denial = self._mutation_denial("pacomind_fail_task")
        if denial:
            return json.dumps({"error": denial})
        try:
            with httpx.Client(timeout=5) as client:
    ...
    def _tool_pacomind_heartbeat_task(self, args: dict) -> str:
        denial = self._mutation_denial("pacomind_heartbeat_task")
        if denial:
            return json.dumps({"error": denial})
        try:
            with httpx.Client(timeout=5) as client:
    ...
    def _tool_pacomind_approve_initiative(self, args: dict) -> str:
        return json.dumps({"error": "initiative approval is operator-only"})

    def _tool_pacomind_initiative_feedback(self, args: dict) -> str:
        denial = self._mutation_denial("pacomind_initiative_feedback")
        if denial:
            return json.dumps({"error": denial})
        try:
            with httpx.Client(timeout=5) as client:
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        logger.debug("PacoMind memory provider switched to session=%s (reset=%s)", new_session_id, reset)

    def on_turn_start(self, turn_number: int, message: str, **kwargs) -> None:
        """Called at the start of each turn."""
        try:
            from pacomind_hermes.native_input import capture
        ...
        logger.debug("PacoMind: turn %d started (session=%s)", turn_number, self._session_id)
    ...
    def on_pre_compress(
        ...
        try:
            ...
            from pacomind_hermes.evidence import checkpoint
            import yaml
            home = Path(self._hermes_home)
            ...
            self._last_checkpoint = checkpoint(
                ...
                outbox_path=str(
                    ...
                    or _profile_env("PACOMIND_HERMES_TURN_OUTBOX", home) or ""
                ),
            )
        except Exception as error:
            self._last_checkpoint = {"state": "failed", "error": type(error).__name__}
            if require_checkpoint:
                raise RuntimeError("PacoMind durable checkpoint failed") from error
            logger.warning("PacoMind durable checkpoint deferred (%s)", type(error).__name__)
        return ""
    ...
    def _format_sections(self, sections: list[dict[str, Any]]) -> str:
        ...
        for section in sections:
            header = section.get("title", section.get("id", "pacomind-context"))
