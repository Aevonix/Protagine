"""CLI commands for the PacoMind memory provider.

Exposes:
  hermes pacomind-memory status     → Health + capabilities
  hermes pacomind-memory goals      → List active goals
  hermes pacomind-memory context    → Fetch current context assembly
  hermes pacomind-memory sync       → Force a turn sync
"""
...
app = typer.Typer(help="PacoMind cognitive infrastructure commands")
...
def register_cli(subparser) -> None:
    ...
    """
    commands = subparser.add_subparsers(dest="pacomind_command", required=True)
    health = commands.add_parser("status", help="Check PacoMind sidecar health")
    health.add_argument("--url", "-u")
    health.set_defaults(func=lambda args: _native_call(status, url=args.url))

    goal_list = commands.add_parser("goals", help="List PacoMind goals")
    ...
    recall = commands.add_parser("context", help="Fetch PacoMind context")
    ...
    turn = commands.add_parser("sync", help="Sync one turn to PacoMind")
...
@app.command()
def status(
    url: Optional[str] = typer.Option(None, "--url", "-u", help="PacoMind sidecar URL"),
) -> None:
    """Check PacoMind sidecar health and capabilities."""
    ...
    try:
        ...
        typer.echo(f"PacoMind sidecar unreachable: {exc}", err=True)
...
def context(
    ...
    """Fetch PacoMind cognitive context for a contact."""
