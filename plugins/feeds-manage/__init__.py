"""feeds-manage — agent tools over the Protagine feeds framework.
...
python install.  Config (~/.protagine-feeds.json):
    {
      "python":      "/path/to/python3",          # with PyYAML available
      "pythonpath":  "/path/to/Protagine/sidecar", # where protagine lives
...
_CFG_PATH = os.path.expanduser("~/.protagine-feeds.json")
_DEFAULTS = {
    "python": "python3",
    "pythonpath": "",
    "specs_dir": "~/.hermes/data/feeds/_specs",
...
def _cli(*args, timeout=180):
    ...
    proc = subprocess.run(
        [os.path.expanduser(cfg["python"]), "-m", "protagine.feeds.cli", *args],
