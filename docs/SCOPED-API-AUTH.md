...
Protagine supports per-service API principals alongside the legacy global
`PROTAGINE_API_KEY`. The dual-accept period is deliberate: consumers can move one
...
Protagine does not infer a wider scope from a prefix.
...
Protagine creates the request at effect-job birth; phone and Operator Deck
...
granted `turns:resolve-sender`. Protagine then ignores the body's initial contact
...
turn from an allowed platform reaches `ParticipantResolver`, Protagine discards
...
`$PROTAGINE_STATE_DIR/api-contact-grants.json`; override it with
`PROTAGINE_API_CONTACT_GRANTS_PATH`. It contains no key material, is required to
...
candidate boundary. Protagine never retries a scoped miss against global memory.
...
WorkerGovernor. `PROTAGINE_WORKER_AUTHORITY_MODE=shadow` is the default: the
...
The embedded in-process worker remains enabled by default for generic Protagine
installs. A health-only deployment can set
`PROTAGINE_EMBEDDED_WORKER_ENABLED=false`; this skips worker construction while
...
`PROTAGINE_AGENT_WORKER_ROUTES=agent_sync`, `hermes_run`, or the combined default
...
Set `PROTAGINE_AGENT_SYNC_WORKER_NODE_ID`,
`PROTAGINE_HERMES_RUN_WORKER_NODE_ID`, and
`PROTAGINE_ACTION_PLANE_WORKER_NODE_ID` to make route ownership exact. The global
`PROTAGINE_AGENT_JOB_CLAIMS_ENABLED=false` containment switch stops every generic
...
P3 startup requires `PROTAGINE_EMBEDDED_WORKER_ENABLED=true`, a configured LLM
router, and `PROTAGINE_THOUGHT_WORKER_NODE_ID` equal to the local durable node ID.
...
match `PROTAGINE_RELEASE_COMMIT` (40 lowercase hex characters) plus
`PROTAGINE_RELEASE_ARTIFACT_MANIFEST_SHA256` (64 lowercase hex characters).
...
Protagine computes the canonical evidence and receipt-reference hashes; callers
...
default is `sidecar-bridge`), while the Hermes `protagine-jobs` route sends the
heartbeat/complete/fail requests. Before worker-authority enforcement, the
credential in Hermes' `PROTAGINE_API_KEY` must therefore have
...
per-route HMAC secret in Hermes and in `PROTAGINE_HERMES_WEBHOOK_SECRET`; Protagine
...
When Hermes' canonical general Protagine plugin is active, the separate
`protagine-memory` provider publishes and dispatches only its non-duplicating
read/context allowlist. Standalone queue mutation tools require the explicit
`PROTAGINE_MEMORY_WORKER_TOOLS=1` opt-in. Initiative approval is never exposed as
a model tool on that provider; approval remains transport/operator authority.

The provider requires `PROTAGINE_PREFETCH_QUERY_CHECK=1` and
`PROTAGINE_PREFETCH_TURN_CONTACT=1`; disabling either is a startup error. Set
`PROTAGINE_MCP_CONTACT_ID` to the exact owner contact and use
`PROTAGINE_MEMORY_DEFAULT_CONTEXT_AUTHORITY=owner_system` only for a non-channel
...
after Protagine can issue and verify a durable target plus transport attestation;
...
| Variable | Default |
|---|---|
| `PROTAGINE_OWNER_PERSON_ID` | `PROTAGINE_OWNER_CONTACT_ID`, then `owner` |
...
scratch `person_id`, and omission derives `PROTAGINE_DEV_PERSON_ID`. It cannot
...
The implementation retains `PROTAGINE_API_KEY` as a global principal with
...
```bash
install -m 600 sidecar/api-keyring.example.json ~/.protagine/api-principals.json
${EDITOR:-vi} ~/.protagine/api-principals.json
chmod 600 ~/.protagine/api-principals.json
```

Set `PROTAGINE_API_KEYRING_PATH` to that absolute path. Protagine rejects a keyring
...
send `X-Protagine-Principal`; if present, it must exactly match the principal
resolved from the token.

## Migration evidence

Protagine persists privacy-safe authentication counters in
`$PROTAGINE_STATE_DIR/protagine-auth-telemetry.db` by default; override that path with
`PROTAGINE_AUTH_TELEMETRY_PATH`. Counters contain only authenticated principal
