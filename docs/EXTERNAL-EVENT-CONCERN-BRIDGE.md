...
Protagine can reduce the already-durable `cognition.external.*` journal lane into
scoped concerns without treating an external producer as an authority. This
bridge is generic Protagine behavior. It does not change the host deployment, Hermes, Voice,
...
`protagine-workspace.db`. The ordinary `EventConcernReducer` may skip the same
...
`PROTAGINE_EXTERNAL_EVENT_CONCERNS=off|shadow|live` defaults to `off`; invalid
values fail off.
...
`PROTAGINE_EXTERNAL_EVENT_CONCERNS_GAP_POLICY=stop|acknowledge` defaults to
...
Before a canary, quiesce Protagine writers and copy `protagine-workspace.db` with its
WAL/SHM files. Start with the workspace and cognition spine in their existing
safe deployment modes, then set:

```text
PROTAGINE_EXTERNAL_EVENT_CONCERNS=shadow
PROTAGINE_EXTERNAL_EVENT_CONCERNS_GAP_POLICY=stop
```
...
Rollback is flag-first: set `PROTAGINE_EXTERNAL_EVENT_CONCERNS=off` and restart.
