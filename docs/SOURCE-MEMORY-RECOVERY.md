...
original pixels. The existing `protagine backup --full` and `protagine restore --full`
...
source-image set, not an atomic snapshot across every Protagine database.
...
```sh
protagine restore --memory-only --input ./backup.tar.gz \
...
requires the surviving protagine identity and `turn-idempotency.db`; the archive
must belong to the same protagine. The caller is responsible for selecting an
...
the memory bundle as a complete Protagine instance. This mode does not certify
