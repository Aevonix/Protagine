...
`PROTAGINE_INITIATIVE_BACKUP_RECEIPT` to its latest-attempt JSON file. This replaces
only the legacy `~/.protagine/backups/*.bak` check. With no setting, the legacy
...
`protagine_work_initiative(initiative_id=...)`. The tool accepts only an existing
...
New tasks use the managed `protagine-reviews` native profile, ordinary completion,
...
`protagine_commitment_work` exposes `claim`, `status` and `release`. The adapter
...
`protagine_accept_local_draft` associates ordinary session requests with one active
