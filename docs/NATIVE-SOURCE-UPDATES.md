...
grants. Canonical source checks remain in Protagine's request path. A false return
...
| `native_request_visible` | The carrier survived Protagine's turn-scoped Relay callback immediately before `next_call`; `boundary` is `relay_before_next_call`. Later downstream callbacks or the SDK may still change or fail the request. |
...
and the `protagine-hermes[native-memory]` extra for the native request boundary.
