...
Hermes requests Protagine memory once at the beginning of a turn. A turn can then
...
`ProtagineWorkSnapshotV1`. This is a bounded, read-only projection of an existing
...
the existing `PROTAGINE_WORKER_STATUS_PATHS` mapping. Unsupported, failed or
...
One request-only block, delimited by `protagine-work-request-v1`, supersedes the
...
same callback. The Protagine memory provider's existing synchronous `on_turn_start`
...
owning a dispatcher task. Protagine corrects this known mismatch through its existing
