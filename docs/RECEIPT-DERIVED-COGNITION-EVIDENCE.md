...
Protagine can now learn from completed project work without trusting a model,
...
`PROTAGINE_COGNITION_EVIDENCE=off|shadow|live` defaults to `off`. Unknown values
resolve to off.
...
- `PROTAGINE_COGNITION_EVIDENCE_BOOTSTRAP=beginning|tail` defaults to
  `beginning`.
- `PROTAGINE_COGNITION_EVIDENCE_INTERVAL_SECONDS` defaults to 30 and is bounded
  to 5–3600 seconds.
- `PROTAGINE_COGNITION_EVIDENCE_GAP_POLICY=stop|acknowledge` defaults to `stop`.
...
- `PROTAGINE_EVENT_CONCERNS_GAP_POLICY` and `PROTAGINE_SITUATION_GAP_POLICY` use the
