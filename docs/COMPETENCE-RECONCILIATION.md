...
Protagine treats competence as earned authority, so an unverified completion may
...
- If the old data cannot be correlated one-to-one, declare an exact domain and
  half-open time window unavailable. Protagine excludes that window from trust
...
PYTHONPATH=sidecar python3 scripts/reconcile_competence.py inspect \
  --db /path/to/protagine-self-model.db \
...
```json
{
  "schema": "protagine.competence-reconciliation/v1",
...
PYTHONPATH=sidecar python3 scripts/reconcile_competence.py apply \
  ...
  --manifest /path/to/reconciliation.json \
  --commit \
  --backup /path/to/backups/protagine-self-model.before-reconciliation.db
```
...
`protagine.actions-success/v2` and its reconciliation revision matches the
