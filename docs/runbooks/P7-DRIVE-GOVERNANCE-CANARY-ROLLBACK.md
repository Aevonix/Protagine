...
Shadow changes only Protagine's optional priority observer. Live additionally
...
1. Deploy a pinned, clean Protagine revision containing P7.
...
Leave `PROTAGINE_DRIVE_GOVERNANCE_MODE` unset or set it to `off`.
...
```text
PROTAGINE_DRIVE_GOVERNANCE_MODE=shadow
```
...
1. Set P3 to `shadow` (or `off`) before setting
   `PROTAGINE_DRIVE_GOVERNANCE_MODE=off`; restart only the Protagine process that
...
4. If a code rollback is required, deploy the prior pinned Protagine revision and
...
the expected missing `protagine.cognition.drive_governance` module. After
implementation:

- P7 focused contract: `30 passed`;
- P7 + P3 + directives + bounded approvals + P4 + Toolsmith matrix:
  `133 passed`; and
- complete Protagine sidecar suite: `2664 passed, 118 skipped, 21 warnings`.
