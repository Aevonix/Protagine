...
Status: integrated into the Protagine sidecar behind an explicit, default-off
...
When and only when `PROTAGINE_RECIPIENT_SIMULATOR_MODE=shadow`, sidecar startup
attaches one process-owned runtime around the canonical `SharedFactsStore` and
three additive ledgers under `PROTAGINE_STATE_DIR`:

- `protagine-p8-visibility.db` — immutable, content-digest-bound fact envelopes;
- `protagine-p8-arcs.db` — the existing append-only conversational arc store; and
- `protagine-p8-recipient-audit.db` — reference/digest-only sample and evaluation
  receipts.
...
request authority plus the person returned by Protagine's server-side resolver.
...
applies the strictly positive `PROTAGINE_P8_FACT_MIN_CONFIDENCE` floor (default
