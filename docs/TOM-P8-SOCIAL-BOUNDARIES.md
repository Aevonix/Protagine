...
Status: core primitives plus a default-off, shared Protagine shadow integration.
See `docs/P8-SHARED-INTEGRATION.md`; live enforcement and every real-time voice
path remain unwired.

## Outcome

P8 adds three small, generic Protagine primitives:
...
`PROTAGINE_P8_FACT_MIN_CONFIDENCE` (default `0.5`; invalid/non-positive fails safe
...
`PROTAGINE_RECIPIENT_SIMULATOR_MODE=off|shadow|live` defaults to `off`; an unknown
value is also `off`.
