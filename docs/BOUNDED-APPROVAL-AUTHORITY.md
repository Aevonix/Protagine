...
Protagine's approval ledger is transport-neutral. A host may collect a decision
from a phone, web console, CLI, or another trusted adapter, but Protagine accepts
...
Only then does Protagine materialize its `ApprovalRequest` or consume an exact
...
Protagine records the request in
`$PROTAGINE_STATE_DIR/approval_authority.db` with:
...
- an immutable, bounded `ProtagineApprovalPresentationV1` containing redacted
...
`PROTAGINE_GRANT_MAX_TTL_SECONDS` (default `2592000`) and
`PROTAGINE_GRANT_MAX_USES` (default `100`). A TTL integer >= 60 seconds and a use
...
standing-grant counts, and `protagine doctor` continues to WARN until those
...
discovery because other Protagine subsystems can share the authority store.
...
`task_type` filter, in `X-Protagine-Blocked-Legacy-Count`. A nonzero count makes
...
Protagine owns the one canonical request. The host may mirror it into a short-lived
phone or Operator Deck challenge, but must not mint a second approval identity
or wait for a second approval. The mirror remains bound to Protagine's canonical
...
`ProtagineApprovalAuthorizationProjectionV1`. The host may import authority only when
...
`ProtagineApprovalPresentationV1`, not the raw queue payload.
...
wrappers. In the default `PROTAGINE_APPROVAL_AUTHORITY_MODE=shadow` they accept
...
`PROTAGINE_APPROVAL_AUTHORITY_MODE=enforce`. In enforcement mode, the middleware
...
Back up `approval_authority.db` with the other Protagine SQLite stores before an
