# Protagine Feeds — spec-driven intelligence pipelines
...
```bash
protagine feeds validate my-feed.yaml     # clear, itemized errors
protagine feeds create   my-feed.yaml     # data dirs + prompts + scheduled jobs
protagine feeds run my-feed collect       # first collection, inline
protagine feeds run my-feed distill       # first brief, in the background
protagine feeds status my-feed
protagine feeds pause|resume|delete my-feed [--purge]
```

Without an installed package: `PYTHONPATH=<repo>/sidecar python3 -m
protagine.feeds.cli ...`. A commented example spec lives at
`sidecar/protagine/feeds/example-feed.yaml`.
...
- **collect** and **alerts** are plain-python cron jobs (`--no-agent`): a
  ...
  (`engine.py`, deployed as `protagine-feed-engine.py`) at the instance config.
...
`~/.protagine-feeds.json` (python interpreter, PYTHONPATH to this repo,
specs dir).
