...
- Config precedent: each subsystem gets `PROTAGINE_<X>_MODE` off|shadow|live and
  a state-dir SQLite db (`protagine-<x>.db`).
...
2. Auto-graduation. PROTAGINE_<X>_MODE shadow phases are CALIBRATION phases,
   not waiting rooms. A capability class auto-graduates shadow -> ask_first
   -> act_first as its track record crosses thresholds
   (PROTAGINE_TRUST_* envs, sensible defaults), with an owner NOTIFICATION on
...
3. Circuit breakers. N failures (PROTAGINE_TRUST_BREAKER_FAILURES, default 3)
   within a window (PROTAGINE_TRUST_BREAKER_WINDOW_HOURS, default 24) or ANY
...
- New `projects/` package.
  ...
  - `store.ProjectStore` (SQLite `protagine-projects.db`): CRUD, list by status,
...
Config flags: `PROTAGINE_PROJECTS_MODE` off|shadow|live (default shadow: plan +
...
shadow/live). `PROTAGINE_PROJECTS_MAX_STEPS` (default 12), `PROTAGINE_PROJECTS_
REVIEW_SECS`, `PROTAGINE_PROJECTS_MAX_REPLANS` (default 3).
...
- `skills_memory/` package (name avoids clash with existing `skills/` executor
  ...
  - `models.Skill`: id, title, situation_signature (normalized terms +
    ...
    last_used, decayed. `store.SkillStore` (SQLite `protagine-skills.db`).
...
Config: `PROTAGINE_SKILLS_ENABLED` (default true - read/inform only),
`PROTAGINE_SKILLS_DISTILL` (default shadow: log the distilled skill without
...
Design:
- `self_model/` package.
  - `store.CompetenceStore` (SQLite `protagine-self-model.db`) keyed by domain
  ...
  - `journal.py`: unified ActionJournal (SQLite `protagine-action-journal.db`):
  ...
  - `trust.py`: TrustEngine.
    ...
    - Auto-graduation on threshold crossings (PROTAGINE_TRUST_ASK_THRESHOLD
      default 0.45/n>=3, PROTAGINE_TRUST_ACT_THRESHOLD default 0.8/n>=5), owner
      notification (not a request) through guarded delivery on each
      graduation; PROTAGINE_TRUST_AUTOGRADUATE=false disables.
...
- Adaptive delivery cap: the per-recipient daily cap scales with the
  "delivery" domain's earned confidence (base PROTAGINE_MAX_DAILY, earned
  upward, bounded by PROTAGINE_TRUST_DELIVERY_CAP_MAX).
...
Config: `PROTAGINE_SELF_MODEL_ENABLED` (default true), PROTAGINE_TRUST_* thresholds
...
Design:
- `beliefs/` package operating over ProtagineGraph + world-model.
...
Config: `PROTAGINE_BELIEFS_MODE` off|shadow|live (shadow: detect + log + surface
...
override `PROTAGINE_SOURCE_TRUST`.

---

## Item 5 - PROTAGINE WORKERS [Phase B, riskiest]
...
- Server side (public, Protagine):
  ...
  - Post-completion audit (reuse `directed/audit.py` pattern) + feedback + self
    model + skill distill.
- Worker daemon (public, Protagine, installable): `workers/protagine_worker.py`
  (extend existing `workers/`): authenticates to the sidecar (PROTAGINE_API_KEY),
...
Config flags: `PROTAGINE_WORKERS_MODE`, `PROTAGINE_WORKER_NODE_ID`, `PROTAGINE_WORKER_MAX_JOBS`, `PROTAGINE_WORKER_CAPABILITIES`,
`PROTAGINE_WORKER_LLM_BASE_URL`/`_MODEL`/`_API_KEY`, `PROTAGINE_WORKER_POLL_SECS`,
server-side `PROTAGINE_WORKERS_MODE` off|shadow|live (shadow: accept registration
...
their LLM endpoints, capabilities per host, launchd plists, PROTAGINE_API_KEY
...
Config: `PROTAGINE_SANDBOX_MODE` off|dry_run|live (dry_run: validate + log the
would-run command, execute nothing), `PROTAGINE_SANDBOX_IMAGE`,
`PROTAGINE_SANDBOX_CPUS`/`_MEMORY`/`_TIMEOUT`/`_EGRESS` (none|allowlist),
`PROTAGINE_SANDBOX_MAX_ARTIFACT_BYTES`. Default off (Docker may be absent on the
...
Config: `PROTAGINE_CONNECTORS_ENABLED`, per-connector `PROTAGINE_CONNECTOR_<NAME>_*`
(enabled, endpoint, token, poll_secs), `PROTAGINE_CONNECTORS_MODE` off|shadow|live
...
register_tool (protagine toolset), register_command with register_slash_command
...
- Item 3 (Skills): Hermes 0.18 has a full agent-facing skills system plus
  /learn and /journey. Protagine's skills_memory remains sidecar-internal
...
- Item 5 (Workers): Hermes kanban's dispatcher/worker model and the delegation
  ...
  work execution with process isolation. Protagine's value-add is SERVER-SIDE
...
- ESCALATION MINER (`sidecar/protagine/mining/`): detects escalation
  events in the live turn stream: build-agent consultations (terminal-class
  tool ran AND text matches PROTAGINE_ESCALATION_CONSULT_REGEX) and heavy-model /
  cloud-failover turns (new optional per-turn `model` field on turns/sync,
  matched against PROTAGINE_ESCALATION_HEAVY_RE). Records bank task context, the
  prior local attempt in-session, the escalated answer, channel, model, ts,
  and a lightweight next-turn outcome into `protagine-mining.db`. Consumers:
  ...
  PROTAGINE_ESCALATION_MINING=off|shadow|live, default shadow.
- TRAINING-CORPUS EXPORTER (`mining/corpus.py` + POST
  ...
  via PROTAGINE_OWNER_CONTACT_ID), quality gate (reachout_policy sanitize +
  is_system_origin, machine-marker stripping, cron/self exclusion), dedup,
  optional redact. PII stance: everything stays under PROTAGINE_STATE_DIR;
...
- Pre-req landed earlier this session (already pushed at tip 2726698):
  ...
  delivery go-LIVE flip (Protagine side).
- RESOLVED (was: KNOWN DEPLOYMENT BLOCKER): proactive delivery now runs
  LIVE via PROTAGINE_DELIVERY_TRANSPORT=gateway to the deployment's message
...
- 2026-07-04 (ops flips, owner-ordered, deployment layer): PROTAGINE_DIRECTED_MODE
  ...
  report was correctly gated by the delivery cooldown). PROTAGINE_THINKING_MODE
  ...
  as internal work). PROTAGINE_WORLD_POPULATE_MODE shadow -> live (first real
  entity writes observed same day). PROTAGINE_DIRECTIVE_LLM_ASSIST off -> on
  (verified against the deployment's fast classifier endpoint). The former
  Protagine-side delivery blocker is resolved at the deployment layer
  (PROTAGINE_DELIVERY_TRANSPORT=gateway); delivery is LIVE with rate caps
...
- Phase A: COMPLETE (2026-07-04). Landed:
  - `self_model/` (item 4 as trust engine): CompetenceStore + event log,
    self-brief, ActionJournal (`protagine-action-journal.db`), TrustEngine
    ...
    PROTAGINE_SELF_MODEL_ENABLED (true), PROTAGINE_TRUST_* thresholds.
  - `skills_memory/` (item 3): SkillStore (`protagine-skills.db`), distillation
    ...
    PROTAGINE_SKILLS_ENABLED (true), PROTAGINE_SKILLS_DISTILL (shadow default;
    live on the reference deployment), PROTAGINE_SKILLS_MAX.
  - `projects/` (item 1): Project/Step models + ProjectStore
    (`protagine-projects.db`), planner (one LLM pass, deterministic
    ...
    Flags: PROTAGINE_PROJECTS_MODE (shadow default), _MAX_STEPS,
  ...
  - `beliefs/` (item 7): claim extraction (conservative), conflict
    ...
    PROTAGINE_SOURCE_TRUST), supersession audit (`protagine-beliefs.db`), inline
  ...
  - Amendment extras: one-command global pause (extractor + guard +
    ...
    journaled writes, PROTAGINE_WORLD_LLM_EXTRACT), action_journal +
...
- 2026-07-04 (live course-corrections, the amendment posture working as
  ...
  world sqlite store is now anchored to PROTAGINE_STATE_DIR (was cwd-relative
...
- Phase B: COMPLETE (2026-07-04, successor session). Landed:
  - `task_queue/governor.py` (item 5, server-side enforcement): WorkerGovernor
    ...
    Mode PROTAGINE_WORKERS_MODE off|shadow|live (default shadow = CALIBRATION:
  ...
  - `workers/protagine_worker.py` (item 5, installable worker daemon): stdlib-only
    console script `protagine-worker`; authenticates (PROTAGINE_API_KEY), registers
    capability-typed (PROTAGINE_WORKER_CAPABILITIES/_JOB_TYPES), polls claim,
  ...
  - `sandbox/` (item 6, exploration sandbox): SandboxBackend ABC + DockerSandbox
    ...
    gate (PROTAGINE_SANDBOX_MODE off|dry_run|live, default off), a DirectiveGuard
  ...
  - Flags added: PROTAGINE_WORKERS_MODE (shadow), PROTAGINE_WORKER_CAPABILITIES/
    _JOB_TYPES/_POLL_SECS/_LLM_BASE_URL/_LLM_MODEL/_LLM_API_KEY,
    PROTAGINE_SANDBOX_MODE (off), PROTAGINE_SANDBOX_IMAGE/_CPUS/_MEMORY/_TIMEOUT/
    _PIDS/_EGRESS/_MAX_ARTIFACT_BYTES.
  - Private deployment layer (private repo + live host): which hosts run
    protagine-worker, their LLM endpoints + capabilities per host, the filled-in
    workers/deploy unit (PROTAGINE_API_KEY provisioning), and -- if a sandbox host
...
- Phase C: COMPLETE (2026-07-04, successor session). Landed:
  - `connectors/` (item 2, senses / connector framework): base Connector ABC +
    ...
    ConnectorConfig (env-only PROTAGINE_CONNECTOR_<NAME>_*, no creds in code).
    ...
    pass. Mode PROTAGINE_CONNECTORS_MODE off|shadow|live (default off; shadow =
  ...
  - Flags added: PROTAGINE_CONNECTORS_ENABLED, PROTAGINE_CONNECTORS_MODE (off),
    PROTAGINE_CONNECTOR_IMAP_* (HOST/PORT/USER/PASSWORD/MAILBOX/MAX/ENABLED/
    POLL_SECS), PROTAGINE_CONNECTOR_CALENDAR_* (ICS_URL/USER/PASSWORD/MAX),
    PROTAGINE_CONNECTOR_FS_* (PATH/EXTENSIONS/MAX), PROTAGINE_CONNECTOR_WEBHOOK_*
...
- PROGRAM STATUS: PROGRAM CLOSED (2026-07-04). All seven capabilities
  ...
  (PROTAGINE_WORKERS_MODE=shadow, PROTAGINE_SANDBOX_MODE=off,
  PROTAGINE_CONNECTORS_MODE=off) so a future code-default change cannot
...
- Repo-hygiene scrub candidates (legacy references that predate this
  ...
  PROTAGINE_WORK_REPO env-driven checkout path, prose trimmed.
- Directed-action delegate shim replacement: the deployment's interim
  ...
  (protagine-worker claiming directed jobs under the WorkerGovernor, or the
...
- Worker fleet rollout (deployment layer): place protagine-worker on hosts
  per the Phase B private deployment layer notes; graduate
  PROTAGINE_WORKERS_MODE shadow -> live once the shadow calibration record
  looks clean.
