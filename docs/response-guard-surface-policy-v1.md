...
Protagine plugin behavior and does not patch Hermes core.
...
enforce. Its owner exemption is derived from Protagine's identity resolver, not
...
- The Protagine Hermes plugin declares `text_chat`.
- Protagine proactive outreach declares `proactive_text`, including its single
...
- Protagine deliberately leaves the Tom2 applied-enforcement evidence probe unset,
...
- `PROTAGINE_GUARD_EXCLUDED_GATEWAYS` is no longer consumed. The constructor
...
mutation. Before a later canary, keep `PROTAGINE_GUARD_MODE=shadow` and the Hermes
...
`off` and Protagine guard mode to `shadow`, then roll back the pinned Protagine
