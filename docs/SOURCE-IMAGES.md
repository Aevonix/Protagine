...
```
$PROTAGINE_STATE_DIR/images/sources/originals/<sha256>.<extension>
$PROTAGINE_STATE_DIR/images/sources/thumbs/<sha256>.jpg
```
...
There is no public static file route. The existing `protagine_memory_read_source`
...
false. Protagine does not select a new model or silently recaption the original.
...
Protagine verifies the exact source, ownership, asset and read revision again.
...
The packaged source-reader test uses installed Protagine tools, the real scoped
