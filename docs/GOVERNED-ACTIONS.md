...
Protagine exposes one narrow mutation boundary for an external, owner-approved
action worker:
...
The PUT body is the exact bounded `ProtagineGovernedActionExecutionV1`
...
Approval data is evidence only: Protagine derives the participant exclusively
...
secret-free `ProtagineGovernedActionExecutionResultV1` projection.

`protagine_research` is a fast handoff, not an inline claim of completed
...
and `PROTAGINE_PROJECTS_MODE` must be `live`, before preparation succeeds. A
...
mode-0700 directory under the Protagine state directory. The database is an
...
the prior Protagine revision. Preserve `governed-actions/ledger.db` with the rest
...
completed and uncertain actions from being dispatched twice. Protagine's full
...
Before rolling Protagine back to a revision that predates governed Project
...
`protagine-projects.db` alongside the governed-action ledger so an exact replay
