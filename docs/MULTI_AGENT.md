# Protagine Multi-Agent Architecture
...
An **agent** is any system that connects to Protagine to receive work or provide capabilities:
...
An **initiative** is a unit of work that Protagine wants an agent to perform:

- Generated by the mind tick from overdue commitments, reply waits and observations, or by users and agents.
...
On your Protagine host:

```bash
protagine agent invite --capabilities messaging,calendar --primary
```

Output:
```
Setup code: PROTAGINE-7X9K-M2P4-QR8W
Expires at: 2026-04-25T18:00:00Z

Run on remote agent:
  protagine agent connect --setup-code PROTAGINE-7X9K-M2P4-QR8W --protagine-url https://protagine.example.com
```
...
Node ID: node-xyz789
Protagine ID: protagine-1
WebSocket URL: wss://protagine.example.com/v1/host/agents/agent-abc123/stream

Config saved to: ~/.protagine/agent.json
```

### 3. Run an Agent

Protagine supports several agent paths out of the box:
...
1. Generate setup code on Protagine host: `protagine agent invite`
2. Connect from remote: `protagine agent connect --setup-code XXX`
...
```bash
# Invite remote agent
protagine agent invite [--expires 900] [--capabilities messaging,calendar] [--primary]

# Connect as remote agent
protagine agent connect --setup-code PROTAGINE-XXXX-XXXX-XXXX --protagine-url https://...

# List agents
protagine agent list [--status online]

# Show agent details
protagine agent show <agent_id>

# Revoke agent
protagine agent revoke <agent_id>

# Disconnect this agent
protagine agent disconnect
```
...
```bash
# List initiatives
protagine initiative list [--status pending] [--agent <id>]

# Show initiative details
protagine initiative show <id>

# Cancel initiative
protagine initiative cancel <id> [--reason "reason"]
```
...
# Load config from ~/.protagine/agent.json
client = AgentClient()
...
|----------|-------------|---------|
| `PROTAGINE_URL` | Public URL for remote agents | — |
| `PROTAGINE_SIDECAR_HOST` | Bind host | `127.0.0.1` |
| `PROTAGINE_SIDECAR_PORT` | Bind port | `7777` |
| `PROTAGINE_API_KEY` | Legacy global API key (migration compatibility) | — |
| `PROTAGINE_API_KEYRING_PATH` | Mode-private scoped-principal keyring | — |
| `PROTAGINE_API_CONTACT_GRANTS_PATH` | Mode-private server-attested exact-contact projection | `$PROTAGINE_STATE_DIR/api-contact-grants.json` |
| `PROTAGINE_AUTH_TELEMETRY_PATH` | Privacy-safe durable auth migration counters | `$PROTAGINE_STATE_DIR/protagine-auth-telemetry.db` |
| `PROTAGINE_AUTH_LEGACY_QUIET_HOURS` | Doctor's required no-legacy observation window | `24` |
| `PROTAGINE_TIMEZONE` | IANA timezone | `UTC` |

### Agent Config (`~/.protagine/agent.json`)
...
```json
{
  ...
  "websocket_url": "wss://protagine.example.com/v1/host/agents/agent-abc123/stream",
  "name": "laptop",
  "capabilities": ["messaging", "calendar"],
  "is_primary": false,
...
2. Verify Protagine URL is reachable
3. Check firewall allows WebSocket connections
4. Run `protagine doctor` for diagnostics

### Initiatives Not Being Assigned

1. Check agent status: `protagine agent show <id>`
