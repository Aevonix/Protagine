...
:mod:`protagine_hostworker.conformance`.  A store that satisfies the method
...
* *gate* — the receipt of ``kind == "gate"`` holding the owner-approval
  evidence validated by :func:`protagine_hostworker.gate.validate_owner_gate`.
...
I4. IN-TRANSACTION GATE RE-VALIDATION.  ``begin_owner_authorized_dispatch``
    ...
    :class:`~protagine_hostworker.gate.GateAuthorization`, returns one marked
    ...
    def begin_owner_authorized_dispatch(
        ...
        *,
        ...
        4. re-read the durable receipts and CALL ``gate_validator(action,
           ...
           :class:`~protagine_hostworker.gate.GateAuthorization` whose
