"""Stdlib-only core for governed Protagine host workers.
...
admission, the :class:`~protagine_hostworker.store.ActionStore` protocol with
...
(:mod:`protagine_hostworker.conformance`).  This distribution must never
import FastAPI or ``protagine`` — see the design rule in
:mod:`protagine_hostworker.contract`.
"""
...
try:  # single source of truth: the installed distribution metadata
    ...
    try:
        __version__ = _dist_version("protagine-hostworker")
