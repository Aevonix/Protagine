...
:mod:`protagine_hostworker.store` through the public store API only, using a
...
A host must pass this suite with its own harness before running live:
    ...
    from protagine_hostworker.conformance import assert_store_conformance
...
def check_non_grantable_tool_with_grant_proof(factory: HarnessFactory) -> None:
    ...
    for tool_name in ("protagine_autonomy_enable", "protagine_autonomy_disable"):
        harness = factory()
        try:
            scenario = _gated(
                harness,
