...
A host proves its :class:`~protagine_hostworker.store.ActionStore` adapter by
...
:func:`protagine_hostworker.conformance.run_store_conformance`.
...
(``python -m protagine_hostworker.conformance``).
"""
...
class StoreHarness(Protocol):
    ...
    ``store`` must implement :class:`~protagine_hostworker.store.ActionStore`.
...
class SqliteStoreHarness:
    ...
    def __init__(
        ...
        *,
        ...
        self._temp = (
            tempfile.mkdtemp(prefix="protagine-hostworker-conformance-")
...
def build_intent(
    *,
    tool_name: str = "protagine_create_commitment",
