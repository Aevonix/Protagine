...
:class:`~protagine_hostworker.store.ActionStore` and any dispatcher exposing
``execute`` (one PUT) and ``observe`` (side-effect-free GET), such as
:class:`~protagine_hostworker.client.GovernedActionClient`.
...
store's half is specified in :mod:`protagine_hostworker.store`):
...
def build_execution_request(
    ...
    """Build the immutable ``ProtagineGovernedActionExecutionV1`` document.
...
class GovernedActionWorker:
    ...
    def _gate_validator(self, tool_name: str):
        ...
        :func:`~protagine_hostworker.gate.assert_dispatchable`.
        """
