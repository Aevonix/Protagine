"""Single source of truth for the governed Protagine tool catalog.
...
``protagine_autonomy_enable`` and ``protagine_autonomy_disable`` are marked
...
grant was issued.  The gate core (:mod:`protagine_hostworker.gate`) fails closed
...
independent validators (Protagine's endpoint ``_validate_args`` and the private
worker's intent ``_validate_args``); the repo-internal agreement test pins
that equivalence against Protagine's endpoint, which deliberately keeps its own
copy (see the design rule in :mod:`protagine_hostworker.contract`).
"""
...
def _display_autonomy_disable(_args: Mapping[str, Any]) -> dict[str, str]:
    return {
        "summary": "Disable Protagine autonomous scheduling",
        "target": "Protagine autonomy scheduler",
...
def _display_autonomy_enable(_args: Mapping[str, Any]) -> dict[str, str]:
    return {
        "summary": "Enable Protagine autonomous scheduling",
        "target": "Protagine autonomy scheduler",
        "risk": (
            "Protagine may begin bounded autonomous work under its configured policies"
        ),
    }
...
def _display_create_commitment(args: Mapping[str, Any]) -> dict[str, str]:
    return {
        "summary": "Create Protagine commitment %s" % _preview(args["description"]),
        "target": "Private Protagine commitment ledger",
        "risk": (
...
def _display_initiative_feedback(args: Mapping[str, Any]) -> dict[str, str]:
    return {
        ...
        "target": "Private Protagine initiative ledger",
        "risk": "Changes Protagine's internal initiative state and learning evidence",
    }
...
def _display_record_insight(args: Mapping[str, Any]) -> dict[str, str]:
    return {
        ...
        "target": "Private Protagine memory and learning store",
        "risk": (
...
def _display_research(args: Mapping[str, Any]) -> dict[str, str]:
    return {
        "summary": "Start %s Protagine research on %s"
        % (args.get("depth", "quick"), _preview(args["topic"])),
        "target": "Protagine research queue",
        "risk": (
...
def _display_task_complete(args: Mapping[str, Any]) -> dict[str, str]:
    return {
        "summary": "Complete Protagine task %s" % _preview(args["task_id"]),
        "target": "Private Protagine task or initiative ledger",
...
def _display_task_dismiss(args: Mapping[str, Any]) -> dict[str, str]:
    return {
        "summary": "Dismiss Protagine task %s as %s"
...
def _display_task_snooze(args: Mapping[str, Any]) -> dict[str, str]:
    return {
        "summary": "Snooze Protagine task %s for %d hours"
...
TOOL_CATALOG: dict[str, ToolSpec] = {
    ...
    for spec in (
        ToolSpec(
            name="protagine_autonomy_disable",
            model_description=(
        ...
        ToolSpec(
            name="protagine_autonomy_enable",
            model_description=(
        ...
        ToolSpec(
            name="protagine_create_commitment",
            model_description=(
        ...
        ToolSpec(
            name="protagine_research",
            model_description=(
                "Submit a governed intent to queue durable Protagine research."
            ),
            model_parameters=_parameters(
                {
                    "depth": {
...
def validate_tool_args(tool_name: Any, raw: Any) -> dict[str, Any]:
    ...
    if spec is None:
        raise ToolCatalogError("tool is not a governed Protagine action")
