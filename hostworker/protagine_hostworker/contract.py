"""Stateless wire contract for governed Protagine host-worker actions.
...
=============================================================
Protagine's endpoint (``sidecar/protagine/governed_actions.py``) MUST KEEP
ITS OWN INDEPENDENT VALIDATOR.  Do NOT refactor the endpoint to import
``protagine_hostworker``.  The two implementations are deliberately separate and
...
* UTF-8 (``canonical_json_utf8`` / ``sha256_json_utf8``): the host Action
  ...
  ``ProtagineGovernedActionExecutionV1``, the returned ``effect_digest``, the
...
EXECUTION_REQUEST_SCHEMA = "ProtagineGovernedActionExecutionV1"
APPROVAL_BINDING_SCHEMA = "ProtagineOwnerApprovalExecutionBindingV1"
EXECUTION_RESULT_SCHEMA = "ProtagineGovernedActionExecutionResultV1"
EFFECT_SCHEMA = "ProtagineGovernedActionEffectV1"
...
# The approval-id form Protagine's endpoint already enforces on the wire.
