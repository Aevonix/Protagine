"""Stdlib-only core for governed PacoMind host workers.
...
admission, the :class:`~pacomind_hostworker.store.ActionStore` protocol with
...
(:mod:`pacomind_hostworker.conformance`).  This distribution must never
import FastAPI or ``pacomind`` — see the design rule in
:mod:`pacomind_hostworker.contract`.
"""
...
try:  # single source of truth: the installed distribution metadata
    ...
    try:
        __version__ = _dist_version("pacomind-hostworker")
