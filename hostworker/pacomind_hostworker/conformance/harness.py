...
A host proves its :class:`~pacomind_hostworker.store.ActionStore` adapter by
...
:func:`pacomind_hostworker.conformance.run_store_conformance`.
...
(``python -m pacomind_hostworker.conformance``).
"""
...
class StoreHarness(Protocol):
    ...
    ``store`` must implement :class:`~pacomind_hostworker.store.ActionStore`.
...
class SqliteStoreHarness:
    ...
    def __init__(
        ...
        *,
        ...
        self._temp = (
            tempfile.mkdtemp(prefix="pacomind-hostworker-conformance-")
...
def build_intent(
    *,
    tool_name: str = "pacomind_create_commitment",
