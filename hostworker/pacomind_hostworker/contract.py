"""Stateless wire contract for governed PacoMind host-worker actions.
...
=============================================================
PacoMind's endpoint (``sidecar/pacomind/governed_actions.py``) MUST KEEP
ITS OWN INDEPENDENT VALIDATOR.  Do NOT refactor the endpoint to import
``pacomind_hostworker``.  The two implementations are deliberately separate and
...
* UTF-8 (``canonical_json_utf8`` / ``sha256_json_utf8``): the host Action
  ...
  ``PacoMindGovernedActionExecutionV1``, the returned ``effect_digest``, the
...
EXECUTION_REQUEST_SCHEMA = "PacoMindGovernedActionExecutionV1"
APPROVAL_BINDING_SCHEMA = "PacoMindOwnerApprovalExecutionBindingV1"
EXECUTION_RESULT_SCHEMA = "PacoMindGovernedActionExecutionResultV1"
EFFECT_SCHEMA = "PacoMindGovernedActionEffectV1"
...
# The approval-id form PacoMind's endpoint already enforces on the wire.
