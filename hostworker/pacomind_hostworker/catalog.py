"""Single source of truth for the governed PacoMind tool catalog.
...
``pacomind_autonomy_enable`` and ``pacomind_autonomy_disable`` are marked
...
grant was issued.  The gate core (:mod:`pacomind_hostworker.gate`) fails closed
...
independent validators (PacoMind's endpoint ``_validate_args`` and the private
worker's intent ``_validate_args``); the repo-internal agreement test pins
that equivalence against PacoMind's endpoint, which deliberately keeps its own
copy (see the design rule in :mod:`pacomind_hostworker.contract`).
"""
...
def _display_autonomy_disable(_args: Mapping[str, Any]) -> dict[str, str]:
    return {
        "summary": "Disable PacoMind autonomous scheduling",
...
def _display_autonomy_enable(_args: Mapping[str, Any]) -> dict[str, str]:
    return {
        ...
        "target": "PacoMind autonomy scheduler",
        "risk": (
            "PacoMind may begin bounded autonomous work under its configured policies"
        ),
    }
...
def _display_create_commitment(args: Mapping[str, Any]) -> dict[str, str]:
    return {
        "summary": "Create PacoMind commitment %s" % _preview(args["description"]),
        "target": "Private PacoMind commitment ledger",
        "risk": (
...
def _display_initiative_feedback(args: Mapping[str, Any]) -> dict[str, str]:
    return {
        ...
        "target": "Private PacoMind initiative ledger",
        "risk": "Changes PacoMind's internal initiative state and learning evidence",
    }
...
def _display_record_insight(args: Mapping[str, Any]) -> dict[str, str]:
    return {
        ...
        "target": "Private PacoMind memory and learning store",
        "risk": (
...
def _display_research(args: Mapping[str, Any]) -> dict[str, str]:
    return {
        "summary": "Start %s PacoMind research on %s"
        % (args.get("depth", "quick"), _preview(args["topic"])),
        "target": "PacoMind research queue",
        "risk": (
...
def _display_task_complete(args: Mapping[str, Any]) -> dict[str, str]:
    return {
        "summary": "Complete PacoMind task %s" % _preview(args["task_id"]),
...
def _display_task_dismiss(args: Mapping[str, Any]) -> dict[str, str]:
    return {
        "summary": "Dismiss PacoMind task %s as %s"
...
def _display_task_snooze(args: Mapping[str, Any]) -> dict[str, str]:
    return {
        "summary": "Snooze PacoMind task %s for %d hours"
...
TOOL_CATALOG: dict[str, ToolSpec] = {
    ...
    for spec in (
        ToolSpec(
            name="pacomind_autonomy_disable",
            model_description=(
        ...
        ToolSpec(
            name="pacomind_create_commitment",
            model_description=(
        ...
        ToolSpec(
            name="pacomind_research",
            model_description=(
                "Submit a governed intent to queue durable PacoMind research."
            ),
            model_parameters=_parameters(
                {
                    "depth": {
...
def validate_tool_args(tool_name: Any, raw: Any) -> dict[str, Any]:
    ...
    if spec is None:
        raise ToolCatalogError("tool is not a governed PacoMind action")
