...
from protagine_hostworker.contract import GovernedContractError, sha256_json_ascii
...
def test_autonomy_tools_are_non_grantable():
    ...
    assert NON_GRANTABLE_TOOL_NAMES == frozenset(
        {"protagine_autonomy_enable", "protagine_autonomy_disable"}
    )
...
def test_unknown_tool_is_refused():
    with pytest.raises(ToolCatalogError):
        validate_tool_args("protagine_send_message", {})
    with pytest.raises(ToolCatalogError):
...
def test_approval_display_matches_deployed_worker_wording():
    display = TOOL_CATALOG["protagine_task_snooze"].approval_display(
        {"task_id": "task-1", "hours": 3}
    )
    assert display == {
        "summary": 'Snooze Protagine task "task-1" for 3 hours',
    ...
    display = TOOL_CATALOG["protagine_autonomy_enable"].approval_display({})
    assert display == {
        "summary": "Enable Protagine autonomous scheduling",
        "target": "Protagine autonomy scheduler",
        "risk": (
            "Protagine may begin bounded autonomous work under its configured "
            "policies"
        ),
    }
    ...
    display = TOOL_CATALOG["protagine_create_commitment"].approval_display(
        {"description": "d" * 400}
    )
    assert display["summary"] == (
        'Create Protagine commitment "%s\\u2026"' % ("d" * 299)
    )
...
def test_every_tool_has_display_metadata():
    samples = {
        "protagine_autonomy_disable": {},
        "protagine_autonomy_enable": {},
        "protagine_create_commitment": {"description": "x"},
        "protagine_initiative_feedback": {
            "initiative_id": "i-1", "action": "actioned",
        },
        "protagine_record_insight": {"content": "x", "insight_type": "fact"},
        "protagine_research": {"topic": "x"},
        "protagine_resolve_commitment": {"commitment_id": "c-1"},
        "protagine_task_complete": {"task_id": "t-1"},
        "protagine_task_dismiss": {"task_id": "t-1"},
        "protagine_task_snooze": {"task_id": "t-1"},
    }
