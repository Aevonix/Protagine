...
from protagine_hostworker.catalog import TOOL_CATALOG
from protagine_hostworker.gate import (
    BOUNDED_GRANT_SHAPE,
    GATE_COMMON_FIELDS,
...
def test_non_grantable_fails_closed_regardless_of_configuration(
    ...
    with pytest.raises(OwnerGateError):
        validate_owner_gate(
            case["action"],
            case["receipts"],
            tool_name="protagine_autonomy_enable",
            now=case["now"],
    ...
    reduced = {
        ...
        if name != "protagine_autonomy_enable"
    }
    with pytest.raises(OwnerGateError):
        validate_owner_gate(
            case["action"],
...
def test_custom_shape_expiries_are_reenforced(golden_vectors):
    ...
    live = validate_owner_gate(
        ...
        tool_name="protagine_record_insight",
        now=now,
        registry=registry,
    )
