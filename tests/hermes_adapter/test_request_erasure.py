...
home.joinpath('config.yaml').write_text(json.dumps({
    ...
    'plugins': {'enabled': ['protagine'], 'protagine': {'owner_contact_id': 'contact-a', 'url': 'http://fixture'}},
    'memory': {'provider': 'protagine-memory', 'config': {'contact_id': 'contact-a', 'url': 'http://fixture'}}}))
...
assert fact in recalled and 'native-erasure-source' in recalled and 'protagine-recall-v1' in recalled, recalled
temporal_only='[protagine-recall-v1 {"contact_id":"contact-a","watermark":0}]\n## Current Time [priority 100]\nOld clock\n[/protagine-recall-v1]'
refreshed=provider._with_fresh_temporal_sync(temporal_only, contact_id='contact-a')
assert 'Old clock' not in refreshed and '[/protagine-recall-v1]' in refreshed
...
if summary_mode:
    tool_response = NS(choices=[NS(message=NS(content='',tool_calls=[
        NS(id='neutral-lookup',type='function',function=NS(name='protagine_get_facts',arguments='{}'))]),
...
with patch(OPENAI_TARGET,return_value=client), patch(TOOLS_TARGET + '.get_tool_definitions',return_value=[]), patch(TOOLS_TARGET + '.check_toolset_requirements',return_value={}):
    ...
    # The supported Protagine request middleware preserves evidence and lineage
    ...
    packets=[line for row in supplied for line in str(row.get('content','')).splitlines()
             if line.startswith('[protagine-recall-v1 ')]
    assert len(packets)==1,packets
    stamp=json.loads(packets[0][len('[protagine-recall-v1 '):-1])
    ...
    forgotten=json.loads(handle_function_call('protagine_memory_forget', {'source_ids':['native-erasure-source']},
    ...
    repeat=json.loads(handle_function_call('protagine_memory_forget', {'source_ids':['native-erasure-source']},
        session_id='forget-request',task_id='forget-task',turn_id='forget-turn'))
    ...
    for args, session, task, turn in [
        ...
        denied=json.loads(handle_function_call('protagine_memory_forget',args,session_id=session,task_id=task,turn_id=turn))
    ...
    denied=json.loads(handle_function_call('protagine_memory_forget', {'source_ids':['native-erasure-source']},
...
skill=home/'skills'/'protagine-erasure-fixture'/'SKILL.md'
skill.parent.mkdir(parents=True)
def write_skill(revision):
    skill.write_text('---\nname: protagine-erasure-fixture\ndescription: Index manuals using '+revision+'.\n---\n'
...
with patch('protagine_hermes.skill_context.SkillContext.__call__',side_effect=OSError('fixture skill read failed')) as refresh_failure:
...
def test_native_persisted_recall_is_not_resent_after_forget(artifacts, tmp_path, mode):
    ...
    run_python('-I','-c',PROBE, artifacts[3], ROOT/'sidecar',
               os.environ.get('PROTAGINE_TEST_DEPENDENCY_PATH',''), mode, cwd=tmp_path, env=env)
