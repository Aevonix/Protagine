...
from protagine_hermes.local_work_runner import main
...
def response(**kwargs):
    ...
    if not rows:
        if MODE=='cancel':
            ...
            calls=[call('tool_call',{'name':'protagine_read_work_source','arguments':{'source':0}})]
        else:
        ...
        if 'protagine_read_work_source' not in search['tools']:
            ...
            assert any(source['name']=='protagine_local_work' for source in sources),search
            calls=[call('tool_search',{'queries':['protagine_local_work read source']})]
        else:
            calls=[call('tool_call',{'name':'protagine_read_work_source','arguments':{'source':0}})]
    else:
        if MODE!='cancel':
...
def initialize(self,*args,**kwargs):
    ...
    def observed_conversation(*a,**k):
        ...
        from protagine_hermes.local_work import ACTIVE
        work=ACTIVE.get()
...
async def authority(request,next_call):
    request.state.protagine_authority=RequestAuthority(principal_id='fixture-native',credential_id='fixture',scopes=frozenset({'turns:write','context:read'}),viewer_person_id='owner',person_ids=frozenset({'owner'}),audiences=frozenset({'viewer'}),authenticated=True)
...
config={'plugins':{'enabled':['protagine'],'protagine':{'owner_contact_id':'owner','url':base,'attested_system_platforms':['cli'],'turn_writer_platforms':[]}},
...
os.environ['PROTAGINE_LOCAL_WORK_JOB_ID']=job['id']
...
accepted=json.loads(handle_function_call('protagine_accept_local_draft',accept_args,session_id='owner-chat',task_id='owner-task',turn_id='owner-turn',tool_call_id='accept'))
...
def test_native_accepted_draft_lifecycle(artifacts, tmp_path, mode):
    ...
    env.update(HERMES_HOME=str(tmp_path/'profile'),PROTAGINE_STATE_DIR=str(tmp_path/'state'),
        ...
        PROTAGINE_SKIP_DOTENV='1',PROTAGINE_LOCAL_WORK_ENABLED='true',PROTAGINE_OWNER_CONTACT_ID='owner',
        LITELLM_LOCAL_MODEL_COST_MAP='True')
    result=run_python('-I','-c',PROBE,artifacts[3],ROOT/'sidecar',os.environ.get('PROTAGINE_TEST_DEPENDENCY_PATH',''),mode,WORKER,cwd=tmp_path,env=env)
...
from protagine_hermes.model_response import decode_json_response
