...
context=ProtagineMemoryProvider.__new__(ProtagineMemoryProvider)._format_sections(sections)
...
from protagine_hermes.skill_context import SkillContext, _tool_names
...
from protagine.setup_skills import BUNDLE_PREFIX
from protagine.setup_hermes import _adapter_resources
...
assert 'protagine-deep-research' in bundled
...
def test_native_skill_index_and_view_remain_authoritative(artifacts, tmp_path):
    native = os.environ.get('PROTAGINE_TEST_HERMES_PATH') or os.environ.get('HERMES_TEST_SOURCE', '')
    ...
    env.update(PROTAGINE_SKIP_DOTENV='1', PYTHON_DOTENV_DISABLED='1', LITELLM_LOCAL_MODEL_COST_MAP='True')
