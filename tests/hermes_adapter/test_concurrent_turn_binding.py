...
original_get=protagine_hermes.ProtagineClient.get
...
assert plugin_manager._plugins['protagine'].enabled
...
try:
    with patch.object(condition,'wait',side_effect=observe_wait),patch(openai_target,side_effect=clients),patch(tools_target+'.get_tool_definitions',return_value=definitions),patch(tools_target+'.check_toolset_requirements',return_value={}):
        for index in range(2):
            ...
            manager=MemoryManager(); manager.add_provider(load_memory_provider('protagine-memory'))
    ...
    for index in range(2):
        ...
        scope=protagine_hermes._TRANSPORT_SCOPES.for_session(agents[index].session_id)
        ...
        summary.append({'index':index,'recall':expected_fact in first_request,'work':'[protagine-work-request-v1]' in first_request,
...
def test_actual_overlapping_native_starts_preserve_recall_authorized_tools_and_work(artifacts,tmp_path,second_person):
    ...
    env.update(HERMES_HOME=str(tmp_path/'profile'),PROTAGINE_STATE_DIR=str(tmp_path/'protagine'),
        ...
        PROTAGINE_GUARD_CHAT_MODE='off',PROTAGINE_OWNER_CONTACT_ID='owner',PROTAGINE_SKIP_DOTENV='1',LITELLM_LOCAL_MODEL_COST_MAP='True')
    result=run_python('-I','-c',PROBE,artifacts[3],ROOT/'sidecar',os.environ.get('PROTAGINE_TEST_DEPENDENCY_PATH',''),second_person,native,cwd=tmp_path,env=env)
