...
for index in range(4):
    opened=dispatch(args,session='reader',task='reader-task',turn='reader-turn',
        call='document-read-'+str(index),tool='protagine_memory_read_source')
...
second=dispatch({**ref,'view':'document','asset_hash':asset,'page':2},
    session='reader',task='reader-task',turn='reader-turn',call='document-second',tool='protagine_memory_read_source')
...
assert 'error' in dispatch({**ref,'view':'document','asset_hash':asset,'page':3},
    session='reader',task='reader-task',turn='reader-turn',call='document-missing',tool='protagine_memory_read_source')
...
for result,selector in zip(results,selectors):
    messages.extend([{'role':'assistant','content':'','tool_calls':[{
        'id':result['tool_call_id'],'type':'function','function':{
            'name':'protagine_memory_read_source','arguments':json.dumps(selector)}}]},result])
...
fresh=dispatch({**ref,'view':'document','asset_hash':asset,'page':2},
    session='reader',task='reader-task',turn='reader-turn',call='document-corrected',tool='protagine_memory_read_source')
