...
def test_sidecar_wheel_contains_product_without_embedded_tests(tmp_path):
    run_python("-m", "build", "--wheel", "--no-isolation", "--outdir", tmp_path,
               cwd=ROOT / "sidecar")
    wheel = next(tmp_path.glob("apsimo-*.whl"))
    ...
    run_python("-c", "from apsimo.briefings.aggregators import CalendarAggregator; "
               "from apsimo.events.bus import EventBus; "
               "from apsimo.intelligence.components.tool_learner import ToolLearner; "
               "from apsimo.intelligence.synthesis.insight_deliverer import InsightDeliverer",
