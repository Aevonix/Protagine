...
config={'plugins':{'enabled':['protagine','protagine'] if duplicate else [selected],selected:settings},
...
if duplicate:
 ...
 path=home/'plugins/protagine';path.mkdir(parents=True)
 (path/'plugin.yaml').write_text('name: protagine\nversion: 1.0.0\nentry_point: __init__.py\n')
 (path/'__init__.py').write_text('from protagine_hermes import register\n')
...
canonical=importlib.import_module('protagine_hermes')
...
assert all(item.startswith('protagine_') for item in names),names
...
intent=intents[0]
assert intent['schema']=='HermesToolActionIntentV1' and intent['tool_name']=='protagine_task_snooze',intent
...
call_digest=digest({'schema':'HermesActionCallV1','tool_name':'protagine_task_snooze',**context})
...
@pytest.mark.parametrize('first', ['protagine_hermes'])
...
def test_native_names_preserve_governed_identity(artifacts, tmp_path, selected, first, discovery):
    ...
    env.update(HOME=str(tmp_path/'user'), HERMES_HOME=str(tmp_path/'profile'),
        ...
        LITELLM_LOCAL_MODEL_COST_MAP='True', PROTAGINE_GENERAL_PLUGIN_ACTIVE='1',
    ...
    assert row['wire_tool'] == 'protagine_task_snooze'
...
home=Path(os.environ['HERMES_HOME']);home.mkdir()
Path(os.environ['HERMES_BUNDLED_PLUGINS']).mkdir()
state=home/'protagine';adapter=state/'adapter'
...
for name,module in [('protagine','protagine_hermes'),('protagine-memory','protagine_memory')]:
 ...
 (path/'__init__.py').write_text(setup_hermes._forwarder(adapter,module,module=='protagine_memory'))
 (path/'plugin.yaml').write_bytes(resources[module+'/plugin.yaml'])
config={'plugins':{'enabled':['protagine'],'protagine':{'instance_dir':str(state),
 'turn_outbox_path':str(home/'turn-outbox.db'),'enabled_read_tools':[]}},
 'memory':{'provider':'protagine-memory','memory_enabled':False,'user_profile_enabled':False},
 'tools':{'tool_search':{'enabled':False}}}
...
assert (home/'plugins/protagine-memory').is_dir()
from hermes_cli.plugins import get_plugin_manager
from plugins.memory import find_provider_dir,load_memory_provider
manager=get_plugin_manager();manager.discover_and_load()
assert manager._plugins['protagine'].enabled,manager.list_plugins()
assert not manager._plugins['protagine'].error,manager.list_plugins()
assert len(manager._hooks['pre_llm_call'])==1
assert find_provider_dir(config['memory']['provider'])==home/'plugins/protagine-memory'
provider=load_memory_provider(config['memory']['provider'],register_skills=False)
assert provider is not None
assert type(provider).__module__=='protagine_memory.provider'
assert {schema['name'] for schema in provider.get_tool_schemas()}=={
 'protagine_check_commitments','protagine_get_affect','protagine_get_facts','protagine_timeline'}
assert all(name.startswith('protagine_') for name in manager._plugins['protagine'].tools_registered)
print(json.dumps({'retained_directory_provider':'protagine-memory','implementation':type(provider).__module__,
 'general_plugin':'protagine','native_loads':1,'network':0,'model_calls':0}))
manager.unload()
'''
...
def test_refreshed_retained_directory_loads_canonical_native_provider(artifacts, tmp_path):
    ...
    env.update(HOME=str(tmp_path/'user'),HERMES_HOME=str(tmp_path/'profile'),
        HERMES_BUNDLED_PLUGINS=str(tmp_path/'bundled'),HERMES_DISABLE_TELEMETRY='1',
        HERMES_DISABLE_LAZY_INSTALLS='1',PYTHON_DOTENV_DISABLED='1',
        PROTAGINE_SKIP_DOTENV='1',PROTAGINE_STATE_DIR=str(tmp_path/'state'),
        LITELLM_LOCAL_MODEL_COST_MAP='True',PROTAGINE_GENERAL_PLUGIN_ACTIVE='1',
        PROTAGINE_MEMORY_WORKER_TOOLS='0',PROTAGINE_MEMORY_TURN_WRITER='disabled')
