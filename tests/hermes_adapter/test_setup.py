...
import runpy
sys.argv = ['protagine', 'init', '--non-interactive', '--hermes-python', sys.argv[7],
...
runpy.run_module('protagine', run_name='__main__')
'''
SERVER = r'''
...
import runpy
sys.argv = ['protagine', 'start']  # Discover the same instance from HERMES_HOME.
runpy.run_module('protagine', run_name='__main__')
'''
TURN = r'''
...
manifest = json.loads((get_hermes_home()/'protagine'/'instance.json').read_text())
binding = manifest['adapter_binding']
roots = (binding['sources'] if binding['mode'] == 'native-installed' else
         {name: str(get_hermes_home()/'protagine'/'adapter'/name) for name in ('protagine_hermes', 'protagine_memory')})
assert Path(protagine_hermes.__file__).resolve().parent == Path(roots['protagine_hermes'])
...
providers = [p for p in agent._memory_manager._providers if type(p).__name__ == 'ProtagineMemoryProvider']
assert len(providers) == 1, 'Native memory provider absent or duplicated'
assert Path(inspect.getfile(type(providers[0]))).resolve().parent == Path(roots['protagine_memory'])
if len(sys.argv)>3 and sys.argv[3]=='defer-capture':
    ...
    ProtagineClient.sync_turn = lambda *args, **kwargs: False
...
client=ProtagineClient(url=config['url'],api_key=os.environ['PROTAGINE_NATIVE_API_KEY'])
...
sys.path.insert(0, sys.argv[1])
if sys.argv[2]: sys.path.append(sys.argv[2])
sys.argv = ['protagine', 'doctor', '--json']
runpy.run_module('protagine', run_name='__main__')
'''
...
def test_native_goal_setup_config_uses_actual_native_tool_and_role_contract(tmp_path, enabled, named_profile):
    ...
    result=run_python('-I','-c',NATIVE_GOALS_CONFIG,ROOT/'sidecar',
        os.environ.get('PROTAGINE_TEST_DEPENDENCY_PATH',''),'enabled' if enabled else 'disabled',
...
home=get_hermes_home();state=home/'protagine';manifest=json.loads((state/'instance.json').read_text())
...
accepted=json.loads(handle_function_call('protagine_accept_local_draft',
...
with sqlite3.connect((state/'protagine-commitments.db').as_uri()+'?mode=ro',uri=True) as db:
...
def _native_interpreter(tmp_path, wheel, adapter_installation):
    ...
    for source in Path(sysconfig.get_path('purelib')).iterdir():
        if 'protagine_hermes' in source.name or source.name in {'protagine_memory', '__pycache__'}:
            continue
        if adapter_installation == 'absent' and source.name.startswith('typer'):
            continue  # Hermes core does not require Protagine's legacy CLI dependency.
...
def test_packaged_guided_setup_captures_and_recalls_with_real_native_sessions(artifacts, tmp_path, adapter_installation, named_home):
    ...
    if not installed_adapter:
        run_native('-I', '-c', "import importlib.util; assert importlib.util.find_spec('typer') is None",
                   cwd=tmp_path)
    # Wheel-install Protagine separately from the source tree; dependencies must be
    ...
    dependency_path = os.environ.get('PROTAGINE_TEST_DEPENDENCY_PATH', '')
    requests = []
        ...
        def do_POST(self):
            ...
            if isinstance(choice, dict) and choice.get('function', {}).get('name') == 'protagine_setup_echo':
                calls = [{'id':'setup-call','type':'function','function':{
                    'name':'protagine_setup_echo','arguments':json.dumps({'token':'protagine-ready'})}}]
            elif 'Produce the accepted local source draft below.' in full:
                results = [m for m in body['messages'] if m.get('role')=='tool']
                names = {tool['function']['name'] for tool in body.get('tools', [])}
                if not results:
                    if 'protagine_read_work_source' in names:
                        name,args='protagine_read_work_source',{'source':0}
                    else:
                        name,args='tool_search',{'queries':['protagine_read_work_source', 'kanban_complete']}
    ...
    env.update(HOME=str(tmp_path), HERMES_HOME=str(home), HERMES_BUNDLED_PLUGINS=str(bundled),
               ...
               HERMES_DISABLE_TELEMETRY='1', PROTAGINE_GUARD_CHAT_MODE='off', LITELLM_LOCAL_MODEL_COST_MAP='True')
    ...
    try:
        ...
        manifest = json.loads((home/'protagine'/'instance.json').read_text())
        assert manifest['adapter_binding']['mode'] == ('native-installed' if installed_adapter else 'private-directory')
        assert (home/'plugins'/'protagine').exists() is not installed_adapter
        assert (home/'plugins'/'protagine-memory').exists() is not installed_adapter
        if adapter_installation == 'editable':
            assert set(manifest['adapter_binding']['external_modules']) == {
                'protagine_hermes.protagine_hostworker.catalog', 'protagine_hermes.protagine_hostworker.contract'}
        ...
        instance = json.loads((home/'protagine'/'instance.json').read_text())
        keyring = json.loads((home/'protagine'/'api-keyring.json').read_text())
        ...
        assert not (home/'protagine'/'lancedb').exists()
        assert (home/'protagine'/'contacts.db').exists()
        ...
        if adapter_installation == 'wheel':
            ...
            client = Path(manifest['adapter_binding']['sources']['protagine_hermes'])/'client.py'
            ...
            assert rejected.returncode != 0 and 'different installed Protagine adapter' in rejected.stdout
