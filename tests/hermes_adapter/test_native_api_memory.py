...
home.joinpath('config.yaml').write_text(json.dumps({
    'plugins':{'enabled':['pacomind'],'pacomind':{'url':'http://fixture','owner_contact_id':owner,
        'api_key':credential,
        'attested_system_platforms':[] if mode=='unattested' else ['api_server'],
        'turn_writer_platforms':['api_server']}},
    'memory':{'provider':'pacomind-memory','config':{'url':'http://fixture','contact_id':'contact-a','api_key':credential}}}))
...
def answer(**kwargs):
    ...
    scope=pacomind_hermes._TRANSPORT_SCOPES.for_execution(
...
print(json.dumps({'mode':mode,'actual_native_api_turn':True,'current_recollection_visible':trusted,
    ...
    'controlled_provider_calls':len(physical),'default_context_authority':os.environ['PACOMIND_MEMORY_DEFAULT_CONTEXT_AUTHORITY']}))
'''
