...
from protagine_hermes import _TransportScope, _TransportScopeRegistry
from protagine_hermes.client import ProtagineClient, TurnOutbox, source_message_hash
from protagine_hermes.input_provenance import supplied_input
from protagine_hermes.task_handoffs import TaskHandoffs, TaskHandoffError, erase_task_handoffs
from protagine_hermes.task_sources import NativeTaskSources

def no_network(*args,**kwargs):raise AssertionError('Native task source qualification has no network access')
socket.socket.connect=no_network;socket.create_connection=no_network
state=Path(os.environ['PROTAGINE_STATE_DIR']);state.mkdir(mode=0o700)
...
async def authority(request,next_call):
 request.state.protagine_authority=RequestAuthority(principal_id='native-task-fixture',credential_id='fixture',
...
client=ProtagineClient('http://fixture')
...
def authority_changes():
 src=sources.capture(scope())
 for platform in ('cron','subagent','background_review','protagine_task'):
...
def existing_handoff_races():
 ...
 from protagine_hermes import task_controller
 ...
 for race in ('completed','failed','retained_reply','stopped','annotated','erased','owner_changed'):
  ...
  if race=='annotated':
   ...
   asyncio.run(contacts.correct_handle_identity(operation_id='handoff-race-owner',performed_by='fixture-owner',
    gateway='sms',address='+15550001',expected_contact_id=owner,contact_id=guest,evidence_refs=['fixture:correction']))
  receipt=controller.finish_handoff(scope=current,tool_name='protagine_task',tool_call_id='accepted-call-'+race,
...
def test_native_task_sources(artifacts, tmp_path, case):
    ...
    env.update(PROTAGINE_STATE_DIR=str(tmp_path/'state'), PROTAGINE_SKIP_DOTENV='1',
        PROTAGINE_INTROSPECTION_ENABLED='false', PROTAGINE_GUARD_CHAT_MODE='off')
    run_python('-I', '-c', PROBE, artifacts[3], ROOT/'sidecar',
        os.environ.get('PROTAGINE_TEST_DEPENDENCY_PATH', ''), case, cwd=tmp_path, env=env)
