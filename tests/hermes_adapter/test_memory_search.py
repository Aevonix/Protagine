...
import copy,time
search_schema=next(s for s in pacomind_hermes._TOOL_SCHEMAS if s['name']=='pacomind_memory_search')
...
PacoMindClient.post=observed_post
...
def search(query,**extra):
    ...
    search_number+=1
    return dispatch({'query':query,**extra},**current,call='search-'+str(search_number),tool='pacomind_memory_search')
def open_original(reference):
    global search_number
    search_number+=1
    return dispatch(reference,**current,call='open-'+str(search_number),tool='pacomind_memory_read_source')
def carry(row):
...
assert first['pacomind_memory_search_v1'] and first['evidence_basis']=='recalled_excerpt'
assert first['full_source_opened'] is False and 'pacomind_source_read_v1' not in first
...
assert opened['pacomind_source_read_v1'] and 'pacomind_memory_search_v1' not in opened
...
for invalid in ({'query':'   '},{'query':'heliograph','limit':True},{'query':'heliograph','person_id':'other'}):
    bad=dispatch(invalid,**current,call='bad-'+str(len(json.dumps(invalid))),tool='pacomind_memory_search')
    assert 'error' in bad or bad.get('status')=='denied',bad
assert len(search_calls)==before_calls
missing=json.loads(handle_function_call('pacomind_memory_search',{'query':'heliograph'}))
...
guest=dispatch({'query':'heliograph'},session='search-guest',task='search-guest-task',turn='search-guest-turn',
    call='guest-search',tool='pacomind_memory_search')
