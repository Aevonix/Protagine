...
def test_native_video_frame_reaches_sdk_and_is_withheld_after_correction_and_erasure(artifacts, tmp_path, monkeypatch, actual_backend):
    ...
    # checks, while all Protagine and SDK traffic stays in controlled transports.
...
opened=dispatch(args,session='reader',task='reader-task',turn='reader-turn',call='call_video',tool='protagine_memory_read_source')
if ACTUAL_BACKEND:
...
if ACTUAL_BACKEND:
    ...
    import protagine.turns.video as decoder_module
...
messages=[{'role':'user','content':compose_user_api_content('',recalled,'')},
    {'role':'assistant','content':'','tool_calls':[{'id':'call_video','type':'function','function':{
        'name':'protagine_memory_read_source','arguments':json.dumps(args)}}]},result]
