"""Write Protagine diagnostic skill to harness skills directories."""
...
SKILL_PATHS = {
    ...
    "crush": "~/.config/crush/skills/protagine-diagnose",
    "codex": "~/.codex/skills/protagine-diagnose",
    "claude-code": "~/.codex/skills/protagine-diagnose",  # Shares with Codex
    "opencode": "~/.config/opencode/skills/protagine-diagnose",
...
PROTAGINE_DIAGNOSTIC_SKILL = """---
name: protagine-diagnose
description: Diagnose Protagine connection failures and missing recalled context using the selected instance.
---

# Protagine diagnostics

1. Run `protagine status` and `protagine doctor`. Read the reported endpoint and instance
   instead of assuming a directory, machine, port, or database backend.
2. For coding harnesses, run `protagine mcp detect`. Confirm one `protagine` MCP server
   is configured using `protagine mcp setup --harness <name>`.
3. Call `protagine_health`. If it fails, report the actual transport or authentication
...
4. If health passes, use `protagine_get_context` for the authenticated contact and
   `protagine_lookup_facts` for a known source-backed fact. Report what was actually
...
def write_protagine_skill(harness_id: str, workspace_dir: Path | None = None) -> bool:
    """Write Protagine diagnostic skill to harness skills directory.
    ...
    if harness_id == "openclaw" and workspace_dir:
        skill_dir = workspace_dir / "skills" / "protagine-diagnose"
    else:
    ...
    try:
        ...
        skill_md.write_text(PROTAGINE_DIAGNOSTIC_SKILL)
...
def remove_protagine_skill(harness_id: str) -> bool:
    """Remove Protagine diagnostic skill from harness skills directory.
...
# ---------------------------------------------------------------------------
# Protagine Check Skill (for on-demand initiative checking)
# ---------------------------------------------------------------------------

PROTAGINE_CHECK_SKILL = """---
name: protagine-check
description: Review current commitments and useful work through the selected Protagine instance.
---

Use `protagine_get_context` and `protagine_check_commitments` for the authenticated
...
def write_protagine_check_skill(workspace_dir: Path) -> bool:
    """Write protagine-check skill to OpenClaw workspace skills directory.
    ...
    """
    skill_dir = workspace_dir / "skills" / "protagine-check"
    try:
        skill_dir.mkdir(parents=True, exist_ok=True)
        (skill_dir / "SKILL.md").write_text(PROTAGINE_CHECK_SKILL)
        return True
    except Exception:
        return False
