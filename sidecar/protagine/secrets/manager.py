"""SecretsManager — unified secrets access layer for Protagine.

All Protagine code MUST use SecretsManager to access secrets.
...
The backend is loaded from ~/.protagine/config.yaml on first instantiation
and cached for the process lifetime.  Override ``PROTAGINE_SECRETS_BACKEND``
...
from protagine.secrets.types import SecretType
...
class SecretsManager:
    """Unified secrets access layer for Protagine.
    ...
    def _load_backend(cls) -> SecretsBackend:
        ...
        backend_name = os.environ.get("PROTAGINE_SECRETS_BACKEND")
        ...
        if not backend.is_available():
            logger.warning(
                ...
                "falling back to 'env'. Run `protagine secrets status` for details.",
                backend_name,
            )
    ...
    def _read_config_backend(cls) -> str | None:
        """Read secrets.backend from ~/.protagine/config.yaml."""
        try:
            import yaml
            from pathlib import Path

            protagine_home = Path(os.environ.get("PROTAGINE_HOME", Path.home() / ".protagine"))
            config_path = protagine_home / "config.yaml"
    ...
    def get_required(self, key: str) -> str:
        ...
        if value is None:
            raise KeyError(
                ...
                f"Run `protagine secrets set {key} <value>` to configure it."
            )
        return value
