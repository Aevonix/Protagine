...
Mode (PROTAGINE_CONNECTORS_MODE, default off):
  off    -> no polling.
...
(PROTAGINE_CONNECTOR_<NAME>_*). Default off; enable per connector.
"""
...
from protagine.connectors.base import Connector, Observation
...
def connectors_enabled() -> bool:
    return os.environ.get(
        "PROTAGINE_CONNECTORS_ENABLED", "false").strip().lower() in (
            "1", "true", "yes", "on")
...
class ConnectorManager:
    ...
    def register_default_connectors(self) -> int:
        ...
        PROTAGINE_CONNECTOR_<NAME>_ACCOUNTS or secret connector/<name>/accounts,
        ...
        from protagine.connectors.caldav_calendar import CalendarConnector
        from protagine.connectors.fs_documents import FSDocumentsConnector
        from protagine.connectors.webhook_pull import WebhookPullConnector
