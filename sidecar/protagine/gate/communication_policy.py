...
separate ``context_digest`` covers the exact, reduced record Protagine received
and made visible to guard/model inputs, so callers can detect substitution or
field-level tampering without Protagine needing the host's private policy store.
"""
...
class CommunicationPolicyContextV1(BaseModel):
    ...
    def context_digest(self) -> str:
        """SHA-256 identity of every field Protagine received and exposed."""
