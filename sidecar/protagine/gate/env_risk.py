...
* **Deployment-configured gateways.** Protagine ships NO gateway names.
  ``PROTAGINE_ENV_RISK_GATEWAY_CLASS`` (default empty) maps gateway families to
...
from protagine.channels.presence import STRONG_METHODS
...
def env_risk_window_hours() -> float:
    """PROTAGINE_ENV_RISK_WINDOW_HOURS (default 48): census recency window."""
    try:
        v = float(os.environ.get("PROTAGINE_ENV_RISK_WINDOW_HOURS", "48"))
...
def gateway_class(gateway: str) -> str:
    ...
    PROTAGINE_ENV_RISK_GATEWAY_CLASS is a comma list of ``gateway:class`` pairs
    ...
    if not g:
        return ""
    raw = os.environ.get("PROTAGINE_ENV_RISK_GATEWAY_CLASS", "")
