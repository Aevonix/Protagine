...
from protagine.gate.layers.l2_pii import PIIScanner
from protagine.gate.layers.l4_trust_tier import TrustTierChecker
from protagine.gate.layers.l5_injection import InjectionDetector
...
_guard_audit = logging.getLogger("protagine.gate.guard")
...
def enforce_allowlist() -> Optional[frozenset]:
    ...
    PROTAGINE_GUARD_ENFORCE_CHECKS, default ``secret_leak,tom2_epistemic`` —
    ...
    """
    raw = os.environ.get("PROTAGINE_GUARD_ENFORCE_CHECKS",
...
def breaker_enabled() -> bool:
    """PROTAGINE_GUARD_BREAKER, default on. off/0/false disables the breaker."""
    return os.environ.get("PROTAGINE_GUARD_BREAKER", "on").strip().lower() not in (
        "off", "0", "false", "no")
...
def breaker_trip_blocks() -> int:
    ...
    <=0 disables tripping."""
    try:
        return int(os.environ.get("PROTAGINE_GUARD_TRIP_BLOCKS", "10"))
