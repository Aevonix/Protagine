...
next PROTAGINE_TOM2_TAINT_TTL_SECS (default 900s) the outbound egress net
...
from protagine.gate.context_provenance import normalize_entity
from protagine.tom.tom2 import INFERENCE_KINDS, _validate_ref
...
def taint_ttl_secs() -> float:
    """PROTAGINE_TOM2_TAINT_TTL_SECS (default 900): how long a registered
    ...
    raw = os.environ.get("PROTAGINE_TOM2_TAINT_TTL_SECS")
    if raw is None or not str(raw).strip():
        return DEFAULT_TTL_SECS
    try:
        v = float(str(raw).strip())
    except (TypeError, ValueError):
        logger.warning("PROTAGINE_TOM2_TAINT_TTL_SECS=%r is malformed — using "
