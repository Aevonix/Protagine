...
def prepare(config, home, *, native_env, observer_env, local_work=False, draft_board=None):
    ...
    observer_env = dict(observer_env)
    ...
    configured = observer_env.get('PROTAGINE_HERMES_WORK_BOARDS')
    if 'PROTAGINE_HERMES_WORK_BOARDS' in observer_env:
        try:
            boards = json.loads(configured)
        except (TypeError, ValueError):
            raise ValueError('Existing PROTAGINE_HERMES_WORK_BOARDS must be a JSON board list') from None
        if (not isinstance(boards, list) or not 1 <= len(boards) <= 8
            ...
            raise ValueError('Existing PROTAGINE_HERMES_WORK_BOARDS must select one to eight board slugs')
...
def describe(details):
    ...
    print('The selected Hermes gateway is required. Protagine does not start or restart it; use its existing lifecycle.')
...
def enable(state):
    ...
    if 'PROTAGINE_HERMES_WORK_BOARDS' not in observer_env:
        suffix = 'PROTAGINE_HERMES_WORK_BOARDS='+json.dumps(details['boards'], separators=(',', ':'))+'\n'
