...
def worker_configuration(state, manifest, owner):
    ...
    config = {**model,
        ...
        'plugins': {'enabled': ['protagine'], 'protagine': {'native_reviews': {
...
def configure(state, *, install=False):
    ...
    if worker.exists():
        ...
        if ownership(plugin_settings(existing).get('native_reviews', {})) != ownership(candidate['plugins']['protagine']['native_reviews']):
        ...
        subprocess.run([manifest['hermes_python'], '-B', '-c',
            ...
            'create_profile("protagine-reviews",no_alias=True,no_skills=True)'],
    ...
    if install:
        ...
        adapter = (state/'adapter/protagine_hermes' if binding_info['mode'] == 'private-directory'
                   else Path(binding_info['sources'].get('protagine_hermes') or binding_info['sources']['protagine_hermes']))
        if not (adapter/'review_worker.py').is_file():
            raise ValueError('Upgrade the selected native adapter before enabling reviews')
        directory = worker/'plugins/protagine'
        write(directory/'__init__.py', _forwarder(adapter.parent, 'protagine_hermes'))
        ...
        write(worker/'SOUL.md',
            ...
            'Retained tasks may name protagine_read_work_source or protagine_review_report; use the '
            'corresponding advertised protagine_read_work_source or protagine_review_report tools.\n'
            'Use protagine_read_work_source(0) for the registered observation. For log-volume reviews, '
            ...
            'Report with protagine_review_report. It is your interface to the existing native '
