"""Agent and Invite stores for multi-agent Protagine.
...
def generate_setup_code() -> str:
    """Generate a random setup code: PROTAGINE-XXXX-XXXX-XXXX."""
...
class AgentStore:
    ...
    def _create_tables(self, conn: sqlite3.Connection) -> None:
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_agents_protagine ON agents(protagine_id)")
    ...
    def create(self, data: Dict[str, Any]) -> Agent:
        ...
        cursor = self._db.execute(
            """
            INSERT INTO agents (
                agent_id, node_id, protagine_id, name,
    ...
    def list(
        ...
        if protagine_id:
            query += " AND protagine_id = ?"
            params.append(protagine_id)
    ...
    def mark_all_offline(self) -> int:
        """Mark all agents as offline (called on Protagine restart)."""
        cursor = self._db.execute(
