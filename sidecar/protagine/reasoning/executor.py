...
Protagine-native tools are defined in tools/definitions.py and handlers
are in tools/handlers.py. These tools provide direct access to
Protagine's intelligence systems (memory, goals, relationships, etc.)
...
class ToolExecutor:
    ...
    handlers :
        ...
        a string result.
    registry :
        Optional SubsystemRegistry for Protagine-native tools. When provided,
        Protagine tools are automatically available.
    """
    ...
    def __init__(
        ...
        # Auto-register Protagine-native tool handlers if registry is provided
    ...
    def register_native_tools(self, search_orchestrator=None, sandbox_dir: str = "") -> None:
        """Register Protagine-native tools that run inside the sidecar.
        ...
        if sandbox_dir:
            try:
                from protagine.reasoning.native_tools.file_ops import (
    ...
    def get_definitions(self, available_tools: list[str] | None = None) -> list[dict[str, Any]]:
        ...
        Returns Protagine-native tool definitions that can be executed server-side.
        ...
        for tc in tool_calls:
            ...
            try:
                if handler is self._canonical_memory_handler:
                    result = await TOOL_HANDLERS['protagine_memory_search'](
