"""Protagine Vector Store — tier migration.
...
def _update_env(model_id: str, pipeline) -> None:
    ...
    try:
        ...
        for i, line in enumerate(lines):
            if line.startswith("PROTAGINE_EMBED_MODEL="):
                lines[i] = f"PROTAGINE_EMBED_MODEL={model_id}"
                updated["model"] = True
            elif line.startswith("PROTAGINE_EMBED_DIMS="):
                dims = pipeline.dimensions if hasattr(pipeline, "dimensions") else ""
                lines[i] = f"PROTAGINE_EMBED_DIMS={dims}"
                updated["dims"] = True
