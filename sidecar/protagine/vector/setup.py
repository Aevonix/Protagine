"""Protagine Vector Store — hardware detection, config recommendation, and CLI.
...
Provides:
  ...
  - CLI entry point for ``protagine vector setup`` and ``protagine vector rebuild``
"""
...
from protagine.vector.config import EmbeddingConfig, HardwareProfile, VectorStoreConfig
...
async def ensure_embed_config() -> Optional[EmbeddingConfig]:
    ...
    allowing Protagine to start without embeddings (graceful degradation).
    """
    ...
    if not sys.stdin.isatty():
        logger.warning(
            "Vector search disabled — no PROTAGINE_EMBED_MODEL configured. "
            "Run 'protagine vector setup' to enable. "
            "Recommended: PROTAGINE_EMBED_MODEL=%s PROTAGINE_EMBED_DIMS=%d PROTAGINE_EMBED_PROVIDER=%s",
    ...
    print(f"  PROTAGINE_EMBED_PROVIDER={recommended.provider}")
    print(f"  PROTAGINE_EMBED_MODEL={recommended.model_id}")
    print(f"  PROTAGINE_EMBED_DIMS={recommended.dimensions}")
    if recommended.quantization:
        print(f"  PROTAGINE_EMBED_QUANTIZATION={recommended.quantization}")
    print()
    return None
...
def cli_setup(args: list[str] | None = None) -> None:
    """CLI handler for ``protagine vector setup``."""
    import argparse

    parser = argparse.ArgumentParser(description="Protagine vector store setup")
    ...
    print("\nAdd to your .env:")
    print(f"  PROTAGINE_EMBED_PROVIDER={config.provider}")
    print(f"  PROTAGINE_EMBED_MODEL={config.model_id}")
    print(f"  PROTAGINE_EMBED_DIMS={config.dimensions}")
    if config.quantization:
        print(f"  PROTAGINE_EMBED_QUANTIZATION={config.quantization}")
...
def cli_rebuild(args: list[str] | None = None) -> None:
    """CLI handler for ``protagine vector rebuild``."""
    import argparse
    ...
    print("NOTE: Vector search requires PROTAGINE_EMBED_MODEL to be set.")
    print("      Run 'protagine vector setup' first, then use scripts/vector_backfill.py")
    print("      for the full backfill process.")


def cli_main() -> None:
    """Entry point for ``protagine vector <subcommand>``."""
    if len(sys.argv) < 3:
        print("Usage: protagine vector {setup|rebuild}")
        sys.exit(1)
