"""Protagine Vector — local image storage.
...
class LocalImageStore:
    ...
    Directory layout:
        $PROTAGINE_STATE_DIR/images/
            originals/{hash}.jpg
            thumbs/{hash}.jpg
    """

    def __init__(self, state_dir: Optional[str] = None, *, source_evidence: bool = False) -> None:
        base = Path(state_dir or os.environ.get("PROTAGINE_STATE_DIR", "."))
    ...
    def store_original(self, image: ImageInput) -> StoredImage:
        ...
        if not thumbnail.exists():
            from protagine.vector.image_preprocess import generate_thumbnail
...
def make_image_store(mode: str = "", state_dir: Optional[str] = None):
    ...
    mode = mode or os.environ.get("PROTAGINE_IMAGE_STORAGE", "local")
