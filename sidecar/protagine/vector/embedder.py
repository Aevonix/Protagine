"""Protagine Vector Store — embedding providers and pipeline.
...
class MLXEmbeddingProvider(EmbeddingProvider):
    ...
    async def warmup(self) -> None:
        ...
        # See: https://github.com/Aevonix/Protagine/issues/17
...
class EmbeddingPipeline:
    ...
    def __init__(
        ...
        self.query_instruction = os.environ.get('PROTAGINE_EMBED_QUERY_INSTRUCTION',
    ...
    def index_identity(self):
        from protagine.vector.indexes import EmbeddingIdentity
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            t0 = time.monotonic()
            vector = await self._provider.embed("protagine health check")
    ...
    async def embed(self, text: str) -> list[float]:
        ...
        if elapsed_ms > self.LATENCY_WARN_MS:
            logger.warning(
                ...
                "check PROTAGINE_EMBED_PROVIDER and model config",
                elapsed_ms,
    ...
    async def embed_image(self, source: str | bytes, mime_type: str = "", caption: str = "") -> tuple[list[float], dict[str, Any]]:
        ...
        if not self._multimodal_enabled or not self._multimodal_provider:
            raise ValueError("Multimodal not enabled. Set PROTAGINE_MULTIMODAL=true and restart.")

        from protagine.vector.multimodal_types import EmbedInput, Modality
        from protagine.vector.image_preprocess import (
            compute_image_hash, extract_exif, load_image, resize_image,
            strip_gps_exif, validate_image,
        )
        from protagine.vector.safety_image import check_image, ImageCheckLevel
        ...
        # Safety check
        check_level = ImageCheckLevel(os.environ.get("PROTAGINE_IMAGE_CHECK", "basic"))
        ...
        strip_gps = os.environ.get("PROTAGINE_STRIP_EXIF_GPS", "true").lower() == "true"
        ...
        # Build ImageInput
        from protagine.vector.multimodal_types import ImageInput
        ...
        if not caption:
            from protagine.vector.caption import caption_image
