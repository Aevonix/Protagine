...
from protagine.task_queue.models import CANONICAL_JOB_ID_PATTERN
...
QUEUE_CONTRACT_SCHEMA = "ProtagineQueueContractV1"
...
def _release_identity() -> Dict[str, str]:
    commit = os.environ.get("PROTAGINE_RELEASE_COMMIT", "").strip().lower()
    manifest_sha256 = os.environ.get(
        "PROTAGINE_RELEASE_ARTIFACT_MANIFEST_SHA256", ""
    ).strip().lower()
    if not _COMMIT_RE.fullmatch(commit) or set(commit) == {"0"}:
        raise QueueContractIdentityError(
            "PROTAGINE_RELEASE_COMMIT must be the exact 40-character lowercase "
            "Git commit deployed"
        )
    if (
        ...
        raise QueueContractIdentityError(
            "PROTAGINE_RELEASE_ARTIFACT_MANIFEST_SHA256 must be the exact "
...
def _deployment_posture() -> Dict[str, Any]:
    ...
    if raw_claims in {"1", "true", "yes", "on"}:
        ...
        raise QueueContractIdentityError(
            "PROTAGINE_AGENT_JOB_CLAIMS_ENABLED must be true or false"
        )
    raw_routes = os.environ.get(
        "PROTAGINE_AGENT_WORKER_ROUTES", "agent_sync,hermes_run"
    )
...
def _work_control_contract() -> Dict[str, Any]:
    ...
    return {
        ...
        "protagine_supplies_host_control_adapter": False,
    ...
    contract: Dict[str, Any] = {
        ...
        "approval_at_birth": {
            ...
            "blocked_discovery": {
                ...
                "legacy_count_header": (
                    "X-Protagine-Blocked-Legacy-Count"
                ),
