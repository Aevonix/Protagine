"""InferenceHandler — context-enriched LLM inference via the Protagine router."""
...
from protagine.task_queue.handlers.base import JobHandler, Job
...
if TYPE_CHECKING:
    ...
    from protagine.world_model.entities import BaseEntity
    from protagine.contacts.store import ContactStore
    from protagine.contacts.models import Contact

logger = logging.getLogger(__name__)

_PROTAGINE_IDENTITY = (
    "You are Protagine, an intelligent personal AI assistant. "
...
async def _update_world_model_async(
    ...
    """
    try:
        from protagine.world_model.extraction.conversation_extractor import ConversationExtractor
        from protagine.world_model.entities import (
...
class InferenceHandler(JobHandler):
    """Run an LLM inference request via the Protagine router.
    ...
    Job payload keys:
        ...
        system_prompt (str, optional): Additional system context appended after
            the enriched Protagine identity + contact + world model prompt.
    ...
    async def _ensure_wm_connected(self) -> None:
        ...
        if self._wm is None:
            try:
                ...
                import os
                protagine_home = os.environ.get("PROTAGINE_HOME", os.path.expanduser("~/.protagine"))
                db_path = os.path.join(protagine_home, "world_model.db")
    ...
    async def _gate_context(
        ...
        *,
        ...
        Uses the context gate (:mod:`protagine.contextgate`): when the
        ...
        if payload.get("context_gate") == "off":
            return messages
        try:
            from protagine.contextgate import (
                GateConfig,
                estimate_tokens,
                prepare_context,
            )
            ...
            from protagine.contextgate import GateDecision
        ...
        if (not cognition_read_only
            ...
            try:
                ...
                owner_id = os.environ.get(
                    ...
                    os.environ.get("PROTAGINE_OWNER_CONTACT_ID", "owner"),
                ).strip() or "owner"
