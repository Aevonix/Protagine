...
_ROUTE_OWNER_ENV = {
    AGENT_SYNC_ROUTE: "PROTAGINE_AGENT_SYNC_WORKER_NODE_ID",
    HERMES_RUN_ROUTE: "PROTAGINE_HERMES_RUN_WORKER_NODE_ID",
    ACTION_PLANE_ROUTE: "PROTAGINE_ACTION_PLANE_WORKER_NODE_ID",
    WORK_ORDER_ROUTE: "PROTAGINE_ACTION_PLANE_WORKER_NODE_ID",
}
...
def bind_thought_route(job: Any) -> str:
    ...
    owner = os.environ.get("PROTAGINE_THOUGHT_WORKER_NODE_ID", "").strip()
    if owner:
...
def configured_route_owners() -> dict[str, str | None]:
    ...
    return {
        "agent_sync": os.environ.get(
            "PROTAGINE_AGENT_SYNC_WORKER_NODE_ID", ""
        ).strip() or None,
        "action_plane": os.environ.get(
            "PROTAGINE_ACTION_PLANE_WORKER_NODE_ID", ""
        ).strip() or None,
        "work_order": os.environ.get(
            "PROTAGINE_ACTION_PLANE_WORKER_NODE_ID", ""
        ).strip() or None,
        "hermes_run": os.environ.get(
            "PROTAGINE_HERMES_RUN_WORKER_NODE_ID", ""
        ).strip() or None,
