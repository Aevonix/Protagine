...
Protagine event bus on each state transition.
"""
...
from protagine.task_queue.config import (
...
class QueueManager:
    ...
    Usage::
        ...
        mgr = QueueManager(db_path=Path("~/.protagine/task_queue.db"))
        await mgr.start()
    ...
    def __init__(
        ...
        from protagine.task_queue.work_control import WorkControlService
    ...
    async def _reconcile_persisted_thought_routes(self) -> None:
        ...
        assert self._db is not None
        from protagine.task_queue.routing import bind_thought_route
    ...
    async def reconcile_governance_holds(self, *, force: bool = False) -> int:
        ...
        from protagine.task_queue.governor import ClaimVerdict, workers_mode
        ...
        try:
            recheck_seconds = max(1.0, float(
                os.environ.get(
                    "PROTAGINE_WORKER_HOLD_RECHECK_SECONDS", "30")
            ))
        except ValueError:
    ...
    async def _mark_pending_work_controls_terminal(
        ...
        *,
        ...
        if not operation_ids:
            return []
        from protagine.task_queue.work_control import canonical_json
            ...
            for row in rows:
                ...
                try:
                    timeout = _positive_seconds(
                        os.environ.get(
                            "PROTAGINE_WORKER_OUTCOME_DELIVERY_TIMEOUT_SECS",
                            "60",
                        ),
                        default=60.0,
                        field="PROTAGINE_WORKER_OUTCOME_DELIVERY_TIMEOUT_SECS",
                    )
    ...
    async def complete_job(
        ...
        if ambiguous_semantic_effect:
            ...
            tags.update({
                "success_verifier_identity": "protagine:embedded-worker",
        ...
        if self._event_bus is not None:
            try:
                if target_status is JobStatus.COMPLETED:
                    from protagine.task_queue.events import JobCompletedEvent
                    ...
                    from protagine.task_queue.events import JobFailedEvent
                    ...
                    from protagine.task_queue.events import JobNeutralEvent
    ...
    async def _work_control_target_row(self, job: Job) -> aiosqlite.Row:
        ...
        run_id = "run-" + _uuid_module.uuid5(
            ...
            f"protagine:queue-job:{target_id}",
        ).hex
        now = _now_iso()
        await self._db.execute(
        ...
        *,
        ...
        if projection["mode"] != "live":
            raise WorkControlError(
                ...
                "WorkControl mutations require PROTAGINE_WORK_CONTROL_MODE=live",
                status_code=503,
            )
    ...
    async def reconcile_inactive_work_controls(self) -> int:
        ...
        assert self._db is not None
        from protagine.task_queue.work_control import work_control_mode
...
class TaskQueueManager:
    """Singleton facade over QueueManager for the Protagine API layer.
    ...
    def get_instance(cls) -> "TaskQueueManager":
        if cls._instance is None:
            raise RuntimeError(
                ...
                "Ensure the Protagine server lifespan has started."
            )
    ...
    async def initialize(
        ...
        if db_path is None:
            protagine_home = Path(
                os.environ.get("PROTAGINE_HOME", "~/.protagine")
            ).expanduser()
            task_db_env = os.environ.get("PROTAGINE_TASK_DB_PATH", "")
            db_path = Path(task_db_env) if task_db_env else protagine_home / "task_queue.db"

        if claim_timeout_secs is None:
            claim_timeout_secs = _positive_seconds(
                os.environ.get("PROTAGINE_QUEUE_CLAIM_TIMEOUT_SECS", "30"),
                default=30.0,
                field="PROTAGINE_QUEUE_CLAIM_TIMEOUT_SECS",
            )
        if worker_heartbeat_ttl_secs is None:
            worker_heartbeat_ttl_secs = validate_worker_heartbeat_ttl(
                os.environ.get(
                    "PROTAGINE_QUEUE_HEARTBEAT_TIMEOUT_SECS",
    ...
    async def list_tasks(
        ...
        """
        from protagine.api.schemas.common import ListResponse
