...
Mode (PROTAGINE_WORKERS_MODE, default shadow):
    ...
    def _read_only_live_trial_status(
        ...
        * every real attempt consumes one of a small, bounded trial budget.
        ...
        try:
            ...
            try:
                ask_min = int(os.environ.get("PROTAGINE_TRUST_ASK_MIN_N", "3"))
            except (TypeError, ValueError):
                ask_min = 3
            ask_min = max(1, min(ask_min, 100))
            try:
                trial_max = int(os.environ.get(
                    "PROTAGINE_TRUST_READ_ONLY_TRIAL_MAX",
                    os.environ.get("PROTAGINE_TRUST_ACT_MIN_N", "5"),
                ))
            except (TypeError, ValueError):
                trial_max = 5
            ...
            for event in shadow_history:
                ...
                if (
                    ...
                    == "protagine.worker-outcome/v1"
                    and evidence.get("schema") == "protagine.worker-outcome/v1"
        ...
        if outcome != "neutral" and self._self_model is not None:
            try:
                recorded = self._self_model.record(
                    ...
                    outcome_contract="protagine.worker-outcome/v1",
                    evidence={
                        "schema": "protagine.worker-outcome/v1",
