...
class RecallSelector:
    ...
    async def rerank(
        ...
        *,
        ...
        PROTAGINE_RECALL_RERANK gates it: ``off`` (default) never calls the
        ...
        if mode not in ("shadow", "on"):
            return memories
        min_score = None
        raw_min_score = os.environ.get("PROTAGINE_RECALL_RERANK_MIN_SCORE", "").strip()
        if raw_min_score:
            try:
        ...
        if min_score is not None:
            ...
            expected = os.environ.get("PROTAGINE_RECALL_RERANK_CALIBRATION", "").strip()
        ...
        try:
            timeout_ms = float(os.environ.get(
                "PROTAGINE_RECALL_RERANK_TIMEOUT_MS", "1200"))
        except (TypeError, ValueError):
            timeout_ms = 1200.0
