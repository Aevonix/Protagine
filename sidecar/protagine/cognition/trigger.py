...
Protagine does not run its own LLM. It sends cognition requests to OpenClaw
...
from protagine.cognition.prompt import COGNITION_SYSTEM_PROMPT, build_cognition_prompt
...
def _cognition_model() -> Optional[str]:
    return os.environ.get("PROTAGINE_COGNITION_MODEL") or None


def _throttle_seconds() -> int:
    try:
        return int(os.environ.get("PROTAGINE_COGNITION_THROTTLE_SECONDS", "30"))
    except ValueError:
        return 30
...
async def trigger_cognition(
    ...
    if not model:
        return {
            "accepted": False,
            "message": "PROTAGINE_COGNITION_MODEL not configured",
            "throttle_seconds": None,
        }
...
def _emit_cognition_event(
    ...
    """
    try:
        from protagine.events.broadcaster import emit
