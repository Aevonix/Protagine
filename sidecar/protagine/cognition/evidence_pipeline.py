...
WorkOrder events are rejoined to Protagine's local immutable project ledger.
...
def cognition_evidence_mode() -> str:
    value = os.environ.get("PROTAGINE_COGNITION_EVIDENCE", "off").strip().lower()
...
def _bootstrap_mode() -> str:
    value = os.environ.get(
        "PROTAGINE_COGNITION_EVIDENCE_BOOTSTRAP", "beginning",
    ).strip().lower()
...
class CognitionEvidenceReducer:
    ...
    def _gap_policy() -> str:
        return os.environ.get(
            "PROTAGINE_COGNITION_EVIDENCE_GAP_POLICY", "stop",
        ).strip().lower()
    ...
    def _apply_expectations(self, projection: EvidenceProjectionV1) -> list[str]:
        ...
        sinks: list[str] = []
        from protagine.self_model.expectations import OutcomeObservationV1
