"""The Protagine agency charter: one shared prompt architecture for every LLM role.
...
- Generic: no deployment identities. The agent name comes from
  PROTAGINE_AGENT_NAME at compose time.
...
_CHARTER = """\
You are the Protagine cognition of {agent_name}: the always-on mind that \
...
def agent_name() -> str:
    return os.environ.get("PROTAGINE_AGENT_NAME", "the agent")
