...
Disabled by default. Activated with ``PROTAGINE_INTROSPECT_ENABLED=true`` and a configured
endpoint; deployment-agnostic (point ``PROTAGINE_INTROSPECT_BASE_URL`` at a local model):

  PROTAGINE_INTROSPECT_ENABLED     "true"|"false"      (default false)
  PROTAGINE_INTROSPECT_BASE_URL    OpenAI-compatible base, e.g. http://127.0.0.1:8000/v1
  PROTAGINE_INTROSPECT_MODEL       model id the endpoint serves (required to run)
  PROTAGINE_INTROSPECT_API_KEY     bearer token, blank for a local model
  PROTAGINE_INTROSPECT_TIMEOUT     seconds (default 30)
  PROTAGINE_INTROSPECT_MAX_TOKENS  response cap (default 400)
...
def _config() -> Dict[str, Any]:
    return {
        "base_url": os.environ.get("PROTAGINE_INTROSPECT_BASE_URL", "http://127.0.0.1:8000/v1").rstrip("/"),
        "model": os.environ.get("PROTAGINE_INTROSPECT_MODEL", "").strip(),
        "api_key": os.environ.get("PROTAGINE_INTROSPECT_API_KEY", "").strip(),
        "timeout": float(os.environ.get("PROTAGINE_INTROSPECT_TIMEOUT", "30")),
        "max_tokens": int(os.environ.get("PROTAGINE_INTROSPECT_MAX_TOKENS", "400")),
    }
