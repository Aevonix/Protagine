...
from protagine.toolsmith.authority import GraduationAuthorityV1
from protagine.toolsmith.integrity import (
    MAX_CAPTURE_BYTES,
    IntegrityError,
    candidate_digest,
    canonical_json,
...
from protagine.toolsmith.miner import ToolCandidate, ToolsmithMiner
from protagine.toolsmith.policy import validate_pure_tool
from protagine.toolsmith.registry import Tool, ToolRegistry, ToolStatus
...
def toolsmith_enabled() -> bool:
    return os.environ.get(
        "PROTAGINE_TOOLSMITH", "off").strip().lower() in ("shadow", "live")


def toolsmith_mode() -> str:
    m = os.environ.get("PROTAGINE_TOOLSMITH", "off").strip().lower()
...
def _shadow_min() -> int:
    try:
        return max(1, min(100, int(os.environ.get(
            "PROTAGINE_TOOLSMITH_SHADOW_MIN", "5"))))
    except ValueError:
        return 5
