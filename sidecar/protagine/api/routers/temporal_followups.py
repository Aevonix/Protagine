...
def review_contract(row):
    ...
    body = ('Review the status of this accepted task and its expected reply. '
            'Read current waiting state and parent work with protagine_read_work_source(0); '
            ...
            'observation. Complete through protagine_review_report with evidence and '
...
async def register(body: ExpectedReply, request: Request):
    ...
    def create():
        ...
        fields['timezone_name'] = body.timezone_name or os.environ.get('PROTAGINE_AGENT_TIMEZONE', 'UTC')
        if body.quiet_start is None and body.quiet_end is None:
            configured = os.environ.get('PROTAGINE_AGENT_QUIET_HOURS', '').strip()
            if configured:
        ...
        if host._comms_log is not None:
            from protagine.api.routers.transport import reconcile_receipts
