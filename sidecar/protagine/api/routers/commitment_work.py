...
def local_store(request, contact_id, *, enabled=False):
    ...
    if enabled and os.environ.get('PROTAGINE_LOCAL_WORK_ENABLED', '').lower() != 'true':
...
def native_run(body, *, finishing=False):
    from protagine.turns.hermes_work import selected_home
    home = selected_home()
    if not home or body.native_job_id != os.environ.get('PROTAGINE_LOCAL_WORK_JOB_ID'):
...
def _accept_local_draft(commitment_id, body, request):
    ...
    backend = os.environ.get('PROTAGINE_LOCAL_WORK_EXECUTOR', 'cron')
...
def pending_native_drafts(contact_id: str, request: Request):
    ...
    if os.environ.get('PROTAGINE_LOCAL_WORK_EXECUTOR', 'cron') != 'kanban':
