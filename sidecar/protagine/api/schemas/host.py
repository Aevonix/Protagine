...
Mirrors the TypeScript types in protagine's src/types.ts.
...
class TurnSyncRequest(BaseModel):
    ...
    def bounded_checkpoint(self):
        ...
        if self.transport_media is not None:
            from protagine.turns.transport_media import native_image_url
...
class RecordOutreachRequest(BaseModel):
    # Agent identity is deployment-specific — configure PROTAGINE_AGENT_NAME
    # rather than relying on this generic fallback.
    agent_id: str = Field(
        default_factory=lambda: _os.environ.get("PROTAGINE_AGENT_NAME", "agent")
    )
