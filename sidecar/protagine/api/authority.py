...
The legacy ``PROTAGINE_API_KEY`` remains a migration credential. New scoped
...
def resolve_request_person(
    ...
    *,
    ...
    if authority.anonymous:
        target = target or (
            os.environ.get("PROTAGINE_DEV_PERSON_ID", "dev-anonymous").strip()
            or "dev-anonymous"
        )
...
def resolve_turn_person(
    ...
    *,
    ...
    sender envelope Protagine then discards the body contact claim, starts from the
...
def required_scope(method: str, path: str) -> str:
    ...
    exact = {
        ...
        ("POST", "/v1/host/memory/recent"): "memory:read",
        ...
        ("GET", "/v1/host/executions/model-calls"): "context:read",
