...
world-model store so Protagine actually learns the owner's people, companies,
...
def _noise_names() -> frozenset:
    ...
    env (persona/agent name, plus PROTAGINE_WORLD_NOISE_NAMES)."""
    extra = set()
    for var in ("PROTAGINE_PERSONA_NAME", "PROTAGINE_AGENT_NAME"):
        v = os.environ.get(var, "").strip().lower()
        if v:
            extra.add(v)
    for t in os.environ.get("PROTAGINE_WORLD_NOISE_NAMES", "").split(","):
        t = t.strip().lower()
        if t:
            extra.add(t)
...
class WorldModelPopulator:
    def __init__(
        ...
        if store is not None:
            try:
                from protagine.world_model.resolution.entity_resolver import EntityResolver
