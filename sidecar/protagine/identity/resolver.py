"""IdentityResolver — normalize person identifiers across Protagine subsystems.
    ...
    async def owner_identities(self) -> frozenset:
        ...
        if not owner_id:
            raise OwnerIdentityError(
                "PROTAGINE_OWNER_CONTACT_ID is not set. Owner-exclusion filters "
        ...
        if not forms:
            raise OwnerIdentityError(
                f"PROTAGINE_OWNER_CONTACT_ID={owner_id!r} does not resolve to "
        ...
        if self._contact_store is not None:
            try:
                ...
                for handle in handles:
                    from protagine.contacts.identity_links import usable_handle
...
def get_identity_resolver(contact_store: Any = None) -> IdentityResolver:
    ...
    if contact_store is None:
        try:
            from protagine.api.routers import host as _host_mod
