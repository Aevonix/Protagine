...
``question:*`` (curiosity), ``self.*`` (the self-narrative sections) and
``consolidation.last`` (the nightly run). A text-only key carries no
half-life, so ``MindState.decay`` never rewrites its ``updated_at``:
``consolidation.last`` reads the last run's moment from it. Affect arrives
