...
class Affect:
    ...
    def _waved_off(self, row: Any) -> bool:
        ...
        contact is not the owner's, and approved work that expired undispatched was never seen (a lapsed
        ask is the owner's silence and counts)."""
        if row.verdict not in {"dismissed", "ignored"}:
            return False
        if row.kind == "message" and row.entity_id != self.owner_id:
            return False
        return not (row.verdict == "ignored" and row.outcome == "expired" and row.decision != "ask")
