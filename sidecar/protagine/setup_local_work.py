...
BOARD = PROFILE = 'protagine-drafts'
...
def verify_tools(endpoint, model, key, *, extra_body=None):
    tool = {'type':'function', 'function':{'name':'protagine_setup_echo',
    ...
    response = httpx.post(endpoint.rstrip('/')+'/chat/completions', headers={'Authorization':'Bearer '+key},
        json={'model':model,'messages':[{'role':'user','content':'Call protagine_setup_echo with token protagine-ready.'}],
              'tools':[tool],'tool_choice':{'type':'function','function':{'name':'protagine_setup_echo'}},
              'max_tokens':256, **(extra_body or {})}, timeout=60, trust_env=False)
    response.raise_for_status()
    choices = response.json().get('choices') or [{}]
    calls = choices[0].get('message', {}).get('tool_calls') or []
    if len(calls) != 1 or calls[0].get('function', {}).get('name') != 'protagine_setup_echo':
    ...
    if json.loads(calls[0]['function']['arguments']) != {'token':'protagine-ready'}:
...
def model_configuration(state, *, configuration_path=None):
    ...
    path = Path(configuration_path) if configuration_path else state/'.protagine-llm-config.json'
    ...
    for index, entry in enumerate([options, *(options['fallback_model'] or [])]):
        name = 'protagine-planning-'+str(index)
...
def worker_configuration(model, policy, plugin, lane):
    ...
    config = {**model, 'agent':{'max_turns':12},
        'toolsets':['protagine_local_work','kanban'], 'platform_toolsets':{'cli':['protagine_local_work','kanban']},
        'plugins':{'enabled':['protagine'], 'protagine':worker_plugin,
...
def refresh_role(state):
    ...
    from .setup import _atomic_hermes_config_write, _align_hermes_memory_spill, _validate_hermes_binding
...
def install(state):
    ...
    if manifest.get('version') != 1 or manifest.get('profile') != 'local':
        raise ValueError('Expected a local Protagine instance')
    adapter = manifest['adapter_binding']
    adapter_root = (state/'adapter/protagine_hermes' if adapter['mode'] == 'private-directory'
                    else Path(adapter['sources'].get('protagine_hermes') or adapter['sources']['protagine_hermes']))
    ...
    if worker.exists() and not owned:
        raise ValueError('The protagine-drafts profile already exists; retain or reconcile its binding')
    from dotenv import dotenv_values
    secret = dotenv_values(home/'.env').get('PROTAGINE_NATIVE_API_KEY')
    if not secret:
    ...
    if manifest['adapter_binding']['mode'] == 'private-directory':
        (worker/'plugins/protagine').mkdir(parents=True, exist_ok=True, mode=0o700)
        write_owned(worker/'plugins/protagine/__init__.py', _forwarder(state/'adapter', 'protagine_hermes'))
        write_owned(worker/'plugins/protagine/plugin.yaml', (state/'adapter/protagine_hermes/plugin.yaml').read_bytes())
    write_owned(worker/'SOUL.md', 'You execute one accepted local draft as part of the same Protagine agent.\n'
    ...
    lines = [line for line in env_before.decode().splitlines() if not line.startswith(('PROTAGINE_LOCAL_WORK_ENABLED=', 'PROTAGINE_LOCAL_WORK_EXECUTOR=', 'PROTAGINE_LOCAL_WORK_BOARD=', 'PROTAGINE_LOCAL_WORK_PROFILE='))]
    lines += ['PROTAGINE_LOCAL_WORK_ENABLED=true', 'PROTAGINE_LOCAL_WORK_EXECUTOR=kanban',
              'PROTAGINE_LOCAL_WORK_BOARD='+board, 'PROTAGINE_LOCAL_WORK_PROFILE='+profile]
    _atomic_hermes_config_write(env_path, env_before, ('\n'.join(lines)+'\n').encode())
