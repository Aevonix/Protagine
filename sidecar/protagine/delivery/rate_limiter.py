...
def _resolve_tz():
    """Owner-local timezone for quiet hours: PROTAGINE_TIMEZONE, else system, else UTC."""
    import os
    name = os.environ.get("PROTAGINE_TIMEZONE", "").strip()
    if name:
        try:
            from zoneinfo import ZoneInfo
            return ZoneInfo(name)
        except Exception:
            logger.warning("rate_limiter: invalid PROTAGINE_TIMEZONE %r, using system tz", name)
    try:
