...
Pipeline (option A: delegation only, Protagine never mutates anything itself):
  ...
  dispatch-> POST the ScopedTask contract to the EXISTING env-configured
             delegate endpoint (PROTAGINE_DIRECTED_TASK_URL, falling back to the
             agent-bridge jobs webhook). Dry-run mode logs the exact would-be
             dispatch and sends nothing (PROTAGINE_DIRECTED_MODE, default dry_run).
...
from protagine.directed.models import ScopedTask, ScopedTaskStore
from protagine.directed.intake import scope_from_directive
from protagine.directed.audit import audit_completion
...
def _dispatch_url() -> str:
    return (os.environ.get("PROTAGINE_DIRECTED_TASK_URL", "")
            or os.environ.get("PROTAGINE_BRIDGE_JOBS_WEBHOOK_URL", "")
            or os.environ.get("PROTAGINE_JOBS_WEBHOOK_URL", ""))


def _strict_reports() -> bool:
    """Strict report-back (default): a completion report is accepted only for
    a task that was actually dispatched. PROTAGINE_DIRECTED_STRICT_REPORTS=0
    restores the legacy accept-in-any-status behavior."""
    return os.environ.get("PROTAGINE_DIRECTED_STRICT_REPORTS", "1").strip() != "0"


def _max_dispatch_per_day() -> int:
    """Daily dispatch cap (rolling 24h). <=0 disables the cap (legacy)."""
    try:
        return int(os.environ.get("PROTAGINE_DIRECTED_MAX_DISPATCH_PER_DAY", "10"))
...
def _hmac_key() -> bytes:
    ...
    return os.environ.get("PROTAGINE_DIRECTED_HMAC_KEY", "").encode("utf-8")
...
class DirectedActionService:
    ...
    async def dispatch(self, task_id: str) -> Dict[str, Any]:
        ...
        if cap > 0:
            ...
            if recent >= cap:
                logger.warning(
                    ...
                    "in 24h; PROTAGINE_DIRECTED_MAX_DISPATCH_PER_DAY)",
                    task.id, recent, cap)
        ...
        # only when PROTAGINE_DIRECTED_HMAC_KEY is set (unset = legacy unsigned).
        ...
        try:
            ...
            if key:
                headers["X-Protagine-Directed-Signature"] = envelope["signature"]
    ...
    def _reject_report(self, task: ScopedTask, reason: str) -> Dict[str, Any]:
        ...
        logger.warning(
            ...
            "(strict reports; PROTAGINE_DIRECTED_STRICT_REPORTS=0 for legacy)",
