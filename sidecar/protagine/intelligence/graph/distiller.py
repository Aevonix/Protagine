...
class MemoryDistiller:
    ...
    Args:
        graph_client: ProtagineGraph instance (or compatible mock with execute())
        metrics: Optional ProtagineMetricsCollector for instrumentation
        min_recalls: Minimum recall count for a memory to be a distillation
            candidate (None = PROTAGINE_DISTILL_MIN_RECALLS, default 3)
        min_strength: Minimum strength for a candidate
            (None = PROTAGINE_DISTILL_MIN_STRENGTH, default 0.5)
        min_age_days: Minimum age in days, only distill established memories
            (None = PROTAGINE_DISTILL_MIN_AGE_DAYS, default 7)
    ...
    def __init__(
        ...
        self._min_recalls = (self._env_int("PROTAGINE_DISTILL_MIN_RECALLS", 3)
                             if min_recalls is None else min_recalls)
        self._min_strength = (self._env_float("PROTAGINE_DISTILL_MIN_STRENGTH", 0.5)
                              if min_strength is None else min_strength)
        self._min_age_days = (self._env_int("PROTAGINE_DISTILL_MIN_AGE_DAYS", 7)
