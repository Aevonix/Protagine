...
class MemoryConsolidator:
    ...
    Designed to be used with ProtagineGraph but accepts any object that exposes
    ...
    Args:
        graph_client: ProtagineGraph instance (or compatible mock)
    ...
    async def _fetch_recent_memories(self) -> List[Dict[str, Any]]:
        ...
        # for hours at 100% CPU (PROTAGINE_CONSOLIDATION_BATCH to tune).
        try:
            cap = max(50, int(os.environ.get("PROTAGINE_CONSOLIDATION_BATCH", "400")))
        except ValueError:
            cap = 400
    ...
    async def _merge_pair(self, keep_rec: Dict[str, Any], merge_rec: Dict[str, Any]) -> None:
        ...
        from protagine.intelligence.graph.client import EpistemicState
    ...
    async def _execute(self, query: str, **params: Any) -> List[Dict[str, Any]]:
        ...
        Supports ProtagineGraph (which exposes an AsyncDriver) or any mock that
        ...
        # ProtagineGraph path: use the driver session directly
