...
``protagine-relationships.db`` and refreshed by the autonomy phase when a
...
def profile_min_interactions() -> int:
    try:
        return max(1, int(os.environ.get(
            "PROTAGINE_RELATIONSHIP_PROFILE_MIN_INTERACTIONS", "5")))
    except ValueError:
        return 5
...
def approach_guidance_enabled() -> bool:
    return os.environ.get(
        "PROTAGINE_APPROACH_GUIDANCE", "true").strip().lower() != "false"
