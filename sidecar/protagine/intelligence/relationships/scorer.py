"""Protagine relationship scoring — evidence-based assessment of Protagine's own social world.

Protagine maintains its own relationship scores for every person it interacts with across
connected gateways (iMessage, Telegram, etc.). These scores represent Protagine's independent
assessment of each relationship, built from interaction patterns Protagine itself observes.

Two-layer scoring model
-----------------------
Owner layer (``owner_guidance`` weight = 0.18): the owner's explicit input informs
Protagine's view but does not dominate it. Owner input is authoritative at onboarding
when Protagine has no history; its relative weight naturally declines as Protagine accumulates
its own evidence.

Protagine observation layer (remaining 0.82): all signals Protagine observes directly —
message frequency, sentiment, latency, initiative, recency — plus three Protagine-specific
signals that capture the quality of Protagine's own engagement: ``protagine_initiative_response``
(do people respond to Protagine's outreach?), ``protagine_effectiveness`` (do Protagine's actions
help?), and ``protagine_style_fit`` (is Protagine's adapted style working?).

A score of 80+ means Protagine has observed strong, consistent engagement with this person
...
class ScoreWeights:
    ...
    Interaction signals are Protagine's direct observations of the exchange.
    Owner guidance informs but does not dominate.
    Protagine-specific signals capture the quality of Protagine's own behavior.
    """
    # Interaction signals (Protagine's observations of the exchange)
    ...
    # Protagine-specific signals (new)
    protagine_initiative_response: float = 0.08  # response rate to Protagine's own outreach
    protagine_effectiveness: float = 0.06        # are Protagine's suggestions/actions valued?
    protagine_style_fit: float = 0.04            # how well Protagine's adapted style is received
    ...
    def total(self) -> float:
        return sum([
            ...
            self.recency,
            self.owner_guidance,
            self.protagine_initiative_response,
            self.protagine_effectiveness,
            self.protagine_style_fit,
        ])


@dataclass
class ScoreBreakdown:
    """Breakdown of Protagine's score into source layers.
    ...
    Attributes:
        ...
        protagine_observation_contribution: Points from contact-behavior signals
        protagine_action_contribution: Points from Protagine's own behavior signals
        dominant_signal: Which single signal most influenced the score
    """
    person_id: str
    total_score: float
    owner_layer_contribution: float
    protagine_observation_contribution: float
    protagine_action_contribution: float
    dominant_signal: str
...
class TierChangeProposal:
    """A Protagine-generated proposal to change a person's tier.

    Protagine surfaces these via the briefing system. Owner approval is required
...
class RelationshipScorer:
    """Protagine's evidence-based scoring of its own relationships with each person.

    Scores are Protagine's independent assessment derived from interaction signals
    ...
    owner_guidance weight (0.18) but does not override Protagine's evidence-based view.

    Protagine-specific signals (protagine_initiative_response, protagine_effectiveness,
    protagine_style_fit) capture the quality of Protagine's own engagement behavior and
    carry a combined 0.18 weight.

    A score of 80+ means Protagine has observed strong, consistent engagement with
    ...
    SIGNAL_RANGES = {
        ...
        # Protagine-specific signals are already normalized 0-1
        "protagine_initiative_response": (0, 1, False),
        "protagine_effectiveness": (0, 1, False),
        "protagine_style_fit": (0, 1, False),
        "contact_response_to_protagine": (0, 1, False),
    }

    # Protagine-specific signal types (direction="protagine" or "bilateral")
    PROTAGINE_SIGNAL_TYPES = frozenset({
        "protagine_initiative",
        "protagine_initiative_response",
        "protagine_effectiveness",
        "contact_response_to_protagine",
        "protagine_style_fit",
    ...
    async def compute_score(
        ...
        weight_map = {
            ...
            # Protagine-specific signals
            "protagine_initiative_response": self.weights.protagine_initiative_response,
            "contact_response_to_protagine": self.weights.protagine_initiative_response,
            "protagine_effectiveness": self.weights.protagine_effectiveness,
            "protagine_style_fit": self.weights.protagine_style_fit,
            "protagine_style_signal": self.weights.protagine_style_fit,
        }
        ...
        # Informs Protagine's assessment without overriding it.
    ...
    async def compute_score_with_breakdown(
        ...
        if not signals:
            bd = ScoreBreakdown(
                ...
                protagine_observation_contribution=0.0,
                protagine_action_contribution=0.0,
        ...
        protagine_action_signals = {
            "protagine_initiative_response", "contact_response_to_protagine",
            "protagine_effectiveness", "protagine_style_fit", "protagine_style_signal",
        }
        ...
        for sig_type, total in type_scores.items():
            if sig_type in weight_map and type_counts[sig_type] > 0:
                ...
                if sig_type in protagine_action_signals:
        ...
        bd = ScoreBreakdown(
            ...
            protagine_observation_contribution=observation_total,
            protagine_action_contribution=action_total,
    ...
    def propose_tier_change(
        ...
        Protagine MAY propose tier changes (upward or downward) based on its
        ...
        if evidence_idx > current_idx:
            ...
            direction = "upward"
            reasoning = (
                f"Protagine has observed consistent engagement from this person "
            ...
            direction = "downward"
            reasoning = (
                f"Protagine's engagement with this person has declined "
