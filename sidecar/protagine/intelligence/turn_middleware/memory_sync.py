...
``protagine.intelligence.components.session_continuity.SessionContinuity``
(expected when protagine runs in-process — no HTTP loopback) or the
...
async def sync_turn_memory(
    ...
    *,
    ...
    outcome so the host stays up even if Protagine's continuity store is
    misbehaving.
    """
