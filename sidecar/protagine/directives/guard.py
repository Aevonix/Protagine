...
def _common_terms() -> frozenset:
    ...
    own name, plus any deployment-supplied extras (PROTAGINE_MATCH_COMMON_TERMS,
    csv). Live incident 2026-07-05: a fragment boundary whose only surviving
    term was "protagine" matched every job that said "report to Protagine"."""
    import os
    terms = {"protagine"}
    agent = os.environ.get("PROTAGINE_AGENT_NAME", "")
    for tok in re.findall(r"[a-z0-9]+", agent.lower()):
        if len(tok) > 1:
            terms.add(tok)
    extra = os.environ.get("PROTAGINE_MATCH_COMMON_TERMS", "")
