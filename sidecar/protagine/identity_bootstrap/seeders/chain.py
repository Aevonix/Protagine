"""Chain seeder — submits a PROTAGINE_REGISTER transaction on first boot."""
...
class ChainSeeder:
    ...
    async def seed(self, corpus: Any) -> None:
        ...
        try:
            from protagine.chain.transactions import Transaction, TxType
        except ImportError:
            logger.debug("chain: transactions module not importable — skipping")
            return

        tx = Transaction(
            tx_id=f"bootstrap-{corpus.protagine_id[:8]}-{uuid.uuid4().hex[:8]}",
            type=TxType.PROTAGINE_REGISTER,
            from_protagine_id="system",
