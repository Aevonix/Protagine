...
class SkillsSeeder:
    ...
    async def seed(self, corpus: Any) -> None:
        ...
        manifest = SkillManifest(
            ...
            version="1.0.0",
            description=(
                "Runs the Protagine Identity Bootstrap self-check matrix (16 checks). "
                "Returns a verification report showing which systems are healthy."
            ),
            author_protagine_id=protagine_id,
            created_at=now,
            updated_at=now,
            status=SkillStatus.ACTIVE,
            entry_point="protagine.identity_bootstrap.skill:run",
