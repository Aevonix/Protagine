...
Uses ProtagineGraph.store_memory() which persists to Neo4j and indexes in
...
class MemorySeeder:
    ...
    def __init__(self, protagine_graph: Optional[Any] = None) -> None:
    ...
    async def seed(self, corpus: Any) -> None:
        ...
        memories = [
            {
                "id": f"mem-bootstrap-identity-{protagine_id[:8]}",
                "type": "episodic",
                "content": (
                    f"I am {corpus.protagine_name}, a Protagine AI agent instance with ID {protagine_id}. "
                    f"I run Protagine v{corpus.protagine_version} on network {corpus.network_id}. "
                    "I was bootstrapped with full self-knowledge at first boot."
                ),
                "tags": ["self", "identity", "bootstrap", "core"],
                "source": "identity_bootstrap",
                "entities": [corpus.protagine_name, "Protagine"],
                "metadata": {
                    "bootstrap_id": f"mem-bootstrap-identity-{protagine_id[:8]}",
            ...
            {
                "id": f"mem-bootstrap-architecture-{protagine_id[:8]}",
                "type": "semantic",
                "content": (
                    "Protagine has 10 architectural layers: Gateway, API, Intelligence, Memory, "
                ...
                "source": "identity_bootstrap",
                "entities": ["Protagine"] + [l.name for l in corpus.layers],
                "metadata": {
                    "bootstrap_id": f"mem-bootstrap-architecture-{protagine_id[:8]}",
            ...
            {
                "id": f"mem-bootstrap-safety-{protagine_id[:8]}",
                "type": "semantic",
                "content": (
                ...
                "source": "identity_bootstrap",
                "entities": ["ResponseGate", "Protagine"],
                "metadata": {
                    "bootstrap_id": f"mem-bootstrap-safety-{protagine_id[:8]}",
            ...
            {
                "id": f"mem-bootstrap-cognition-{protagine_id[:8]}",
                "type": "semantic",
                "content": (
                    "Protagine's cognition pipeline runs 8 phases: MetaLearning, StrategyAdjustment, "
                ...
                "source": "identity_bootstrap",
                "entities": ["Protagine", "CognitionPipeline"],
                "metadata": {
                    "bootstrap_id": f"mem-bootstrap-cognition-{protagine_id[:8]}",
        ...
        if self._graph is not None:
            ...
            logger.warning(
                "memory: no protagine_graph available — falling back to ephemeral API dict"
            )
            try:
