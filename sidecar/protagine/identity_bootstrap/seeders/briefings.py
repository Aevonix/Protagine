...
class BriefingsSeeder:
    ...
    async def seed(self, corpus: Any) -> None:
        ...
        briefing_id = f"briefing-bootstrap-{protagine_id[:8]}"
        ...
        welcome.sections = [
            BriefingSection(
                section_id=f"sec-bootstrap-identity-{protagine_id[:8]}",
                name="Identity",
                content={
                ...
                narrative=(
                    f"This is {corpus.protagine_name} (Protagine v{corpus.protagine_version}), "
                    f"instance ID {protagine_id}, on network {corpus.network_id}. "
                    "Bootstrap complete."
                ),
                priority=100,
            ),
            BriefingSection(
                section_id=f"sec-bootstrap-systems-{protagine_id[:8]}",
                name="Active Systems",
                content={
                ...
                narrative=(
                    f"Protagine has {len(corpus.layers)} architectural layers and "
                    f"{len(corpus.api_endpoints)} API endpoints ready."
                ),
                priority=80,
            ),
            BriefingSection(
                section_id=f"sec-bootstrap-safety-{protagine_id[:8]}",
                name="Safety Status",
                content={
