...
class WorldModelSeeder:
    ...
    async def seed(self, corpus: Any) -> None:
        ...
        from protagine.world_model.entities import ConceptEntity
        ...
        # Self entity
        self_id = f"protagine-self-{protagine_id}"
        self_entity = ConceptEntity(
            id=self_id,
            name=corpus.protagine_name,
            entity_type="concept",
            concept_type="protagine_instance",
            aliases=[protagine_id, "self", corpus.protagine_name],
            confidence=1.0,
            properties={
        ...
        for subsystem in SUBSYSTEMS:
            sub_id = f"protagine-subsystem-{protagine_id}-{subsystem}"
            sub_entity = ConceptEntity(
                id=sub_id,
                name=f"{subsystem} subsystem",
                entity_type="concept",
                concept_type="protagine_subsystem",
                aliases=[subsystem],
                confidence=1.0,
                properties={
                    "subsystem_name": subsystem,
                    "parent_protagine_id": protagine_id,
                },
                first_seen=now,
                last_seen=now,
                created_at=now,
        ...
        for src_sub, tgt_sub in dep_pairs:
            src_id = f"protagine-subsystem-{protagine_id}-{src_sub}"
            tgt_id = f"protagine-subsystem-{protagine_id}-{tgt_sub}"
