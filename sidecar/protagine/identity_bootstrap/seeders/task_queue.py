...
class TaskQueueSeeder:
    ...
    async def seed(self, corpus: Any) -> None:
        ...
        caps = WorkerCapabilities(
            node_id=f"bootstrap-{protagine_id[:8]}",
        ...
        job_id = f"job-bootstrap-selfcheck-{protagine_id[:8]}"
        job = Job(
            job_id=job_id,
            ...
            posted_by=f"bootstrap-{protagine_id[:8]}",
            tags={"type": "bootstrap", "protagine_id": protagine_id[:8]},
        )
        try:
