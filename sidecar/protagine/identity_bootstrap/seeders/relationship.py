...
class RelationshipSeeder:
    ...
    async def seed(self, corpus: Any) -> None:
        ...
        contact_id = f"self:{protagine_id}"
        now = _now_iso()
    ...
    async def _seed_sqlite(self, store: Any, contact_id: str, corpus: Any, now: str) -> None:
        ...
        protagine_id = corpus.protagine_id
        tags_json = json.dumps(["self", "system", "protagine"])
        ...
        # Add internal handle
        hdl_id = f"hdl-self-{protagine_id[:8]}"
        try:
            await db.execute(
                """
                ...
                """,
                (hdl_id, contact_id, "custom", f"protagine://{protagine_id}", 1, 1, 1.0, "identity_bootstrap", now),
            )
            await db.commit()
        except Exception as exc:
    ...
    def _seed_memory(self, contacts_mod: Any, contact_id: str, corpus: Any, now: str) -> None:
        ...
        in_mem[contact_id] = {
            ...
            "display_name": corpus.protagine_name,
            "given_name": "Protagine",
            "family_name": "Self",
            "organization": "Protagine AI",
            "relationship_score": 1.0,
            "trust_tier": "inner_circle",
            "interaction_allowed": True,
            "tags": ["self", "system", "protagine"],
            "privacy_level": "private",
            "person_node_id": f"protagine-self-{corpus.protagine_id}",
