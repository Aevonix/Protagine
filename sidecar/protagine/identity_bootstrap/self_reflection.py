"""Protagine Identity Bootstrap — SelfReflectionComponent.
...
class SelfReflectionComponent:
    ...
    async def reflect(
        ...
        notes = self._build_notes(protagine_id, corpus, critical, warnings, health_score)
        ...
        logger.info(
            "self_reflection: protagine=%s health=%.2f critical=%d warnings=%d",
            protagine_id,
            health_score,
            len(critical),
            len(warnings),
    ...
    def _generate_proposals(self, critical: List[Any], warnings: List[Any]) -> List[str]:
        ...
        if "corpus" in systems_with_issues:
            proposals.append(
                "Update the identity_bootstrap corpus to match the current Protagine version."
            )
        if any(a.check == "protagine_id_set" for a in critical):
            proposals.append(
                "Set PROTAGINE_ID env var or ensure ChainManager is initialized before bootstrap."
            )
    ...
    def _build_notes(
        ...
        lines = [
            f"Protagine: {corpus.protagine_name} ({protagine_id})",
            f"Version: {corpus.protagine_version}",
