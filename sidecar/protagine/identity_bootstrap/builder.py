"""Protagine Identity Bootstrap — IdentityBootstrapBuilder.

Assembles a SelfKnowledgeCorpus at runtime by reading protagine_id, version,
...
from protagine.identity_bootstrap.corpus import (
    CORPUS_VERSION,
    GATE_LAYERS,
    INFERENCE_TIERS,
    API_ENDPOINTS,
...
def _protagine_version() -> str:
    """Return Protagine version from canonical source."""
    # Canonical: protagine_cli.__init__ (full_version_string returns "0.3.0-h6")
    try:
        from protagine_cli import full_version_string
    ...
    try:
        from importlib.metadata import version
        return version("protagine-ai")
    except Exception:
        pass
    return "0.3.0-h6"


def _read_protagine_id(chain_manager: Optional[Any]) -> str:
    """Read protagine_id from ChainManager, chain.db, or environment.

    Priority order:
    1. ChainManager.protagine_id (if ChainManager is initialized)
    2. PROTAGINE_ID environment variable
    3. Genesis block genesis_protagine_id from chain.db (crypto identity)
    4. Hostname-based fallback (last resort)
    """
    if chain_manager is not None:
        try:
            cid = chain_manager.protagine_id
            if cid and cid != "local":
                return str(cid)
        except Exception as exc:
            logger.debug("ChainManager.protagine_id unavailable: %s", exc)

    env_id = os.environ.get("PROTAGINE_ID", "")
    if env_id:
        return env_id

    # Read genesis_protagine_id directly from chain.db — this is the
    ...
    chain_db = os.path.join(protagine_home, "chain.db")
    if os.path.exists(chain_db):
        try:
            ...
            if row:
                ...
                cid = genesis_data.get("metadata", {}).get("genesis_protagine_id")
                if cid:
                    return cid
        except Exception as exc:
            logger.debug("_read_protagine_id chain.db fallback failed: %s", exc)

    # Last resort: hostname-based stable ID
    import socket
    import hashlib
    raw = f"{socket.gethostname()}-protagine"
...
def _read_network_id(chain_manager: Optional[Any]) -> str:
    try:
        ...
        pass
    return os.environ.get("PROTAGINE_NETWORK_ID", "local")
...
def _resolve_inference_tiers() -> list[InferenceTier]:
    ...
    reads them from ``protagine.router.tiers.DEFAULT_TIERS`` at runtime so the
    ...
    try:
        from protagine.router.tiers import DEFAULT_TIERS, ModelTier  # type: ignore[import]
...
class IdentityBootstrapBuilder:
    """Builds a SelfKnowledgeCorpus by introspecting the running Protagine instance."""

    def __init__(self, chain_manager: Optional[Any] = None) -> None:
        self._chain_manager = chain_manager

    def build(self) -> SelfKnowledgeCorpus:
        protagine_id = _read_protagine_id(self._chain_manager)
        protagine_name = os.environ.get("PROTAGINE_NAME", f"protagine-{protagine_id[:8]}")
        protagine_version = _protagine_version()
        ...
        corpus = SelfKnowledgeCorpus(
            ...
            protagine_name=protagine_name,
            protagine_version=protagine_version,
