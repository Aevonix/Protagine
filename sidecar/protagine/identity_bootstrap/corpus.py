"""Protagine Identity Bootstrap — SelfKnowledgeCorpus.

Static catalog of Protagine's own architecture, layers, endpoints, and phases.
...
@dataclass
class LayerRecord:
    """A single architectural layer in Protagine."""
    name: str
    description: str
...
class CognitionPhase:
    """A phase in the Protagine cognition / meta-learning pipeline."""
    name: str
    description: str
...
@dataclass
class InferenceTier:
    """A tier in the Protagine smart model routing system."""
    name: str
    description: str
...
class ArchitectureManifest:
    """Top-level container describing Protagine's own architecture."""
    protagine_id: str
    protagine_name: str
    protagine_version: str
    network_id: str
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "protagine_name": self.protagine_name,
            "protagine_version": self.protagine_version,
...
LAYERS: List[LayerRecord] = [
    ...
    LayerRecord(
        name="API",
        description="FastAPI REST + WebSocket server exposing all Protagine capabilities. Handles auth, routing, audit logging.",
        subsystems=["protagine.api.server", "protagine.api.routers", "protagine.api.middleware", "protagine.api.auth"],
        layer_index=1,
    ),
    LayerRecord(
        name="Intelligence",
        ...
        subsystems=[
            "protagine.intelligence.cognition",
            "protagine.intelligence.mind_model",
            "protagine.intelligence.synthesis",
            "protagine.intelligence.relationships",
            "protagine.intelligence.components",
        ],
        layer_index=2,
    ),
    LayerRecord(
        ...
        subsystems=["protagine.world_model", "protagine.contacts", "protagine.api.routers.memory", "protagine.intelligence.graph"],
        layer_index=3,
    ),
    LayerRecord(
        name="Goals",
    ...
    LayerRecord(
        ...
        subsystems=["protagine.skills", "protagine.skill_security", "protagine.plugins"],
        layer_index=5,
    ),
    LayerRecord(
        name="TaskQueue",
    ...
    LayerRecord(
        ...
        subsystems=["protagine.inference", "protagine.router", "agent.context_compressor"],
        layer_index=9,
    ),
]
...
API_ENDPOINTS: List[EndpointRecord] = [
    # ── Admin (/v1/admin) ────────────────────────────────────────────────────
    EndpointRecord("/v1/admin/register", "POST", "Register a new protagine node", "admin", tags=["admin"]),
    EndpointRecord("/v1/admin/attest", "POST", "Attest trust to a target protagine (genesis admin only)", "admin", tags=["admin"]),
    EndpointRecord("/v1/admin/revoke", "POST", "Revoke trust from a target protagine", "admin", tags=["admin"]),
    EndpointRecord("/v1/admin/network", "GET", "List all registered protagine nodes", "admin", tags=["admin"]),
    EndpointRecord("/v1/admin/suspend", "POST", "Suspend a registered protagine node", "admin", tags=["admin"]),
    EndpointRecord("/v1/admin/reinstate", "POST", "Reinstate a suspended protagine node", "admin", tags=["admin"]),
    ...
    EndpointRecord("/v1/chain/register", "POST", "Register a protagine on-chain", "chain", tags=["chain"]),
