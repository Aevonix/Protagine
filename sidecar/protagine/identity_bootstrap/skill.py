...
Registered in the Protagine skill registry as 'bootstrap-self-check'.
...
async def run(protagine_id: Optional[str] = None, mode: str = "full") -> dict:
    """Run the Protagine Identity Bootstrap self-check.

    Args:
        protagine_id: Override protagine_id (default: read from running instance).
    ...
    """
    from protagine.identity_bootstrap.runner import IdentityBootstrap

    bootstrap = IdentityBootstrap()
    report = await bootstrap.verify_only()

    return {
        "protagine_id": report.protagine_id,
        "mode": report.mode,
