"""Protagine Identity Bootstrap — self-referential query detection.

Detects user messages that ask about Protagine's own capabilities,
...
# Each pattern targets a class of question about Protagine's own capabilities.
_SELF_PATTERNS = [
    ...
    re.compile(r"\byour\b.*\b(?:version|protagine.?id|identity)\b", re.I),
...
def query_is_self_referential(message: str) -> bool:
    """Return True if *message* is asking about Protagine's own capabilities."""
...
def build_self_context_from_corpus() -> Optional[str]:
    ...
    lines = [
        "# Protagine Self-Knowledge (auto-injected)",
        "",
        f"**Identity:** {corpus.protagine_name} (ID: {corpus.protagine_id}, "
        f"v{corpus.protagine_version}, network: {corpus.network_id})",
        "",
