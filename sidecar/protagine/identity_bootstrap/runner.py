"""Protagine Identity Bootstrap — IdentityBootstrap orchestrator.
...
BootstrapReport to ~/.protagine/bootstrap.db.
"""
...
from protagine.identity_bootstrap.builder import IdentityBootstrapBuilder
from protagine.identity_bootstrap.models import BootstrapAnomaly, BootstrapReport
from protagine.identity_bootstrap.verifier import BootstrapVerifier
from protagine.identity_bootstrap.self_reflection import SelfReflectionComponent
...
CREATE INDEX IF NOT EXISTS idx_bootstrap_protagine ON bootstrap_runs(protagine_id, completed_at DESC);
"""
...
class _BootstrapDB:
    ...
    def save_report(self, report: BootstrapReport, protagine_version: str = "") -> None:
        import uuid
        run_id = str(uuid.uuid4())
        report_dict = report.to_dict()
        if protagine_version:
            report_dict["protagine_version"] = protagine_version
        with self._lock:
            self._conn.execute(
                """
                INSERT OR REPLACE INTO bootstrap_runs
                    (run_id, protagine_id, mode, started_at, completed_at, success,
    ...
    def last_protagine_version(self, protagine_id: str) -> Optional[str]:
        """Return the protagine_version stored in the most recent bootstrap run."""
        last = self.last_run(protagine_id)
        if last is None:
            return None
        try:
            data = json.loads(last.get("report_json", "{}"))
            return data.get("protagine_version") or None
        except Exception:
            return None

    def last_run(self, protagine_id: str) -> Optional[dict]:
        with self._lock:
            row = self._conn.execute(
                "SELECT * FROM bootstrap_runs WHERE protagine_id = ? ORDER BY completed_at DESC LIMIT 1",
                (protagine_id,),
            ).fetchone()
        if row is None:
            return None
...
class IdentityBootstrap:
    """Orchestrates the Protagine Identity Bootstrap process."""
    ...
    def __init__(
        ...
        protagine_home = os.environ.get(
            "PROTAGINE_HOME",
            os.path.join(os.path.expanduser("~"), ".protagine"),
        )
        db_path = os.path.join(protagine_home, "bootstrap.db")
        try:
    ...
    async def _determine_mode(self, corpus) -> str:
        ...
        if self._db is not None:
            try:
                last = self._db.last_run(protagine_id)
                if last is not None:
                    # Fix 6: version guard — only run full bootstrap if protagine_version changed
                    last_protagine_ver = self._db.last_protagine_version(protagine_id)
                    if last_protagine_ver and last_protagine_ver == corpus.protagine_version:
                        logger.debug(
                            "_determine_mode: protagine_version=%s unchanged → VERIFY",
                            corpus.protagine_version,
                        )
                        return "VERIFY"
        ...
        try:
            import protagine.api.routers.world as wm_mod
            backend = getattr(wm_mod, "_wm_backend", None)
            if backend is not None:
                entity = await backend.get_entity(f"protagine-self-{protagine_id}")
    ...
    async def _run_seeders(self, corpus) -> tuple[List[str], List[str]]:
        ...
        group1 = [
            WorldModelSeeder(),
            RelationshipSeeder(),
            MemorySeeder(protagine_graph=self._graph),
    ...
    async def run(self) -> BootstrapReport:
        ...
        logger.info(
            "IdentityBootstrap starting (mode=%s, protagine=%s, version=%s)",
            mode,
            corpus.protagine_id,
            corpus.protagine_version,
        )
        ...
        # Always run verifier
        verifier = BootstrapVerifier(corpus, protagine_graph=self._graph)
        ...
        if self._db is not None and mode != "VERIFY":
            try:
                self._db.save_report(report, protagine_version=corpus.protagine_version)
            except Exception as exc:
