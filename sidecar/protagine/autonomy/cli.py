"""CLI entry point for Protagine autonomy loop management.

Usage:
    protagine autonomy status     # live loop stats from the running sidecar
    protagine autonomy cycle      # wake the loop / run one cycle now
...
sidecar over HTTP, resolved the same way the doctor does (PROTAGINE_URL /
PROTAGINE_SIDECAR_HOST/PORT, PROTAGINE_API_KEY).
"""
...
def _request(path: str, method: str = "GET") -> dict:
    ...
    url = default_protagine_url().rstrip("/") + path
...
def run_autonomy_command(args: list) -> int:
    """Dispatch 'protagine autonomy <subcommand>'."""
    if not args:
        print("Usage: protagine autonomy <status|cycle>", file=sys.stderr)
        return 1
    ...
    if sub == "start":
        ...
        print("The autonomy loop runs inside the sidecar and starts with it "
              "(protagine serve). Use 'protagine autonomy status' to inspect it or "
              "'protagine autonomy cycle' to wake it.", file=sys.stderr)
        return 1
