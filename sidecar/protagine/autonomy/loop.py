"""AutonomyLoop — Protagine's continuous operating cycle.
...
def _get_broadcast():
    ...
    if _broadcast is None:
        try:
            from protagine.api.routers.host import broadcast_event
...
def _proactive_enforce_verdict_error(
    ...
    from protagine.gate.surface_policy import ResponseGuardSurfacePolicyV1
...
class AutonomyLoop:
    """Protagine's continuous operating cycle.
    ...
    Each tick:
      ...
      4. Run initiative engine — Protagine decides whether to act
    ...
    def _start_governed_delivery_reconciler(self) -> None:
        ...
        if task is None or task.done():
            self._governed_reconcile_task = asyncio.create_task(
                ...
                name="protagine-governed-delivery-reconciliation",
            )
    ...
    async def _tick_phases(self) -> None:
        ...
        # Phase 11d2: tom2 knowledge asymmetry (daily; PROTAGINE_TOM2, default off)
    ...
    async def _phase_telemetry(self) -> None:
        try:
            from protagine.api.routers.host import _telemetry
        ...
        try:
            ...
            initiatives = await engine.generate(
                ...
                cooldown_tasks=float(os.environ.get(
                    "PROTAGINE_INITIATIVE_COOLDOWN_TASKS", "12",
                )),
                cooldown_contacts=float(os.environ.get(
                    "PROTAGINE_INITIATIVE_COOLDOWN_CONTACTS", "72",
                )),
            )
    ...
    async def _phase_thinking(self) -> None:
        ...
        On a slow cadence (PROTAGINE_THINKING_INTERVAL_SECS), hand the LLM a
        ...
        Disabled unless PROTAGINE_ENABLE_INTERNAL_THINKING=true.
        """
        # Mode: off | shadow | live (PROTAGINE_THINKING_MODE). Back-compat:
        # PROTAGINE_ENABLE_INTERNAL_THINKING=true means "live". The autonomy
        # preset fills the unset case (explicit env always wins).
        mode = os.environ.get("PROTAGINE_THINKING_MODE", "").strip().lower()
        ...
        if thinker is None:
            from protagine.intelligence.components.self_directed_thinker import (
                SelfDirectedThinker,
            )
        ...
        try:
            ...
            from protagine.proposals import build_from_thinker, proposal_to_payload
    ...
    def _build_thinking_situation(self) -> dict:
        ...
        try:
            from protagine.api.routers.observations import (
    ...
    async def _recipient_is_owner(self, person_id: str, payload: dict) -> bool:
        ...
        if not person_id or person_id == "owner":
            return True
        try:
            from protagine.identity.resolver import get_identity_resolver
    ...
    async def _route_reachout_delivery(self, payload: dict, delivery: Any) -> bool:
        ...
        from protagine.delivery import reachout_policy as rp
        ...
        if directives is not None:
            try:
                ...
                from protagine.directives.guard import boundary_fail_closed
        ...
        # logs, in enforce (PROTAGINE_GUARD_MODE=enforce) it blocks a leaking
        ...
        # gate above.
        guard_enforcing = (
            os.environ.get("PROTAGINE_GUARD_MODE", "").strip().lower()
            == "enforce"
        )
        try:
            from protagine.api.routers.host import _response_guard
    ...
    def _get_rejection_store(self) -> Any:
        ...
        if not hasattr(self, "_rejection_store"):
            ...
            try:
                ...
                from protagine.gate.rejection import RejectionStore
                state_dir = Path(os.environ.get(
                    "PROTAGINE_STATE_DIR", os.path.expanduser("~/.protagine")))
                self._rejection_store = RejectionStore(
                    str(state_dir / "protagine-gate-rejections.db"))
            except Exception:
    ...
    async def _run_rejection_feedback(self, text: str, guard_result: Any, *,
        ...
        from protagine.api.routers.host import _response_guard
        from protagine.gate.rejection import RejectionFeedbackLoop
        if _response_guard is None:
            return None
    ...
    async def _phase_observation_sync(self) -> None:
        ...
        The agent is Protagine's sensor array: when a domain's newest
        ...
        snapshots back to /v1/host/observations. Protagine never calls
        ...
        from protagine.observations.store import OBSERVATION_SYNC_INTERVALS

        enabled = os.environ.get(
            "PROTAGINE_SYNC_DOMAINS",
                ...
                await task_queue.submit(
                    ...
                    params={
                        ...
                        "description": (
                            ...
                            f"connections and report snapshots to Protagine"
                        ),
    ...
    async def _post_agent_action_to_queue(
        ...
        legacy_auto_approve_requested = (
            os.environ.get("PROTAGINE_AGENT_AUTO_APPROVE", "false").lower()
            == "true"
        )
        ...
        if legacy_auto_approve_requested and is_gated:
            logger.warning(
                "Ignoring retired PROTAGINE_AGENT_AUTO_APPROVE for effectful %s",
                action_hint,
            )
        ...
        try:
            from protagine.task_queue.models import JobStatus
            ...
            if bounded_grant is not None and job_id:
                ...
                try:
                    from protagine.events.broadcaster import emit as broadcast
            ...
            if gate_pending and job_id:
                ...
                if delivery and hasattr(delivery, "push_initiative"):
                    ...
                    # gates Protagine's *own* proactive outreach, not the owner's need to
                    ...
                    await delivery.push_initiative({
                        ...
                        "description": getattr(initiative, "description", ""),
                        "rationale": getattr(initiative, "rationale", ""),
                        "suggested_action": "protagine_approve_initiative",
                        "entity_id": getattr(initiative, "entity_id", None),
                        "channel_hint": "dm",
                        "context": {
                            "job_id": job_id,
    ...
    async def _feed_introduction_candidates(self, engine: Any) -> None:
        ...
        PROTAGINE_INTROS_ENABLED=false; trust floor via PROTAGINE_INTRO_TRUST_FLOOR.
        """
        if os.environ.get("PROTAGINE_INTROS_ENABLED", "true").lower() != "true":
            return
        ...
        try:
            floor = os.environ.get("PROTAGINE_INTRO_TRUST_FLOOR", "regular")
    ...
    async def _phase_job_writeback(self) -> None:
        ...
        was invisible to memory — Protagine could act but never learn from
    ...
    async def _writeback_one_job(self, job: Any) -> None:
        ...
        if graph is not None and hasattr(graph, "store_memory"):
            ...
            await graph.store_memory(
                ...
                source_uri=f"protagine://jobs/{job.job_id}",
            )
        ...
        # 4. Skill capture (v0.17.0, PROTAGINE_ENABLE_SKILL_SYNTHESIS) — feed
    ...
    def _get_skill_learning(self) -> Any:
        ...
        if os.environ.get("PROTAGINE_ENABLE_SKILL_SYNTHESIS",
                          "false").lower() != "true":
            return None
        ...
        try:
            ...
            from protagine.skills.learning import (
                NoveltyDetector,
                PatternExtractor,
                SkillLearningService,
            )
            from protagine.skills.packager import SkillPackager

            library = pathlib.Path(
                os.environ.get("PROTAGINE_SKILL_LIBRARY")
                or os.path.join(os.environ.get("PROTAGINE_STATE_DIR", "."),
                                "skill_library"))
            packager = SkillPackager(
                registry=skills_registry,
                protagine_id=os.environ.get("PROTAGINE_NODE_ID", "protagine"),
    ...
    async def _maybe_capture_skill(self, job: Any, action: str,
        ...
        try:
            ...
            from protagine.skills.learning.triggers import (
    ...
    async def _phase_project_result_reconciliation(self) -> None:
        ...
        try:
            raw_limit = int(os.environ.get(
                "PROTAGINE_PROJECT_RECONCILIATION_LIMIT", "25",
            ))
        except (TypeError, ValueError):
            raw_limit = 25
        ...
        try:
            raw_budget = float(os.environ.get(
                "PROTAGINE_PROJECT_RECONCILIATION_BUDGET_SECS", "5",
            ))
        except (TypeError, ValueError):
            raw_budget = 5.0
    ...
    async def _phase_trust_notices(self) -> None:
        ...
        if delivery is None:
            return
        try:
            from protagine.proposals import Proposal, proposal_to_payload
        except Exception:
            return
    ...
    async def _phase_selfhood_benchmark(self) -> None:
        ...
        try:
            ...
            logger.error("Phase selfhood_benchmark error: %s", exc,
                         exc_info=True)
            return
        if os.environ.get("PROTAGINE_BENCHMARK_REPORT",
    ...
    def _phase_budget_secs(self) -> float:
        try:
            return float(os.environ.get("PROTAGINE_PHASE_BUDGET_SECS", "150"))
        except ValueError:
            return 150.0
    ...
    def _tick_budget_secs(self) -> float:
        ...
        try:
            v = float(os.environ.get("PROTAGINE_TICK_BUDGET_SECS", "0") or 0)
        except ValueError:
            v = 0.0
        if v > 0:
            return v
    ...
    async def _toolsmith_body(self) -> None:
        ...
        try:
            from protagine.toolsmith.registry import ToolStatus
    ...
    async def _phase_relationship_profiling(self) -> None:
        ...
        try:
            from protagine.api.routers.host import _relationship_profiler
        except ImportError:
            return
        ...
        try:
            refresh_secs = float(os.environ.get(
                "PROTAGINE_RELATIONSHIP_PROFILE_REFRESH_SECS", "21600"))
        except ValueError:
            pass
        now = time.time()
    ...
    async def _phase_tom2_asymmetry(self) -> None:
        ...
        PROTAGINE_TOM2 is set (default off; shadow computes counts only, live
    ...
    async def _phase_connectors(self) -> None:
        ...
        if manager is None:
            return
        try:
            from protagine.connectors import connectors_mode
    ...
    async def _phase_condition_checks(self) -> None:
        ...
        if self._periodic_last.get("condition_checks") != key:
            ...
            from protagine.autonomy.condition_worker import (
    ...
    async def _phase_memory_pruning(self) -> None:
        ...
        PROTAGINE_MEMORY_PRUNE_MODE gates the phase: ``off`` disables it,
        ...
        """
        mode = os.environ.get(
            "PROTAGINE_MEMORY_PRUNE_MODE", "shadow").strip().lower()
        if mode == "off":
            return
    ...
    async def _phase_memory_distillation(self) -> None:
        ...
        async def work(graph):
            try:
                from protagine.intelligence.graph.distiller import MemoryDistiller
    ...
    async def _phase_memory_reconciliation(self) -> None:
        async def work(graph):
            from protagine.intelligence.graph.reconciler import FileReconciler
    ...
    async def _phase_approval_timeout(self) -> None:
        ...
        PROTAGINE_APPROVAL_TIMEOUT_HOURS (default 72) are failed with reason
        ...
        if task_queue is None:
            return
        try:
            timeout_hours = float(os.environ.get("PROTAGINE_APPROVAL_TIMEOUT_HOURS", "72"))
    ...
    def _in_quiet_hours(self) -> bool:
        ...
        from protagine.util.quiet_hours import in_quiet_window
