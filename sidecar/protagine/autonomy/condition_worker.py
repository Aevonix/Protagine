"""Poll blocked goal records and system conditions from the autonomy loop.

Resolving a condition updates the record; it never dispatches execution.
"""
...
async def _check_email_reply(params: dict) -> dict:
    ...
    Credentials are loaded from SecretsManager:
        PROTAGINE_IMAP_HOST, PROTAGINE_IMAP_PORT, PROTAGINE_IMAP_USERNAME, PROTAGINE_IMAP_PASSWORD
    ...
    try:
        ...
        from protagine.email.providers import IMAPProvider
    except ImportError:
    ...
    sm = SecretsManager()
    host = sm.get_required("PROTAGINE_IMAP_HOST")
    port = int(sm.get("PROTAGINE_IMAP_PORT") or "993")
    username = sm.get_required("PROTAGINE_IMAP_USERNAME")
    password = sm.get_required("PROTAGINE_IMAP_PASSWORD")
...
async def _check_delivery_status(params: dict) -> dict:
    ...
    Carrier API keys are loaded from SecretsManager:
        PROTAGINE_UPS_API_KEY, PROTAGINE_FEDEX_API_KEY
    ...
    try:
        ...
        raise RuntimeError(
            "_check_delivery_status requires protagine.secrets"
        ) from exc
...
async def _check_commitment_overdue(params: dict) -> dict:
    ...
    from protagine.api.routers.host import _commitment_store, emit
...
async def _check_surprise_accumulation(params: dict) -> dict:
    ...
    from protagine.api.routers.host import _surprise_store
