"""Tool handlers for Protagine-native server-side execution.
...
for calling Protagine's intelligence systems.
"""
...
async def handle_get_relationship(
    ...
    try:
        ...
        logger.error("protagine_get_relationship failed: %s", e)
...
async def handle_list_goals(
    ...
    try:
        ...
        logger.error("protagine_list_goals failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_get_briefing(
    ...
    try:
        ...
        logger.error("protagine_get_briefing failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_record_insight(
    ...
    try:
        ...
        logger.error("protagine_record_insight failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_query_entities(
    ...
    try:
        ...
        logger.error("protagine_query_entities failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_start_research(
    ...
    try:
        ...
        logger.error("protagine_start_research failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_discover_connections(
    ...
    try:
        ...
        logger.error("protagine_discover_connections failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_list_boundaries(
    ...
    try:
        ...
        logger.error("protagine_list_boundaries failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_recent_boundary_blocks(
    ...
    try:
        ...
        logger.error("protagine_recent_boundary_blocks failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_flag_boundary_concern(
    ...
    try:
        ...
        logger.error("protagine_flag_boundary_concern failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_initiative_feedback(
    ...
    try:
        ...
        logger.error("protagine_initiative_feedback failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_recall_skills(
    ...
    try:
        ...
        from protagine.skills_memory import relevant_skills
...
TOOL_HANDLERS: dict[str, callable] = {
    "protagine_memory_search": handle_memory_search,
    "protagine_get_relationship": handle_get_relationship,
    "protagine_list_goals": handle_list_goals,
    "protagine_get_briefing": handle_get_briefing,
    "protagine_record_insight": handle_record_insight,
    "protagine_query_entities": handle_query_entities,
    "protagine_start_research": handle_start_research,
    "protagine_discover_connections": handle_discover_connections,
    "protagine_task_complete": handle_task_complete,
    "protagine_task_snooze": handle_task_snooze,
    "protagine_task_dismiss": handle_task_dismiss,
    "protagine_initiative_feedback": handle_initiative_feedback,
    "protagine_list_boundaries": handle_list_boundaries,
    "protagine_recent_boundary_blocks": handle_recent_boundary_blocks,
    "repo_list_files": handle_repo_list_files,
    "repo_read_file": handle_repo_read_file,
    "repo_search": handle_repo_search,
    "protagine_flag_boundary_concern": handle_flag_boundary_concern,
