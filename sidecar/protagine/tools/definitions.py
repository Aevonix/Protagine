"""Protagine-native tool definitions for server-side execution.

These tools are advertised to the LLM via the ReasoningLoop and executed
by the ToolExecutor. They provide direct access to Protagine's intelligence
...
# Core Protagine tools — memory, relationships, goals
PROTAGINE_CORE_TOOLS: list[dict[str, Any]] = [
    {
    ...
    {
        "name": "protagine_get_relationship",
        "description": (
...
PROTAGINE_EXTENDED_TOOLS: list[dict[str, Any]] = [
    {
        "name": "protagine_query_entities",
        "description": (
            "Query Protagine's world model for entities (people, places, organizations, concepts). "
    ...
    {
        "name": "protagine_discover_connections",
        "description": (
    ...
    {
        "name": "protagine_list_boundaries",
        "description": (
    ...
    {
        "name": "protagine_recent_boundary_blocks",
        "description": (
    ...
    {
        "name": "protagine_flag_boundary_concern",
        "description": (
...
# All tools combined
PROTAGINE_TOOLS: list[dict[str, Any]] = PROTAGINE_CORE_TOOLS + PROTAGINE_EXTENDED_TOOLS + NATIVE_TOOLS
...
STATIC_TOOL_NAMES = frozenset(str(tool["name"]) for tool in PROTAGINE_TOOLS)
...
def get_tool_definitions(
    ...
    """
    tools = PROTAGINE_CORE_TOOLS if not include_extended else PROTAGINE_TOOLS
