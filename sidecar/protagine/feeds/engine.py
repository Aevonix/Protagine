#!/usr/bin/env python3
"""Protagine feed collector engine — self-contained, stdlib-only.

This file is BOTH a package module and a deployable script: the feeds manager
copies it verbatim into the harness scripts dir (as ``protagine-feed-engine.py``)
where per-instance shims execute it under any python3.  Keep it stdlib-only.

Stages (selected via $PROTAGINE_FEED_STAGE, default ``collect``):
...
Config file (path in $PROTAGINE_FEED_CONFIG) is either the spec-rendered shape
...
# --------------------------------------------------------------------- config
def load_config():
    path = os.environ.get("PROTAGINE_FEED_CONFIG")
    if not path:
        raise SystemExit("PROTAGINE_FEED_CONFIG not set")
...
def main():
    cfg = load_config()
    stage = os.environ.get("PROTAGINE_FEED_STAGE", "collect")
    if stage == "collect":
        stage_collect(cfg)
    elif stage == "alert":
        stage_alert(cfg)
