...
in this registry, never a raw command string. Protagine builds initiatives
...
Approval policy (``PROTAGINE_APPROVAL_POLICY``, default ``strict``):
...
``PROTAGINE_AGENT_AUTO_APPROVE=true`` collapses the gate for trusted
...
# to /v1/host/observations. Protagine never calls external APIs itself.
...
_SPECS: List[ActionSpec] = [
    ...
    ActionSpec(
        ...
        tool="terminal",
        # The working checkout is deployment-specific: PROTAGINE_WORK_REPO names
        # the repo the hygiene check inspects (default: the worker's own cwd).
        command=f"git -C {os.environ.get('PROTAGINE_WORK_REPO', '.')} status --short",
        risk=RiskTier.READ_ONLY,
    ...
    ActionSpec(
        ...
        tool="terminal",
        command="protagine doctor --subsystem $ID",
        risk=RiskTier.READ_ONLY,
        description="Investigate a degraded Protagine subsystem",
        initiative_type="agent_action",
        required_params=["ID"],
    ),
    ActionSpec(
    ...
    ActionSpec(
        ...
        tool="terminal",
        command="protagine graph cleanup-orphans $ID",
...
def get_approval_policy() -> str:
    ...
    ``PROTAGINE_APPROVAL_POLICY=graduated`` opts a deployment into the
    graduated gate; anything else (including unset/unknown values) is
    ``strict`` — fail closed on typos.
    """
    mode = os.environ.get("PROTAGINE_APPROVAL_POLICY", APPROVAL_POLICY_STRICT)
