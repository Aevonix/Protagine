"""Durable, transport-neutral authority for Protagine action approvals.

Protagine owns approval *state* but deliberately knows nothing about WhatsApp,
...
SQLite transactions make decision and grant consumption atomic across Protagine
workers.  The database lives in ``PROTAGINE_STATE_DIR`` and is safe to copy with
the rest of Protagine state for rollback.
"""
...
GRANT_MAX_TTL_ENV = "PROTAGINE_GRANT_MAX_TTL_SECONDS"
GRANT_MAX_USES_ENV = "PROTAGINE_GRANT_MAX_USES"
GRANT_UNLIMITED_SENTINEL = "unlimited"
PRESENTATION_SCHEMA = "ProtagineApprovalPresentationV1"
AUTHORIZATION_PROJECTION_SCHEMA = "ProtagineApprovalAuthorizationProjectionV1"
TYPED_APPROVAL_SUBJECT_SCHEMA = "ProtagineApprovalSubjectBindingV1"
...
def canonical_approval_timeout_seconds() -> int:
    ...
    raw = os.environ.get("PROTAGINE_APPROVAL_TIMEOUT_HOURS")
    if raw in (None, ""):
        return DEFAULT_REQUEST_TTL_SECONDS
    try:
...
def build_action_binding(
    *,
    ...
    try:
        ...
        # than Protagine's built-in registry. Their exact action/risk/type tuple is
...
def authority_mode() -> str:
    ...
    value = os.environ.get("PROTAGINE_APPROVAL_AUTHORITY_MODE", "shadow").strip().lower()
