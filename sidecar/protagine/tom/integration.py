...
from protagine.tom.fact_adapters import (
    FactPayloadV1,
...
from protagine.tom.visibility_store import FactVisibilityStore
...
def p8_min_confidence() -> float:
    ...
    try:
        value = float(os.environ.get(
            "PROTAGINE_P8_FACT_MIN_CONFIDENCE",
...
class P8Runtime:
    ...
    def _fresh_until(record: Mapping[str, Any]) -> datetime:
        ...
        if record.get("expires_at"):
            ...
            return fresh
        raw_days = os.environ.get("PROTAGINE_P8_FACT_TTL_DAYS", "30")
        try:
            days = int(raw_days)
        except ValueError:
            days = 30
