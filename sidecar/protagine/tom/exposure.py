...
from protagine.tom.tom2 import _validate_ref
...
def budget_pair_day() -> int:
    """PROTAGINE_TOM2_BUDGET_PAIR_DAY (default 1): renderings per (reader,
    subject) pair per rolling 24h."""
    return _budget("PROTAGINE_TOM2_BUDGET_PAIR_DAY", DEFAULT_PAIR_DAY)


def budget_reader_day() -> int:
    """PROTAGINE_TOM2_BUDGET_READER_DAY (default 3): renderings per reader
    per rolling 24h."""
    return _budget("PROTAGINE_TOM2_BUDGET_READER_DAY", DEFAULT_READER_DAY)


def budget_global_day() -> int:
    """PROTAGINE_TOM2_BUDGET_GLOBAL_DAY (default 10): renderings across all
    readers per rolling 24h."""
    return _budget("PROTAGINE_TOM2_BUDGET_GLOBAL_DAY", DEFAULT_GLOBAL_DAY)
