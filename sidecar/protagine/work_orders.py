...
Protagine owns the goal/work ledger; it does not execute external effects.  A
...
from protagine.execution_results import (
...
from protagine.task_queue.models import Job, JobPriority, JobStatus, JobType
from protagine.task_queue.models import JobCapabilityRequirement
...
def load_receipt_verifier_from_env(environ: Optional[Mapping[str, str]] = None) -> Any:
    ...
    The verifier remains an operator-selected plugin.  Protagine knows only a
    ...
    spec = str(env.get("PROTAGINE_WORK_ORDER_RECEIPT_VERIFIER", "") or "").strip()
    if not spec:
        return None
    if len(spec) > 512 or not _IMPORT_PATH.fullmatch(spec):
        raise ReceiptVerifierConfigurationError(
            "PROTAGINE_WORK_ORDER_RECEIPT_VERIFIER must be module:attribute"
        )

    raw_config = str(
        env.get("PROTAGINE_WORK_ORDER_RECEIPT_VERIFIER_CONFIG", "{}") or "{}"
    )
...
class QueueWorkOrderAdapter:
    """Idempotently post/poll WorkOrderV1 jobs on Protagine's durable queue."""
