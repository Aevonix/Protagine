...
def work_snapshot(value, *, now=None):
    ...
    if value is None:
        return None
    if not isinstance(value, dict) or value.get('schema') != 'ProtagineWorkSnapshotV1':
        return {'schema': 'ProtagineWorkSnapshotV1', 'available': False, 'reason': 'unsupported_work_snapshot', 'complete': False}
    if type(value.get('available')) is not bool or not _number(value.get('observed_at')):
        return {'schema': 'ProtagineWorkSnapshotV1', 'available': False, 'reason': 'invalid_work_snapshot', 'complete': False}
    now = time.time() if now is None else now
    age = now - value['observed_at']
    result = {'schema': 'ProtagineWorkSnapshotV1', 'available': value['available'], 'complete': False, 'observed_at': value['observed_at'],
...
def reported_worker_view(*, limit=8, now=None):
    configured = os.environ.get('PROTAGINE_WORKER_STATUS_PATHS', '').strip()
    if not configured:
        return None
