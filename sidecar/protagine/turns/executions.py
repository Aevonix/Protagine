...
class ExecutionRegistry:
    ...
    def view(self, *, contact_id: str, owner: bool = False, session_id: str = "", limit: int = 20,
        ...
        for row in rows:
            ...
            if owner:
                from protagine.self_model.execution_forecasts import project
        ...
        for row in recent_rows:
            ...
            recent.append(item)
            break
        return {"schema": "ProtagineExecutionViewV1", "items": items, "total": total,
...
def request_work_context(view: dict, *, limit: int = 8, max_chars: int = 4000,
    ...
    def line_for(row):
        ...
        return json.dumps(row, sort_keys=True, ensure_ascii=True).replace(
            '[/protagine-work-request-v1]', r'\u005b/protagine-work-request-v1\u005d') + '\n'
    ...
    for source, group in groups:
        ...
        for is_recent, row in [(False, row) for row in group.get('items', [])] + [
            ...
            if source == 'reported_worker':
                from protagine.turns.reported_workers import work_details
    ...
    if any(item.get('input_source') for item in rows + stale_executions):
        header += 'Open input_source with protagine_memory_read_source for the original request.\n'
    ...
    return {'schema': 'ProtagineRequestWorkV1', 'observed_at': time.time(),
            ...
            'native_task_ids': list(dict.fromkeys(item['task_id'] for item in shown_executions
                if item.get('platform') == 'protagine_task' and item.get('task_id'))),
