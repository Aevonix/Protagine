...
def observed_boards():
    ...
    configured = os.environ.get('PROTAGINE_HERMES_WORK_BOARDS')
...
def selected_board():
    ...
    if home is not None and home.parent.name == 'profiles':
        home = home.parent.parent
    board = os.environ.get('PROTAGINE_LOCAL_WORK_BOARD', 'protagine-drafts').strip()
    profile = os.environ.get('PROTAGINE_LOCAL_WORK_PROFILE', 'protagine-drafts').strip()
    ...
    with closing(sqlite3.connect(path.as_uri()+'?mode=ro', uri=True,
        ...
        if review or followup:
            ...
            if task is not None and task['assignee'] == 'default' and db.execute(
                ...
                profile = 'default'
        creator = 'protagine-followup' if followup else 'protagine-initiative' if review else 'protagine-local-work'
        ...
        if review or followup:
            ...
            if review:
                ...
                import yaml
                from protagine.util.instance import plugin_settings
                try:
