...
- PROTAGINE_PROJECTS_MODE=shadow (default): plans for real, logs the exact
...
from protagine.projects.models import (
...
class ProjectEngine:
        ...
        if self._self_model is not None:
            try:
                ...
                if int(load.get("total") or 0) >= _int_env(
                        "PROTAGINE_PROJECTS_DEFER_LOAD", 10):
    ...
    async def _adopt_initiatives(self) -> int:
        ...
        if open_projects >= _int_env("PROTAGINE_PROJECTS_MAX_CONCURRENT", 3):
            return 0
        try:
    ...
    async def _run_reasoning_step(self, project: Project, step: Step,
        ...
        tier = os.environ.get("PROTAGINE_PROJECTS_MODEL_TIER", "small")
    ...
    async def _complete_project(self, project: Project, steps: List[Step],
        ...
        if project.outcome == "succeeded" and len(steps) >= 3 and mode == "live":
            try:
                from protagine.skills_memory import distill_from_completion
                transcript = "\n".join(
    ...
    def _record_outcome(self, outcome: str,
        ...
        try:
            ...
            if os.environ.get(
                "PROTAGINE_COGNITION_EVIDENCE", "off"
