...
def projects_max_steps() -> int:
    try:
        return max(1, min(50, int(os.environ.get("PROTAGINE_PROJECTS_MAX_STEPS", "12"))))
...
def projects_max_replans() -> int:
    try:
        return max(0, int(os.environ.get("PROTAGINE_PROJECTS_MAX_REPLANS", "3")))
    except (TypeError, ValueError):
        return 3
...
def projects_review_secs() -> float:
    try:
        return max(30.0, float(os.environ.get("PROTAGINE_PROJECTS_REVIEW_SECS", "900")))
    except (TypeError, ValueError):
        return 900.0
