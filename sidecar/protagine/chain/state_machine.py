...
class ChainStateMachine:
    ...
    def _apply_tx(
        ...
        dispatch = {
            TxType.PROTAGINE_REGISTER: self._apply_protagine_register,
            TxType.PROTAGINE_ROTATE_KEY: self._apply_protagine_rotate_key,
            TxType.PROTAGINE_REVOKE_KEY: self._apply_protagine_revoke_key,
            TxType.PROTAGINE_RELEASE_NAME: self._apply_protagine_release_name,
            ...
            TxType.PROTAGINE_SUSPEND: self._apply_protagine_suspend,
            TxType.PROTAGINE_REINSTATE: self._apply_protagine_reinstate,
    ...
    def _apply_protagine_register(
        ...
        state.protagine_registry[protagine_id] = record
        state.name_registry[name] = protagine_id
        state.key_history[protagine_id] = [
            KeyHistoryEntry(
        ...
        if is_genesis_admin and not state.genesis_admin_id:
            state.genesis_admin_id = protagine_id
        return state

    def _apply_protagine_rotate_key(
        self, state: ChainState, tx: Transaction, height: int
    ) -> ChainState:
        p = tx.payload
        protagine_id = tx.from_protagine_id
        new_key = p["new_public_key_hex"]

        record = state.protagine_registry.get(protagine_id)
        if record is None:
            return state

        # Mark old key as rotated
        history = state.key_history.get(protagine_id, [])
        ...
        state.key_history[protagine_id] = history
        record.active_public_key_hex = new_key
        return state

    def _apply_protagine_revoke_key(
        self, state: ChainState, tx: Transaction, height: int
    ) -> ChainState:
        p = tx.payload
        ...
        if cascade:
            ...
            # all attestations FROM this protagine)
            for edge_key, edge in list(state.trust_graph.items()):
                if edge.from_protagine_id == protagine_id:
                    edge.trust_level = 0  # downgrade to DISCOVERY

        return state

    def _apply_protagine_release_name(
        self, state: ChainState, tx: Transaction, height: int
    ) -> ChainState:
        p = tx.payload
        name = p["name"].lower()
    ...
    def _apply_trust_attest(
        ...
        p = tx.payload
        target_id = p["target_protagine_id"]
        edge_key = (tx.from_protagine_id, target_id)
        state.trust_graph[edge_key] = TrustEdge(
            from_protagine_id=tx.from_protagine_id,
    ...
    def _apply_untrust_attest(
        ...
        # Check for duplicate from same protagine
        existing = [
            e for e in state.untrust_counters[target_id]
            if e.from_protagine_id == tx.from_protagine_id
        ]
        if not existing:
            event = UntrustEvent(
                from_protagine_id=tx.from_protagine_id,
                target_protagine_id=target_id,
                at_height=height,
                tx_id=tx.tx_id,
        ...
        if total_count >= threshold and not state.is_suspended(target_id):
            ...
            record = state.protagine_registry.get(target_id)
            if record and record.status == "active":
                record.status = "suspended"
                state.suspended_colonies.add(target_id)
                logger.info(
                    "Auto-suspended protagine %s: untrust threshold %d reached",
                    target_id,
                    threshold,
                )
    ...
    def _apply_sentinel_register(
        ...
        state.sentinel_roster[sentinel_id] = SentinelRecord(
            ...
            protagine_id=p.get("protagine_id", tx.from_protagine_id),
            host=p.get("host", ""),
    ...
    def _apply_protagine_reinstate(
        self, state: ChainState, tx: Transaction, height: int
    ) -> ChainState:
        p = tx.payload
        target_id = p["target_protagine_id"]
    ...
    def _apply_protocol_upgrade(
        ...
        if title == "transfer_genesis_admin":
            ...
            if new_admin_id and tx.from_protagine_id == state.genesis_admin_id:
                old_admin_id = state.genesis_admin_id
                state.genesis_admin_id = new_admin_id
                # Update ProtagineRecord flags
                if old_admin_id in state.protagine_registry:
                    state.protagine_registry[old_admin_id].is_genesis_admin = False
                if new_admin_id in state.protagine_registry:
                    state.protagine_registry[new_admin_id].is_genesis_admin = True
                logger.info(
                ...
                logger.warning(
                    ...
                    tx.from_protagine_id[:16], state.genesis_admin_id[:16] if state.genesis_admin_id else "(none)",
                )
        ...
        if title == "strip_genesis_admin":
            ...
            if unanimous:
                ...
                if old_admin and old_admin in state.protagine_registry:
                    state.protagine_registry[old_admin].is_genesis_admin = False
    ...
    def _apply_plugin_publish(
        ...
        publisher_id = tx.from_protagine_id
        ...
        # Rate limiting: track publish timestamps per protagine
    ...
    def _apply_plugin_attestation(
        ...
        p = tx.payload
        plugin_hash = p.get("plugin_hash", "")
        scanner_id = tx.from_protagine_id
        ...
        # Deduplicate: one attestation per (plugin_hash, scanner_protagine_id) — supersede old
        record.attestations = [
            a for a in record.attestations
            if a.get("scanner_protagine_id") != scanner_id
        ]
        record.attestations.append({
            "scanner_protagine_id": scanner_id,
        ...
        sentinel_ids = {s.protagine_id for s in state.sentinel_roster.values() if s.status == "active"}
    ...
    def _apply_plugin_flag(
        ...
        p = tx.payload
        plugin_hash = p.get("plugin_hash", "")
        reporter_id = tx.from_protagine_id
        ...
        for existing_flag in record.flags:
            if (
                existing_flag.get("reporter_protagine_id") == reporter_id
                and existing_flag.get("vulnerability_description") == vuln_desc
            ):
                return state  # duplicate

        record.flags.append({
            "reporter_protagine_id": reporter_id,
            "severity": severity,
        ...
        for flag in record.flags:
            if (
                ...
                trusted_high_reporters.add(flag["reporter_protagine_id"])
