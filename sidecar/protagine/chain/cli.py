"""CLI commands for ProtagineChain management.

Usage:
    protagine chain status                     # Show chain state summary
    protagine chain genesis --name NAME        # Initialize genesis block
    protagine chain info --block N             # Show block details
    protagine chain tx --tx-id ID              # Show transaction details
    protagine chain list-colonies              # List registered colonies
    protagine chain list-sentinels             # List active sentinels
    protagine chain verify                     # Verify chain integrity
    protagine chain mempool                    # Show pending mempool transactions
"""
...
def _load_state(db_path: str | None = None) -> "ChainState":
    from protagine.chain.state_machine import ChainStateMachine
...
def cmd_chain_status(args) -> int:
    ...
    try:
        ...
        print(f"  Colonies:         {len(state.protagine_registry)}")
...
def cmd_chain_genesis(args) -> int:
    """Initialize a new ProtagineChain with a genesis block."""
    from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
    from cryptography.hazmat.primitives.serialization import (
        Encoding, PublicFormat, PrivateFormat, NoEncryption
    )
    from protagine.chain.genesis import GenesisConfig, create_genesis_block
    from protagine.chain.storage import ChainStore
    ...
    protagine_id = hashlib.sha256(bytes.fromhex(pub_hex)).hexdigest()

    def sign_fn(data: bytes) -> str:
        return priv.sign(data).hex()

    cfg = GenesisConfig(
        genesis_protagine_name=name,
        genesis_protagine_id=protagine_id,
...
def cmd_chain_tx(args) -> int:
    ...
    try:
        store = _load_store(getattr(args, "db", None))
        txs = store.get_transactions_for_protagine("")
...
def cmd_chain_list_colonies(args) -> int:
    ...
    try:
        ...
        if not state.protagine_registry:
            print("No colonies registered.")
            return 0
        fmt = "{:<20} {:<12} {:<16} {}"
        print(fmt.format("NAME", "STATUS", "PROTAGINE_ID (prefix)", "ENDPOINT"))
        print("-" * 80)
        for protagine_id, record in sorted(state.protagine_registry.items(), key=lambda x: x[1].name):
            flags = ""
...
def cmd_chain_list_sentinels(args) -> int:
    ...
    try:
        ...
        if not state.sentinel_roster:
            ...
            return 0
        fmt = "{:<36} {:<12} {:<20} {}"
        print(fmt.format("SENTINEL_ID", "STATUS", "HOST:PORT", "PROTAGINE_ID (prefix)"))
        print("-" * 80)
...
def cmd_chain_verify(args) -> int:
    ...
    try:
        ...
        for idx in range(height + 1):
            ...
            from protagine.chain.block import build_merkle_root
...
def cmd_chain_state(args) -> int:
    ...
    try:
        ...
        summary = {
            ...
            "protagine_count": len(state.protagine_registry),
...
def cmd_chain_register(args) -> int:
    """Register this protagine on-chain (protagine_register transaction)."""
    from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
    from cryptography.hazmat.primitives.serialization import Encoding, PublicFormat, PrivateFormat, NoEncryption
    from protagine.chain.transactions import TxType, Transaction
    import hashlib
    ...
    try:
        ...
        nonce_map = {}
        for cid, rec in state.protagine_registry.items():
            pass
        last_nonce = 0
        for block_idx in range(store.get_height() + 1):
            ...
            if block:
                for tx in block.transactions:
                    if tx.from_protagine_id == protagine_id and tx.nonce > last_nonce:
                        last_nonce = tx.nonce

        tx = Transaction.create(
            tx_type=TxType.PROTAGINE_REGISTER,
            from_protagine_id=protagine_id,
            nonce=last_nonce + 1,
            payload={
                "name": name,
        ...
        print(f"Submitted protagine_register to mempool:")
        print(f"  tx_id:      {tx.tx_id}")
        print(f"  protagine_id:  {protagine_id}")
        print(f"  name:       {name}")
        return 0
    except Exception as exc:
        print(f"Error: {exc}", file=sys.stderr)
...
def cmd_chain_rotate_key(args) -> int:
    """Rotate this protagine's Ed25519 key on-chain."""
    from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
    from cryptography.hazmat.primitives.serialization import Encoding, PublicFormat, PrivateFormat, NoEncryption
    from protagine.chain.transactions import TxType, Transaction
    import hashlib
    ...
    try:
        ...
        protagine_id = hashlib.sha256(bytes.fromhex(old_pub_hex)).hexdigest()
        ...
        tx = Transaction.create(
            tx_type=TxType.PROTAGINE_ROTATE_KEY,
            from_protagine_id=protagine_id,
            nonce=last_nonce + 1,
            payload={
        ...
        store.add_to_mempool(tx)
        print(f"Submitted protagine_rotate_key to mempool:")
        print(f"  tx_id:      {tx.tx_id}")
        print(f"  protagine_id:  {protagine_id}")
...
def cmd_chain_trust(args) -> int:
    """Submit trust_attest for a peer protagine."""
    from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
    ...
    key_file = getattr(args, "key_file", None)
    target = getattr(args, "target", None) or getattr(args, "protagine", None)
    level_raw = getattr(args, "level", None) or getattr(args, "trust_level", "1")
    db_path = getattr(args, "db", None)

    if not key_file or not target:
        print("Error: --key-file and --target (or positional protagine name) required", file=sys.stderr)
        return 1
    ...
    try:
        ...
        # Resolve target: may be a name or protagine_id
        target_id = state.protagine_id_for_name(target) or (target if target in state.protagine_registry else None)
        if not target_id:
            print(f"Error: protagine {target!r} not found in chain state", file=sys.stderr)
            return 1
...
def cmd_chain_untrust(args) -> int:
    """Submit untrust_attest for a peer protagine."""
    from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
    from cryptography.hazmat.primitives.serialization import Encoding, PublicFormat
    from protagine.chain.transactions import TxType, Transaction
    import hashlib
    ...
    try:
        ...
        target_id = state.protagine_id_for_name(target) or (target if target in state.protagine_registry else None)
        if not target_id:
            print(f"Error: protagine {target!r} not found", file=sys.stderr)
            return 1
...
def run_chain_command(args: list) -> int:
    """Dispatch 'protagine chain <subcommand>'."""
    if not args:
        _print_chain_help()
        return 1
...
def _print_chain_help() -> None:
    print("""Usage: protagine chain <subcommand> [options]
...
Subcommands:
  ...
  register            Register this protagine on-chain
  rotate-key          Rotate protagine Ed25519 key on-chain
  trust               Submit trust_attest (--target NAME --level 0-4)
  untrust             Submit untrust_attest (--target NAME)

Options:
  --db PATH              SQLite database path (default: ~/.protagine/chain.db)
  --name NAME            Protagine name
  ...
  --target NAME          Target protagine name or ID (trust/untrust)
  --level N              Trust level 0-4 (trust command)
  --endpoint URL         Protagine endpoint URL (register)
  --description TEXT     Protagine description (register)
""")
