"""Transaction types and chain state dataclasses for ProtagineChain.
...
class TxType(str, Enum):
    PROTAGINE_REGISTER = "protagine_register"
    PROTAGINE_ROTATE_KEY = "protagine_rotate_key"
    PROTAGINE_REVOKE_KEY = "protagine_revoke_key"
    PROTAGINE_RELEASE_NAME = "protagine_release_name"
    ...
    PROTAGINE_SUSPEND = "protagine_suspend"
    PROTAGINE_REINSTATE = "protagine_reinstate"
...
class Transaction:
    ...
    def create(
        ...
        signing_payload = {
            "tx_id": tx_id,
            "type": tx_type.value,
            "from_protagine_id": from_protagine_id,
            "timestamp": ts,
            "nonce": nonce,
            "payload": payload,
        }
    ...
    def signing_bytes(self) -> bytes:
        ...
        payload = {
            ...
            "from_protagine_id": self.from_protagine_id,
    ...
    def from_dict(cls, d: dict[str, Any]) -> "Transaction":
        ...
        return cls(
            tx_id=d["tx_id"],
            type=tx_type,
            from_protagine_id=d["from_protagine_id"],
...
class ChainState:
    ...
    protagine_registry: dict[str, ProtagineRecord] = field(default_factory=dict)
    name_registry: dict[str, str] = field(default_factory=dict)  # name -> protagine_id
    ...
    # Track publish counts per protagine for rate limiting
    plugin_publish_counts: dict[str, list[str]] = field(default_factory=dict)  # protagine_id -> list of timestamps

    def protagine_id_for_name(self, name: str) -> str | None:
        return self.name_registry.get(name.lower())

    def active_key_for_protagine(self, protagine_id: str) -> str | None:
        record = self.protagine_registry.get(protagine_id)
        if record:
            return record.active_public_key_hex
        return None

    def is_active(self, protagine_id: str) -> bool:
        record = self.protagine_registry.get(protagine_id)
        return record is not None and record.status == "active"

    def is_suspended(self, protagine_id: str) -> bool:
        return protagine_id in self.suspended_colonies
