...
class ChainManager:
    """Main coordinator for ProtagineChain.
    ...
    async def submit_transaction(self, tx: Transaction) -> SubmitResult:
        ...
        if tx.from_protagine_id != "system":
            persisted_nonce = self.store.get_nonce(tx.from_protagine_id)
    ...
    async def sync_from_sentinel(
        ...
        req = ChainSyncRequest(
            ...
            requester_id=self.protagine_id,
        )
    ...
    async def handle_tx_submit(
        ...
        """Handle a TX_SUBMIT message from another protagine."""
    ...
    async def get_chain_state_summary(
        ...
        return ChainStateResponse(
            ...
            n_colonies=len(state.protagine_registry),
            n_sentinels=len(state.active_sentinels()),
            n_suspended=len(state.suspended_colonies),
            network_id=state.network_id,
            responder_id=responder_id or self.protagine_id,
        )
    ...
    def get_instance(cls) -> "ChainManager":
        """Return the process-wide singleton backed by ~/.protagine/data/chain.db."""
        global _chain_manager_instance
        if _chain_manager_instance is None:
            protagine_home = get_state_dir()
            db_path = protagine_home / "chain.db"
            # Derive protagine_id from genesis block cryptographic identity
            protagine_id = "local"
            if db_path.exists():
                try:
                    ...
                    if row:
                        ...
                        # Use the cryptographic protagine_id (sha256 of Ed25519 pubkey)
                        # from genesis block metadata — not producer_id which is
                        # just a human-readable label and trivially spoofable.
                        protagine_id = (
                            genesis_data.get("metadata", {}).get("genesis_protagine_id")
                            or genesis_data.get("producer_id", "local")
                        )
                    conn.close()
                except Exception:
                    pass
            _chain_manager_instance = cls(db_path=db_path, protagine_id=protagine_id)
