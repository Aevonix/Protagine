"""SQLite-backed persistent storage for ProtagineChain.
...
CREATE TABLE IF NOT EXISTS chain_transactions (
    ...
    from_protagine_id  TEXT    NOT NULL,
    timestamp       TEXT    NOT NULL,
    nonce           INTEGER NOT NULL,
    payload_json    TEXT    NOT NULL,
    signature       TEXT    NOT NULL,
    UNIQUE(from_protagine_id, nonce)
);
...
CREATE INDEX IF NOT EXISTS idx_tx_protagine    ON chain_transactions(from_protagine_id);
...
class ChainStore:
            ...
            for tx in block.transactions:
                conn.execute(
                    """INSERT OR IGNORE INTO chain_transactions
                       (tx_id, block_index, tx_type, from_protagine_id, timestamp,
        ...
        with self._tx() as conn:
            conn.execute(
                """INSERT OR IGNORE INTO chain_mempool
                   (tx_id, tx_type, from_protagine_id, received_at, priority,
    ...
    def get_nonce(self, protagine_id: str) -> int:
        """Return the highest accepted nonce for protagine_id (0 if none)."""
        with self._get_conn() as conn:
            row = conn.execute(
                "SELECT MAX(nonce) as n FROM chain_transactions WHERE from_protagine_id = ?",
                (protagine_id,),
            ).fetchone()
    ...
    def get_transactions_for_protagine(self, protagine_id: str) -> list[Transaction]:
        with self._get_conn() as conn:
            rows = conn.execute(
                "SELECT tx_id, tx_type, from_protagine_id, timestamp, nonce, "
                "payload_json, signature FROM chain_transactions "
                "WHERE from_protagine_id = ? ORDER BY nonce ASC",
                (protagine_id,),
            ).fetchall()
        result = []
        for r in rows:
            result.append(
                Transaction(
                    ...
                    from_protagine_id=r["from_protagine_id"],
...
def _serialize_chain_state(state: ChainState) -> str:
    ...
    def _te(e: TrustEdge) -> dict:
        return {
            "from_protagine_id": e.from_protagine_id,
            "to_protagine_id": e.to_protagine_id,
    ...
    def _ue(e: UntrustEvent) -> dict:
        return {
            "from_protagine_id": e.from_protagine_id,
            "target_protagine_id": e.target_protagine_id,
    ...
    def _cr(r: ProtagineRecord) -> dict:
        return {
            "protagine_id": r.protagine_id,
            "name": r.name,
    ...
    data: dict[str, Any] = {
        ...
        "protagine_registry": {k: _cr(v) for k, v in state.protagine_registry.items()},
...
def _deserialize_chain_state(state_json: str) -> ChainState:
    ...
    for k, v in data["protagine_registry"].items():
        state.protagine_registry[k] = ProtagineRecord(**v)
