"""Protagine Local Key Manager — PEM-based Ed25519 signing for Phase 1/2.

Provides the same interface as ProtagineKeyManager (sign, public_key_hex, rotate)
but stores the key as a simple PEM file. Designed to be swapped out for the
Shamir-backed ProtagineKeyManager when Phase 3/4/5 ship.

Key storage:
    {state_dir}/protagine-keys/
...
Migration path:
    ...
    Phase 3:   LocalKeyManager with passphrase (protagine key set-passphrase)
    Phase 4/5: ProtagineKeyManager (Shamir-split, protagine key migrate-to-shamir)
"""
...
class LocalKeyManager:
    """PEM-based Ed25519 key manager for single-node Protagine deployments."""
    ...
    # ------------------------------------------------------------------
    # Public interface (matches ProtagineKeyManager)
    # ------------------------------------------------------------------
    ...
    def rotate(self, new_passphrase: Optional[bytes] = None) -> None:
        ...
        The protagine_id does NOT change — only the signing key rotates.
        """
        from cryptography.hazmat.primitives import serialization
        from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
        ...
        logger.info("Key rotated for protagine %s", self.protagine_id)
    ...
    def set_passphrase(self, new_passphrase: bytes) -> None:
        ...
        logger.info("Passphrase updated for protagine %s", self.protagine_id)
    ...
    def _load_private_key(self):
        ...
        if not priv_path.exists():
            raise FileNotFoundError(
                f"No private key found at {priv_path}. Run 'protagine init' or 'protagine key generate'."
            )
    ...
    def generate(
        ...
        mgr = cls(keys_dir=keys_dir, protagine_id=protagine_id, passphrase=passphrase)
        private_key = Ed25519PrivateKey.generate()
        mgr._save_keypair(private_key, passphrase)
        logger.info("Generated new Ed25519 keypair for protagine %s", protagine_id)
        return mgr
