...
class GenesisConfig:
    ...
    network_name: str = "protagine-federation"
    genesis_protagine_name: str = "genesis"
    genesis_protagine_id: str = ""         # sha256(genesis_pubkey)
    genesis_pubkey_hex: str = ""        # 32-byte Ed25519 pubkey, hex
    genesis_endpoint: str = ""
    genesis_description: str = "Genesis protagine — network founder"
...
def _derive_protagine_id(public_key_hex: str) -> str:
    """Compute protagine_id = sha256(public_key_bytes)."""
...
def create_genesis_block(
    ...
    """Create the genesis block with the founding protagine's registration.
    ...
    # Derive protagine_id if not provided
    if not config.genesis_protagine_id and config.genesis_pubkey_hex:
        config.genesis_protagine_id = _derive_protagine_id(config.genesis_pubkey_hex)

    # Build the genesis protagine_register transaction
    register_payload = {
        "name": config.genesis_protagine_name,
        "public_key_hex": config.genesis_pubkey_hex,
        "protagine_id": config.genesis_protagine_id,
    ...
    register_tx = Transaction.create(
        ...
        from_protagine_id=config.genesis_protagine_id,
        nonce=1,
    ...
    metadata: dict[str, Any] = {
        ...
        "genesis_protagine_id": config.genesis_protagine_id,
...
class GenesisAdmin:
    ...
    def __init__(
        self,
        genesis_protagine_id: str,
        sign_fn: Callable[[bytes], str],
        get_nonce: Callable[[str], int],
    ) -> None:
        self.genesis_protagine_id = genesis_protagine_id
        self.sign_fn = sign_fn
        self.get_nonce = get_nonce

    def _next_nonce(self) -> int:
        return self.get_nonce(self.genesis_protagine_id) + 1

    def suspend_protagine(
        self, target_protagine_id: str, reason: str = "admin_action"
    ) -> Transaction:
        """Create a protagine_suspend transaction as Genesis admin."""
        return Transaction.create(
            tx_type=TxType.PROTAGINE_SUSPEND,
            from_protagine_id=self.genesis_protagine_id,
            nonce=self._next_nonce(),
            payload={
                "target_protagine_id": target_protagine_id,
                "reason": reason,
    ...
    def reinstate_protagine(
        self,
        target_protagine_id: str,
        conditions: str = "",
        protagine_acknowledgment: str = "",
    ) -> Transaction:
        """Create a protagine_reinstate transaction as Genesis admin."""
        return Transaction.create(
            tx_type=TxType.PROTAGINE_REINSTATE,
            from_protagine_id=self.genesis_protagine_id,
            nonce=self._next_nonce(),
            payload={
                "target_protagine_id": target_protagine_id,
                ...
                "protagine_acknowledgment": protagine_acknowledgment,
            },
            sign_fn=self.sign_fn,
        )
    ...
    def force_release_name(self, name: str) -> Transaction:
        ...
        return Transaction.create(
            tx_type=TxType.PROTAGINE_RELEASE_NAME,
            from_protagine_id=self.genesis_protagine_id,
            nonce=self._next_nonce(),
    ...
    def transfer_admin(
        self, new_admin_protagine_id: str, new_admin_pubkey_hex: str
    ) -> Transaction:
        """Transfer Genesis admin role to another protagine.
        ...
        return Transaction.create(
            ...
            payload={
                ...
                "description": f"Transfer genesis admin to {new_admin_protagine_id}",
                "spec_url": "",
                "changes": {
                    "new_genesis_admin_id": new_admin_protagine_id,
