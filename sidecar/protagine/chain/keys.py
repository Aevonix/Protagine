"""Protagine key resilience: Shamir Secret Sharing, encrypted share persistence, key rotation.
...
class KeyShare:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "protagine_id": self.protagine_id,
...
class ShareBackend(Protocol):
    ...
    def retrieve_share(self, protagine_id: str, share_index: int) -> Optional[KeyShare]:
        """Retrieve a share. Returns None if not present."""
        ...

    def delete_share(self, protagine_id: str, share_index: int) -> None:
        """Delete a share (used during rotation)."""
        ...

    def list_shares(self, protagine_id: str) -> List[int]:
...
class KeyShareStore:
    ...
    - MUST bind share to its protagine_id and share_index via AAD.
    - MUST support multiple backends (local file, remote node, backup file).
    - MUST detect and reject share transplantation (wrong protagine_id or index).
    """
    ...
    def __init__(
        ...
        self.protagine_id = protagine_id
    ...
    def _aad(self, share_index: int) -> bytes:
        ...
        return (
            bytes.fromhex(self.protagine_id)
            + bytes([share_index])
    ...
    def decrypt_share(self, share: KeyShare, passphrase: bytes) -> bytes:
        ...
        Raises:
            ValueError: If protagine_id mismatch (transplantation attack).
            cryptography.exceptions.InvalidTag: If decryption fails.
        """
        if share.protagine_id != self.protagine_id:
            raise ValueError(
                f"Share protagine_id mismatch: expected {self.protagine_id}, "
                f"got {share.protagine_id}"
            )
    ...
    def collect(
        ...
        for backend in self.backends:
            indices = backend.list_shares(self.protagine_id)
            for idx in indices:
                if idx in seen_indices:
                    continue
                share = backend.retrieve_share(self.protagine_id, idx)
                if share is None:
                    continue
...
class ProtagineKeyManager:
    """High-level manager for protagine signing operations.
    ...
    def sign(self, payload: bytes) -> str:
        ...
        self._audit(f"key_reconstruct protagine={self.protagine_id}")
        ...
        try:
            ...
            self._audit(f"key_zeroed protagine={self.protagine_id}")
    ...
    def rotate(
        ...
        Signs a protagine_rotate_key payload with the OLD key first,
        ...
        for backend in self.store.backends:
            old_indices.extend(backend.list_shares(self.protagine_id))
        for backend in self.store.backends:
            for idx in set(old_indices):
                backend.delete_share(self.protagine_id, idx)
        ...
        logger.info("Key rotation complete for protagine %s", self.protagine_id)
...
class RemoteNodeShareBackend:
    ...
    authenticated share transport across Protagine mesh nodes.

    API contract (served by the remote node at BASE_PATH):
      PUT    /{protagine_id}/{share_index}  — store or replace a share
      GET    /{protagine_id}/{share_index}  — retrieve a share (404 → None)
      DELETE /{protagine_id}/{share_index}  — delete a share (404 → silent)
      GET    /{protagine_id}               — list indices {"indices": [...]}
    ...
    def store_share(self, share: KeyShare) -> None:
        ...
        try:
            self._request(
                "PUT",
                f"{self.BASE_PATH}/{share.protagine_id}/{share.share_index}",
                json=share.to_dict(),
            )
        except Exception as exc:
            logger.error(
                "store_share failed [node=%s protagine=%s index=%d]: %s",
                self.node_id, share.protagine_id, share.share_index, exc,
            )
            raise
    ...
    def retrieve_share(self, protagine_id: str, share_index: int) -> Optional[KeyShare]:
        ...
        try:
            resp = self._request(
                "GET",
                f"{self.BASE_PATH}/{protagine_id}/{share_index}",
            )
            ...
            if exc.response.status_code == 404:
                return None
            logger.error(
                "retrieve_share failed [node=%s protagine=%s index=%d]: HTTP %d",
                self.node_id, protagine_id, share_index, exc.response.status_code,
            )
            return None
        except Exception as exc:
            logger.error(
                "retrieve_share failed [node=%s protagine=%s index=%d]: %s",
                self.node_id, protagine_id, share_index, exc,
            )
            return None
    ...
    def delete_share(self, protagine_id: str, share_index: int) -> None:
        ...
        try:
            ...
            logger.error(
                "delete_share failed [node=%s protagine=%s index=%d]: HTTP %d",
                self.node_id, protagine_id, share_index, exc.response.status_code,
            )
            raise
        except Exception as exc:
            logger.error(
                "delete_share failed [node=%s protagine=%s index=%d]: %s",
                self.node_id, protagine_id, share_index, exc,
            )
            raise
    ...
    def list_shares(self, protagine_id: str) -> List[int]:
        ...
        """
        try:
            resp = self._request("GET", f"{self.BASE_PATH}/{protagine_id}")
            return sorted(resp.json().get("indices", []))
        except Exception as exc:
            logger.error(
                "list_shares failed [node=%s protagine=%s]: %s",
                self.node_id, protagine_id, exc,
            )
            return []


class BackupFileShareBackend:
    """Read/write a portable .protagineshare backup file.
    ...
    def __init__(
        ...
        *,
        ...
        self._protagine_id = protagine_id
    ...
    def _load(self) -> None:
        if self._share is None and self.file_path.exists():
            ...
            # Populate protagine/network IDs from file if not set at ctor.
            if self._protagine_id is None:
                self._protagine_id = data.get("protagine_id")
    ...
    def store_share(self, share: KeyShare) -> None:
        ...
        if self._protagine_id is None or self._network_id is None:
            self._load()
        if self._protagine_id is None or self._network_id is None:
            raise ValueError(
                "protagine_id and network_id are required to write a backup file"
            )
        ...
        data = {
            ...
            "protagine_id": self._protagine_id,
...
def export_share_file(
    ...
    """Write a .protagineshare backup file."""
    data = {
        "file_type": "protagine_key_share",
        "file_version": 1,
...
def import_share_file(file_path: Path) -> tuple:
    """Read a .protagineshare backup file.

    Returns:
        (protagine_id, network_id, KeyShare) tuple.
    """
    data = json.loads(file_path.read_text())
    if data.get("file_type") != "protagine_key_share":
        raise ValueError(f"Not a protagine_key_share file: {file_path}")
    protagine_id = data["protagine_id"]
    network_id = data["network_id"]
    share = KeyShare.from_dict(data["share"])
    return protagine_id, network_id, share
