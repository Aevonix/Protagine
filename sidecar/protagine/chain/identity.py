...
select its verification key with PROTAGINE_GENESIS_TRUST_PUBLIC_KEY. The public
...
def is_genesis(protagine_id: str, public_key_hex: str) -> bool:
    """Check if a protagine_id + public_key combination matches the verified Genesis manifest.
    ...
    if manifest is None:
        return False
    return (
        protagine_id == manifest.get("protagine_id")
...
def verify_genesis_manifest(manifest: dict) -> bool:
    """Verify a manifest against PROTAGINE_GENESIS_TRUST_PUBLIC_KEY, with no default.

    Returns True if the signature is valid, False otherwise.
    A manifest without a signature is automatically invalid.
    """
    trust_key = os.environ.get("PROTAGINE_GENESIS_TRUST_PUBLIC_KEY", "").strip()
    if not trust_key:
        return False
...
def load_genesis_manifest(path: str | Path) -> Optional[dict]:
    ...
    try:
        ...
        required = {"protagine_id", "public_key_ed25519", "alias", "genesis_version", "signature"}
...
# ---------------------------------------------------------------------------
# Protagine ID management
# ---------------------------------------------------------------------------

PROTAGINE_ID_FILE = "protagine-id"


def get_or_create_protagine_id(state_dir: str | Path) -> str:
    """Get the existing protagine_id or create a new random UUID.

    The protagine_id is stored in {state_dir}/protagine-id.
    It is generated once and never changes.
    For disaster recovery, use 'protagine backup' / 'protagine restore'.
    """
    id_path = Path(state_dir) / PROTAGINE_ID_FILE
    if id_path.exists():
        protagine_id = id_path.read_text().strip()
        if protagine_id:
            return protagine_id

    # Generate new
    protagine_id = str(uuid.uuid4())
    id_path.parent.mkdir(parents=True, exist_ok=True)
    id_path.write_text(protagine_id)
    logger.info("Generated new protagine_id: %s", protagine_id)
    return protagine_id
...
def create_genesis_manifest(
    ...
    """Create a signed Genesis manifest for the Genesis Protagine.
...
def create_protagine_manifest(
    protagine_id: str,
    public_key_hex: str,
    output_path: str | Path,
) -> dict:
    """Create a standard (non-Genesis) protagine manifest.

    Every Protagine can generate a manifest to share its public identity.
    ...
    Path(output_path).parent.mkdir(parents=True, exist_ok=True)
    Path(output_path).write_text(json.dumps(manifest, indent=2) + "\n")
    logger.info("Protagine manifest created at %s", output_path)
    return manifest


def backup_protagine(state_dir: str | Path, passphrase: Optional[bytes] = None) -> dict:
    """Export protagine identity as a portable backup.

    Returns a dict containing:
        - protagine_id
        - public_key_ed25519
        - private_key_pem (optionally encrypted)
        - genesis_manifest (if this Protagine is Genesis)
        - backup_version
        - created_at

    This is everything needed to restore a Protagine on a new machine.
    """
    state_dir = Path(state_dir)

    # Protagine ID
    id_path = state_dir / PROTAGINE_ID_FILE
    if not id_path.exists():
        raise FileNotFoundError("No protagine-id found — nothing to back up")
    protagine_id = id_path.read_text().strip()
    ...
    # Genesis manifest (if this Protagine is Genesis)
    genesis_data = None
...
def restore_protagine(
    state_dir: str | Path,
    backup_data: dict,
    passphrase: Optional[bytes] = None,
) -> str:
    """Restore a Protagine from a backup.

    Writes protagine-id, private key, public key, and optionally genesis manifest.
    Returns the restored protagine_id.
    """
    state_dir = Path(state_dir)
    ...
    protagine_id = backup_data["protagine_id"]
    ...
    (state_dir / PROTAGINE_ID_FILE).write_text(protagine_id)
    ...
    logger.info("Protagine restored: %s", protagine_id)
    return protagine_id
