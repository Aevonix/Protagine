...
class TransactionValidator:
    ...
    def validate(self, tx: Transaction) -> ValidationResult:
        ...
        if not tx.from_protagine_id:
            return ValidationResult.fail("missing from_protagine_id")

        # System transactions (auto-generated) skip signature check
        is_system = tx.from_protagine_id == "system"
    ...
    def _verify_signature(self, tx: Transaction) -> ValidationResult:
        """Verify Ed25519 signature against the protagine's active key."""
        protagine = self.state.protagine_registry.get(tx.from_protagine_id)

        # For protagine_register, use the public key from the payload itself
        if tx.type == TxType.PROTAGINE_REGISTER:
            pub_key_hex = tx.payload.get("public_key_hex", "")
        elif protagine is not None:
            pub_key_hex = protagine.active_public_key_hex
        else:
            return ValidationResult.fail(
                f"protagine {tx.from_protagine_id!r} not registered"
            )

        if not pub_key_hex:
            return ValidationResult.fail("no public key for protagine")
    ...
    def _check_nonce(self, tx: Transaction) -> ValidationResult:
        """Verify nonce is strictly greater than protagine's last accepted nonce.

        The authoritative nonce check is enforced by ChainManager.submit_transaction()
        via ChainStore.get_nonce() + DB UNIQUE(protagine_id, nonce). This method is a
        ...
        last_nonce = self._chain_store.get_nonce(tx.from_protagine_id)
    ...
    def _validate_by_type(self, tx: Transaction) -> ValidationResult:
        dispatch = {
            TxType.PROTAGINE_REGISTER: self._validate_protagine_register,
            TxType.PROTAGINE_ROTATE_KEY: self._validate_protagine_rotate_key,
            TxType.PROTAGINE_REVOKE_KEY: self._validate_protagine_revoke_key,
            TxType.PROTAGINE_RELEASE_NAME: self._validate_protagine_release_name,
            ...
            TxType.PROTAGINE_SUSPEND: self._validate_protagine_suspend,
            TxType.PROTAGINE_REINSTATE: self._validate_protagine_reinstate,
    ...
    def _validate_protagine_register(self, tx: Transaction) -> ValidationResult:
        p = tx.payload
        name = p.get("name", "").lower()
        protagine_id = p.get("protagine_id", "")
        public_key_hex = p.get("public_key_hex", "")

        if not name:
            return ValidationResult.fail("protagine_register: missing name")
        if not _NAME_PATTERN.match(name):
            return ValidationResult.fail(
                f"protagine_register: name {name!r} does not match required pattern"
            )
        if name in self.state.name_registry:
            return ValidationResult.fail(
                f"protagine_register: name {name!r} already registered"
            )
        if protagine_id in self.state.protagine_registry:
            return ValidationResult.fail(
                f"protagine_register: protagine_id {protagine_id!r} already registered"
            )
        # Verify protagine_id = sha256(pubkey)
        try:
            expected_id = hashlib.sha256(bytes.fromhex(public_key_hex)).hexdigest()
        except ValueError:
            return ValidationResult.fail("protagine_register: invalid public_key_hex")
        if expected_id != protagine_id:
            return ValidationResult.fail(
                "protagine_register: protagine_id does not match sha256(public_key)"
            )
        # Genesis admin can only appear once (in genesis block)
        if p.get("genesis_admin", False) and self.state.genesis_admin_id:
            return ValidationResult.fail(
                "protagine_register: genesis_admin=true only allowed in genesis block"
            )
        return ValidationResult.success()

    def _validate_protagine_rotate_key(self, tx: Transaction) -> ValidationResult:
        protagine = self.state.protagine_registry.get(tx.from_protagine_id)
        if protagine is None:
            return ValidationResult.fail("protagine_rotate_key: protagine not registered")
        if protagine.status == "suspended":
            return ValidationResult.fail("protagine_rotate_key: protagine is suspended")
        p = tx.payload
        new_key = p.get("new_public_key_hex", "")
        if not new_key:
            return ValidationResult.fail("protagine_rotate_key: missing new_public_key_hex")
        # New key must not be registered to another protagine
        for cid, record in self.state.protagine_registry.items():
            if cid != tx.from_protagine_id and record.active_public_key_hex == new_key:
                return ValidationResult.fail(
                    "protagine_rotate_key: new key already registered to another protagine"
                )
        return ValidationResult.success()

    def _validate_protagine_revoke_key(self, tx: Transaction) -> ValidationResult:
        p = tx.payload
        revoked_key = p.get("revoked_key_hex", "")
        if not revoked_key:
            return ValidationResult.fail("protagine_revoke_key: missing revoked_key_hex")
        # Key must be in history of the target protagine
        # from_protagine_id is either the protagine itself, or "system"/"genesis"
        target_id = tx.from_protagine_id
        history = self.state.key_history.get(target_id, [])
        if not any(e.public_key_hex == revoked_key for e in history):
            return ValidationResult.fail(
                "protagine_revoke_key: key not found in protagine key history"
            )
        return ValidationResult.success()

    def _validate_protagine_release_name(self, tx: Transaction) -> ValidationResult:
        p = tx.payload
        name = p.get("name", "").lower()
        owner_id = self.state.name_registry.get(name)
        if owner_id is None:
            return ValidationResult.fail(
                f"protagine_release_name: name {name!r} not registered"
            )
        is_genesis = tx.from_protagine_id == self.state.genesis_admin_id
        if owner_id != tx.from_protagine_id and not is_genesis:
            return ValidationResult.fail(
                "protagine_release_name: not authorized (not owner or genesis admin)"
            )
        return ValidationResult.success()

    def _validate_trust_attest(self, tx: Transaction) -> ValidationResult:
        p = tx.payload
        target_id = p.get("target_protagine_id", "")
        if not self.state.is_active(tx.from_protagine_id):
            return ValidationResult.fail("trust_attest: attesting protagine not active")
        if not self.state.is_active(target_id):
            return ValidationResult.fail("trust_attest: target protagine not active")
    ...
    def _validate_untrust_attest(self, tx: Transaction) -> ValidationResult:
        p = tx.payload
        target_id = p.get("target_protagine_id", "")
        if not self.state.protagine_registry.get(tx.from_protagine_id):
            return ValidationResult.fail("untrust_attest: attesting protagine not registered")
        if not self.state.protagine_registry.get(target_id):
            return ValidationResult.fail("untrust_attest: target protagine not registered")
        return ValidationResult.success()

    def _validate_sentinel_register(self, tx: Transaction) -> ValidationResult:
        protagine = self.state.protagine_registry.get(tx.from_protagine_id)
        if protagine is None:
            return ValidationResult.fail("sentinel_register: protagine not registered")
        if protagine.status == "suspended":
            return ValidationResult.fail("sentinel_register: protagine is suspended")
        p = tx.payload
    ...
    def _validate_sentinel_deregister(self, tx: Transaction) -> ValidationResult:
        ...
        is_genesis = tx.from_protagine_id == self.state.genesis_admin_id
        is_system = tx.from_protagine_id == "system"
        if record.protagine_id != tx.from_protagine_id and not is_genesis and not is_system:
            return ValidationResult.fail(
                "sentinel_deregister: not authorized (not owner or genesis admin)"
            )
        return ValidationResult.success()

    def _validate_protagine_suspend(self, tx: Transaction) -> ValidationResult:
        p = tx.payload
        target_id = p.get("target_protagine_id", "")
        if not target_id:
            return ValidationResult.fail("protagine_suspend: missing target_protagine_id")
        if target_id not in self.state.protagine_registry:
            return ValidationResult.fail("protagine_suspend: target protagine not registered")
        if self.state.is_suspended(target_id):
            return ValidationResult.fail("protagine_suspend: protagine already suspended")
        is_genesis = tx.from_protagine_id == self.state.genesis_admin_id
        is_system = tx.from_protagine_id == "system"
        if not is_genesis and not is_system:
            return ValidationResult.fail(
                "protagine_suspend: only genesis admin or system may suspend"
            )
        return ValidationResult.success()

    def _validate_protagine_reinstate(self, tx: Transaction) -> ValidationResult:
        p = tx.payload
        target_id = p.get("target_protagine_id", "")
        if not target_id:
            return ValidationResult.fail("protagine_reinstate: missing target_protagine_id")
        if not self.state.is_suspended(target_id):
            return ValidationResult.fail("protagine_reinstate: protagine is not suspended")
        is_genesis = tx.from_protagine_id == self.state.genesis_admin_id
        is_system = tx.from_protagine_id == "system"
        if not is_genesis and not is_system:
            return ValidationResult.fail(
                "protagine_reinstate: only genesis admin or system may reinstate"
            )
        return ValidationResult.success()
