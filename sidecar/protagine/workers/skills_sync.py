"""Report the agent's skill index to Protagine.

Installed as the ``protagine-skills-sync`` console command.

Scans the Hermes skills directory for SKILL.md files, parses the
frontmatter (name / description / tags), and POSTs the index to
Protagine's push-only "skills" observation domain. Protagine's self-directed
thinking and capability-gap machinery read from there, so Protagine
...
def report(observations: list) -> int:
    """POST the skill index to Protagine's skills observation domain.
    ...
    req = urllib.request.Request(
        f"{protagine_url}/v1/host/observations",
...
def main(argv: Optional[list] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="protagine-skills-sync",
        description="Report the agent's installed skill index to Protagine.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="scan and print what would be reported without POSTing to Protagine",
    )
    ...
    if not observations:
        ...
        return 0
    if args.dry_run:
        print("protagine-skills-sync (dry run — nothing reported):")
