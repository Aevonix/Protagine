"""Claim Protagine agent_action jobs and hand them to the agent.

Installed as the ``protagine-queue-worker`` console command.

This is the execution half of the agent-as-sensor loop:

  Protagine notices a domain is stale → posts a read-only agent_sync_<domain>
  ...
  the agent POSTs observations back to Protagine and completes the job →
  Protagine generates initiatives from what the agent saw.
...
close the loop with plain curl — no special tooling needed:
  ...
  - report observations:  POST {PROTAGINE_URL}/v1/host/observations
  - complete the job:     POST {PROTAGINE_URL}/v1/host/queue/jobs/{id}/complete
  - fail the job:         POST {PROTAGINE_URL}/v1/host/queue/jobs/{id}/fail

Environment:
  PROTAGINE_URL                sidecar URL       (default http://127.0.0.1:7777)
  PROTAGINE_API_KEY            API key           (default dev-mode-no-key)
  PROTAGINE_JOBS_WEBHOOK_URL   agent webhook     (default http://127.0.0.1:8644/webhooks/protagine-jobs)
  PROTAGINE_WORKER_NODE_ID     worker identity   (default derived from PROTAGINE_AGENT_NAME)
  PROTAGINE_WORKER_MAX_JOBS    jobs per run      (default 1 — each is a full agent invocation)
  PROTAGINE_AGENT_WORKER_ROUTES comma-separated exact no-effect lanes:
...
def agent_action_capabilities(routes: Optional[str] = None) -> list[str]:
    ...
    configured = (
        os.environ.get("PROTAGINE_AGENT_WORKER_ROUTES", "agent_sync,hermes_run")
    ...
    if not selected or unknown:
        ...
        raise ValueError(
            "PROTAGINE_AGENT_WORKER_ROUTES must select agent_sync and/or "
...
def encode_hermes_webhook(
    ...
    *,
    ...
    configured = (
        os.environ.get("PROTAGINE_HERMES_WEBHOOK_SECRET", "")
    ...
    if configured and configured != "INSECURE_NO_AUTH":
        ...
        if os.environ.get(
            "PROTAGINE_HERMES_WEBHOOK_V1_COMPAT", "false"
...
def work_order_webhook_fields(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "result_contract": {
            ...
            "verification_note": (
                "Executor verification claims are observational; Protagine "
...
def load_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "webhook_url": os.environ.get(
            "PROTAGINE_JOBS_WEBHOOK_URL", "http://127.0.0.1:8644/webhooks/protagine-jobs"
        ),
        "node_id": node_id,
...
def register_worker(cfg: Dict[str, Any]) -> None:
    ...
    _post(
        cfg,
        f"{cfg['protagine_url']}/v1/host/queue/workers/register",
        {
            "node_id": cfg["node_id"],
...
def claim_job(cfg: Dict[str, Any]) -> Optional[dict]:
    ...
    return _post(
        cfg,
        f"{cfg['protagine_url']}/v1/host/queue/jobs/claim",
        {
            "node_id": cfg["node_id"],
            "capabilities": capabilities,
            "job_types": ["agent_action"],
...
def main(argv: Optional[list] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="protagine-queue-worker",
        description="Claim Protagine agent_action jobs and hand them to the agent webhook.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    cfg = load_config()
    if args.dry_run:
        print("protagine-queue-worker (dry run — no network calls):")
        print(f"  protagine_url:  {cfg['protagine_url']}")
