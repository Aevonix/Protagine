"""Protagine worker daemon -- claims typed jobs and executes them with an LLM
(cognition program, Phase B item 5).

This is the installable, capability-typed worker half of the Protagine queue.
...
Environment:
  PROTAGINE_URL                 sidecar base URL     (default http://127.0.0.1:7777)
  PROTAGINE_API_KEY             sidecar API key      (default dev-mode-no-key)
  PROTAGINE_WORKER_NODE_ID      worker identity      (default <agent>-worker)
  PROTAGINE_WORKER_CAPABILITIES csv capabilities     (default research,analyst,read)
  PROTAGINE_WORKER_JOB_TYPES    csv job types        (default research)
  PROTAGINE_WORKER_MAX_JOBS     jobs per cycle       (default 1)
  PROTAGINE_WORKER_POLL_SECS    seconds between polls (default 30; daemon mode)
  PROTAGINE_WORKER_HEARTBEAT_SECS heartbeat cadence while running (default 20)
  PROTAGINE_WORKER_LLM_BASE_URL OpenAI-compatible base (default OPENAI_BASE_URL)
  PROTAGINE_WORKER_LLM_MODEL    model name           (default gpt-4o-mini)
  PROTAGINE_WORKER_LLM_API_KEY  LLM key              (default OPENAI_API_KEY)
"""
...
_WORKER_SYSTEM = (
    "You are an Protagine worker agent executing ONE claimed job with a read and "
...
def load_config() -> Dict[str, Any]:
    ...
    node_id = os.environ.get("PROTAGINE_WORKER_NODE_ID") or (
        os.environ.get("PROTAGINE_AGENT_NAME", "protagine").strip().lower().replace(" ", "-")
        + "-worker"
    )
    caps = [c.strip() for c in os.environ.get(
        "PROTAGINE_WORKER_CAPABILITIES", "research,analyst,read").split(",") if c.strip()]
    job_types = [t.strip() for t in os.environ.get(
        "PROTAGINE_WORKER_JOB_TYPES", "research").split(",") if t.strip()]
    if os.environ.get(
        "PROTAGINE_AGENT_JOB_CLAIMS_ENABLED", "true"
    ).strip().lower() not in {"1", "true", "yes", "on"}:
        job_types = [item for item in job_types if item != "agent_action"]
    return {
        "protagine_url": os.environ.get("PROTAGINE_URL", "http://127.0.0.1:7777").rstrip("/"),
        "api_key": os.environ.get("PROTAGINE_API_KEY", "dev-mode-no-key"),
        "node_id": node_id,
        "capabilities": caps,
        "job_types": job_types,
        "max_jobs": int(os.environ.get("PROTAGINE_WORKER_MAX_JOBS", "1")),
        "poll_secs": float(os.environ.get("PROTAGINE_WORKER_POLL_SECS", "30")),
        "heartbeat_secs": max(
            1.0,
            float(os.environ.get("PROTAGINE_WORKER_HEARTBEAT_SECS", "20")),
        ),
        "llm_base_url": (os.environ.get("PROTAGINE_WORKER_LLM_BASE_URL")
                         or os.environ.get("OPENAI_BASE_URL", "")).rstrip("/"),
        "llm_model": os.environ.get("PROTAGINE_WORKER_LLM_MODEL", "gpt-4o-mini"),
        "llm_api_key": (os.environ.get("PROTAGINE_WORKER_LLM_API_KEY")
...
def claim_job(cfg: Dict[str, Any]) -> Optional[dict]:
    return _post(cfg, f"{cfg['protagine_url']}/v1/host/queue/jobs/claim", {
        "node_id": cfg["node_id"],
        "capabilities": cfg["capabilities"],
        "job_types": cfg["job_types"],
    }) or None
...
def _heartbeat_loop(
    ...
    while not stop.wait(interval):
        try:
            _post(
                cfg,
                f"{cfg['protagine_url']}/v1/host/queue/jobs/{job_id}/heartbeat",
...
def call_llm(cfg: Dict[str, Any], messages: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    if not cfg["llm_base_url"]:
        raise RuntimeError("no PROTAGINE_WORKER_LLM_BASE_URL configured")
...
def execute_job(cfg: Dict[str, Any], job: dict) -> bool:
    ...
    try:
        started = _post(
            cfg,
            f"{cfg['protagine_url']}/v1/host/queue/jobs/{job_id}/start",
            {"claim_attempt_id": claim_attempt_id},
        )
        ...
        try:
            _post(
                cfg,
                f"{cfg['protagine_url']}/v1/host/queue/jobs/{job_id}/release",
                {"claim_attempt_id": claim_attempt_id},
            )
        except Exception:
            pass
    ...
    heartbeat = threading.Thread(
        ...
        daemon=True,
        name=f"protagine-heartbeat-{job_id}",
    )
    try:
        ...
        try:
            _post(cfg, f"{cfg['protagine_url']}/v1/host/queue/jobs/{job_id}/fail",
    ...
    try:
        resp = _post(cfg, f"{cfg['protagine_url']}/v1/host/queue/jobs/{job_id}/complete",
...
def run_forever(cfg: Dict[str, Any]) -> None:
    print(f"protagine-worker {cfg['node_id']} polling {cfg['protagine_url']} "
...
def main(argv: Optional[list] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="protagine-worker",
        description="Claim typed Protagine jobs and execute them read/analyse-only.")
    ...
    if args.dry_run:
        ...
        print("protagine-worker (dry run -- no network calls):")
