"""The guided Hermes path of ``protagine init``.
...
def _select_state_environment(state):
    os.environ['PROTAGINE_STATE_DIR'] = str(state)
...
def _expose_task_control(config):
    ...
    if 'protagine_task' in eager:
        return False
    # Detach all touched branches from unrelated YAML aliases.
    config['tools'] = {**tools, 'tool_search': {**search, 'eager': [*eager, 'protagine_task']}}
    return True
...
def _owner_handles(args, ask, noninteractive):
    ...
    for value in values:
        ...
        if (not separator or not re.fullmatch(r'[a-z0-9][a-z0-9_.-]{0,63}', platform)
                or platform in {'cli', 'internal', 'system', 'background_review', 'protagine_task'}
...
def _adapter_resources(wheel=None):
    packages = ('protagine_hermes/', 'protagine_memory/')
    if wheel:
        ...
        try:
            distribution = importlib.metadata.distribution('protagine-hermes')
        except importlib.metadata.PackageNotFoundError:
            raise ValueError('Install the canonical protagine-hermes package alongside protagine, or supply --adapter-wheel') from None
    ...
    for name in ('protagine_hermes/__init__.py', 'protagine_hermes/evidence.py', 'protagine_hermes/client.py',
                 'protagine_hermes/commitment_work.py', 'protagine_memory/__init__.py', 'protagine_memory/provider.py'):
...
def _preflight_outbox(home, resources):
    ...
    """
    name = '_protagine_setup_pathcheck'
    module = types.ModuleType(name)
    sys.modules[name] = module
    selected = home/'state'/'protagine-turn-outbox.sqlite3'
    inspected = selected
    try:
        exec(compile(resources['protagine_hermes/client.py'], '<canonical Protagine client>', 'exec'), module.__dict__)
...
selected = []
for group, name, module in [('hermes_agent.plugins', 'protagine', 'protagine_hermes'),
                            ('hermes_agent.memory_providers', 'protagine-memory', 'protagine_memory')]:
    matches = [ep for ep in entries.select(group=group) if ep.name == name]
    if len(matches) > 1 or (matches and matches[0].value != module):
        raise ValueError('Conflicting Protagine entry point')
    selected.append(matches[0] if matches else None)
if not any(selected):
    print(json.dumps({'mode': 'private-directory'}))
    sys.exit(0)
if not all(selected):
    raise ValueError('Both canonical native Protagine entry points are required')
...
'''], input=json.dumps(expected), capture_output=True, text=True, timeout=30)
    if probe.returncode:
        raise ValueError('Hermes has an incomplete or different installed Protagine adapter; select its matching artifact, upgrade that package explicitly, or use a separate Hermes interpreter')
...
def _forwarder(site, module, memory=False):
    ...
    hint = '# ProtagineMemoryProvider native provider entry point.\n' if memory else ''
...
def refresh_adapter(state, args):
    ...
    from .setup import _atomic_hermes_config_write, _read_hermes_config, _align_hermes_memory_spill, _validate_hermes_binding
    ...
    if binding['mode'] == 'private-directory':
        ...
        def retain_forwarder(profile, *, memory=False):
            names = ('protagine-memory',) if memory else ('protagine',)
            ...
            modules = ('protagine_memory',) if memory else ('protagine_hermes',)
            ...
            canonical = 'protagine_memory' if memory else 'protagine_hermes'
    ...
    try:
        if copied_change:
            staged = Path(tempfile.mkdtemp(prefix='.protagine-adapter-', dir=state))
            for name, content in resources.items():
                _private_write(staged/name, content)
            # Hermes and Protagine are stopped by the operator's existing lifecycle.
    ...
    print('Start this Protagine instance and new Hermes processes through their existing lifecycle.')
...
def run(root_dir=None, args=None):
    ...
    try:
        ...
        env = dict(os.environ)
        state = Path(root_dir or env.get('PROTAGINE_STATE_DIR') or
                     plugin_settings(current_config).get('instance_dir') or home/'protagine').expanduser().resolve()
        if state == home or home.is_relative_to(state):
            raise ValueError('The private Protagine directory must not contain the Hermes home')
        ...
        if any(name in config.get('plugins', {}).get('disabled', []) for name in ('protagine',)):
            raise ValueError('Protagine is explicitly disabled in this home; resolve that setting before attachment')
        if any((home/name).exists() for name in ('protagine-memory.json',)):
            raise ValueError('Existing native Protagine settings need an explicit migration; select a new home')
        if (state/'instance.json').exists():
            ...
            if getattr(args, 'local_work', False):
                ...
                options, _ = asyncio.run(planning(json.loads((state/'.protagine-llm-config.json').read_text())))
                verify_tools(options['base_url'], options['model'], options['api_key'],
                             **options['request_overrides'])
                install(state)
                print('Accepted local drafts use native Kanban. Restart this Protagine instance and Hermes gateway to load the binding.')
            if getattr(args, 'native_goals', False):
        ...
        for name in ('protagine', 'protagine-memory'):
            if (home/'plugins'/name).exists():
                raise ValueError('An existing Protagine directory adapter needs an explicit upgrade; choose another home for this installer')
        selected_provider = config.get('memory', {}).get('provider')
        replace_provider = bool(getattr(args, 'replace_memory_provider', False))
        if selected_provider not in (None, '', 'protagine', 'protagine-memory') and not replace_provider:
        ...
        model_key = env.get('PROTAGINE_MODEL_API_KEY', '')
        ...
        if ordinary_skill_review:
            if 'protagine_hermes/ordinary_skill_review.py' not in resources:
        ...
        env_updates = {'PROTAGINE_NATIVE_API_KEY': key, 'PROTAGINE_GENERAL_PLUGIN_ACTIVE': '1',
        ...
        staged = Path(tempfile.mkdtemp(prefix='.protagine-init-', dir=state.parent))
        try:
            ...
            values = {**PRESETS['passive'], 'PROTAGINE_INSTALL_PROFILE': 'local', 'PROTAGINE_STATE_DIR': str(state),
                'PROTAGINE_SIDECAR_HOST': '127.0.0.1', 'PROTAGINE_SIDECAR_PORT': str(port),
                'PROTAGINE_OWNER_CONTACT_ID': owner_id, 'PROTAGINE_OWNER_NAME': owner_name,
                'PROTAGINE_PERSONA_NAME': agent_name, 'PROTAGINE_CONTACTS_DB': str(state/'contacts.db'),
                'PROTAGINE_API_KEYRING_PATH': str(state/'api-keyring.json'), 'PROTAGINE_API_KEY': '',
                'PROTAGINE_CLIENT_API_KEY': key, 'PROTAGINE_GRAPH_ENABLED': 'false',
                'PROTAGINE_EMBED_PROVIDER': 'skip',
                'PROTAGINE_AUTONOMY_PRESET': 'passive', 'PROTAGINE_EMBEDDED_WORKER_ENABLED': 'false',
                'PROTAGINE_SOURCE_CLAIMS': 'on'}
            values.update({
                'PROTAGINE_AGENT_VALUES': json.dumps(agent_preferences['values'], ensure_ascii=True),
                'PROTAGINE_AGENT_TIMEZONE': agent_preferences['timezone'],
                'PROTAGINE_AGENT_QUIET_HOURS': agent_preferences['quiet_hours'],
            })
            if goal_details is not None:
                values['PROTAGINE_HERMES_WORK_BOARDS'] = json.dumps(goal_details['boards'], separators=(',', ':'))
            ...
            _private_write(staged/'.protagine-llm-config.json', _json(model_configuration))
            for name, content in resources.items():
            ...
            if replace_provider:
                ...
                candidate['memory']['provider'] = 'protagine-memory'
            ...
            plugin = candidate['plugins']['protagine']
            plugin.update(instance_dir=str(state), owner_contact_id=owner_id,
                api_key='${PROTAGINE_NATIVE_API_KEY}', execution_registry_enabled=True,
                attested_system_platforms=['cli'], enabled_action_tools=[], enabled_message_tools=[],
                turn_outbox_path=str(home/'state'/'protagine-turn-outbox.sqlite3'))
            candidate['memory']['config'].update(api_key='${PROTAGINE_NATIVE_API_KEY}', turn_writer='disabled')
            enabled = candidate['plugins'].setdefault('enabled', [])
            if not isinstance(enabled, list):
                raise ValueError('Hermes plugins.enabled must be a list')
            if 'protagine' not in enabled:
                enabled.append('protagine')
        ...
        try:
            ...
            if binding['mode'] == 'private-directory':
                for directory, module in (('protagine', 'protagine_hermes'), ('protagine-memory', 'protagine_memory')):
                    create(home/'plugins'/directory/'__init__.py', _forwarder(state/'adapter', module, directory == 'protagine-memory'))
                    create(home/'plugins'/directory/'plugin.yaml', resources[module+'/plugin.yaml'])
                create(home/'plugins'/'protagine-memory'/'cli.py', _forwarder(state/'adapter', 'protagine_memory.cli'))
