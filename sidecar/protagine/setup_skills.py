...
SKILL_NAME = 'protagine-deep-research'
BUNDLE_PREFIX = 'protagine_hermes/bundled_skills/'
...
def prepare(home, resources, *, refresh=False):
    ...
    for path, content in sorted(resources.items()):
        ...
        if not re.fullmatch(r'protagine-[a-z0-9-]{1,56}', name):
            raise ValueError('Bundled skill must have a canonical protagine- name')
...
def _prepare_skill(home, name, content, *, refresh):
    ...
    skill, marker = directory/'SKILL.md', directory/'.protagine-owned.json'
    ...
    if recorded is None:
        ...
        try:
            ...
            owned = (record['owner'] == 'protagine-hermes' and record['version'] == 1
        ...
        if previous != content and not refresh:
            raise ValueError('Bundled skill update requires explicit protagine init --skills-only')
    after = (json.dumps({'owner': 'protagine-hermes', 'version': 1, 'sha256': digest},
...
def install(updates):
    ...
    print('Bundled skills available: '+', '.join(names)+'. With this Protagine adapter version active, ongoing conversations refresh skill discovery on their next request.')
