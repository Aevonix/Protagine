"""Read-only local repo mirrors for Protagine's own research (best-of-B).

Owner-designated repos (env/config only, never hardcoded) are cloned/pulled
into a local mirror directory. Protagine gets low-latency read access -- list,
...
Config: PROTAGINE_REPO_MIRRORS = "name=url[|alias1|alias2][,name2=url2...]"
...
def parse_mirror_config(raw: str) -> Dict[str, Dict[str, str]]:
    """Parse PROTAGINE_REPO_MIRRORS into {name: {url, aliases}}."""
