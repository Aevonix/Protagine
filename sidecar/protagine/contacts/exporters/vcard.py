...
class VCardExporter:
    ...
    Args:
        include_tags:  If True, export Protagine tags as X-PROTAGINE-TAGS property.
        include_tier:  If True, export trust tier as X-PROTAGINE-TIER property.
    """
    ...
    def export_one(self, contact: Contact, handles: List[ContactHandle]) -> str:
        ...
        if self._include_tags and contact.tags:
            lines.append(f"X-PROTAGINE-TAGS:{','.join(contact.tags)}")

        if self._include_tier:
            lines.append(f"X-PROTAGINE-TIER:{contact.trust_tier}")
