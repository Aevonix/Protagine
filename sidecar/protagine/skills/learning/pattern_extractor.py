"""Protagine Skills — pattern extraction from task solutions."""
...
class PatternExtractor:
    ...
    def _synthesize_source(
        ...
        each call via ``protagine.tools.invoke`` so the skill actually runs
        ...
        # ``protagine.tools.invoke(name, args)`` for dynamic dispatch; skills
        ...
        for i, entry in enumerate(tool_calls[:10]):
            ...
            replay_lines.append(
                f"    _r{i} = await protagine.tools.invoke({tool_name!r}, {args_repr})"
            )
        replay_block = "\n".join(replay_lines)
        ...
        return textwrap.dedent(f"""\
            ...
            async def run(protagine, {params}) -> Any:
