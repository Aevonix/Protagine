"""Protagine Skills — learning triggers and coordination service."""
...
from protagine.skills.learning.novelty_detector import NoveltyDetector, NoveltyResult
...
class TriggerCoordinator:
    ...
    the protagine EventBus instance after construction.
    """
...
class LearningTriggerEvent:
    ...
    requestor_protagine_id: Optional[str] = None
...
class SkillLearningService:
    ...
    async def post_task_hook(self, solution: TaskSolution) -> None:
        """Wired as a post-task completion callback in protagine/task_queue/."""
    ...
    async def explicit_remember(
        ...
        event = LearningTriggerEvent(
            ...
            solution=solution,
            force_capture=True,
            requestor_protagine_id=requestor,
        )
