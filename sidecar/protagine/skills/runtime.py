"""Protagine skill runtime handle.

Provides synthesized skills with a ``protagine`` object that exposes
...
class _ToolProxy:
    """Namespace that lets skills call ``protagine.tools.invoke(name, args)``."""
    ...
    async def invoke(self, name: str, args: dict[str, Any] | None = None) -> Any:
        """Invoke a Protagine MCP tool by name through the sidecar API.
        ...
        try:
            ...
            logger.error("protagine.tools.invoke(%s) failed: %s", name, exc)
            raise


class ProtagineRuntime:
    """Thin runtime handle injected into synthesized skills that declare
    a ``protagine`` parameter in their ``run()`` signature.

    Usage inside a skill::

        async def run(protagine, query: str) -> str:
            result = await protagine.tools.invoke("protagine_lookup_facts", {"query": query})
            return str(result)
    """
