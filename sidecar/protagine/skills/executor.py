"""Protagine Skills — skill executor with capability-gated sandbox."""
...
def _resolve_sandbox_mode() -> str:
    ...
    override = (os.environ.get("PROTAGINE_SKILL_SANDBOX") or "").strip().lower()
    if override in ("subprocess", "inprocess"):
        if override == "inprocess" and _DEFAULT_SANDBOX_MODE == "subprocess":
            logger.warning(
                "PROTAGINE_SKILL_SANDBOX=inprocess overrides the default subprocess "
...
class SkillExecutor:
    """Executes Protagine skills inside a capability-gated sandbox.
    ...
    Execution pipeline:
      ...
      4. Execute either in a hardened subprocess (default on Linux) or
         in-process (rollback path, controlled by PROTAGINE_SKILL_SANDBOX).
    ...
    async def _run_subprocess(
        ...
        proc = await asyncio.create_subprocess_exec(
            sys.executable,
            "-m",
            "protagine.skills.sandbox_runner",
    ...
    async def _run_skill_inprocess(
        ...
        """Legacy in-process execution. Used only when PROTAGINE_SKILL_SANDBOX=inprocess."""
        ...
        # Inject protagine runtime if the skill signature expects it
        import inspect
        sig = inspect.signature(run_fn)
        if "protagine" in sig.parameters:
            from protagine.skills.runtime import ProtagineRuntime
            protagine_rt = ProtagineRuntime(base_url=os.environ.get("PROTAGINE_URL", "http://127.0.0.1:7777"))
            inputs = {"protagine": protagine_rt, **inputs}
