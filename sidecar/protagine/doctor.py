"""Protagine doctor — configuration and health diagnostics (v0.19.0).
...
- ``PROTAGINE_OWNER_CONTACT_ID`` unset or unresolvable (relationship +
...
WORKER_CRON_REMEDY = (
    "install the cron: re-run 'protagine init' (Step 10e installs it), or add "
    "'*/5 * * * * protagine-queue-worker' to your crontab — the console script "
    "ships with the pip package (or use "
    "'python -m protagine.workers.queue_worker')"
)
...
PLIST_ENV_REMEDY = (
    ...
    "'launchctl bootout gui/$(id -u)/ai.aevonix.protagine-sidecar && "
    "launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/ai.aevonix.protagine-sidecar.plist' — "
...
def check_state_dir() -> CheckResult:
    """1. PROTAGINE_STATE_DIR exists and is writable."""
    path = _state_dir()
    if not path.exists():
        return CheckResult(
            "state-dir", FAIL,
            detail=f"state dir {path} does not exist",
            remedy=f"mkdir -p {path} (or run 'protagine init'); set PROTAGINE_STATE_DIR if it should live elsewhere",
        )
    if not path.is_dir():
        return CheckResult(
            "state-dir", FAIL,
            detail=f"{path} exists but is not a directory",
            remedy="point PROTAGINE_STATE_DIR at a writable directory",
        )
...
def check_llm_config() -> List[CheckResult]:
    ...
    path = _state_dir() / ".protagine-llm-config.json"
    ...
    if not path.exists():
        results = [CheckResult(
            ...
            remedy="run 'protagine init' or POST /v1/host/configure from the host to persist an LLM config",
        )]
    ...
    try:
        ...
        results = [CheckResult(
            ...
            remedy="fix or delete the file, then re-run 'protagine init' / POST /v1/host/configure",
        )]
...
def check_contacts_db() -> CheckResult:
    ...
    if sqlite_path == ":memory:":
        return CheckResult(
            ...
            remedy="set PROTAGINE_CONTACTS_DB to a file path, or set PROTAGINE_STATE_DIR so the "
                   "default $PROTAGINE_STATE_DIR/protagine-contacts.db applies",
        )
    ...
    if not path.parent.exists():
        return CheckResult(
            ...
            remedy=f"mkdir -p {path.parent} (or fix PROTAGINE_CONTACTS_DB / PROTAGINE_STATE_DIR)",
        )
    if not path.exists():
        ...
        # running service (whose PROTAGINE_CONTACTS_DB/PROTAGINE_STATE_DIR live in
        ...
        if siblings:
            return CheckResult(
                ...
                detail=f"{path} does not exist, but {siblings[0]} does — this "
                       ...
                       "running service",
                remedy="run doctor with the service's env (PROTAGINE_CONTACTS_DB / "
                       "PROTAGINE_STATE_DIR), or align the two",
            )
        return CheckResult(
            "contacts-db", PASS,
    ...
    if big_siblings:
        return CheckResult(
            ...
            remedy="run doctor with the service's env (PROTAGINE_CONTACTS_DB / "
                   "PROTAGINE_STATE_DIR), or delete the stale empty stub",
        )
    try:
...
def check_owner_contact_id() -> CheckResult:
    """4. PROTAGINE_OWNER_CONTACT_ID must be set for owner-aware subsystems."""
    canonical = os.environ.get("PROTAGINE_OWNER_CONTACT_ID", "")
    legacy = os.environ.get("PROTAGINE_HOST_CONTACT_ID", "")
    if canonical:
        return CheckResult(
            "owner-contact-id", PASS, detail=f"PROTAGINE_OWNER_CONTACT_ID={canonical}",
        )
    if legacy:
        return CheckResult(
            "owner-contact-id", WARN,
            detail=f"owner only set via deprecated PROTAGINE_HOST_CONTACT_ID={legacy}",
            remedy="rename the variable to PROTAGINE_OWNER_CONTACT_ID (same value)",
        )
    return CheckResult(
        "owner-contact-id", WARN,
        detail="PROTAGINE_OWNER_CONTACT_ID is not set — owner-exclusion filters fail closed, so "
        ...
        remedy="set PROTAGINE_OWNER_CONTACT_ID to the owner's contact CID (create one via "
    ...
    if authority_mode == "shadow":
        ...
        if effects:
            return CheckResult(
                ...
                remedy="set PROTAGINE_APPROVAL_AUTHORITY_MODE=enforce (or take "
...
def check_approval_policy() -> CheckResult:
    ...
    authority_raw = os.environ.get("PROTAGINE_APPROVAL_AUTHORITY_MODE")
    if authority_raw and authority_raw.strip().lower() not in ("shadow", "enforce"):
        return CheckResult(
            "approval-policy", FAIL,
            detail=(
                f"PROTAGINE_APPROVAL_AUTHORITY_MODE={authority_raw!r} is invalid; "
                "approval control surfaces fail closed with invalid configuration"
            ),
            remedy="set PROTAGINE_APPROVAL_AUTHORITY_MODE to 'shadow' or 'enforce'",
        )
    authority_mode = (authority_raw or "shadow").strip().lower()
    raw = os.environ.get("PROTAGINE_APPROVAL_POLICY")
    if raw is None or not raw.strip():
        return _approval_result(
            "PROTAGINE_APPROVAL_POLICY unset — defaults to strict; approval "
    ...
    if value in ("strict", "graduated"):
        return _approval_result(
            f"PROTAGINE_APPROVAL_POLICY={value}; approval authority "
            f"mode={authority_mode}",
            authority_mode,
        )
    return CheckResult(
        "approval-policy", FAIL,
        detail=f"PROTAGINE_APPROVAL_POLICY={raw!r} is not a valid mode — the gate fails closed "
               "to strict, so the policy you intended is silently NOT active",
        remedy="set PROTAGINE_APPROVAL_POLICY to 'strict' or 'graduated' (or unset it for strict)",
    )
...
def check_feature_gates() -> CheckResult:
    ...
    problems: List[str] = []
    notes: List[str] = []
    for var in ("PROTAGINE_ENABLE_INTERNAL_THINKING", "PROTAGINE_ENABLE_SKILL_SYNTHESIS"):
    ...
    thinking_on = (
        os.environ.get("PROTAGINE_ENABLE_INTERNAL_THINKING", "false").strip().lower() == "true"
    )
    if thinking_on:
        notes.append(
    ...
    preset = os.environ.get("PROTAGINE_AUTONOMY_PRESET", "").strip().lower()
    if preset:
        try:
            from protagine.util.autonomy_preset import PRESETS
            if preset in PRESETS:
                notes.append(f"PROTAGINE_AUTONOMY_PRESET={preset}")
            else:
                problems.append(
                    f"PROTAGINE_AUTONOMY_PRESET={preset!r} is not a known preset "
    ...
    return CheckResult(
        "feature-gates", PASS,
        detail="; ".join(notes)
               or "gates unset (features off; set PROTAGINE_AUTONOMY_PRESET or "
...
def check_tom2_cross_context() -> CheckResult:
    ...
    Hermes cannot mutate a post-LLM reply and the Protagine plugin honestly
    ...
    on = os.environ.get("PROTAGINE_TOM2_CROSS_CONTEXT", "0").strip().lower()\
        in ("1", "true", "yes", "on")
    if not on:
        return CheckResult(
            "tom2-cross-context", PASS,
            detail="PROTAGINE_TOM2_CROSS_CONTEXT off (default; the "
                   "cross-contact render path ships dark)")
    chat_mode = os.environ.get("PROTAGINE_GUARD_CHAT_MODE", "").strip().lower()
    if chat_mode != "enforce":
        return CheckResult(
            "tom2-cross-context", WARN,
            detail="PROTAGINE_TOM2_CROSS_CONTEXT is ON while the chat guard is "
                   f"not enforcing (PROTAGINE_GUARD_CHAT_MODE={chat_mode or '(unset)'}) — "
                   "cross-contact epistemic rendering without an enforcing "
                   "outbound guard risks implication leaks",
            remedy="set PROTAGINE_TOM2_CROSS_CONTEXT=0 (recommended; this path "
                   "is built but deliberately unwired) or finish a "
                   "transport-owned applied-enforcement mediator; setting "
                   "PROTAGINE_GUARD_CHAT_MODE=enforce alone does not qualify")
    return CheckResult(
        "tom2-cross-context", WARN,
        detail="PROTAGINE_TOM2_CROSS_CONTEXT is ON and chat enforce was "
               "requested, but no applied-enforcement capability truth "
               "exists: current Hermes cannot apply post-LLM reply "
               "mutations, so the Protagine plugin runs the chat guard in "
               "SHADOW",
        remedy="set PROTAGINE_TOM2_CROSS_CONTEXT=0; do not treat "
               "PROTAGINE_GUARD_CHAT_MODE=enforce as applied enforcement. "
...
def check_tom2_risk_caps() -> CheckResult:
    """Leveled tom2 (L1.3): PROTAGINE_TOM2_RISK_CAPS must parse. A malformed
    ...
    from protagine.tom.levels import DEFAULT_RISK_CAPS, risk_caps_valid
    raw = os.environ.get("PROTAGINE_TOM2_RISK_CAPS", "").strip()
    if risk_caps_valid():
        return CheckResult(
            "tom2-risk-caps", PASS,
            detail=f"PROTAGINE_TOM2_RISK_CAPS={raw or f'(default {DEFAULT_RISK_CAPS})'}")
    return CheckResult(
        "tom2-risk-caps", WARN,
        detail=f"PROTAGINE_TOM2_RISK_CAPS={raw!r} is malformed — the level "
               "resolver fails closed to all-0 caps (every environment "
               "renders level 0), NOT the caps you configured",
        remedy="set PROTAGINE_TOM2_RISK_CAPS to four 'risk:cap' pairs covering "
               f"risks 0-3 with caps 0-2, e.g. {DEFAULT_RISK_CAPS}, or unset "
               "it for the default")


def check_tom2_level_coherence() -> CheckResult:
    """Leveled cross-contact tom2 (L4.3): a raised PROTAGINE_TOM2_LEVEL must
    ...
    from protagine.gate.response_guard import enforce_allowlist
    ...
    if lvl >= 2:
        ...
        if allowed is not None and "tom2_epistemic" not in allowed:
            problems.append("level 2 unreachable: tom2_epistemic is not on "
                            "PROTAGINE_GUARD_ENFORCE_CHECKS, so enforce "
    ...
    if problems:
        return CheckResult(
            ...
            detail=f"PROTAGINE_TOM2_LEVEL={lvl} but " + "; ".join(problems),
            remedy="either lower PROTAGINE_TOM2_LEVEL to the level you can "
    ...
    return CheckResult(
        "tom2-level-coherence", PASS,
        detail=f"PROTAGINE_TOM2_LEVEL={lvl} with a coherent brake posture")
...
def check_hermes_skills_dir() -> CheckResult:
    ...
    from protagine.skills.hermes_export import hermes_base_dir, hermes_export_enabled

    if not hermes_export_enabled():
        return CheckResult(
            "hermes-skills-dir", SKIP, detail="PROTAGINE_EMIT_HERMES_SKILLS not enabled",
        )
    ...
    return CheckResult(
        ...
        detail=f"PROTAGINE_EMIT_HERMES_SKILLS is on but {parent} does not exist — skill exports "
               "will fail",
        remedy=f"mkdir -p {parent} (or point PROTAGINE_HERMES_SKILLS_DIR at an existing skills tree)",
    )
...
def check_relationship_attribution() -> CheckResult:
    ...
    comms = _state_dir() / "protagine-comms.db"
    if not comms.exists():
...
def check_optional_video() -> CheckResult:
    ...
    try:
        ...
        return CheckResult('optional-video', SKIP, detail=
            "Decoder unavailable in this interpreter. For selected clip memory install 'protagine[video]' here; see docs/SOURCE-VIDEOS.md.")
...
def check_server_auth(base_url: str, api_key: str, timeout: float) -> CheckResult:
    ...
    if status in (401, 403):
        return CheckResult(
            ...
            remedy="set PROTAGINE_API_KEY (CLI side) to the key in the sidecar's environment "
                   "(~/.protagine/.env), or pass --api-key. " + PLIST_ENV_REMEDY,
        )
...
def check_server_auth_migration(
    ...
    if auth.get("dual_accept") and legacy:
        try:
            ...
            quiet_hours = max(
                0.0, float(os.environ.get("PROTAGINE_AUTH_LEGACY_QUIET_HOURS", "24")),
            )
            age_hours = max(
                0.0,
...
def check_server_owner_contact(base_url: str, api_key: str, timeout: float) -> CheckResult:
    ...
    from protagine.identity.resolver import get_owner_contact_id

    owner = get_owner_contact_id()
    if not owner:
        return CheckResult(
            "server-owner-contact", SKIP,
            detail="PROTAGINE_OWNER_CONTACT_ID not set (see owner-contact-id)",
        )
    ...
    if status == 404:
        return CheckResult(
            ...
            detail=f"PROTAGINE_OWNER_CONTACT_ID={owner} does not resolve to any contact (404) — "
            ...
            remedy="create the owner contact via POST /v1/host/contacts and set "
                   "PROTAGINE_OWNER_CONTACT_ID to the returned contact_id. " + PLIST_ENV_REMEDY,
        )
    return CheckResult(
        "server-owner-contact", FAIL,
...
def check_server_llm(base_url: str, api_key: str, timeout: float) -> CheckResult:
    ...
    return CheckResult(
        ...
        remedy="the common causes are a baseUrl missing the /v1 suffix (LiteLLM 404s against "
               ...
               ".protagine-llm-config.json, and restart the sidecar. " + PLIST_ENV_REMEDY,
    )
    ...
    if status == 200 and isinstance(body, dict):
        ...
        return CheckResult(
            ...
            remedy="check the sidecar log for EmbeddingPipeline init errors; verify "
                   "PROTAGINE_EMBED_PROVIDER / PROTAGINE_EMBED_MODEL and restart the sidecar",
        )
...
def check_server_source_memory(base_url: str, api_key: str, timeout: float) -> CheckResult:
    ...
    owner = os.environ.get("PROTAGINE_OWNER_CONTACT_ID", "").strip()
    if not owner or not api_key:
        return CheckResult(
            "server-source-memory", FAIL,
            detail="An owner contact and scoped client credential are required.",
            remedy="Use the selected instance environment and its PROTAGINE_CLIENT_API_KEY.",
        )
    try:
        status, body = _http_get(
...
def check_server_worker_liveness(base_url: str, api_key: str, timeout: float) -> CheckResult:
    ...
    the cron-driven ``protagine-queue-worker`` is absent or broken.
    """
    status, body = _http_get(
...
def check_server_skills_observations(base_url: str, api_key: str, timeout: float) -> CheckResult:
    ...
    if not observations:
        return CheckResult(
            ...
            detail="no skill observations recorded — Protagine does not know which skills the "
                   "agent has installed",
            remedy="run the protagine-skills-sync console command (installed with the pip "
    ...
    if age_days > 7:
        return CheckResult(
            ...
            remedy="run the protagine-skills-sync console command (or re-enable the plugin "
...
def check_server_grant_envelope(
    ...
    if (
        ...
        return CheckResult(
            ...
            remedy="check the sidecar log and restart with valid PROTAGINE_GRANT_MAX_* settings",
        )

    if expected_standing or active_standing:
        dimensions = []
        if ttl_state == "unbounded":
            dimensions.append("PROTAGINE_GRANT_MAX_TTL_SECONDS=unlimited (no expiry)")
        if uses_state == "unbounded":
            dimensions.append("PROTAGINE_GRANT_MAX_USES=unlimited (no use cap)")
        if active_standing:
            dimensions.append(
    ...
    return CheckResult(
        ...
        detail=(
            ...
            f"PROTAGINE_GRANT_MAX_TTL_SECONDS={ttl}, "
            f"PROTAGINE_GRANT_MAX_USES={uses}"
        ),
    )
...
def check_server_autonomy_posture(base_url: str, api_key: str, timeout: float) -> CheckResult:
    ...
    # with a reactive loop is a Protagine that never thinks, calibrates, or
    ...
    # only mean an explicit PROTAGINE_AUTONOMY_MODE=reactive pin, coupling
    ...
    autonomy_mode = str(posture.get("PROTAGINE_AUTONOMY_MODE", "") or "").lower()
    mode_source = str(posture.get("PROTAGINE_AUTONOMY_MODE_SOURCE", "") or "").lower()
    coupling = str(posture.get("PROTAGINE_PRESET_LOOP_COUPLING", "") or "").lower()
    if preset in ("calibration", "autonomous") and autonomy_mode == "reactive":
        ...
        if mode_source == "env":
            return CheckResult(
                ...
                detail=base + " (PROTAGINE_AUTONOMY_MODE=reactive is explicitly "
                       "pinned below the preset)",
                remedy="unset PROTAGINE_AUTONOMY_MODE and restart (the preset "
                       "supplies proactive via PROTAGINE_PRESET_LOOP_COUPLING), "
                       "or leave the pin if the rollback is deliberate — "
                       "then this FAIL is the reminder")
        if coupling == "off":
            return CheckResult(
                "server-autonomy-posture", FAIL,
                detail=base + " (PROTAGINE_PRESET_LOOP_COUPLING=off disabled "
                       "the preset's mode default)",
                remedy="unset PROTAGINE_PRESET_LOOP_COUPLING (it defaults on) "
                       "or set PROTAGINE_AUTONOMY_MODE=proactive and restart")
        if not mode_source:
            ...
            return CheckResult(
                ...
                remedy="set PROTAGINE_AUTONOMY_MODE=proactive and restart (or "
        ...
        return CheckResult(
            ...
            remedy="set PROTAGINE_AUTONOMY_MODE=proactive explicitly and "
                   "check server logs for preset resolution errors")

    live = sorted(k.replace("PROTAGINE_", "").replace("_MODE", "").lower()
                  for k, v in posture.items() if v == "live")
    shadow = sorted(k.replace("PROTAGINE_", "").replace("_MODE", "").lower()
                    for k, v in posture.items() if v in ("shadow", "dry_run"))
    on = sorted(k.replace("PROTAGINE_", "").replace("_ENABLED", "").lower()
    ...
    if not (on or live or shadow):
        return CheckResult(
            "server-autonomy-posture", WARN,
            detail="everything is off — this Protagine observes but never thinks or acts",
            remedy="set PROTAGINE_AUTONOMY_PRESET=calibration (shadow everything, earn "
                   "autonomy via the trust engine) or flip individual PROTAGINE_*_MODE flags")
    ...
    if downgraded:
        return CheckResult(
            ...
            remedy="unset the overriding PROTAGINE_* env var(s) to inherit the "
...
def _preset_downgrades(preset: str, posture: dict) -> List[str]:
    ...
    for flag, want in PRESETS.get(preset, {}).items():
        ...
        if have and rank.get(have, 99) < rank.get(want, 0):
            out.append(
                flag.replace("PROTAGINE_", "").replace("_MODE", "").lower()
                + f"={have}")
    return sorted(out)
...
def check_server_self_model(base_url: str, api_key: str, timeout: float) -> CheckResult:
    ...
    if not body.get("available"):
        return CheckResult(
            ...
            remedy="unset PROTAGINE_SELF_MODEL_ENABLED=false (it defaults on) and restart")
...
def check_server_benchmark(base_url: str, api_key: str, timeout: float) -> CheckResult:
    ...
    if not body.get("available"):
        return CheckResult(
            ...
            remedy="unset PROTAGINE_BENCHMARK_ENABLED=false (it defaults on) and restart")
    ...
    if falling:
        return CheckResult(
            ...
            detail=f"{len(weeks)} week(s) of rollups; regressing week-over-week: "
                   + ", ".join(sorted(falling)),
            remedy="run `protagine benchmark` and review the regressing metrics' detail")
    return CheckResult(
        "server-benchmark", PASS,
...
def check_server_expectations(base_url: str, api_key: str, timeout: float) -> CheckResult:
    ...
    if not body.get("available"):
        return CheckResult(
            ...
            detail="expectations off (PROTAGINE_EXPECTATIONS=off) — no calibration signal")
...
def check_server_workspace(base_url: str, api_key: str, timeout: float) -> CheckResult:
    ...
    if not body.get("available"):
        return CheckResult(
            ...
            detail="workspace off (PROTAGINE_WORKSPACE=off) — no continuous thought")
...
def check_server_toolsmith(base_url: str, api_key: str, timeout: float) -> CheckResult:
    ...
    if not body.get("available"):
        return CheckResult(
            ...
            detail="toolsmith off (PROTAGINE_TOOLSMITH=off) — no self-built tools")
    ...
    if mode == "off" and domains:
        return CheckResult(
            ...
            detail=f"workers have a track record ({len(domains)} domain(s)) but the "
                   ...
                   "server-side",
            remedy="set PROTAGINE_WORKERS_MODE=shadow (observe) or live (enforce)")
    return CheckResult(
    ...
    if mode != "off" and not connectors:
        return CheckResult(
            ...
            detail=f"mode={mode} but no connector is registered — the agent has no "
                   "senses configured",
            remedy="enable at least one: PROTAGINE_CONNECTOR_FS_PATH (folder watch), "
                   "PROTAGINE_CONNECTOR_IMAP_* (email), PROTAGINE_CONNECTOR_CALENDAR_ICS_URL, "
                   "or PROTAGINE_CONNECTOR_WEBHOOK_URL; each also needs "
                   "PROTAGINE_CONNECTOR_<NAME>_ENABLED=true")
...
def check_server_directives(base_url: str, api_key: str, timeout: float) -> CheckResult:
    ...
    ("attempt them", "that and wipe it from protagine") and repeated captures
...
def run_server_checks(base_url: str, api_key: str, timeout: float = 10.0) -> List[CheckResult]:
    ...
    try:
        ...
        results.append(CheckResult(
            ...
            remedy="start it with 'protagine start' (or 'protagine service start'), then re-run "
                   "'protagine doctor'",
        ))
    ...
    if status == 200 and isinstance(body, dict):
        ...
        if health_status == "ok":
            ...
            results.append(CheckResult(
                ...
                remedy="check the sidecar log; 'protagine status' shows the degraded subsystems",
            ))
    else:
        results.append(CheckResult(
            "server-health", FAIL,
...
def default_api_key() -> str:
    if os.environ.get('PROTAGINE_INSTALL_PROFILE') == 'local':
        return os.environ.get('PROTAGINE_CLIENT_API_KEY', '')
