"""Temporal reference frame for Protagine (v0.21.0).
...
Three independent, editable timezones (per the design):
  * AGENT timezone   — where the Protagine agent "lives" (its home reference frame).
                       Editable; defaults to PROTAGINE_AGENT_TIMEZONE, then the host
                       system tz, then UTC.
...
def agent_timezone() -> str:
    ...
    env = os.environ.get("PROTAGINE_AGENT_TIMEZONE")
...
def default_contact_timezone() -> Optional[str]:
    ...
    env = os.environ.get("PROTAGINE_DEFAULT_CONTACT_TIMEZONE")
    if is_valid_timezone(env):
        return env  # type: ignore[return-value]
