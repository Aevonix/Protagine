...
The env PROTAGINE_<X>_MODE remains the owner override: "off" stays off and
...
class TrustEngine:
    ...
    def snapshot(self) -> List[Dict[str, Any]]:
        from protagine.self_model.supervised import supervised_enabled
        with self._lock:
            rows = self._conn.execute(
    ...
    def gate(self, domain: str, description: str, *,
        ...
        if floor:
            ...
            if conf >= _fenv("PROTAGINE_TRUST_ACT_THRESHOLD", 0.8):
                decision = "act"
    ...
    def after_outcome(self, domain: str) -> None:
        ...
        try:
            ...
            pass
        window = _fenv("PROTAGINE_TRUST_BREAKER_WINDOW_HOURS", 24.0) * 3600.0
        ...
        if stage == "act_first" and (
                violations > 0
                or failures >= _ienv("PROTAGINE_TRUST_BREAKER_FAILURES", 3)):
            self.set_stage(
        ...
        if stage == "shadow":
            ...
            if n >= _ienv("PROTAGINE_TRUST_ASK_MIN_N", 3) and bad == 0:
            ...
            conf = self.confidence(domain)
            if (len(real) >= _ienv("PROTAGINE_TRUST_ACT_MIN_N", 5)
                    and conf >= _fenv("PROTAGINE_TRUST_ACT_THRESHOLD", 0.8)
