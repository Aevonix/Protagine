...
def _parts():
    ...
    owner = (os.environ.get('PROTAGINE_OWNER_PERSON_ID') or os.environ.get('PROTAGINE_OWNER_CONTACT_ID', '')).strip()
...
def _parent_current(ledger, commitments, conditions, owner):
    from protagine.api.routers.temporal_followups import source_bindings
...
def _stop_facts(ledger, commitments, prediction, now):
    ...
    from protagine.commitments.store import OPEN_STATUSES
    c = prediction.detail['conditions']
