...
``PROTAGINE_COGNITION_P4_MODE`` defaults to ``off``. In ``shadow`` the complete
...
def experiments_enabled() -> bool:
    return os.environ.get(
        "PROTAGINE_EXPERIMENTS_ENABLED", "true").strip().lower() != "false"


def _max_running() -> int:
    try:
        return max(1, min(10, int(os.environ.get(
            "PROTAGINE_EXPERIMENTS_MAX_RUNNING", "2"))))
    except ValueError:
        return 2
...
def experiment_pregrants_from_env() -> Dict[str, Tuple[float, float]]:
    ...
    raw = os.environ.get("PROTAGINE_EXPERIMENT_PREGRANTS_JSON", "").strip()
    if not raw:
        return {}
    try:
        decoded = json.loads(raw)
    except (TypeError, ValueError) as exc:
        raise ValueError("PROTAGINE_EXPERIMENT_PREGRANTS_JSON is invalid JSON") from exc
