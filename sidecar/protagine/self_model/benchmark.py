...
Storage: protagine-benchmark.db (samples append-only + weekly rollups).
...
def cognition_p4_mode() -> str:
    ...
    value = os.environ.get("PROTAGINE_COGNITION_P4_MODE", "off").strip().lower()
...
def benchmark_enabled() -> bool:
    return os.environ.get(
        "PROTAGINE_BENCHMARK_ENABLED", "true").strip().lower() != "false"
...
class SelfhoodBenchmark:
    ...
    def probe_count(self) -> int:
        if self._probes is not None:
            return self._probes
        try:
            return int(os.environ.get("PROTAGINE_BENCHMARK_PROBES", "8"))
        except ValueError:
            return 8
    ...
    def _m_delivery(self, start, end, since, until):
        ...
        return {"value": ok / len(evs), "numerator": ok,
                ...
                "detail": {
                    "n": len(evs),
                    "metric_definition": "protagine.delivery-success/v1",
                    "competence_reconciliation_revision": revision,
                }}
    ...
    def _m_actions(self, start, end, since, until):
        ...
        return {"value": ok / n, "numerator": ok, "denominator": n,
                "detail": {
                    "domains": per,
                    "metric_definition": "protagine.actions-success/v2",
                    "competence_reconciliation_revision": revision,
                }}
    ...
    def _m_acceptance(self, start, end, since, until):
        ...
        return {"value": accepted / len(deliveries), "numerator": accepted,
                ...
                "detail": {
                    ...
                    "metric_definition": "protagine.initiative-acceptance/v1",
                    "competence_reconciliation_revision": revision,
                }}
    ...
    def snapshot(self, weeks: int = 8) -> Dict[str, Any]:
        ...
        return {
            "format": "protagine.selfhood-benchmark/v2",
