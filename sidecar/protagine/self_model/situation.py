...
understands Protagine's durable host-journal envelope plus the structured event
families shared with the host.  Deployments may construct ``SituationObservationV1``
directly for attested probes/transports without patching Protagine.
"""
...
_EVIDENCE_PREFIXES = frozenset({
    "journal", "event", "receipt", "host-event", "protagine-event",
...
def situation_spine_mode() -> str:
    ...
    value = os.environ.get("PROTAGINE_SITUATION_SPINE", "off").strip().lower()
...
class JournalSituationAdapter:
    ...
    def _scope(data: Mapping[str, Any]) -> tuple[str, str, str]:
        subject = str(
            ...
            or os.environ.get("PROTAGINE_OWNER_PERSON_ID", "").strip()
    ...
    def adapt(
        ...
        *,
        source_kind: str = "protagine_event",
...
class SituationReducer:
    ...
    def _gap_policy() -> str:
        return os.environ.get(
            "PROTAGINE_SITUATION_GAP_POLICY", "stop",
        ).strip().lower()
