...
def _align_hermes_memory_spill(config: dict) -> bool:
    """Keep native head/tail previews from cutting Protagine's evidence envelope.
...
def _prepare_hermes_config(
    ...
    if provider not in (None, "", "protagine-memory"):
        raise ValueError("Another memory provider is configured; migrate it explicitly before staging Protagine")
    memory_config = mapping(memory, "config")
    plugins = mapping(config, "plugins")
    plugin_config = mapping(plugins, "protagine")
    native_path = config_path.with_name("protagine-memory.json")
    try:
        native_config = json.loads(native_path.read_text()) if native_path.exists() else {}
    except (OSError, ValueError):
        raise ValueError("Native Protagine memory configuration is invalid") from None
    if not isinstance(native_config, dict):
        raise ValueError("Native Protagine memory configuration must be an object")
    ...
    for settings in (memory_config, plugin_config, native_config):
        if settings.get("url") not in (None, "", sidecar_url):
            raise ValueError("Existing Protagine endpoint differs; use a reviewed instance migration")
        if contact_id and settings.get("contact_id") not in (None, "", contact_id):
            raise ValueError("Existing Protagine contact differs; preserve its identity or migrate explicitly")
    ...
    if not isinstance(selected_contact, str) or not selected_contact.strip():
        raise ValueError("Provide --contact-name or retain an existing Protagine contact binding")
    if any(value not in (None, "", selected_contact) for value in bindings):
        raise ValueError("Existing Protagine contact bindings disagree; reconcile them before staging")
    ...
    for settings in (memory_config, plugin_config):
        ...
        if settings.get("api_key") in (None, ""):
            settings["api_key"] = "${PROTAGINE_API_KEY}"
...
def _atomic_hermes_config_write(config_path: Path, original: bytes | None, updated: bytes) -> None:
    ...
    if original is not None:
        with tempfile.NamedTemporaryFile(
            prefix=f".{config_path.name}.protagine-backup-", dir=config_path.parent, delete=False,
        ) as backup:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            prefix=f".{config_path.name}.protagine-stage-", dir=config_path.parent, delete=False,
        ) as staged:
...
def run_init(root_dir: str | None = None, args=None) -> int:
    ...
    from protagine.setup_hermes import run
