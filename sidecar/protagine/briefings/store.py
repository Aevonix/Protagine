"""Protagine Briefing System — SQLite persistence store.
...
class BriefingStore:
    ...
    def get_instance(cls) -> "BriefingStore":
        """Return the process-wide singleton backed by ~/.protagine/data/briefings.db."""
        global _briefing_store_instance
        if _briefing_store_instance is None:
            protagine_home = get_state_dir()
            _briefing_store_instance = cls(str(protagine_home / "briefings.db"))
