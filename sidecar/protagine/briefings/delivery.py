"""Protagine Briefing System — delivery gateways and delivery engine.
...
class IMessageBriefingGateway(BriefingGateway):
    ...
    def format(self, briefing: Briefing) -> str:
        ...
        lines = [f"{emoji} *Protagine {briefing.briefing_type.value.title()} Briefing*", ""]
        for section in briefing.active_sections():
        ...
        if len(text) > _IMESSAGE_MAX_CHARS:
            trailer = "\n... [see Protagine for full briefing]"
