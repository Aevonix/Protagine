...
Usage::
    ...
    from protagine.contextgate import prepare_context
...
- ``PROTAGINE_CONTEXT_GATE``            — ``auto`` (default) | ``on`` | ``off``
- ``PROTAGINE_CONTEXT_GATE_HEADROOM``   — fraction of budget that triggers
  gating (default 0.8)
- ``PROTAGINE_CONTEXT_GATE_BUDGET``     — default token budget when neither
  the caller nor the model tier provides one (default 0 = no gating)
- ``PROTAGINE_CONTEXT_CHUNK_TOKENS``    — target chunk size (default 1024)
- ``PROTAGINE_CONTEXT_OVERLAP_TOKENS``  — chunk overlap (default 128)
- ``PROTAGINE_CONTEXT_CHARS_PER_TOKEN`` — token estimator ratio (default 4.0)
"""

from protagine.contextgate.estimate import estimate_tokens
from protagine.contextgate.chunker import Chunk, chunk_text
from protagine.contextgate.retrieve import rank_chunks
from protagine.contextgate.gate import (
    GateConfig,
    GateDecision,
    PreparedContext,
    classify_task,
