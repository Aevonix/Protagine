"""Durable, generic execution boundary for owner-governed Protagine actions.
...
def _validate_args(tool_name: str, raw: Any) -> dict[str, Any]:
    if tool_name in {"protagine_autonomy_disable", "protagine_autonomy_enable"}:
        return _exact_mapping(raw, set(), name="args")
    if tool_name == "protagine_create_commitment":
        args = _exact_mapping(
            raw,
        ...
        return args
    if tool_name == "protagine_initiative_feedback":
        ...
        return args
    if tool_name == "protagine_record_insight":
        if not isinstance(raw, Mapping):
            raise GovernedActionValidationError("args must be an object")
        args = dict(raw)
        if (
        ...
        return args
    if tool_name == "protagine_research":
        if not isinstance(raw, Mapping):
            raise GovernedActionValidationError("args must be an object")
        args = dict(raw)
        ...
        return args
    if tool_name == "protagine_resolve_commitment":
        if not isinstance(raw, Mapping):
            raise GovernedActionValidationError("args must be an object")
        args = dict(raw)
        ...
        return args
    if tool_name == "protagine_task_complete":
        ...
        return args
    if tool_name == "protagine_task_dismiss":
        if not isinstance(raw, Mapping):
            raise GovernedActionValidationError("args must be an object")
        args = dict(raw)
        ...
        return args
    if tool_name == "protagine_task_snooze":
        if not isinstance(raw, Mapping):
            raise GovernedActionValidationError("args must be an object")
        args = dict(raw)
        ...
        return args
    raise GovernedActionValidationError("tool is not a governed Protagine action")
...
def parse_execution_request(
    ...
    *,
    ...
    if (
        document["schema"] != "ProtagineGovernedActionExecutionV1"
    ...
    if (
        approval["schema"] != "ProtagineOwnerApprovalExecutionBindingV1"
...
def _validated_effect(value: Any) -> dict[str, Any]:
    ...
    if (
        effect["schema"] != "ProtagineGovernedActionEffectV1"
...
def _state_result(
    ...
    *,
    ...
    result = {
        "schema": "ProtagineGovernedActionExecutionResultV1",
        "version": 1,
...
class ProtagineSubsystemActionExecutor:
    """Map the ten generic action names onto existing Protagine subsystems."""
    ...
    def _governed_research_project(
        ...
        from protagine.projects import Project
        from protagine.work_orders import action_authority

        args = request["args"]
        approval = request["approval"]
        identity_material = {
            "schema": "ProtagineGovernedResearchProjectIdentityV1",
            "version": 1,
    ...
    async def prepare(
        ...
        if tool_name not in ACTION_TOOL_NAMES:
            raise RuntimeError("unknown governed Protagine action")
        if tool_name == "protagine_autonomy_enable":
            self._dependency(self.autonomy_enable, "autonomy control")
            self._dependency(self.autonomy_running, "autonomy status")
        elif tool_name == "protagine_autonomy_disable":
            self._dependency(self.autonomy_disable, "autonomy control")
            self._dependency(self.autonomy_running, "autonomy status")
        elif tool_name == "protagine_create_commitment":
            self._dependency(self.commitments, "commitment store")
        elif tool_name == "protagine_resolve_commitment":
        ...
        elif tool_name == "protagine_initiative_feedback":
        ...
        elif tool_name == "protagine_record_insight":
        ...
        elif tool_name == "protagine_research":
    ...
    async def perform(
        ...
        if tool_name == "protagine_autonomy_disable":
