"""Protagine MCP harness configuration.

Handles detecting installed harnesses and configuring them to use Protagine's MCP server.
"""
...
def _mcp_config(contact_id: str, source: str, include_type: bool = False,
    ...
    """New Protagine config. Authentication is inherited, never a literal placeholder."""
    env = dict(os.environ)
    url = sidecar_url or env.get("PROTAGINE_SIDECAR_URL") or (
        "http://127.0.0.1:" + env.get("PROTAGINE_SIDECAR_PORT", "7777"))
    custom = env.get("PROTAGINE_MCP_COMMAND")
    result = {
        "command": custom or "protagine",
        "args": shlex.split(env.get("PROTAGINE_MCP_ARGS", "")) if custom else ["mcp"],
        "env": {"PROTAGINE_URL": url, "PROTAGINE_MCP_CONTACT_ID": contact_id,
                "PROTAGINE_MCP_SOURCE": source},
    }
    if include_type:
        result["type"] = "stdio"
    return result
...
def _selected(servers: dict) -> dict:
    selected = servers.get("protagine")
    if selected is not None and not isinstance(selected, dict):
        raise ValueError("The Protagine MCP entry must be a configuration table")
    return selected or {}
...
def _merged(existing: dict, desired: dict, sidecar_url: Optional[str]) -> dict:
    ...
    if (sidecar_url is None and not any(os.environ.get(k) for k in ('PROTAGINE_SIDECAR_URL', 'PROTAGINE_SIDECAR_PORT'))
            and "PROTAGINE_URL" in env):
        selected_env["PROTAGINE_URL"] = env["PROTAGINE_URL"]
    env.update(selected_env)
    result = {**existing, **desired, "env": env}
    if (existing.get("command") not in {None, 'protagine'}
            and not (os.environ.get("PROTAGINE_MCP_COMMAND"))):
...
def _update_servers(servers: dict, desired: dict, sidecar_url: Optional[str]) -> bool:
    ...
    if servers.get("protagine") == new:
        return False
    servers["protagine"] = new
    return True
...
def _add_to_json_config(hdef: dict, contact_id: str, source: str,
    ...
    return "  Configure one Protagine MCP entry; preserve other servers and credentials"
...
def _without_toml_entries(content: str) -> str:
    ...
    for line in content.splitlines(keepends=True):
        ...
        if stripped.startswith("["):
            match = re.match(r"^\[\s*mcp_servers\s*\.\s*protagine(?:\s*\.[^]]*)?\s*\]", stripped)
            skipping = bool(match)
        if not skipping:
            output.append(line)
    return "".join(output)
...
def _add_to_toml_config(config_path: str, contact_id: str, source: str,
    ...
    return "  Configure one Protagine MCP entry; preserve other TOML tables and credentials"
...
def _add_to_yaml_config(config_path: str, contact_id: str, source: str,
    ...
    return "  Configure one Protagine MCP entry; preserve other settings and credentials"


def add_to_harness(harness_id: str, contact_id: str, dry_run: bool = False,
                   sidecar_url: Optional[str] = None) -> Optional[str]:
    """Configure a single Protagine MCP server without mutating process settings."""
...
def remove_from_harness(harness_id: str, dry_run: bool = False) -> Optional[str]:
    ...
    if not any(name in servers for name in ('protagine',)):
        return None
    for name in ('protagine',):
    ...
    if not dry_run:
        if fmt == "toml":
            ...
            _write_json(path, data)
    return f"  Removed Protagine MCP integration from {hdef['display']} config"
