"""Protagine MCP Server implementation.

Thin adapter that translates MCP tool calls into Protagine sidecar HTTP requests.
...
def _require_contact(override: str | None = None) -> tuple[str, dict[str, str]]:
    ...
    return "", {
        ...
        "message": "No contact_id provided and PROTAGINE_MCP_CONTACT_ID is not set",
        "suggestion": "Set PROTAGINE_MCP_CONTACT_ID in your MCP config or pass contact_id explicitly",
    }
...
def _sidecar_error(exc: Exception) -> dict[str, str]:
    ...
    return {
        ...
        "message": f"Protagine sidecar not reachable at {_base_url()}",
        "suggestion": "Start with: protagine start",
    }
...
async def _post(path: str, data: dict) -> dict | None:
    try:
        # Inject provenance from PROTAGINE_MCP_SOURCE into metadata so it
...
def create_server() -> FastMCP:
    """Create the Protagine MCP server with all tools registered."""
    ...
    async def protagine_health() -> dict:
        """Check if Protagine sidecar is running and healthy. Call at session start or when other tools fail."""
        return await _get("/v1/host/health")

    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    async def protagine_get_context(
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    async def protagine_check_commitments(
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    async def protagine_lookup_facts(
        contact_id: str | None = None,
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    async def protagine_check_affect(
        contact_id: str | None = None,
    ) -> dict:
    ...
    async def protagine_create_commitment(
        description: str,
        person_id: str | None = None,
    ...
    async def protagine_fulfill_commitment(
        commitment_id: str,
        note: str | None = None,
    ) -> dict:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": True})
    async def protagine_cancel_commitment(
        commitment_id: str,
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": False})
    async def protagine_remember_fact(
        fact: str,
        contact_id: str | None = None,
        source: str | None = None,
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": True})
    async def protagine_forget_sources(source_ids: list[str], contact_id: str | None = None) -> dict:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": True})
    async def protagine_forget_fact(
        fact_id: str,
    ) -> dict:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": False})
    async def protagine_record_affect(
        valence: float,
        trigger: str,
        contact_id: str | None = None,
        arousal: float = 0.5,
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": False})
    async def protagine_record_surprise(
        observation: str,
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": True})
    async def protagine_task_complete(
        task_id: str,
    ) -> dict:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": True})
    async def protagine_task_snooze(
        task_id: str,
        hours: int = 24,
        reason: str = "",
    ) -> dict:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": True})
    async def protagine_task_dismiss(
        task_id: str,
        reason: str = "stale",
    ) -> dict:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": True})
    async def protagine_initiative_feedback(
        initiative_id: str,
        action: str,
        details: dict[str, Any] | None = None,
    ) -> dict:
        """Provide feedback on how an initiative was handled. Action can be: acknowledged, actioned, dismissed, snoozed. Call after handling a protagine_initiative."""
    ...
    @mcp.resource("protagine://status")
    async def status_resource() -> dict:
        """Current Protagine system status."""
        return await _get("/v1/host/health")

    @mcp.resource("protagine://commitments")
    ...
    @mcp.resource("protagine://affect/{contact_id}")
    ...
    @mcp.resource("protagine://facts/{contact_id}")
    ...
    async def daily_briefing() -> str:
        ...
        cid = _contact_id()
        if not cid:
            return "Set PROTAGINE_MCP_CONTACT_ID to get your daily briefing."
        return (
            f"Review the following for {cid}:\n"
            "1. Check protagine_check_commitments for pending and overdue items\n"
            "2. Check protagine_get_context for relevant source-backed context\n"
            "3. Check protagine_get_patterns for workflow patterns\n"
            "4. Check protagine://surprises/unresolved for anything unexpected\n"
    ...
    async def pre_task() -> str:
        ...
        cid = _contact_id()
        if not cid:
            return "Set PROTAGINE_MCP_CONTACT_ID to use pre-task context."
        return (
            f"Before starting this task:\n"
            f"1. Call protagine_check_commitments to see what {cid} has pending\n"
            f"2. Call protagine_lookup_facts to recall relevant context\n"
            f"3. Call protagine_get_context for source-backed appraisals and working perspective\n"
    ...
    async def post_task() -> str:
        ...
        cid = _contact_id()
        if not cid:
            return "Set PROTAGINE_MCP_CONTACT_ID to use post-task recording."
        return (
            f"I just completed a task. For {cid}:\n"
            "1. If there was a commitment for this task, call protagine_fulfill_commitment\n"
            "2. If anything unexpected happened, call protagine_record_surprise\n"
            "3. If I learned something worth remembering, call protagine_remember_fact with its source.\n"
