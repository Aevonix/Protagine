...
Resolution ladder (first hit wins):
  ...
  2. scoped display-name: propose a candidate in the existing audit trail;
     never attribute a sender from a name or install a guessed handle
...
class ParticipantResolver:
    ...
    async def resolve(self, *, platform: str, user_id: str,
        ...
        # 2. A display name may suggest a link but cannot establish identity.
        filed = False
        ...
        if not shadow_contacts_enabled():
            return Resolution(None, "none", proposal_filed=filed)
        try:
            ...
            logger.info("Shadow contact %s created for %s:%s (%r)",
                        contact.contact_id, platform, user_id,
                        display_name or "?")
            return Resolution(contact.contact_id, "shadow", created=True, proposal_filed=filed)
        except Exception:
    ...
    async def _file_handle_proposal(self, contact_id: str, platform: str,
        ...
        """Keep a suggestion in the existing audit, outside usable handles."""
        try:
