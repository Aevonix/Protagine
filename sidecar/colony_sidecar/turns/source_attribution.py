...
def correct(ledger, *, operation_id, performed_by, old_contact_id, contact_id,
    ...
    """
    from colony_sidecar.contacts.identity_links import _refs, _json
    ...
    with closing(ledger._connect()) as conn, conn:
        ...
        from colony_sidecar.beliefs.source_projection import erase_removed as erase_claims, enqueue
        ...
        for sid in descendants:
            ...
            if row:
                ...
                erase_judgments(conn, sid, row['session_id'], [])
        try:
            from colony_sidecar.self_model.appraisals import invalidate_source_attribution
