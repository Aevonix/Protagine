...
* **Passive.** Recording is fed from the turns/sync attribution chokepoint
  ...
  changes nothing about turn processing. ``PACOMIND_CONV_PRESENCE`` (default
...
def conv_presence_enabled() -> bool:
    """PACOMIND_CONV_PRESENCE (default on): passive presence recording."""
    return os.environ.get("PACOMIND_CONV_PRESENCE", "on").strip().lower() not in (
        "off", "0", "false", "no")
...
class ConversationPresenceStore:
    ...
    def record(self, conversation_key: str, contact_id: str, *,
        ...
        must not care); the PACOMIND_CONV_PRESENCE gate turns the whole write
