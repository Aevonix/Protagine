"""PacoMind Identity Bootstrap — IdentityBootstrapBuilder.

Assembles a SelfKnowledgeCorpus at runtime by reading pacomind_id, version,
...
from pacomind.identity_bootstrap.corpus import (
    CORPUS_VERSION,
    GATE_LAYERS,
    INFERENCE_TIERS,
    API_ENDPOINTS,
...
def _pacomind_version() -> str:
    """Return PacoMind version from canonical source."""
    # Canonical: pacomind_cli.__init__ (full_version_string returns "0.3.0-h6")
    try:
        from pacomind_cli import full_version_string
    ...
    try:
        from importlib.metadata import version
        return version("pacomind-ai")
    except Exception:
        pass
    return "0.3.0-h6"


def _read_pacomind_id(chain_manager: Optional[Any]) -> str:
    """Read pacomind_id from ChainManager, chain.db, or environment.

    Priority order:
    1. ChainManager.pacomind_id (if ChainManager is initialized)
    2. PACOMIND_ID environment variable
    3. Genesis block genesis_pacomind_id from chain.db (crypto identity)
    4. Hostname-based fallback (last resort)
    """
    if chain_manager is not None:
        try:
            cid = chain_manager.pacomind_id
            if cid and cid != "local":
                return str(cid)
        except Exception as exc:
            logger.debug("ChainManager.pacomind_id unavailable: %s", exc)

    env_id = os.environ.get("PACOMIND_ID", "")
    if env_id:
        return env_id

    # Read genesis_pacomind_id directly from chain.db — this is the
    ...
    chain_db = os.path.join(pacomind_home, "chain.db")
    if os.path.exists(chain_db):
        try:
            ...
            if row:
                ...
                cid = genesis_data.get("metadata", {}).get("genesis_pacomind_id")
                if cid:
                    return cid
        except Exception as exc:
            logger.debug("_read_pacomind_id chain.db fallback failed: %s", exc)

    # Last resort: hostname-based stable ID
    import socket
    import hashlib
    raw = f"{socket.gethostname()}-pacomind"
...
def _read_network_id(chain_manager: Optional[Any]) -> str:
    try:
        ...
        pass
    return os.environ.get("PACOMIND_NETWORK_ID", "local")
...
def _resolve_inference_tiers() -> list[InferenceTier]:
    ...
    reads them from ``pacomind.router.tiers.DEFAULT_TIERS`` at runtime so the
    ...
    try:
        from pacomind.router.tiers import DEFAULT_TIERS, ModelTier  # type: ignore[import]
...
class IdentityBootstrapBuilder:
    """Builds a SelfKnowledgeCorpus by introspecting the running PacoMind instance."""

    def __init__(self, chain_manager: Optional[Any] = None) -> None:
        self._chain_manager = chain_manager

    def build(self) -> SelfKnowledgeCorpus:
        pacomind_id = _read_pacomind_id(self._chain_manager)
        pacomind_name = os.environ.get("PACOMIND_NAME", f"pacomind-{pacomind_id[:8]}")
        pacomind_version = _pacomind_version()
        ...
        corpus = SelfKnowledgeCorpus(
            ...
            pacomind_name=pacomind_name,
            pacomind_version=pacomind_version,
