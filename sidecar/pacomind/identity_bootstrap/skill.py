...
Registered in the PacoMind skill registry as 'bootstrap-self-check'.
...
async def run(pacomind_id: Optional[str] = None, mode: str = "full") -> dict:
    """Run the PacoMind Identity Bootstrap self-check.

    Args:
        pacomind_id: Override pacomind_id (default: read from running instance).
    ...
    """
    from pacomind.identity_bootstrap.runner import IdentityBootstrap

    bootstrap = IdentityBootstrap()
    report = await bootstrap.verify_only()

    return {
        "pacomind_id": report.pacomind_id,
        "mode": report.mode,
