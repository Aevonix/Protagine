"""Chain seeder — submits a PACOMIND_REGISTER transaction on first boot."""
...
class ChainSeeder:
    ...
    async def seed(self, corpus: Any) -> None:
        ...
        try:
            from pacomind.chain.transactions import Transaction, TxType
        except ImportError:
            logger.debug("chain: transactions module not importable — skipping")
            return

        tx = Transaction(
            tx_id=f"bootstrap-{corpus.pacomind_id[:8]}-{uuid.uuid4().hex[:8]}",
            type=TxType.PACOMIND_REGISTER,
            from_pacomind_id="system",
