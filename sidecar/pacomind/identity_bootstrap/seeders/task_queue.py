...
class TaskQueueSeeder:
    ...
    async def seed(self, corpus: Any) -> None:
        ...
        caps = WorkerCapabilities(
            node_id=f"bootstrap-{pacomind_id[:8]}",
        ...
        job_id = f"job-bootstrap-selfcheck-{pacomind_id[:8]}"
        job = Job(
            job_id=job_id,
            ...
            posted_by=f"bootstrap-{pacomind_id[:8]}",
            tags={"type": "bootstrap", "pacomind_id": pacomind_id[:8]},
        )
        try:
