...
class RelationshipSeeder:
    ...
    async def _seed_sqlite(self, store: Any, contact_id: str, corpus: Any, now: str) -> None:
        ...
        pacomind_id = corpus.pacomind_id
        tags_json = json.dumps(["self", "system", "pacomind"])
        ...
        # Add internal handle
        hdl_id = f"hdl-self-{pacomind_id[:8]}"
        try:
            await db.execute(
                """
                ...
                """,
                (hdl_id, contact_id, "custom", f"pacomind://{pacomind_id}", 1, 1, 1.0, "identity_bootstrap", now),
            )
            await db.commit()
        except Exception as exc:
    ...
    def _seed_memory(self, contacts_mod: Any, contact_id: str, corpus: Any, now: str) -> None:
        ...
        in_mem[contact_id] = {
            ...
            "display_name": corpus.pacomind_name,
            "given_name": "PacoMind",
            "family_name": "Self",
            "organization": "PacoMind AI",
            "relationship_score": 1.0,
            "trust_tier": "inner_circle",
            "interaction_allowed": True,
            "tags": ["self", "system", "pacomind"],
            "privacy_level": "private",
            "person_node_id": f"pacomind-self-{corpus.pacomind_id}",
