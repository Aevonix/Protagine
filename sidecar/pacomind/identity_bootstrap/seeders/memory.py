...
Uses PacoMindGraph.store_memory() which persists to Neo4j and indexes in
...
class MemorySeeder:
    ...
    def __init__(self, pacomind_graph: Optional[Any] = None) -> None:
    ...
    async def seed(self, corpus: Any) -> None:
        ...
        memories = [
            {
                "id": f"mem-bootstrap-identity-{pacomind_id[:8]}",
                "type": "episodic",
                "content": (
                    f"I am {corpus.pacomind_name}, a PacoMind AI agent instance with ID {pacomind_id}. "
                    f"I run PacoMind v{corpus.pacomind_version} on network {corpus.network_id}. "
                    "I was bootstrapped with full self-knowledge at first boot."
                ),
                "tags": ["self", "identity", "bootstrap", "core"],
                "source": "identity_bootstrap",
                "entities": [corpus.pacomind_name, "PacoMind"],
                "metadata": {
                    "bootstrap_id": f"mem-bootstrap-identity-{pacomind_id[:8]}",
            ...
            {
                "id": f"mem-bootstrap-architecture-{pacomind_id[:8]}",
                "type": "semantic",
                "content": (
                    "PacoMind has 10 architectural layers: Gateway, API, Intelligence, Memory, "
                ...
                "source": "identity_bootstrap",
                "entities": ["PacoMind"] + [l.name for l in corpus.layers],
                "metadata": {
                    "bootstrap_id": f"mem-bootstrap-architecture-{pacomind_id[:8]}",
            ...
            {
                "id": f"mem-bootstrap-safety-{pacomind_id[:8]}",
                "type": "semantic",
                "content": (
                ...
                "source": "identity_bootstrap",
                "entities": ["ResponseGate", "PacoMind"],
                "metadata": {
                    "bootstrap_id": f"mem-bootstrap-safety-{pacomind_id[:8]}",
            ...
            {
                "id": f"mem-bootstrap-cognition-{pacomind_id[:8]}",
                "type": "semantic",
                "content": (
                    "PacoMind's cognition pipeline runs 8 phases: MetaLearning, StrategyAdjustment, "
                ...
                "source": "identity_bootstrap",
                "entities": ["PacoMind", "CognitionPipeline"],
                "metadata": {
                    "bootstrap_id": f"mem-bootstrap-cognition-{pacomind_id[:8]}",
        ...
        if self._graph is not None:
            ...
            logger.warning(
                "memory: no pacomind_graph available — falling back to ephemeral API dict"
            )
            try:
