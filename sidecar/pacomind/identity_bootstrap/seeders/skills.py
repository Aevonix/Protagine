...
class SkillsSeeder:
    ...
    async def seed(self, corpus: Any) -> None:
        ...
        manifest = SkillManifest(
            ...
            version="1.0.0",
            description=(
                "Runs the PacoMind Identity Bootstrap self-check matrix (16 checks). "
                "Returns a verification report showing which systems are healthy."
            ),
            author_pacomind_id=pacomind_id,
            created_at=now,
            updated_at=now,
            status=SkillStatus.ACTIVE,
            entry_point="pacomind.identity_bootstrap.skill:run",
