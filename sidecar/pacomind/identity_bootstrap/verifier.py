"""PacoMind Identity Bootstrap — 16-point self-check matrix.
...
from pacomind.identity_bootstrap.models import BootstrapAnomaly

logger = logging.getLogger(__name__)


class BootstrapVerifier:
    """Runs 16 self-checks against live PacoMind subsystems."""

    def __init__(self, corpus: Any, pacomind_graph: Optional[Any] = None) -> None:
        self._corpus = corpus
    ...
    async def run_all(self) -> List[BootstrapAnomaly]:
        ...
        checks = [
            ...
            self._check_pacomind_id_consistent,
        ]
    ...
    async def _check_wm_self_entity(self) -> Optional[BootstrapAnomaly]:
        try:
            ...
            entity = await backend.get_entity(f"pacomind-self-{self._corpus.pacomind_id}")
            if entity is None:
                return BootstrapAnomaly(
                    system="world_model",
                    check="self_entity_exists",
                    expected=f"pacomind-self-{self._corpus.pacomind_id}",
                    actual="not found",
                    severity="CRITICAL",
                )
        except Exception as exc:
    ...
    async def _check_wm_subsystem_count(self) -> Optional[BootstrapAnomaly]:
        try:
            ...
            if backend is None:
                return None
            pacomind_id = self._corpus.pacomind_id
            found = 0
            for sub in SUBSYSTEMS:
                ent = await backend.get_entity(f"pacomind-subsystem-{pacomind_id}-{sub}")
                if ent is not None:
                    found += 1
    ...
    async def _check_wm_part_of_relationships(self) -> Optional[BootstrapAnomaly]:
        try:
            ...
            self_id = f"pacomind-self-{self._corpus.pacomind_id}"
    ...
    async def _check_contacts_self_entry(self) -> Optional[BootstrapAnomaly]:
        try:
            import pacomind.api.routers.contacts as contacts_mod
            store = getattr(contacts_mod, "_contact_store", None)
            contact_id = f"self:{self._corpus.pacomind_id}"
    ...
    async def _check_memory_identity(self) -> Optional[BootstrapAnomaly]:
        mem_id = f"mem-bootstrap-identity-{self._corpus.pacomind_id[:8]}"
    ...
    async def _check_memory_architecture(self) -> Optional[BootstrapAnomaly]:
        mem_id = f"mem-bootstrap-architecture-{self._corpus.pacomind_id[:8]}"
        found = await self._check_memory_in_graph(mem_id)
        if not found:
            return BootstrapAnomaly(
                system="memory",
    ...
    async def _check_memory_safety(self) -> Optional[BootstrapAnomaly]:
        mem_id = f"mem-bootstrap-safety-{self._corpus.pacomind_id[:8]}"
        found = await self._check_memory_in_graph(mem_id)
        if not found:
            return BootstrapAnomaly(
                system="memory",
    ...
    async def _check_memory_cognition(self) -> Optional[BootstrapAnomaly]:
        mem_id = f"mem-bootstrap-cognition-{self._corpus.pacomind_id[:8]}"
        found = await self._check_memory_in_graph(mem_id)
        if not found:
            return BootstrapAnomaly(
                system="memory",
    ...
    async def _check_memory_in_graph(self, bootstrap_id: str) -> bool:
        ...
        try:
            import pacomind.api.server as server_mod
            # Prefer the graph passed directly to the verifier, then server module
            graph = self._graph or getattr(server_mod, "_pacomind_graph", None)
        ...
        try:
            import pacomind.api.routers.memory as memory_mod
    ...
    async def _check_goals_bootstrap_present(self) -> Optional[BootstrapAnomaly]:
        try:
            ...
            goal_id = f"goal-bootstrap-selfknowledge-{self._corpus.pacomind_id[:8]}"
    ...
    async def _check_briefing_present(self) -> Optional[BootstrapAnomaly]:
        try:
            ...
            briefing_id = f"briefing-bootstrap-{self._corpus.pacomind_id[:8]}"
    ...
    async def _check_session_present(self) -> Optional[BootstrapAnomaly]:
        try:
            import pacomind.api.routers.sessions as sessions_mod
            session_id = f"sess-bootstrap-{self._corpus.pacomind_id[:8]}"
    ...
    # ── Check 16: pacomind_id consistency ───────────────────────────────────────

    async def _check_pacomind_id_consistent(self) -> Optional[BootstrapAnomaly]:
        pacomind_id = self._corpus.pacomind_id
        if not pacomind_id or pacomind_id == "unknown":
            return BootstrapAnomaly(
                system="corpus",
                check="pacomind_id_set",
                expected="non-empty pacomind_id",
                actual=repr(pacomind_id),
                severity="CRITICAL",
            )
        return None
