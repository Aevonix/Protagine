"""PacoMind Identity Bootstrap — SelfKnowledgeCorpus.

Static catalog of PacoMind's own architecture, layers, endpoints, and phases.
...
@dataclass
class LayerRecord:
    """A single architectural layer in PacoMind."""
    name: str
    description: str
...
class CognitionPhase:
    """A phase in the PacoMind cognition / meta-learning pipeline."""
    name: str
    description: str
...
class GateLayerRecord:
    """A single layer in the PacoMind ResponseGate safety pipeline."""
    layer_id: str
    name: str
    description: str
    layer_index: int = 0
...
@dataclass
class InferenceTier:
    """A tier in the PacoMind smart model routing system."""
    name: str
    description: str
...
class ArchitectureManifest:
    """Top-level container describing PacoMind's own architecture."""
    pacomind_id: str
    pacomind_name: str
    pacomind_version: str
    network_id: str
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "pacomind_name": self.pacomind_name,
            "pacomind_version": self.pacomind_version,
...
LAYERS: List[LayerRecord] = [
    ...
    LayerRecord(
        name="API",
        description="FastAPI REST + WebSocket server exposing all PacoMind capabilities. Handles auth, routing, audit logging.",
        subsystems=["pacomind.api.server", "pacomind.api.routers", "pacomind.api.middleware", "pacomind.api.auth"],
        layer_index=1,
    ),
    LayerRecord(
        name="Intelligence",
        ...
        subsystems=[
            "pacomind.intelligence.cognition",
            "pacomind.intelligence.mind_model",
            "pacomind.intelligence.synthesis",
            "pacomind.intelligence.relationships",
            "pacomind.intelligence.components",
        ],
        layer_index=2,
    ),
    LayerRecord(
        ...
        subsystems=["pacomind.world_model", "pacomind.contacts", "pacomind.api.routers.memory", "pacomind.intelligence.graph"],
        layer_index=3,
    ),
    LayerRecord(
        name="Goals",
        description="DAG-based goal decomposition, lifecycle management, priority queue, replan engine, autonomy loop.",
        subsystems=["pacomind.goals", "pacomind.autonomy"],
        layer_index=4,
    ),
    LayerRecord(
        name="Skills",
        description="Self-creating skill system: registry, executor, marketplace, security scanner, progressive loading, MCP bridge.",
        subsystems=["pacomind.skills", "pacomind.skill_security", "pacomind.plugins"],
        layer_index=5,
    ),
    LayerRecord(
        name="TaskQueue",
        description="Distributed hardware-aware task queue: job scheduler, worker nodes, handler registry, monitoring.",
        subsystems=["pacomind.task_queue"],
        layer_index=6,
    ),
    LayerRecord(
        name="Federation",
        description="Multi-agent mesh networking: pacomind chain, P2P protocol, ledger sync, consensus, identity federation.",
        subsystems=["pacomind.chain", "pacomind.mesh", "pacomind.network", "pacomind.federation"],
        layer_index=7,
    ),
    LayerRecord(
        name="Safety",
        description="ResponseGate pipeline (7 layers), PII scrubbing, injection detection, trust-tier gating, rate limiting.",
        subsystems=["pacomind.gate", "pacomind.gate.layers"],
        layer_index=8,
    ),
    LayerRecord(
        name="Inference",
        description="On-device inference, model routing, TurboQuant quantization backends (CPU/CUDA/MLX), context compression.",
        subsystems=["pacomind.inference", "pacomind.router", "agent.context_compressor"],
        layer_index=9,
    ),
]
...
API_ENDPOINTS: List[EndpointRecord] = [
    # ── Admin (/v1/admin) ────────────────────────────────────────────────────
    EndpointRecord("/v1/admin/register", "POST", "Register a new pacomind node", "admin", tags=["admin"]),
    EndpointRecord("/v1/admin/attest", "POST", "Attest trust to a target pacomind (genesis admin only)", "admin", tags=["admin"]),
    EndpointRecord("/v1/admin/revoke", "POST", "Revoke trust from a target pacomind", "admin", tags=["admin"]),
    EndpointRecord("/v1/admin/network", "GET", "List all registered pacomind nodes", "admin", tags=["admin"]),
    EndpointRecord("/v1/admin/suspend", "POST", "Suspend a registered pacomind node", "admin", tags=["admin"]),
    EndpointRecord("/v1/admin/reinstate", "POST", "Reinstate a suspended pacomind node", "admin", tags=["admin"]),
    ...
    EndpointRecord("/v1/chain/register", "POST", "Register a pacomind on-chain", "chain", tags=["chain"]),
