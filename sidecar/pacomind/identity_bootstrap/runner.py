"""PacoMind Identity Bootstrap — IdentityBootstrap orchestrator.
...
BootstrapReport to ~/.pacomind/bootstrap.db.
"""
...
from pacomind.identity_bootstrap.builder import IdentityBootstrapBuilder
from pacomind.identity_bootstrap.models import BootstrapAnomaly, BootstrapReport
from pacomind.identity_bootstrap.verifier import BootstrapVerifier
from pacomind.identity_bootstrap.self_reflection import SelfReflectionComponent
...
CREATE INDEX IF NOT EXISTS idx_bootstrap_pacomind ON bootstrap_runs(pacomind_id, completed_at DESC);
"""
...
class _BootstrapDB:
    ...
    def save_report(self, report: BootstrapReport, pacomind_version: str = "") -> None:
        import uuid
        run_id = str(uuid.uuid4())
        report_dict = report.to_dict()
        if pacomind_version:
            report_dict["pacomind_version"] = pacomind_version
        with self._lock:
            self._conn.execute(
                """
                INSERT OR REPLACE INTO bootstrap_runs
                    (run_id, pacomind_id, mode, started_at, completed_at, success,
    ...
    def last_pacomind_version(self, pacomind_id: str) -> Optional[str]:
        """Return the pacomind_version stored in the most recent bootstrap run."""
        last = self.last_run(pacomind_id)
        if last is None:
            return None
        try:
            data = json.loads(last.get("report_json", "{}"))
            return data.get("pacomind_version") or None
        except Exception:
            return None

    def last_run(self, pacomind_id: str) -> Optional[dict]:
        with self._lock:
            row = self._conn.execute(
                "SELECT * FROM bootstrap_runs WHERE pacomind_id = ? ORDER BY completed_at DESC LIMIT 1",
                (pacomind_id,),
            ).fetchone()
        if row is None:
            return None
...
class IdentityBootstrap:
    """Orchestrates the PacoMind Identity Bootstrap process."""

    def __init__(
        self,
        pacomind_graph: Optional[Any] = None,
        chain_manager: Optional[Any] = None,
        queue_manager: Optional[Any] = None,
        skill_registry: Optional[Any] = None,
        metrics_collector: Optional[Any] = None,
    ) -> None:
        self._graph = pacomind_graph
        self._chain_manager = chain_manager
        self._queue_manager = queue_manager
        self._skill_registry = skill_registry
        self._metrics = metrics_collector

        pacomind_home = os.environ.get(
            "PACOMIND_HOME",
            os.path.join(os.path.expanduser("~"), ".pacomind"),
        )
        db_path = os.path.join(pacomind_home, "bootstrap.db")
        try:
    ...
    async def _determine_mode(self, corpus) -> str:
        ...
        if self._db is not None:
            try:
                last = self._db.last_run(pacomind_id)
                if last is not None:
                    # Fix 6: version guard — only run full bootstrap if pacomind_version changed
                    last_pacomind_ver = self._db.last_pacomind_version(pacomind_id)
                    if last_pacomind_ver and last_pacomind_ver == corpus.pacomind_version:
                        logger.debug(
                            "_determine_mode: pacomind_version=%s unchanged → VERIFY",
                            corpus.pacomind_version,
                        )
                        return "VERIFY"
        ...
        try:
            ...
            if backend is not None:
                entity = await backend.get_entity(f"pacomind-self-{pacomind_id}")
    ...
    async def _run_seeders(self, corpus) -> tuple[List[str], List[str]]:
        ...
        group1 = [
            WorldModelSeeder(),
            RelationshipSeeder(),
            MemorySeeder(pacomind_graph=self._graph),
    ...
    async def run(self) -> BootstrapReport:
        ...
        logger.info(
            "IdentityBootstrap starting (mode=%s, pacomind=%s, version=%s)",
            mode,
            corpus.pacomind_id,
            corpus.pacomind_version,
        )
        ...
        # Always run verifier
        verifier = BootstrapVerifier(corpus, pacomind_graph=self._graph)
        ...
        if self._db is not None and mode != "VERIFY":
            try:
                self._db.save_report(report, pacomind_version=corpus.pacomind_version)
            except Exception as exc:
