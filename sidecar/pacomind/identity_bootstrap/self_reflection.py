"""PacoMind Identity Bootstrap — SelfReflectionComponent.
...
class SelfReflectionComponent:
    ...
    async def reflect(
        ...
        notes = self._build_notes(pacomind_id, corpus, critical, warnings, health_score)
        ...
        if self._metrics is not None:
            try:
                await self._metrics.record(
                    ...
                    value=health_score,
                    domain="identity",
                    context={"pacomind_id": pacomind_id},
                )
            except Exception as exc:
                logger.debug("self_reflection: metrics record failed: %s", exc)

        logger.info(
            "self_reflection: pacomind=%s health=%.2f critical=%d warnings=%d",
            pacomind_id,
            health_score,
            len(critical),
            len(warnings),
    ...
    def _generate_proposals(self, critical: List[Any], warnings: List[Any]) -> List[str]:
        ...
        if "corpus" in systems_with_issues:
            proposals.append(
                "Update the identity_bootstrap corpus to match the current PacoMind version."
            )
        if any(a.check == "pacomind_id_set" for a in critical):
            proposals.append(
                "Set PACOMIND_ID env var or ensure ChainManager is initialized before bootstrap."
            )
    ...
    def _build_notes(
        ...
        lines = [
            f"PacoMind: {corpus.pacomind_name} ({pacomind_id})",
            f"Version: {corpus.pacomind_version}",
