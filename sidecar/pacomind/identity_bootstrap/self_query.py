"""PacoMind Identity Bootstrap — self-referential query detection.

Detects user messages that ask about PacoMind's own capabilities,
...
# Each pattern targets a class of question about PacoMind's own capabilities.
_SELF_PATTERNS = [
    ...
    re.compile(r"\byour\b.*\b(?:version|pacomind.?id|identity)\b", re.I),
...
def query_is_self_referential(message: str) -> bool:
    """Return True if *message* is asking about PacoMind's own capabilities."""
...
def build_self_context_from_corpus() -> Optional[str]:
    ...
    lines = [
        "# PacoMind Self-Knowledge (auto-injected)",
        "",
        f"**Identity:** {corpus.pacomind_name} (ID: {corpus.pacomind_id}, "
        f"v{corpus.pacomind_version}, network: {corpus.network_id})",
        "",
