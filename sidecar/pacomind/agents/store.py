"""Agent and Invite stores for multi-agent PacoMind.
...
# Type alias for PacoMind key manager (avoid circular import)
LocalKeyManager = Any


def get_state_dir() -> Path:
    """Get PacoMind state directory."""
...
def generate_setup_code() -> str:
    """Generate a random setup code: PACOMIND-XXXX-XXXX-XXXX."""
...
class AgentStore:
    ...
    def __init__(
        ...
        self._pacomind_km = pacomind_key_manager
    ...
    def _create_tables(self, conn: sqlite3.Connection) -> None:
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_agents_pacomind ON agents(pacomind_id)")
    ...
    def create(self, data: Dict[str, Any]) -> Agent:
        ...
        cursor = self._db.execute(
            """
            INSERT INTO agents (
                agent_id, node_id, pacomind_id, name,
    ...
    def list(
        ...
        if pacomind_id:
            query += " AND pacomind_id = ?"
            params.append(pacomind_id)
    ...
    def mark_all_offline(self) -> int:
        """Mark all agents as offline (called on PacoMind restart)."""
        cursor = self._db.execute(
    ...
    async def sign_node_certificate(
        ...
        if not self._pacomind_km:
            raise ValueError("PacoMind key not available for signing")

        # Import here to avoid circular dependency
        from pacomind.chain.identity import get_or_create_pacomind_id

        pacomind_id = get_or_create_pacomind_id(self._state_dir)
        ...
        signature = self._pacomind_km.sign(payload)
