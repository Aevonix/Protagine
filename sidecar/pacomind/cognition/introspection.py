...
Disabled by default. Activated with ``PACOMIND_INTROSPECT_ENABLED=true`` and a configured
endpoint; deployment-agnostic (point ``PACOMIND_INTROSPECT_BASE_URL`` at a local model):

  PACOMIND_INTROSPECT_ENABLED     "true"|"false"      (default false)
  PACOMIND_INTROSPECT_BASE_URL    OpenAI-compatible base, e.g. http://127.0.0.1:8000/v1
  PACOMIND_INTROSPECT_MODEL       model id the endpoint serves (required to run)
  PACOMIND_INTROSPECT_API_KEY     bearer token, blank for a local model
  PACOMIND_INTROSPECT_TIMEOUT     seconds (default 30)
  PACOMIND_INTROSPECT_MAX_TOKENS  response cap (default 400)
...
def introspect_enabled() -> bool:
    ...
    return resolve_bool("PACOMIND_INTROSPECT_ENABLED", False)


def _config() -> Dict[str, Any]:
    return {
        "base_url": os.environ.get("PACOMIND_INTROSPECT_BASE_URL", "http://127.0.0.1:8000/v1").rstrip("/"),
        "model": os.environ.get("PACOMIND_INTROSPECT_MODEL", "").strip(),
        "api_key": os.environ.get("PACOMIND_INTROSPECT_API_KEY", "").strip(),
        "timeout": float(os.environ.get("PACOMIND_INTROSPECT_TIMEOUT", "30")),
        "max_tokens": int(os.environ.get("PACOMIND_INTROSPECT_MAX_TOKENS", "400")),
    }
...
async def run_turn_introspection(
    *,
    ...
    from pacomind.commitments.store import _normalize_desc, _similar_desc
