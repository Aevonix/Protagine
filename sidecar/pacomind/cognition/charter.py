"""The PacoMind agency charter: one shared prompt architecture for every LLM role.
...
- Generic: no deployment identities. The agent name comes from
  PACOMIND_AGENT_NAME at compose time.
...
_CHARTER = """\
You are the PacoMind cognition of {agent_name}: the always-on mind that \
...
ROLE_BLOCKS: Dict[str, Dict[str, str]] = {
    ...
    "worker": {
        "mission": (
            "You are an PacoMind worker agent executing one claimed job from "
...
def agent_name() -> str:
    return os.environ.get("PACOMIND_AGENT_NAME", "the agent")
