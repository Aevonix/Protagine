"""Durable append-only event journal for PacoMind.
...
try:  # PacoMind is deployed on Linux; retain a safe process-local fallback.
    import fcntl
...
def _state_dir() -> Path:
    return Path(os.environ.get("PACOMIND_STATE_DIR", ".")).resolve()


def _journal_dir() -> Path:
    configured = os.environ.get("PACOMIND_EVENT_JOURNAL_DIR", "").strip()
    return Path(configured).resolve() if configured else _state_dir() / "events"


def _retention() -> int:
    try:
        return max(1, int(os.environ.get("PACOMIND_EVENT_JOURNAL_RETENTION", "500")))
    except ValueError:
        return 500
