...
Paths/binaries are overridable via env for non-standard installs:
  PACOMIND_HERMES_BIN   hermes CLI (default: ``hermes`` on PATH, then the
                      conventional venv location)
  PACOMIND_HERMES_HOME  harness home (default ~/.hermes)
"""
...
def hermes_home() -> str:
    return os.path.expanduser(os.environ.get("PACOMIND_HERMES_HOME", "~/.hermes"))


def hermes_bin() -> str:
    env = os.environ.get("PACOMIND_HERMES_BIN")
    if env:
