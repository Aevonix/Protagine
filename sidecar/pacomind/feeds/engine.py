#!/usr/bin/env python3
"""PacoMind feed collector engine — self-contained, stdlib-only.

This file is BOTH a package module and a deployable script: the feeds manager
copies it verbatim into the harness scripts dir (as ``pacomind-feed-engine.py``)
where per-instance shims execute it under any python3.  Keep it stdlib-only.

Stages (selected via $PACOMIND_FEED_STAGE, default ``collect``):
...
Config file (path in $PACOMIND_FEED_CONFIG) is either the spec-rendered shape
...
UA = "PacoMind-Feed-Collector/1.0"


# --------------------------------------------------------------------- config
def load_config():
    path = os.environ.get("PACOMIND_FEED_CONFIG")
    if not path:
        raise SystemExit("PACOMIND_FEED_CONFIG not set")
...
def main():
    cfg = load_config()
    stage = os.environ.get("PACOMIND_FEED_STAGE", "collect")
    if stage == "collect":
        stage_collect(cfg)
    elif stage == "alert":
        stage_alert(cfg)
