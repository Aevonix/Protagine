...
def boundary_fail_closed() -> bool:
    ...
    PACOMIND_BOUNDARY_FAIL_CLOSED=false to restore the legacy allow-on-error
    behavior.
    """
    from pacomind.util.autonomy_preset import resolve_bool
    return resolve_bool("PACOMIND_BOUNDARY_FAIL_CLOSED", True)
...
def _common_terms() -> frozenset:
    ...
    own name, plus any deployment-supplied extras (PACOMIND_MATCH_COMMON_TERMS,
    csv). Live incident 2026-07-05: a fragment boundary whose only surviving
    term was "pacomind" matched every job that said "report to PacoMind"."""
    import os
    terms = {"pacomind"}
    agent = os.environ.get("PACOMIND_AGENT_NAME", "")
    for tok in re.findall(r"[a-z0-9]+", agent.lower()):
        if len(tok) > 1:
            terms.add(tok)
    extra = os.environ.get("PACOMIND_MATCH_COMMON_TERMS", "")
