...
Resolution ladder (first hit wins):
  ...
  3. shadow contact (tier unknown, interaction_allowed=false, provenance
     recorded), when PACOMIND_IDENTITY_SHADOW_CONTACTS (default true)
"""
...
def shadow_contacts_enabled() -> bool:
    return os.environ.get(
        "PACOMIND_IDENTITY_SHADOW_CONTACTS", "true").strip().lower() != "false"


def machine_channel_prefixes() -> tuple:
    raw = os.environ.get("PACOMIND_IDENTITY_MACHINE_CHANNELS", "cron,api,internal")
