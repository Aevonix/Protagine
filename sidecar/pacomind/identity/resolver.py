"""IdentityResolver — normalize person identifiers across PacoMind subsystems.
...
- The owner is resolved once from ``PACOMIND_OWNER_CONTACT_ID`` and cached.
- ``PACOMIND_HOST_CONTACT_ID`` is accepted as a deprecated alias.
...
from pacomind.channels.phone_gateways import get_phone_gateways as _get_phone_gateways
...
def get_owner_contact_id() -> Optional[str]:
    ...
    ``PACOMIND_OWNER_CONTACT_ID`` is canonical. ``PACOMIND_HOST_CONTACT_ID``
    ...
    """
    owner = os.environ.get("PACOMIND_OWNER_CONTACT_ID")
    if owner:
        return owner
    legacy = os.environ.get("PACOMIND_HOST_CONTACT_ID")
    if legacy:
        logger.warning(
            "PACOMIND_HOST_CONTACT_ID is deprecated, use PACOMIND_OWNER_CONTACT_ID"
        )
        return legacy
    return None
...
def get_owner_name(default: str = "the owner") -> str:
    ...
    Set ``PACOMIND_OWNER_NAME`` in the deployment env; otherwise a neutral term
    ("the owner") is used so the public code carries no personal identity.
    """
    return os.environ.get("PACOMIND_OWNER_NAME") or default


def get_persona_name(default: str = "the assistant") -> str:
    """Human-readable persona/agent name for tool descriptions and prompts.

    Same rule: the deployment sets ``PACOMIND_PERSONA_NAME``; the generic default
    is neutral so the public code names no specific product.
    """
    return os.environ.get("PACOMIND_PERSONA_NAME") or default
    ...
    async def owner_identities(self) -> frozenset:
        ...
        if not owner_id:
            raise OwnerIdentityError(
                "PACOMIND_OWNER_CONTACT_ID is not set. Owner-exclusion filters "
        ...
        if not forms:
            raise OwnerIdentityError(
                f"PACOMIND_OWNER_CONTACT_ID={owner_id!r} does not resolve to "
        ...
        if self._contact_store is not None:
            try:
                ...
                for handle in handles:
                    from pacomind.contacts.identity_links import usable_handle
...
def get_identity_resolver(contact_store: Any = None) -> IdentityResolver:
    ...
    if contact_store is None:
        try:
            from pacomind.api.routers import host as _host_mod
