...
from pacomind.skills_memory.models import Skill, signature_overlap
...
def skills_enabled() -> bool:
    return os.environ.get("PACOMIND_SKILLS_ENABLED", "true").strip().lower() != "false"


def skills_distill_mode() -> str:
    from pacomind.util.autonomy_preset import resolve
    return resolve("PACOMIND_SKILLS_DISTILL",
                   ("off", "shadow", "live"), "shadow")


def skills_max() -> int:
    try:
        return max(10, int(os.environ.get("PACOMIND_SKILLS_MAX", str(_DEFAULT_MAX))))
