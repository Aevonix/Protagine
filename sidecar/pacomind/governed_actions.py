"""Durable, generic execution boundary for owner-governed PacoMind actions.
...
def _validate_args(tool_name: str, raw: Any) -> dict[str, Any]:
    if tool_name in {"pacomind_autonomy_disable", "pacomind_autonomy_enable"}:
        return _exact_mapping(raw, set(), name="args")
    if tool_name == "pacomind_create_commitment":
        args = _exact_mapping(
            raw,
        ...
        return args
    if tool_name == "pacomind_initiative_feedback":
        ...
        return args
    if tool_name == "pacomind_record_insight":
        if not isinstance(raw, Mapping):
            raise GovernedActionValidationError("args must be an object")
        args = dict(raw)
        if (
        ...
        return args
    if tool_name == "pacomind_research":
        if not isinstance(raw, Mapping):
            raise GovernedActionValidationError("args must be an object")
        args = dict(raw)
        ...
        return args
    if tool_name == "pacomind_resolve_commitment":
        if not isinstance(raw, Mapping):
            raise GovernedActionValidationError("args must be an object")
        args = dict(raw)
        ...
        return args
    if tool_name == "pacomind_task_complete":
        ...
        return args
    if tool_name == "pacomind_task_dismiss":
        if not isinstance(raw, Mapping):
            raise GovernedActionValidationError("args must be an object")
        args = dict(raw)
        ...
        return args
    if tool_name == "pacomind_task_snooze":
        if not isinstance(raw, Mapping):
            raise GovernedActionValidationError("args must be an object")
        args = dict(raw)
        ...
        return args
    raise GovernedActionValidationError("tool is not a governed PacoMind action")
...
def parse_execution_request(
    ...
    *,
    ...
    if (
        document["schema"] != "PacoMindGovernedActionExecutionV1"
    ...
    if (
        approval["schema"] != "PacoMindOwnerApprovalExecutionBindingV1"
...
def _validated_effect(value: Any) -> dict[str, Any]:
    ...
    if (
        effect["schema"] != "PacoMindGovernedActionEffectV1"
...
def _state_result(
    ...
    *,
    ...
    result = {
        "schema": "PacoMindGovernedActionExecutionResultV1",
        "version": 1,
...
class PacoMindSubsystemActionExecutor:
    """Map the ten generic action names onto existing PacoMind subsystems."""
    ...
    def _governed_research_project(
        ...
        from pacomind.projects import Project
        from pacomind.work_orders import action_authority
    ...
    async def prepare(
        ...
        elif tool_name == "pacomind_initiative_feedback":
        ...
        elif tool_name == "pacomind_record_insight":
        ...
        elif tool_name == "pacomind_research":
    ...
    async def perform(
        ...
        if tool_name == "pacomind_autonomy_disable":
