"""Temporal reference frame for PacoMind (v0.21.0).
...
Three independent, editable timezones (per the design):
  * AGENT timezone   — where the PacoMind agent "lives" (its home reference frame).
                       Editable; defaults to PACOMIND_AGENT_TIMEZONE, then the host
                       system tz, then UTC.
...
def _state_dir() -> Path:
    return Path(os.environ.get("PACOMIND_STATE_DIR", os.path.expanduser("~/.pacomind")))
...
def agent_timezone() -> str:
    ...
    env = os.environ.get("PACOMIND_AGENT_TIMEZONE")
...
def default_contact_timezone() -> Optional[str]:
    ...
    env = os.environ.get("PACOMIND_DEFAULT_CONTACT_TIMEZONE")
    if is_valid_timezone(env):
        return env  # type: ignore[return-value]
