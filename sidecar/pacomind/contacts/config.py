"""PacoMind Contacts — configuration."""
...
class ContactsConfig:
    ...
    def from_env(cls) -> "ContactsConfig":
        ...
        server path resolves PACOMIND_CONTACTS_DB or falls back to
        ``$PACOMIND_STATE_DIR/pacomind-contacts.db``.
        """
        path = os.environ.get("PACOMIND_CONTACTS_DB")
        if not path:
            ...
            if not state_dir:
                state_dir = os.path.join(os.path.expanduser("~"),
                                         ".pacomind", "data")
            path = os.path.join(state_dir, "pacomind-contacts.db")
        auto_promote = os.environ.get("PACOMIND_AUTO_PROMOTE_GROUP_TO_1ON1", "").strip().lower() in (
            "1", "true", "yes", "on")
        try:
            min_int = int(os.environ.get("PACOMIND_GROUP_PROMOTE_MIN_INTERACTIONS", "") or 5)
        except ValueError:
            min_int = 5
