...
class VCardExporter:
    ...
    Args:
        include_tags:  If True, export PacoMind tags as X-PACOMIND-TAGS property.
        include_tier:  If True, export trust tier as X-PACOMIND-TIER property.
    """
    ...
    def export_one(self, contact: Contact, handles: List[ContactHandle]) -> str:
        ...
        if self._include_tags and contact.tags:
            lines.append(f"X-PACOMIND-TAGS:{','.join(contact.tags)}")

        if self._include_tier:
            lines.append(f"X-PACOMIND-TIER:{contact.trust_tier}")
