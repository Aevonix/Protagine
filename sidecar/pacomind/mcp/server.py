"""PacoMind MCP Server implementation.

Thin adapter that translates MCP tool calls into PacoMind sidecar HTTP requests.
...
def _base_url() -> str:
    return os.environ.get("PACOMIND_URL") or os.environ.get("PACOMIND_URL", "http://127.0.0.1:7777")


def _api_key() -> str:
    return os.environ.get("PACOMIND_API_KEY") or os.environ.get("PACOMIND_API_KEY", "")
...
def _source() -> str | None:
    return os.environ.get("PACOMIND_MCP_SOURCE") or os.environ.get("PACOMIND_MCP_SOURCE")


def _contact_id(override: str | None = None) -> str | None:
    return override or os.environ.get("PACOMIND_MCP_CONTACT_ID") or os.environ.get("PACOMIND_MCP_CONTACT_ID")
...
def _require_contact(override: str | None = None) -> tuple[str, dict[str, str]]:
    ...
    return "", {
        ...
        "message": "No contact_id provided and PACOMIND_MCP_CONTACT_ID is not set",
        "suggestion": "Set PACOMIND_MCP_CONTACT_ID in your MCP config or pass contact_id explicitly",
    }
...
def _sidecar_error(exc: Exception) -> dict[str, str]:
    ...
    return {
        ...
        "message": f"PacoMind sidecar not reachable at {_base_url()}",
        "suggestion": "Start with: pacomind start",
    }
...
async def _post(path: str, data: dict) -> dict | None:
    try:
        # Inject provenance from PACOMIND_MCP_SOURCE into metadata so it
...
def create_server() -> FastMCP:
    """Create the PacoMind MCP server with all tools registered."""

    mcp = FastMCP(
        "pacomind",
        instructions=(
            "PacoMind provides a cognitive substrate for AI agents: commitments, facts, "
    ...
    async def pacomind_health() -> dict:
        """Check if PacoMind sidecar is running and healthy. Call at session start or when other tools fail."""
        return await _get("/v1/host/health")

    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    async def pacomind_get_context(
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    async def pacomind_check_commitments(
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    async def pacomind_lookup_facts(
        contact_id: str | None = None,
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    async def pacomind_check_affect(
        contact_id: str | None = None,
    ) -> dict:
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    async def pacomind_search_world(
        query: str,
    ...
    @mcp.tool(annotations={"readOnlyHint": True, "idempotentHint": True})
    async def pacomind_get_patterns(
    ...
    async def pacomind_create_commitment(
        description: str,
        person_id: str | None = None,
    ...
    async def pacomind_fulfill_commitment(
        commitment_id: str,
        note: str | None = None,
    ) -> dict:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": True})
    async def pacomind_cancel_commitment(
        commitment_id: str,
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": False})
    async def pacomind_remember_fact(
        fact: str,
        contact_id: str | None = None,
        source: str | None = None,
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": True})
    async def pacomind_forget_sources(source_ids: list[str], contact_id: str | None = None) -> dict:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": True})
    async def pacomind_forget_fact(
        fact_id: str,
    ) -> dict:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": False})
    async def pacomind_record_affect(
        valence: float,
        trigger: str,
        contact_id: str | None = None,
        arousal: float = 0.5,
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": False})
    async def pacomind_record_surprise(
        observation: str,
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": True})
    async def pacomind_task_complete(
        task_id: str,
    ) -> dict:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": True})
    async def pacomind_task_snooze(
        task_id: str,
        hours: int = 24,
        reason: str = "",
    ) -> dict:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": True})
    async def pacomind_task_dismiss(
        task_id: str,
        reason: str = "stale",
    ) -> dict:
    ...
    @mcp.tool(annotations={"readOnlyHint": False, "idempotentHint": True})
    async def pacomind_initiative_feedback(
        initiative_id: str,
        action: str,
        details: dict[str, Any] | None = None,
    ) -> dict:
        """Provide feedback on how an initiative was handled. Action can be: acknowledged, actioned, dismissed, snoozed. Call after handling a pacomind_initiative."""
    ...
    @mcp.resource("pacomind://status")
    async def status_resource() -> dict:
        """Current PacoMind system status."""
        return await _get("/v1/host/health")

    @mcp.resource("pacomind://commitments")
    ...
    @mcp.resource("pacomind://affect/{contact_id}")
    ...
    @mcp.resource("pacomind://facts/{contact_id}")
    ...
    @mcp.resource("pacomind://world/entities")
    ...
    @mcp.resource("pacomind://surprises/unresolved")
    ...
    async def daily_briefing() -> str:
        ...
        cid = _contact_id()
        if not cid:
            return "Set PACOMIND_MCP_CONTACT_ID to get your daily briefing."
        return (
            f"Review the following for {cid}:\n"
            "1. Check pacomind_check_commitments for pending and overdue items\n"
            "2. Check pacomind_get_context for relevant source-backed context\n"
            "3. Check pacomind_get_patterns for workflow patterns\n"
            "4. Check pacomind://surprises/unresolved for anything unexpected\n"
    ...
    async def pre_task() -> str:
        ...
        cid = _contact_id()
        if not cid:
            return "Set PACOMIND_MCP_CONTACT_ID to use pre-task context."
        return (
            f"Before starting this task:\n"
            f"1. Call pacomind_check_commitments to see what {cid} has pending\n"
            f"2. Call pacomind_lookup_facts to recall relevant context\n"
            f"3. Call pacomind_get_context for source-backed appraisals and working perspective\n"
    ...
    async def post_task() -> str:
        ...
        cid = _contact_id()
        if not cid:
            return "Set PACOMIND_MCP_CONTACT_ID to use post-task recording."
        return (
            f"I just completed a task. For {cid}:\n"
            "1. If there was a commitment for this task, call pacomind_fulfill_commitment\n"
            "2. If anything unexpected happened, call pacomind_record_surprise\n"
            "3. If I learned something worth remembering, call pacomind_remember_fact with its source.\n"
