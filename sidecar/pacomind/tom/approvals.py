...
(PACOMIND_TOM2_APPROVAL_TTL_DAYS, default 30). Per-message prompts were
...
through the same guarded proposal path as everything else PacoMind wants
...
def approval_ttl_days() -> float:
    """PACOMIND_TOM2_APPROVAL_TTL_DAYS (default 30): how long an approval
    stays live. Malformed values read as the default; negative values read
    as 0 (nothing is approved)."""
    try:
        return max(0.0, float(
            os.environ.get("PACOMIND_TOM2_APPROVAL_TTL_DAYS", "30")))
