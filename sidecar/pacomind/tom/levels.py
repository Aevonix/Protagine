...
action required:
    ...
    effective = min(PACOMIND_TOM2_LEVEL          [default 0],
                    ...
                    cap(environment risk)      [PACOMIND_TOM2_RISK_CAPS],
...
def configured_level() -> int:
    """PACOMIND_TOM2_LEVEL (default 0): the owner's requested level."""
    return _env_int("PACOMIND_TOM2_LEVEL", 0)


def configured_max_level() -> int:
    """PACOMIND_TOM2_MAX_LEVEL (default 1): the hard ceiling."""
    return _env_int("PACOMIND_TOM2_MAX_LEVEL", 1)


def risk_caps_valid() -> bool:
    """True when PACOMIND_TOM2_RISK_CAPS parses cleanly (doctor surface)."""
    return _parse_risk_caps_strict() is not None


def _parse_risk_caps_strict() -> Optional[Dict[int, int]]:
    raw = os.environ.get("PACOMIND_TOM2_RISK_CAPS", DEFAULT_RISK_CAPS).strip()
    caps: Dict[int, int] = {}
    try:
...
def parse_risk_caps() -> Dict[int, int]:
    """Risk->cap map from PACOMIND_TOM2_RISK_CAPS (default '0:2,1:2,2:1,3:0').
    ...
    if caps is None:
        logger.warning("PACOMIND_TOM2_RISK_CAPS=%r is malformed — failing "
                       "closed to all-0 caps",
                       os.environ.get("PACOMIND_TOM2_RISK_CAPS"))
...
async def _resolve(conversation_key: str, reader_contact_id: str, *,
    ...
    from pacomind.gate.env_risk import classify
