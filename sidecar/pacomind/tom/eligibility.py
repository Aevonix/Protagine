...
Order (each check exists for a specific threat):
    ...
    5. mutual-knowledge    — M1: reader and subject demonstrably know of each
                             ...
                             PACOMIND_TOM2_MUTUAL_WINDOW_DAYS, default 30).
...
from pacomind.gate.env_risk import _tier_rank, env_risk_window_hours
from pacomind.identity.participants import SYSTEM_CONTACT_ID
from pacomind.tom.tom2 import render_inference_for_contact
...
def l2_approval_mode() -> str:
    """PACOMIND_TOM2_L2_APPROVAL: 'required' (default) or 'off'. Any other
    value reads as 'required' (fail closed)."""
    raw = os.environ.get("PACOMIND_TOM2_L2_APPROVAL", "required").strip().lower()
    return "off" if raw == "off" else "required"


def mutual_window_days() -> float:
    """PACOMIND_TOM2_MUTUAL_WINDOW_DAYS (default 30): how recent the reader/
    subject co-sighting must be to count as mutual knowledge."""
    try:
        v = float(os.environ.get("PACOMIND_TOM2_MUTUAL_WINDOW_DAYS", "30"))
...
async def _evaluate(inference, *, reader_contact_id, conversation_key,
    ...
    # PACOMIND_TOM2_CROSS_CONTEXT flag being off.
