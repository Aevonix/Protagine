...
(PACOMIND_WORLD_LLM_BASE_URL / _MODEL / _API_KEY, falling back to the
PACOMIND_INTROSPECT_* fast-classifier endpoint convention).

Modes (PACOMIND_WORLD_LLM_EXTRACT): off (default) | shadow (log the would-be
...
from pacomind.world_model.constants import CAUSAL_RELATIONSHIP_TYPES
from pacomind.world_model.populator import _is_low_quality, _looks_like_fragment
...
def llm_extract_mode() -> str:
    ...
    return resolve("PACOMIND_WORLD_LLM_EXTRACT",
                   ("off", "shadow", "live"), "off")
...
def world_supervised_max_writes() -> int:
    ...
    try:
        v = int(os.environ.get("PACOMIND_WORLD_SUPERVISED_MAX_WRITES", "25"))
...
def causal_extract_mode() -> str:
    """Effective causal-extraction mode: min(PACOMIND_CAUSAL_EXTRACT,
    PACOMIND_WORLD_LLM_EXTRACT) over off < shadow < live — causal extraction
    can never be MORE live than the extractor that carries it."""
    from pacomind.util.autonomy_preset import resolve
    order = ("off", "shadow", "live")
    causal = resolve("PACOMIND_CAUSAL_EXTRACT", order, "off")
    return order[min(order.index(causal), order.index(llm_extract_mode()))]


def _endpoint() -> Dict[str, str]:
    base = (os.environ.get("PACOMIND_WORLD_LLM_BASE_URL", "")
            or os.environ.get("PACOMIND_INTROSPECT_BASE_URL", "")).rstrip("/")
    model = (os.environ.get("PACOMIND_WORLD_LLM_MODEL", "")
             or os.environ.get("PACOMIND_INTROSPECT_MODEL", ""))
    key = (os.environ.get("PACOMIND_WORLD_LLM_API_KEY", "")
           or os.environ.get("PACOMIND_INTROSPECT_API_KEY", ""))
...
class WorldLLMExtractor:
    def __init__(self, store: Any, *, graph: Any = None,
        ...
        if store is not None:
            try:
                from pacomind.world_model.resolution.entity_resolver import (
                    EntityResolver,
                )
    ...
    async def _llm_batch(self, texts: List[str]) -> Optional[dict]:
        if self._router_provider is not None:
            ...
            import asyncio
            from pacomind.beliefs.source_claims import final_text, extraction_timeout_seconds
        ...
        try:
            ...
            timeout = aiohttp.ClientTimeout(total=float(
                os.environ.get("PACOMIND_WORLD_LLM_TIMEOUT", "40")))
    ...
    def _effective_mode(self) -> str:
        ...
        domain is explicitly enrolled in PACOMIND_SUPERVISED_LIVE_DOMAINS —
    ...
    def _record_outcome(self, mode: str, report: Dict[str, Any]) -> None:
        ...
        if self._self_model is None:
            return
        try:
            from pacomind.self_model.supervised import supervised_domains
            if "world_model" not in supervised_domains():
                return
    ...
    async def _upsert(self, name: str, etype: str, conf: float, mode: str,
        ...
        if not self._may_write(mode, "entity_upsert", report):
            return None
        try:
            from pacomind.world_model.entities import ENTITY_CLASS_MAP, BaseEntity
