...
THE INVARIANT
    ...
    NEVER, on its own, cause PacoMind to take or schedule an action. An
...
``causal_edges_actionable()``. The flag PACOMIND_CAUSAL_ACT defaults to off,
...
def causal_edges_actionable() -> bool:
    ...
    PACOMIND_CAUSAL_ACT=1 is the only unlock; anything else (unset, 0, junk)
    keeps causal edges query-only.
    """
    return os.environ.get("PACOMIND_CAUSAL_ACT", "0").strip().lower() in (
        "1", "true", "yes", "on")
