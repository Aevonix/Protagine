...
world-model store so PacoMind actually learns the owner's people, companies,
...
def populate_mode() -> str:
    ...
    return resolve("PACOMIND_WORLD_POPULATE_MODE",
...
def _noise_names() -> frozenset:
    ...
    env (persona/agent name, plus PACOMIND_WORLD_NOISE_NAMES)."""
    extra = set()
    for var in ("PACOMIND_PERSONA_NAME", "PACOMIND_AGENT_NAME"):
        v = os.environ.get(var, "").strip().lower()
        if v:
            extra.add(v)
    for t in os.environ.get("PACOMIND_WORLD_NOISE_NAMES", "").split(","):
        t = t.strip().lower()
        if t:
            extra.add(t)
...
class WorldModelPopulator:
    def __init__(
        ...
        if store is not None:
            try:
                from pacomind.world_model.resolution.entity_resolver import EntityResolver
