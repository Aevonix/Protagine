"""Full-state backup and restore for PacoMind.

Creates a compressed, optionally encrypted archive of all PacoMind state:
- All SQLite databases in PACOMIND_STATE_DIR (auto-discovered)
- Original source images referenced by the canonical ledger snapshot
- Identity files (pacomind-id, keys, genesis)
...
Usage::
    ...
    # Backup
    pacomind backup --full --output ~/backups/

    # Restore
    pacomind restore --full pacomind-backup-20260627T120000.tar.gz
"""
...
def create_full_backup(
    ...
    *,
    ...
    """Create a full backup archive of PacoMind state.
    ...
    pacomind_id = _read_pacomind_id(state_dir)
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    base_name = f"pacomind-backup-{timestamp}"

    with tempfile.TemporaryDirectory(prefix="pacomind-backup-") as tmp:
        ...
        meta = {
            ...
            "pacomind_version": _get_pacomind_version(),
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "pacomind_id": pacomind_id,
            "pacomind_id_hmac": _compute_identity_hmac(pacomind_id),
...
def restore_full_backup(
    ...
    *,
    ...
    """Restore PacoMind state from a full backup archive.
    ...
    with tempfile.TemporaryDirectory(prefix="pacomind-restore-") as tmp:
        ...
        if meta.get("backup_version", 0) > BACKUP_VERSION:
            raise ValueError(
                ...
                f"this PacoMind supports (max {BACKUP_VERSION})"
            )

        existing_id = _read_pacomind_id(state_dir)
        backup_id = meta.get("pacomind_id", "")

        if existing_id and existing_id != backup_id and not force_identity:
            raise ValueError(
                f"Backup is from pacomind {backup_id} but this instance is "
        ...
        summary: dict[str, Any] = {
            "pacomind_id": backup_id, "databases": [], "errors": [],
...
def restore_source_memory(
    ...
    *,
    ...
    current_id = _read_pacomind_id(current_state)
    ledger = current_state / "turn-idempotency.db"
    if not current_id or not ledger.is_file():
        raise ValueError("Memory recovery requires the surviving pacomind identity and source ledger")

    from pacomind.vector.image_store import LocalImageStore

    destination.parent.mkdir(mode=0o700, parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="pacomind-memory-recovery-", dir=destination.parent) as temporary:
        ...
        if meta.get("backup_version", 0) > BACKUP_VERSION:
            raise ValueError("Memory archive version is newer than this PacoMind supports")
        if meta.get("pacomind_id") != current_id:
            raise ValueError("Memory archive and surviving state have different pacomind identities")
        ...
        if _read_pacomind_id(current_state) != current_id:
            raise ValueError("Surviving pacomind identity changed during memory recovery")
...
def _snapshot_identity(state_dir: Path, dest: Path) -> None:
    dest.mkdir(parents=True, exist_ok=True)
    identity_files = ["pacomind-id", "genesis.json"]
...
def _snapshot_config(state_dir: Path, dest: Path) -> None:
    dest.mkdir(parents=True, exist_ok=True)
    config_files = [
        ".env", "channels.json", ".pacomind-llm-config.json",
...
def _decrypt_file(
    ...
    data = src.read_bytes()
    header = b"PACOMIND_ENC_V1\n"
    if not data.startswith(header):
        raise ValueError("Not a PacoMind encrypted backup (invalid header)")
...
def _read_pacomind_id(state_dir: Path) -> str:
    id_path = state_dir / "pacomind-id"
    if id_path.exists():
        return id_path.read_text().strip()
    return ""


def _compute_identity_hmac(pacomind_id: str) -> str:
    return hmac.new(
        pacomind_id.encode(), b"pacomind-backup-binding", hashlib.sha256
    ).hexdigest()


def _get_pacomind_version() -> str:
    try:
        from importlib.metadata import version
        return version("pacomind")
    except Exception:
        return "unknown"
