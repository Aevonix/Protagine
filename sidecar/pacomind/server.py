"""PacoMind sidecar FastAPI server.
...
def _state_dir() -> Path:
    """Resolve the PacoMind state directory (wrapper for get_state_dir)."""
...
def _embedded_worker_enabled() -> bool:
    ...
    cutovers can pin ``PACOMIND_EMBEDDED_WORKER_ENABLED=false`` while keeping
    ...
    if raw in {"0", "false", "no", "off"}:
        return False
    raise RuntimeError(
        "PACOMIND_EMBEDDED_WORKER_ENABLED must be true or false"
    )
...
def _resolve_guard_mode(raw: Optional[str]):
    """Strict PACOMIND_GUARD_MODE parser: 'enforce', 'shadow', or unset only.
    ...
    logger.error(
        "PACOMIND_GUARD_MODE=%r is not a recognised guard mode "
        "(expected 'enforce' or 'shadow') — refusing to silently "
        "fall back to shadow", raw,
    )
    raise RuntimeError(
        f"PACOMIND_GUARD_MODE must be 'enforce' or 'shadow', got {raw!r}"
    )
...
def _cognition_owner_spec(*, router, node_id: str):
    ...
    from pacomind.task_queue.models import JobType, WorkerCapabilities
    from pacomind.task_queue.routing import THOUGHT_ROUTE
...
def _attach_p8_runtime(*, state_dir: Path, facts_store, graph=None):
    ...
    from pacomind.tom.integration import P8Runtime, p8_integration_mode
    ...
    audit = None
    try:
        from pacomind.tom.arcs import ArcStore
        ...
        visibility = open_visibility_envelope_store(
            state_dir / "pacomind-p8-visibility.db", enabled=True)
        arcs = ArcStore(str(state_dir / "pacomind-p8-arcs.db"))
        audit = open_recipient_simulation_audit_store(
            state_dir / "pacomind-p8-recipient-audit.db", mode="shadow")
...
def _attach_cognition_spine(
    *,
    ...
    if cognition_spine_mode() == "live":
        from pacomind.directives import Action
        ...
        try:
            ...
            boundary_probe = directive_manager.check(Action(
                kind="read",
                text="pacomind cognition startup boundary health probe",
        ...
        if not configured_owner:
            raise RuntimeError(
                "P3 live cognition requires PACOMIND_THOUGHT_WORKER_NODE_ID"
            )
        ...
        if configured_owner != actual_owner:
            raise RuntimeError(
                "PACOMIND_THOUGHT_WORKER_NODE_ID does not match the local "
                "cognition worker node"
            )

    try:
        project_limit = int(
            os.environ.get("PACOMIND_PROJECTS_MAX_CONCURRENT", "3")
        )
    except ValueError as exc:
        raise RuntimeError(
            "PACOMIND_PROJECTS_MAX_CONCURRENT must be an integer"
        ) from exc
    if not 1 <= project_limit <= 100:
        raise RuntimeError(
            "PACOMIND_PROJECTS_MAX_CONCURRENT must be between 1 and 100"
        )
    ...
    def _runtime_contract():
        ...
        from pacomind.self_model.event_concerns import event_concern_mode
        from pacomind.self_model.workspace import workspace_mode
    ...
    cognition_store = CognitionSpineStore(
        str(state_dir / "pacomind-cognition.db")
    )
    spine = CognitionSpine(
        ...
        owner_person_id=(
            os.environ.get("PACOMIND_OWNER_PERSON_ID", "").strip()
            or os.environ.get("PACOMIND_OWNER_CONTACT_ID", "").strip()
            or "owner"
        ),
    )
...
def _attach_situation_spine(
    *,
    ...
    store = SituationStore(str(state_dir / "pacomind-situation.db"))
    try:
    ...
    try:
        ...
        try:
            interval = int(os.environ.get(
                "PACOMIND_COGNITION_EVIDENCE_INTERVAL_SECONDS", "30",
            ))
        except ValueError as exc:
            raise RuntimeError(
                "PACOMIND_COGNITION_EVIDENCE_INTERVAL_SECONDS must be an integer"
            ) from exc
        interval = max(5, min(3600, interval))

        evidence_store = CognitionEvidenceStore(
            str(state_dir / "pacomind-cognition-evidence.db")
        )
...
def _initialize_controlled_learning(
    *,
    ...
    correction_store = FeedbackStore(
        db_path=str(state_dir / "pacomind-learning-feedback.db"))

    benchmark = None
    if benchmark_enabled():
        benchmark = SelfhoodBenchmark(
            BenchmarkStore(db_path=str(state_dir / "pacomind-benchmark.db")),
    ...
    if experiments_enabled():
        ...
        experiments = ExperimentEngine(
            ExperimentStore(
                db_path=str(state_dir / "pacomind-experiments.db")),
...
def _scheduler_health_check(autonomy_loop) -> dict:
    ...
    """
    import pacomind.api.routers.host as _h
    wired = 0
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        _adaptive_params = AdaptiveParamStore(
            db_path=str(state_dir / "pacomind-params.db"))
        register_core_params(_adaptive_params)
        try:
            from pacomind.api.routers.host import set_adaptive_params
            set_adaptive_params(_adaptive_params)
        except ImportError:
            pass
        logger.info("AdaptiveParamStore initialized (db=%s)",
                    state_dir / "pacomind-params.db")
    except Exception as exc:
    ...
    try:
        ...
        config_path = state_dir / ".pacomind-llm-config.json"
    ...
    graph = None
    if os.environ.get("PACOMIND_GRAPH_ENABLED", "true").lower() not in {"0", "false", "off"}:
        try:
            from pacomind.intelligence.graph.client import PacoMindGraph, GraphConfig
            ...
            graph = PacoMindGraph(graph_config)
            # Apply graph schema constraints/indexes before any queries run
            try:
                from pacomind.intelligence.graph.migrations import run_migrations
            ...
            try:
                import pacomind.api.routers.host as _host_router
                te = _host_router._tool_executor
                if te is not None:
                    te._graph = graph
            except Exception:
                logger.warning("ToolExecutor graph wiring failed (capability-gap detection degraded)")
            logger.info("PacoMindGraph initialized (uri=%s db=%s)", neo4j_uri, neo4j_db)

            # Ensure PacoMind self-representation in graph (v0.11.0)
            try:
                await graph.ensure_pacomind_self()
            except Exception as self_exc:
                logger.warning("PacoMind self-representation setup skipped: %s", self_exc)
            ...
            logger.warning("PacoMindGraph init failed — memory endpoints will be degraded: %s", exc)
    ...
    _gate_audit = None
    try:
        from pacomind.gate import ResponseGate, GateConfig
        from pacomind.gate.audit import InMemoryAuditLog
        # L7 send-delay (the cancel window) is env-tunable; default 0 = no
        # hold. Set PACOMIND_GATE_SEND_DELAY_SECS>0 to enable a real cancel
        # window on the request-path gate.
        try:
            _gate_delay = float(os.environ.get("PACOMIND_GATE_SEND_DELAY_SECS", "0"))
        except ValueError:
            _gate_delay = 0.0
    ...
    if graph is not None:
        try:
            from pacomind.intelligence.mind_model.graph_baseline import GraphBaselineStore
            from pacomind.intelligence.mind_model.signal_collector import SignalCollector
        ...
        logger.warning("SignalCollector skipped — PacoMindGraph not available")

    # --- 6. Embedding pipeline ---
    embed_provider = os.environ.get("PACOMIND_EMBED_PROVIDER", "")
    embed_model = os.environ.get("PACOMIND_EMBED_MODEL", "")
    embed_dims = os.environ.get("PACOMIND_EMBED_DIMS", "")
    reranker_model = os.environ.get("PACOMIND_RERANKER_MODEL", "")

    # Auto-detect tier if not explicitly configured
    if embed_provider != "skip" and (not embed_provider or not embed_model):
        try:
            from pacomind.vector.scanner import scan
            from pacomind.vector.tiers import get_tier_by_memory
            hw = scan()
    ...
    try:
        ...
        if provider is not None and embed_provider == "openai_api" and hasattr(provider, "configure"):
            ...
            provider.configure(
                os.environ.get("PACOMIND_EMBED_BASE_URL", ""),
                os.environ.get("PACOMIND_EMBED_API_KEY", ""),
            )
        if provider is None:
            ...
            multimodal_enabled = os.environ.get("PACOMIND_MULTIMODAL", "false").lower() == "true"
            if multimodal_enabled:
                try:
                    from pacomind.vector.multimodal_provider import make_multimodal_provider
                    from pacomind.vector.image_store import make_image_store
                    ...
                    img_store = make_image_store(
                        mode=os.environ.get("PACOMIND_IMAGE_STORAGE", "local"),
                    ...
                    logger.info("Multimodal enabled (model=%s, storage=%s)", embed_model, os.environ.get("PACOMIND_IMAGE_STORAGE", "local"))
                except Exception as exc:
            ...
            # Wire embedding pipeline into PacoMindGraph for vector-backed recall
            try:
                ...
                from pacomind.vector import set_store, set_pipeline
                ...
                embed_dims = int(os.environ.get("PACOMIND_EMBED_DIMS", pipeline.dimensions or 384))
                ...
                logger.info("PacoMindGraph wired to vector store (path=%s)", vector_db_path)

                if graph is not None and graph._embed_fn and graph._vector_store:
                    logger.info("PacoMindGraph fully operational (Neo4j + embeddings + vector store)")
                else:
                    logger.warning("PacoMindGraph partially wired — memory may be degraded")
            ...
            llm_config_path = Path(os.environ.get("PACOMIND_STATE_DIR", ".")) / ".pacomind-llm-config.json"
    ...
    reranker_provider_name = os.environ.get("PACOMIND_RERANKER_PROVIDER", "")
    if reranker_model and reranker_model.lower() not in ("none", "", "null"):
        try:
            ...
            reranker_base_url = os.environ.get("PACOMIND_RERANKER_BASE_URL", "")
            reranker_api_key = os.environ.get("PACOMIND_RERANKER_API_KEY", "")
            if reranker_provider_name == "openai_api" or reranker_base_url:
                ...
                # PACOMIND_RERANKER_PROMPT_STYLE=qwen3 applies the Qwen3-Reranker
                ...
                reranker_provider.configure(
                    reranker_base_url,
                    reranker_api_key,
                    os.environ.get("PACOMIND_RERANKER_PROMPT_STYLE", ""),
                )
            else:
                from pacomind.vector.scanner import scan
                hw = scan()
            ...
            # Wire the reranker into PacoMindGraph recall (mirrors the
            # set_embed_fn wiring above). Registration alone changes
            # nothing: use is gated by PACOMIND_RECALL_RERANK (default off).
            if graph is not None and hasattr(graph, "set_rerank_fn"):
                ...
                logger.info(
                    "PacoMindGraph wired to reranker for recall "
                    "(gated by PACOMIND_RECALL_RERANK)")
        except Exception as exc:
    ...
    try:
        ...
        goals_db = os.path.join(state_dir, "pacomind-goals.db")
    ...
    try:
        ...
        commitments_db = state_dir / "pacomind-commitments.db"
        ...
        from pacomind.self_model.settlement import register_settler
    ...
    try:
        ...
        affect_db = state_dir / "pacomind-affect.db"
        ...
        facts_db = state_dir / "pacomind-facts.db"
        ...
        # store + daily asymmetry engine. Inert unless PACOMIND_TOM2 is set
        ...
        from pacomind.api.routers.host import set_tom2_store, set_tom2_engine
        tom2_db = state_dir / "pacomind-tom2.db"
        ...
        from pacomind.api.routers.host import set_tom2_exposure_store
        tom2_exposure_db = state_dir / "pacomind-tom2-exposure.db"
        ...
        # gated by PACOMIND_CONV_PRESENCE (default on).
        from pacomind.channels.presence import ConversationPresenceStore
        from pacomind.api.routers.host import set_presence_store
        presence_db = state_dir / "pacomind-presence.db"
        ...
        provenance_db = state_dir / "pacomind-context-provenance.db"
        ...
        # no bypass authority.
        from pacomind.gate.response_guard import ResponseGuard, GuardMode
        from pacomind.gate.surface_policy import (
            POLICY_DIGEST as _guard_policy_digest,
            POLICY_ID as _guard_policy_id,
        )
        from pacomind.world_model.extraction.conversation_extractor import (
            ConversationExtractor)
        _guard_mode = _resolve_guard_mode(os.environ.get("PACOMIND_GUARD_MODE"))
        from pacomind.gate.guard_audit import GuardAuditStore
        guard_audit_db = state_dir / "pacomind-guard-audit.db"
        ...
        from pacomind.api.routers.host import set_taint_registry
        taint_db = state_dir / "pacomind-tom2-taint.db"
        ...
        from pacomind.tom.levels import set_evidence_probe
        set_evidence_probe(None)

        from pacomind.tom.engagement import EngagementStore
        engagement_db = state_dir / "pacomind-engagement.db"
        ...
        comms_log = CommsLog(db_path=state_dir / "pacomind-comms.db", source_ledger=source_ledger)
        ...
        preference_learner = PreferenceLearner(db_path=str(state_dir / "pacomind-preferences.db"),
        ...
        logger.info("PreferenceLearner initialized (db=%s)", state_dir / "pacomind-preferences.db")
    except Exception as exc:
        try:
            from pacomind.tom.levels import set_evidence_probe
            set_evidence_probe(None)
    ...
    try:
        ...
        from pacomind.api.routers.host import set_directive_manager
        from pacomind.turns import get_turn_idempotency_ledger
        _directive_store = DirectiveStore(db_path=str(state_dir / "pacomind-directives.db"),
        ...
        logger.info(
            ...
            state_dir / "pacomind-directives.db", _directive_store.count_active(),
        )
    except Exception as exc:
        if locals().get("tool_executor") is not None:
    ...
    try:
        ...
        from pacomind.api.routers.host import set_proposal_store
        _proposal_store = ProposalStore(db_path=str(state_dir / "pacomind-proposals.db"))
        set_proposal_store(_proposal_store)
        logger.info("ProposalStore initialized (db=%s)", state_dir / "pacomind-proposals.db")
    except Exception as exc:
    ...
    try:
        ...
        from pacomind.api.routers.host import set_feedback_store
        set_feedback_store(TypeFeedbackStore(db_path=str(state_dir / "pacomind-feedback.db")))
        logger.info("TypeFeedbackStore initialized (db=%s)", state_dir / "pacomind-feedback.db")
    except Exception as exc:
    ...
    try:
        ...
        if self_model_enabled():
            from pacomind.autonomy.registry import SubsystemRegistry as _Reg
            _competence = CompetenceStore(
                db_path=str(state_dir / "pacomind-self-model.db"))
            _journal = ActionJournal(
                db_path=str(state_dir / "pacomind-action-journal.db"))
            _trust = TrustEngine(
                _competence, db_path=str(state_dir / "pacomind-self-model.db"),
            ...
            logger.info(
                ...
                "autograduate=%s)",
                state_dir / "pacomind-self-model.db",
                state_dir / "pacomind-action-journal.db",
                os.environ.get("PACOMIND_TRUST_AUTOGRADUATE", "true"))
        else:
            logger.info("SelfModel disabled (PACOMIND_SELF_MODEL_ENABLED=false)")
    except Exception as exc:
    ...
    try:
        ...
        if _controlled_learning["benchmark"] is not None:
            logger.info(
                ...
                state_dir / "pacomind-benchmark.db",
                state_dir / "pacomind-learning-feedback.db",
            )
        else:
            logger.info(
                "Selfhood benchmark disabled (PACOMIND_BENCHMARK_ENABLED=false)")
        if _controlled_learning["experiments"] is not None:
            logger.info(
                "Controlled experiment framework ready (db=%s, shared_authority=%s)",
                state_dir / "pacomind-experiments.db",
                state_dir / "approval_authority.db",
            )
        else:
            logger.info(
                "Experiment framework disabled "
                "(PACOMIND_EXPERIMENTS_ENABLED=false)")
    except Exception as exc:
        logger.error("Controlled learning init failed closed: %s", exc)

    # --- Toolsmith (Mind M1): self-built, sandbox-verified tools ---
    try:
        from pacomind.toolsmith import (
            Toolsmith, ToolRegistry, toolsmith_enabled,
        )
        from pacomind.api.routers.host import set_toolsmith
        if toolsmith_enabled():
            _tool_registry = ToolRegistry(
                db_path=str(state_dir / "pacomind-toolsmith.db"),
            ...
            try:
                from pacomind.api.routers.host import _tool_executor
            ...
            logger.info("Toolsmith ready (mode=%s, db=%s)",
                        os.environ.get("PACOMIND_TOOLSMITH", "off"),
                        state_dir / "pacomind-toolsmith.db")
        else:
            logger.info("Toolsmith disabled (PACOMIND_TOOLSMITH=off)")
    except Exception as exc:
    ...
    try:
        ...
        from pacomind.api.routers.host import set_expectations
        if expectations_enabled():
            _exp_store = ExpectationStore(
                db_path=str(state_dir / "pacomind-expectations.db"))
            _exp_journal = None
            try:
                from pacomind.api.routers.host import _self_model as _sm_e
            ...
            logger.info("Expectation engine ready (mode=%s, db=%s)",
                        expectations_mode(),
                        state_dir / "pacomind-expectations.db")
        else:
            logger.info("Expectation engine disabled (PACOMIND_EXPECTATIONS=off)")
    except Exception as exc:
    ...
    try:
        ...
        from pacomind.self_model.thinker import build_thinker
        from pacomind.self_model.event_concerns import (
        ...
        from pacomind.api.routers.host import set_workspace
        if workspace_enabled():
            _concern_store = ConcernStore(
                db_path=str(state_dir / "pacomind-workspace.db"))
            _thinker = (build_thinker(llm_router, graph=graph)
                        if llm_router is not None else None)
            _ws_journal = None
            try:
                from pacomind.api.routers.host import _self_model as _sm_ws
            ...
            if event_concerns_enabled():
                ...
                logger.info(
                    ...
                    event_concern_mode(),
                    os.environ.get("PACOMIND_EVENT_CONCERNS_BOOTSTRAP", "tail"),
                )
            ...
            if turn_concerns_enabled():
                ...
                logger.info(
                    ...
                    turn_concern_mode(),
                    os.environ.get("PACOMIND_TURN_CONCERNS_BOOTSTRAP", "tail"),
                    ",".join(turn_concern_channels()) or "<none>",
                )
            set_workspace(_workspace)
            logger.info("Cognitive workspace ready (mode=%s, db=%s)",
                        workspace_mode(), state_dir / "pacomind-workspace.db")
        else:
            logger.info("Cognitive workspace disabled (PACOMIND_WORKSPACE=off)")
            if external_event_concerns_enabled():
                logger.warning(
    ...
    try:
        from pacomind.skills_memory import SkillStore, skills_distill_mode
        from pacomind.api.routers.host import set_skill_store
        _skills_mem_store = SkillStore(
            db_path=str(state_dir / "pacomind-skills.db"))
        set_skill_store(_skills_mem_store)
        logger.info("SkillStore initialized (db=%s, %d skill(s), distill=%s)",
                    state_dir / "pacomind-skills.db",
    ...
    try:
        from pacomind.mining import EscalationMiner, MiningStore, mining_mode
        from pacomind.api.routers.mining import set_mining

        if mining_mode() != "off":
            _mining_store_obj = MiningStore(
                db_path=str(state_dir / "pacomind-mining.db"))

            def _mining_router_getter():
                try:
                    from pacomind.api.routers.host import _reasoning_loop
            ...
            logger.info("EscalationMiner initialized (db=%s, mode=%s)",
                        state_dir / "pacomind-mining.db", mining_mode())
        else:
            logger.info("Mining disabled (PACOMIND_ESCALATION_MINING=off)")
    except Exception as exc:
        logger.warning("Mining init failed: %s", exc)

    # --- Read-only repo mirrors + directed action (option A) ---
    try:
        from pacomind.repos import RepoMirrorManager
        ...
        if _n_repos:
            ...
            async def _sync_mirrors():
                try:
                    ...
                    from pacomind.api.routers.host import _world_store as _ws
        ...
        async def _directed_deliver(payload: dict) -> bool:
            ...
            try:
                from pacomind.api.routers.host import _autonomy_loop, _delivery_bridge
        ...
        from pacomind.api.routers.host import _feedback_store as _fb_store
        _directed_svc = DirectedActionService(
            store=ScopedTaskStore(db_path=str(state_dir / "pacomind-directed.db")),
            directive_manager=_get_dm2(),
    ...
    try:
        ...
        patterns_db = state_dir / "pacomind-patterns.db"
        ...
        surprise_db = state_dir / "pacomind-surprise.db"
    ...
    try:
        ...
        from pacomind.channels.router import set_channel_store
        from pacomind.channels.phone_gateways import set_channel_store_ref

        channels_db = os.path.join(state_dir, "pacomind-channels.db")
        ...
        from pacomind.api.routers.host import set_channel_store as _host_set_channel_store
    ...
    if contacts_store is not None:
        try:
            ...
            import pacomind.api.routers.host as _host_mod
    ...
    try:
        ...
        # via the directive manager. Mode from PACOMIND_WORLD_POPULATE_MODE
        # (off|shadow|live, default shadow).
        try:
            from pacomind.world_model.populator import WorldModelPopulator, populate_mode
            from pacomind.api.routers.host import (
        ...
        try:
            from pacomind.beliefs import BeliefEngine, BeliefStore, beliefs_mode
            from pacomind.api.routers.host import set_belief_engine
            from pacomind.world_model.store import set_property_audit_hook
            _belief_store = BeliefStore(
                db_path=str(state_dir / "pacomind-beliefs.db"))
            ...
            logger.info("BeliefEngine initialized (db=%s, mode=%s)",
                        state_dir / "pacomind-beliefs.db", beliefs_mode())
        ...
        try:
            ...
            from pacomind.world_model.extraction.formats import (
            ...
            if llm_router is not None:
                try:
                    from pacomind.world_model.extraction.llm_extractor import (
    ...
    try:
        from pacomind.intelligence.cognition.registry import CognitionPipeline
        ...
        if graph is not None:
            ...
            logger.warning("CognitionPipeline skipped — PacoMindGraph not available")
    except Exception as exc:
    ...
    try:
        ...
        search_provider = os.environ.get("PACOMIND_SEARCH_PROVIDER", "")
        if search_provider == "tavily" and os.environ.get("TAVILY_API_KEY"):
            from pacomind.research.search.tavily import TavilyProvider
            ...
            from pacomind.research.search.serpapi import SerpAPIProvider
            ...
            from pacomind.research.search.brave import BraveSearchProvider
        ...
        try:
            ...
            te = None
        if te is not None:
            sandbox_dir = os.environ.get("PACOMIND_SANDBOX_DIR", str(state_dir / "sandbox"))
    ...
    try:
        ...
        try:
            ...
            if (_trust_for_cap is not None
                ...
                logger.info("Delivery rate cap wired to trust engine "
                            "(adaptive, max=%s)",
                            os.environ.get("PACOMIND_TRUST_DELIVERY_CAP_MAX", "6"))
        except Exception:
        ...
        # under PACOMIND_STATE_DIR, and the scheduler fires daily/weekly briefings and
        # drains pending deliveries. All deployment specifics come from env.
        try:
            from pathlib import Path as _P

            from pacomind.briefings.config import BriefingConfig
            from pacomind.briefings.engine import BriefingEngine
            from pacomind.briefings.scheduler import BriefingScheduler
            from pacomind.briefings.store import BriefingStore

            b_cfg = BriefingConfig()
            b_cfg.daily.time = os.environ.get("PACOMIND_BRIEFING_DAILY_TIME", b_cfg.daily.time)
            b_cfg.daily.timezone = os.environ.get("PACOMIND_BRIEFING_TZ", b_cfg.daily.timezone)
            b_cfg.weekly.timezone = b_cfg.daily.timezone
            b_cfg.delivery_gateway = os.environ.get("PACOMIND_BRIEFING_GATEWAY", "whatsapp")
            b_cfg.lm_enhancement_enabled = (
                os.environ.get("PACOMIND_BRIEFING_LM_ENHANCE", "0") not in ("0", "false", "no")
            )
            _b_state_dir = os.environ.get("PACOMIND_STATE_DIR", ".")
            ...
            try:
                if graph is not None:
                    from pacomind.briefings.aggregators import RelationshipAggregator
            ...
            set_briefings_engine(briefings)
            if os.environ.get("PACOMIND_BRIEFINGS_SCHEDULE", "1") not in ("0", "false", "no"):
    ...
    try:
        from pacomind.intelligence.synthesis.connection_discoverer import ConnectionDiscoverer
        if graph is not None:
            ...
            logger.warning("ConnectionDiscoverer skipped — PacoMindGraph not available")
    except Exception as exc:
    ...
    try:
        from pacomind.intelligence.learning.continuous_learner import ContinuousLearner
    ...
    # --- 17. Chain / Identity ---
    try:
        from pacomind.chain.identity import (
            get_or_create_pacomind_id,
        ...
        from pacomind.chain.manager import ChainManager
        chain = ChainManager(
            db_path=state_dir / "chain.db",
            pacomind_id=pacomind_id,
        )
        set_chain_manager(chain)
        logger.info("ChainManager initialized (pacomind_id=%s)", pacomind_id)

        # Wire local key manager
        try:
            from pacomind.chain.local_keys import LocalKeyManager
            keys_dir = state_dir / "pacomind-keys"
            key_passphrase = os.environ.get("PACOMIND_KEY_PASSPHRASE", "")
            passphrase = key_passphrase.encode() if key_passphrase else None

            if (keys_dir / "private.pem").exists():
                key_mgr = LocalKeyManager(keys_dir=keys_dir, pacomind_id=pacomind_id, passphrase=passphrase)
                logger.info("LocalKeyManager loaded (public_key=%s...)", key_mgr.public_key_hex()[:16])
            else:
                key_mgr = LocalKeyManager.generate(keys_dir=keys_dir, pacomind_id=pacomind_id, passphrase=passphrase)
                logger.info("LocalKeyManager generated new keypair for pacomind %s", pacomind_id)
        ...
        try:
            from pacomind.chain.node import get_or_create_node_id, ensure_node_keypair, create_node_certificate
            ...
            if not cert_path.exists():
                create_node_certificate(state_dir, pacomind_key_manager=key_mgr)
                logger.info("Node certificate created and signed by PacoMind key")
            else:
                logger.info("Node certificate exists")
        except Exception as nexc:
            logger.warning("Node identity init skipped: %s", nexc)

        # The LOCAL identity anchor (pacomind_id, node keypair, signed node
    ...
    # --- 19. Session store ---
    try:
        from pacomind.sessions.store import InMemorySessionStore
        ...
        if _gate_ref is not None:
            from pacomind.gate import ResponseGate
    ...
    try:
        ...
        try:
            ...
            from pacomind.api.routers.observations import set_observation_store
    ...
    try:
        ...
        # PACOMIND_PRESET_LOOP_COUPLING, default on) rather than an explicit
        # PACOMIND_AUTONOMY_MODE, say so loudly at startup and leave ONE
        ...
        if getattr(autonomy_config, "mode_source", "") == "preset":
            try:
                from pacomind.util.autonomy_preset import preset_name
                _preset = preset_name() or "(unknown)"
            except Exception:
                _preset = "(unknown)"
            logger.warning(
                "Autonomy loop mode %s inherited from PACOMIND_AUTONOMY_PRESET=%s "
                "via preset-loop coupling (PACOMIND_PRESET_LOOP_COUPLING=on by "
                "default). Set PACOMIND_AUTONOMY_MODE explicitly to override, or "
                "PACOMIND_PRESET_LOOP_COUPLING=off to restore env-only resolution.",
                autonomy_config.mode.value, _preset)
            try:
                from pacomind.api.routers.host import _self_model as _sm_j
                ...
                if _cj is not None and not _cj.recent(
                    ...
                    _cj.record(
                        ...
                        reasoning="PACOMIND_AUTONOMY_MODE unset; "
                                  "PACOMIND_PRESET_LOOP_COUPLING on (default). "
                                  "Rollback: set PACOMIND_AUTONOMY_MODE=reactive "
                                  "or PACOMIND_PRESET_LOOP_COUPLING=off.",
        ...
        async def _run_memory_consolidate():
            from pacomind.api.routers.host import _consolidator as c
            if c is None:
        ...
        async def _run_cpi_track():
            from pacomind.api.routers.host import _metalearner as ml
            if ml is None:
        ...
        async def _run_world_model_prune():
            from pacomind.api.routers.host import _world_store as ws
            if ws is None:
        ...
        scheduler.register("mining_prune", _run_mining_prune, interval_seconds=86400, metadata={"description": "Prune banked mining turns per PACOMIND_MINING_RETENTION_DAYS / PACOMIND_MINING_MAX_TURNS (0 = keep everything)"})
        ...
        if os.environ.get("PACOMIND_PATTERNS_SCHEDULE",
                          "off").strip().lower() == "on":
            async def _run_pattern_extract():
                ...
                from pacomind.patterns.extract import extract_patterns
                return {"status": "ok",
                        **extract_patterns(world_store=ws, pattern_store=ps)}

            scheduler.register("pattern_extract", _run_pattern_extract, interval_seconds=86400, metadata={"description": "Extract world-model patterns into the pattern store (PACOMIND_PATTERNS_SCHEDULE=on)"})

        async def _run_surprise_ttl():
            from pacomind.api.routers.host import _surprise_store as ss
            if ss is None:
                return {"status": "skipped",
                        "reason": "surprise_store_not_wired"}
            try:
                ttl_days = float(os.environ.get(
                    "PACOMIND_SURPRISE_TTL_DAYS", "14"))
            except ValueError:
                ttl_days = 14.0
            if ttl_days <= 0:
        ...
        scheduler.register("surprise_ttl", _run_surprise_ttl, interval_seconds=86400, metadata={"description": "Auto-resolve surprises unaddressed past PACOMIND_SURPRISE_TTL_DAYS (default 14; 0 disables)"})

        async def _run_digest_flush():
            from pacomind.api.routers.host import _delivery_bridge as bridge
            if bridge is None:
                return {"status": "skipped", "reason": "delivery_bridge_not_wired"}
            header = os.environ.get("PACOMIND_DIGEST_HEADER", "Daily digest")
            return await bridge.flush_digests_to_gateway(header=header)

        digest_interval = int(os.environ.get("PACOMIND_DIGEST_INTERVAL_SECONDS", "86400"))
        scheduler.register(
            "digest_flush",
            _run_digest_flush,
    ...
    # Wire SubsystemRegistry into ToolExecutor so PacoMind-native tools
    ...
    if registry is not None and locals().get("tool_executor") is not None:
        ...
        from pacomind.tools.handlers import TOOL_HANDLERS as _pacomind_handlers
        for _tname, _thandler in _pacomind_handlers.items():
            if _tname not in te._handlers:
                te._handlers[_tname] = lambda args, h=_thandler, r=registry: h(args, r)
        logger.info(
            "PacoMind tool handlers wired into ToolExecutor (%d pacomind tools, %d total)",
            len(_pacomind_handlers),
            len(te._handlers),
        )

    # --- 22b. Project engine (goal persistence, cognition item 1) ---
    try:
        from pacomind.projects import ProjectEngine, ProjectStore, projects_mode
        ...
        _project_store = ProjectStore(db_path=str(state_dir / "pacomind-projects.db"))
        ...
        logger.info("ProjectEngine initialized (db=%s, mode=%s)",
                    state_dir / "pacomind-projects.db", projects_mode())
        ...
        try:
            from pacomind.api.routers.host import _belief_engine as _be
            if _be is not None:
    ...
    try:
        ...
        if _evidence_wiring is not None:
            logger.info(
                ...
                (
                    state_dir / "pacomind-cognition-evidence.db"
    ...
    try:
        ...
        if _cognition_spine is not None:
            from pacomind.cognition.goal_spine import cognition_spine_mode
            logger.info(
                "Typed cognition spine attached (db=%s, mode=%s)",
                state_dir / "pacomind-cognition.db",
                cognition_spine_mode(),
            )
    except Exception as exc:
    ...
    try:
        ...
        if _situation_wiring is not None:
            logger.info(
                ...
                state_dir / "pacomind-situation.db",
        ...
        try:
            from pacomind.self_model.situation import situation_spine_mode
    ...
    try:
        ...
        from pacomind.task_queue.governor import WorkerGovernor, workers_mode
        ...
        try:
            ...
            queue_scheduler = Scheduler(
                ...
                tick_interval_secs=_queue_seconds_env(
                    "PACOMIND_QUEUE_SCHEDULER_TICK_SECS", 2.0,
                ),
                claim_timeout_secs=_queue_seconds_env(
                    "PACOMIND_QUEUE_CLAIM_TIMEOUT_SECS", 30.0,
                ),
    ...
    try:
        from pacomind.services.agent_bridge import (
    ...
    if task_queue is not None and _embedded_worker_enabled():
        try:
            ...
            from pacomind.task_queue.worker import WorkerNode
    ...
    try:
        from pacomind.sandbox import SandboxManager, sandbox_mode
        from pacomind.api.routers.host import (
    ...
    try:
        from pacomind.connectors import (
    ...
    try:
        ...
        from pacomind.api.routers.host import get_directive_manager as _get_dm
    ...
    from pacomind.sessions.reports import SessionReportStore
    ...
    try:
        if (
            ...
            from pacomind.autonomy.synthesis import ConversationSynthesisTask
    ...
    try:
        import pacomind.api.routers.host as _host_actions
        from pacomind.api.routers.governed_actions import (
    ...
    claims_enabled = os.environ.get("PACOMIND_SOURCE_CLAIMS", "on").strip().lower() in {"on", "1", "true"}
    from pacomind.vector import get_store as source_vector_store
    if claims_enabled or source_vector_store() is not None:
        ...
        from pacomind.api.routers import host as source_claim_host
    ...
    try:
        from pacomind.channels.router import set_channel_store as _set_ch_store
        _set_ch_store(None)
    except Exception:
        pass
    ...
    # Stop task queue
    try:
        from pacomind.api.routers.host import _task_queue
    ...
    try:
        from pacomind.api.routers.host import _autonomy_loop
    ...
    try:
        from pacomind.api.routers.host import set_project_engine
        set_project_engine(None)
        project_store = locals().get("_project_store")
...
def create_app() -> FastAPI:
    ...
    from pacomind.runtime_logging import RequestLogTiming, configure_runtime_logging
    if os.environ.get('PACOMIND_RUNTIME_LOGGING') == '1':
    ...
    from pacomind.api.routers import executions as executions_router
    app.include_router(executions_router.router)
    from pacomind.api.routers import commitment_work as commitment_work_router
    app.include_router(commitment_work_router.router)
    from pacomind.api.routers import initiative_work as initiative_work_router
    app.include_router(initiative_work_router.router)
    from pacomind.api.routers import social_state as social_state_router
    app.include_router(social_state_router.router)
    from pacomind.api.routers import temporal_followups as temporal_followups_router
    app.include_router(temporal_followups_router.router)
    from pacomind.api.routers import transport as transport_router
    app.include_router(transport_router.router)
    from pacomind.api.routers import followup_plans
    ...
    from pacomind.api.routers import governed_actions as governed_actions_router
    app.include_router(governed_actions_router.router)

    # Channel registration router
    from pacomind.channels.router import router as channels_router
    app.include_router(channels_router)

    # Task queue router (v0.13.0)
    from pacomind.api.routers import task_queue as task_queue_router
    app.include_router(task_queue_router.router)

    # Observations router (v0.16.0) — agent-as-sensor ingestion
    from pacomind.api.routers import observations as observations_router
    app.include_router(observations_router.router)
    from pacomind.api.routers import mining as mining_router
    app.include_router(mining_router.router)

    # Context gate (v0.32.0) — budget-aware context preparation
    from pacomind.api.routers import context_gate as context_gate_router
    ...
    try:
        ...
        logger.debug("MCP SDK not installed — /mcp endpoint not available (install pacomind[mcp])")
    except Exception as exc:
