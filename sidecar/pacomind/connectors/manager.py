...
Mode (PACOMIND_CONNECTORS_MODE, default off):
  off    -> no polling.
...
(PACOMIND_CONNECTOR_<NAME>_*). Default off; enable per connector.
"""
...
from pacomind.connectors.base import Connector, Observation
...
def connectors_enabled() -> bool:
    return os.environ.get(
        "PACOMIND_CONNECTORS_ENABLED", "false").strip().lower() in (
            "1", "true", "yes", "on")


def connectors_mode() -> str:
    from pacomind.util.autonomy_preset import resolve
    return resolve("PACOMIND_CONNECTORS_MODE", ("off", "shadow", "live"), "off")
...
class ConnectorManager:
    ...
    def register_default_connectors(self) -> int:
        ...
        PACOMIND_CONNECTOR_<NAME>_ACCOUNTS or secret connector/<name>/accounts,
        ...
        from pacomind.connectors.caldav_calendar import CalendarConnector
        from pacomind.connectors.fs_documents import FSDocumentsConnector
        from pacomind.connectors.webhook_pull import WebhookPullConnector
