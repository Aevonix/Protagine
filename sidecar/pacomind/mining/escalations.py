...
1. CONSULTATION: the agent shelled out to a build/coding agent during the
   ...
   PACOMIND_ESCALATION_CONSULT_REGEX). The situation was hard enough that the
...
2. PROVIDER ESCALATION: the turn's model (optional per-turn metadata from the
   host) matches PACOMIND_ESCALATION_HEAVY_RE: a heavy-model or cloud-failover
   turn.
...
Modes (PACOMIND_ESCALATION_MINING): shadow banks + journals only; live also
feeds records into skills-memory distillation (domain "escalation"), which
itself honors PACOMIND_SKILLS_DISTILL. Everything is best-effort: a miner
...
from pacomind.mining.models import (
    EscalationRecord,
    MinedTurn,
    consult_regex,
    heavy_model_regex,
...
from pacomind.mining.store import MiningStore
...
def _session_tool_activity(session_id: str, max_lines: int = 400):
    ...
    its tool-activity stream (PACOMIND_TOOL_ACTIVITY_FILE, a JSONL of
    ...
    path = _os.environ.get("PACOMIND_TOOL_ACTIVITY_FILE", "")
    ...
    def _detect(
        ...
        if ran_terminal and pattern:
            try:
                ...
                m = None
                logger.warning("invalid PACOMIND_ESCALATION_CONSULT_REGEX; detector idle")
            if m:
        ...
        if heavy and turn.model:
            try:
                ...
                hm = None
                logger.warning("invalid PACOMIND_ESCALATION_HEAVY_RE; detector idle")
            if hm:
                return EscalationRecord(
