...
NAME = 'PacoMind ordinary skill review'
...
if operation == 'prepare':
    if job:
        pause_job(job['id'], reason='PacoMind installer is refreshing the managed binding')
        ...
        job = create_job(prompt='', schedule=data['schedule'], name=data['name'],
            ...
            paused=True, paused_reason='PacoMind installer is preparing the managed binding')
elif operation == 'pause' and job:
    job = pause_job(job['id'], reason='PacoMind ordinary-skill review is being disabled')
    ...
    try:
        ...
        pause_job(job['id'], reason='PacoMind scheduler registration failed; retry setup')
        raise
...
def launcher(state, manifest):
    ...
    adapter = (state/'adapter'/'pacomind_hermes' if binding['mode'] == 'private-directory'
               else Path(binding['sources']['pacomind_hermes']))
...
def configure(state, *, enabled=True, schedule=None, evaluator_path=None):
    ...
    script = home/'scripts'/('pacomind-skill-review-'+hashlib.sha256(str(state).encode()).hexdigest()[:16]+'.sh')
    ...
    import asyncio
    configuration = Path(manifest.get('model_configuration_path') or state/'.pacomind-llm-config.json')
