...
class RecallSelector:
    ...
    async def rerank(
        ...
        *,
        ...
        PACOMIND_RECALL_RERANK gates it: ``off`` (default) never calls the
        ...
        is inline but hard-capped by PACOMIND_RECALL_RERANK_TIMEOUT_MS
        ...
        """
        mode = os.environ.get("PACOMIND_RECALL_RERANK", "off").strip().lower()
        if mode not in ("shadow", "on"):
            return memories
        min_score = None
        raw_min_score = os.environ.get("PACOMIND_RECALL_RERANK_MIN_SCORE", "").strip()
        if raw_min_score:
            try:
        ...
        if min_score is not None:
            ...
            expected = os.environ.get("PACOMIND_RECALL_RERANK_CALIBRATION", "").strip()
        ...
        try:
            timeout_ms = float(os.environ.get(
                "PACOMIND_RECALL_RERANK_TIMEOUT_MS", "1200"))
        except (TypeError, ValueError):
            timeout_ms = 1200.0
