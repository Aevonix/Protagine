...
Runs gathered evidence through PacoMind's intelligence synthesis layer to
...
class ResearchSynthesizer:
    ...
    Tries to use PacoMind's intelligence synthesis layer when available;
    ...
    async def _extract_insights(
        ...
        # Attempt to use PacoMind's CrossDomainAnalyzer
        try:
            return await self._run_pacomind_synthesis(evidence, goal_text)
        except Exception as exc:
            logger.debug("CrossDomainAnalyzer unavailable (%s), using built-in", exc)
            return self._builtin_extract(evidence, goal_text)

    async def _run_pacomind_synthesis(
        self,
        evidence: EvidencePackage,
        goal_text: str,
    ) -> List[DomainInsight]:
        """Run PacoMind intelligence synthesis layer."""
        from pacomind.intelligence.synthesis.cross_domain_analyzer import CrossDomainAnalyzer
        from pacomind.intelligence.synthesis.novelty_scorer import NoveltyScorer
        from pacomind.intelligence.synthesis.insight_validator import InsightValidator
