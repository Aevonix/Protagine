...
Collects evidence from multiple sources in parallel:
  ...
  - GRAPH    — knowledge graph traversal (PacoMindGraph)
...
class GraphGatherer:
    """Gather evidence from the PacoMind knowledge graph."""
...
class DocumentGatherer:
    ...
    async def gather(
        ...
        if not document_ids:
            return results
        try:
            from pacomind.plugins.document_intelligence.pipeline import (
...
class EmailGatherer:
    ...
    async def gather(
        ...
        results: List[EvidenceItem] = []
        try:
            from pacomind.email.store import EmailStore
