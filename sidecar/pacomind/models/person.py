...
Core identity and relationship tracking for PacoMind's social graph.
Each person has a tier (computed from behavioral signals) and contact info
across multiple channels.

Two-layer model
---------------
PacoMind maintains two distinct layers of relationship data:

- **PacoMind layer** (``score``, ``tier``): PacoMind's own evidence-based assessment,
  derived from interaction signals observed across connected gateways. A score
  of 80 means PacoMind has observed strong, consistent engagement — not that the
  owner designated this person as important.

- **Owner layer** (``notes``, manual tier overrides in TrustTierManager): explicit
  guidance the owner provides to inform PacoMind's initial orientation. Owner input
  carries weight but does not override PacoMind's evidence once sufficient history
  exists. The ``notes`` field is an owner-layer input: free-form context the
  owner wants PacoMind to keep in mind.
"""
...
@dataclass
class Person:
    """A person in PacoMind's social graph.

    Represents anyone the PacoMind owner interacts with. The owner themselves
    ...
    Attributes:
        ...
        is_user: True if this represents the PacoMind owner
    """

    id: str
    name: str
    tier: RelationshipTier = RelationshipTier.PERIPHERAL
    score: float = 0.0  # PacoMind's evidence-based assessment (0–100); not owner-assigned
    aliases: List[str] = field(default_factory=list)
    contacts: List[ContactInfo] = field(default_factory=list)
    first_seen: Optional[datetime] = None
    last_interaction: Optional[datetime] = None
    notes: Optional[str] = None  # Owner-layer input: context the owner wants PacoMind to know
    is_user: bool = False
