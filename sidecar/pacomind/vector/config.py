"""PacoMind Vector Store — configuration dataclasses.
...
class EmbeddingConfig:
    ...
    def from_env(cls) -> Optional[EmbeddingConfig]:
        """Build config from PACOMIND_EMBED_* environment variables.

        Returns ``None`` if the required variables (MODEL, DIMS) are not set.
        """
        model_id = os.environ.get("PACOMIND_EMBED_MODEL")
        dims_str = os.environ.get("PACOMIND_EMBED_DIMS")
        if not model_id or not dims_str:
            return None
        return cls(
            provider=os.environ.get("PACOMIND_EMBED_PROVIDER", "cpu"),
            model_id=model_id,
            dimensions=int(dims_str),
            max_batch_size=int(os.environ.get("PACOMIND_EMBED_BATCH_SIZE", "64")),
            device=os.environ.get("PACOMIND_EMBED_DEVICE") or None,
...
class VectorStoreConfig:
    ...
    def from_env(cls) -> VectorStoreConfig:
        """Build from PACOMIND_VECTOR_* environment variables."""
        pacomind_home = os.path.expanduser(
            os.environ.get("PACOMIND_HOME", os.path.join(os.path.expanduser("~"), ".pacomind"))
        )
        return cls(
            data_dir=os.environ.get(
                "PACOMIND_VECTOR_STORE_PATH",
                os.path.join(pacomind_home, "vector"),
            ),
            cache_size=int(os.environ.get("PACOMIND_EMBED_CACHE_SIZE", "4096")),
            batch_window_ms=float(os.environ.get("PACOMIND_EMBED_BATCH_WINDOW_MS", "5")),
        )
