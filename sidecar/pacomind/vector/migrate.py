"""PacoMind Vector Store — tier migration.
...
async def migrate_tier(
    ...
    from pacomind.vector.backfill import _backfill_collection
...
async def _migrate_generation(store, pipeline, *, graph, old_model_id, batch_size):
    ...
    from pacomind.vector.collections import Collection
    from pacomind.vector.indexes import IncompatibleIndex
...
def _update_env(model_id: str, pipeline) -> None:
    ...
    try:
        ...
        for i, line in enumerate(lines):
            if line.startswith("PACOMIND_EMBED_MODEL="):
                lines[i] = f"PACOMIND_EMBED_MODEL={model_id}"
                updated["model"] = True
            elif line.startswith("PACOMIND_EMBED_DIMS="):
                dims = pipeline.dimensions if hasattr(pipeline, "dimensions") else ""
                lines[i] = f"PACOMIND_EMBED_DIMS={dims}"
                updated["dims"] = True
