"""PacoMind Vector Store — hardware detection, config recommendation, and CLI.
...
Provides:
  ...
  - CLI entry point for ``pacomind vector setup`` and ``pacomind vector rebuild``
"""
...
from pacomind.vector.config import EmbeddingConfig, HardwareProfile, VectorStoreConfig
...
async def ensure_embed_config() -> Optional[EmbeddingConfig]:
    ...
    allowing PacoMind to start without embeddings (graceful degradation).
    """
    ...
    if not sys.stdin.isatty():
        logger.warning(
            "Vector search disabled — no PACOMIND_EMBED_MODEL configured. "
            "Run 'pacomind vector setup' to enable. "
            "Recommended: PACOMIND_EMBED_MODEL=%s PACOMIND_EMBED_DIMS=%d PACOMIND_EMBED_PROVIDER=%s",
    ...
    print(f"  PACOMIND_EMBED_PROVIDER={recommended.provider}")
    print(f"  PACOMIND_EMBED_MODEL={recommended.model_id}")
    print(f"  PACOMIND_EMBED_DIMS={recommended.dimensions}")
    if recommended.quantization:
        print(f"  PACOMIND_EMBED_QUANTIZATION={recommended.quantization}")
    print()
    return None
...
def cli_setup(args: list[str] | None = None) -> None:
    """CLI handler for ``pacomind vector setup``."""
    import argparse

    parser = argparse.ArgumentParser(description="PacoMind vector store setup")
    ...
    print("\nAdd to your .env:")
    print(f"  PACOMIND_EMBED_PROVIDER={config.provider}")
    print(f"  PACOMIND_EMBED_MODEL={config.model_id}")
    print(f"  PACOMIND_EMBED_DIMS={config.dimensions}")
    if config.quantization:
        print(f"  PACOMIND_EMBED_QUANTIZATION={config.quantization}")
...
def cli_rebuild(args: list[str] | None = None) -> None:
    """CLI handler for ``pacomind vector rebuild``."""
    import argparse
    ...
    print("NOTE: Vector search requires PACOMIND_EMBED_MODEL to be set.")
    print("      Run 'pacomind vector setup' first, then use scripts/vector_backfill.py")
    print("      for the full backfill process.")


def cli_main() -> None:
    """Entry point for ``pacomind vector <subcommand>``."""
    if len(sys.argv) < 3:
        print("Usage: pacomind vector {setup|rebuild}")
        sys.exit(1)
