"""PacoMind Vector Store — embedding providers and pipeline.
...
class MLXEmbeddingProvider(EmbeddingProvider):
    ...
    async def warmup(self) -> None:
        ...
        # See: https://github.com/Aevonix/PacoMind/issues/17
...
class EmbeddingPipeline:
    ...
    def __init__(
        ...
        self.query_instruction = os.environ.get('PACOMIND_EMBED_QUERY_INSTRUCTION',
    ...
    def index_identity(self):
        from pacomind.vector.indexes import EmbeddingIdentity
    ...
    def _check_identity(self):
        if self._index_identity is not None:
            from pacomind.vector.indexes import EmbeddingIdentity, IncompatibleIndex
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            t0 = time.monotonic()
            vector = await self._provider.embed("pacomind health check")
    ...
    async def embed(self, text: str) -> list[float]:
        ...
        if elapsed_ms > self.LATENCY_WARN_MS:
            logger.warning(
                ...
                "check PACOMIND_EMBED_PROVIDER and model config",
                elapsed_ms,
    ...
    async def embed_image(self, source: str | bytes, mime_type: str = "", caption: str = "") -> tuple[list[float], dict[str, Any]]:
        ...
        if not self._multimodal_enabled or not self._multimodal_provider:
            raise ValueError("Multimodal not enabled. Set PACOMIND_MULTIMODAL=true and restart.")

        from pacomind.vector.multimodal_types import EmbedInput, Modality
        from pacomind.vector.image_preprocess import (
            compute_image_hash, extract_exif, load_image, resize_image,
            strip_gps_exif, validate_image,
        )
        from pacomind.vector.safety_image import check_image, ImageCheckLevel
        ...
        # Safety check
        check_level = ImageCheckLevel(os.environ.get("PACOMIND_IMAGE_CHECK", "basic"))
        ...
        strip_gps = os.environ.get("PACOMIND_STRIP_EXIF_GPS", "true").lower() == "true"
        ...
        if not caption:
            from pacomind.vector.caption import caption_image
    ...
    @property
    def embed_fn(self):
        """Drop-in callable for PacoMindGraph.set_embed_fn()."""
        return self.embed
