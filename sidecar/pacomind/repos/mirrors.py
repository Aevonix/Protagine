"""Read-only local repo mirrors for PacoMind's own research (best-of-B).

Owner-designated repos (env/config only, never hardcoded) are cloned/pulled
into a local mirror directory. PacoMind gets low-latency read access -- list,
...
Config: PACOMIND_REPO_MIRRORS = "name=url[|alias1|alias2][,name2=url2...]"
...
def parse_mirror_config(raw: str) -> Dict[str, Dict[str, str]]:
    """Parse PACOMIND_REPO_MIRRORS into {name: {url, aliases}}."""
...
class RepoMirrorManager:
    def __init__(
        ...
        self._config = config if config is not None else parse_mirror_config(
            os.environ.get("PACOMIND_REPO_MIRRORS", ""))
    ...
    async def register_entities(self, world_store: Any) -> int:
        ...
        n = 0
        try:
            from pacomind.world_model.entities import ProjectEntity
