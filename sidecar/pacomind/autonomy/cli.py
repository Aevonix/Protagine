"""CLI entry point for PacoMind autonomy loop management.

Usage:
    pacomind autonomy status     # live loop stats from the running sidecar
    pacomind autonomy cycle      # wake the loop / run one cycle now
...
sidecar over HTTP, resolved the same way the doctor does (PACOMIND_URL /
PACOMIND_SIDECAR_HOST/PORT, PACOMIND_API_KEY).
"""
...
def _request(path: str, method: str = "GET") -> dict:
    from pacomind.doctor import default_pacomind_url

    url = default_pacomind_url().rstrip("/") + path
...
def run_autonomy_command(args: list) -> int:
    """Dispatch 'pacomind autonomy <subcommand>'."""
    if not args:
        print("Usage: pacomind autonomy <status|cycle>", file=sys.stderr)
        return 1
    ...
    if sub == "start":
        ...
        print("The autonomy loop runs inside the sidecar and starts with it "
              "(pacomind serve). Use 'pacomind autonomy status' to inspect it or "
              "'pacomind autonomy cycle' to wake it.", file=sys.stderr)
        return 1
