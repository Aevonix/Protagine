...
class AutonomyConfig:
    """Configuration for the PacoMind autonomy loop.
    ...
    # Where the mode came from: "env" (explicit PACOMIND_AUTONOMY_MODE),
    # "preset" (PACOMIND_AUTONOMY_PRESET via preset-loop coupling, H4.1),
    ...
    @classmethod
    def from_pacomind_config(cls, pacomind_cfg: object) -> "AutonomyConfig":
        """Construct config from a PacoMind config object or dict.

        Looks for an ``autonomy`` sub-key/attribute. If not found, returns
        defaults. Supports both dict-like (``pacomind_cfg["autonomy"]``) and
        object-like (``pacomind_cfg.autonomy``) configs.

        Args:
            pacomind_cfg: PacoMind config object/dict, or None for defaults.
        """
        if pacomind_cfg is None:
            return cls()

        # Extract the autonomy sub-section
        autonomy_section = None
        if isinstance(pacomind_cfg, dict):
            autonomy_section = pacomind_cfg.get("autonomy")
        else:
            autonomy_section = getattr(pacomind_cfg, "autonomy", None)
    ...
    def from_env(cls) -> "AutonomyConfig":
        ...
        Environment variables:
            PACOMIND_AUTONOMY_MODE
            PACOMIND_AUTONOMY_PHASES (comma-separated existing phase names)
            PACOMIND_AUTONOMY_PROPOSALS_ONLY
            PACOMIND_PRESET_LOOP_COUPLING (default on: an active
                PACOMIND_AUTONOMY_PRESET supplies the mode when
                PACOMIND_AUTONOMY_MODE is unset)
            PACOMIND_TIMEZONE
            PACOMIND_AUTONOMY_TICK_INTERVAL_SECS
            PACOMIND_AUTONOMY_INITIATIVE_CONFIDENCE_THRESHOLD
            PACOMIND_AUTONOMY_MAX_ACTIONS_PER_HOUR
            PACOMIND_AUTONOMY_QUIET_HOURS_START
            PACOMIND_AUTONOMY_QUIET_HOURS_END
            PACOMIND_AUTONOMY_ANOMALY_SEVERITY_THRESHOLD
            PACOMIND_AUTONOMY_PREDICTION_CONFIDENCE_THRESHOLD
            PACOMIND_AUTONOMY_GOAL_STALE_THRESHOLD_HOURS
            PACOMIND_OWNER_CHECK_IN_ENABLED
            PACOMIND_OWNER_CHECK_IN_SILENT_HOURS
            PACOMIND_OWNER_CHECK_IN_COOLDOWN_HOURS
            PACOMIND_OWNER_CONTACT_ID
            PACOMIND_PROACTIVE_DELIVERY_ENABLED
            PACOMIND_DELIVERY_SHADOW
            PACOMIND_CONVERSATION_SYNTHESIS_ENABLED
            PACOMIND_CONVERSATION_SYNTHESIS_INTERVAL_SECS
            PACOMIND_CONVERSATION_SYNTHESIS_LOOKBACK_HOURS
            PACOMIND_CONVERSATION_SYNTHESIS_MIN_CONFIDENCE
        """
        logger = logging.getLogger(__name__)

        # Mode selection. Precedence (H4.1):
        #   explicit PACOMIND_AUTONOMY_MODE  >  coupled preset mode
        #   >  legacy tick-interval migration  >  default (reactive)
        # Explicit env ALWAYS wins — PACOMIND_AUTONOMY_MODE=reactive is the
        ...
        mode_source = "default"
        raw_mode = os.environ.get("PACOMIND_AUTONOMY_MODE")
        ...
        # Timezone
        timezone = os.environ.get("PACOMIND_TIMEZONE", "UTC")
        try:
            ZoneInfo(timezone)
        except Exception:
            logger.warning("Invalid PACOMIND_TIMEZONE '%s', falling back to UTC", timezone)
            timezone = "UTC"

        # Legacy migration: if tick interval set without mode (and no coupled
        # preset supplied one), assume proactive
        legacy_tick = os.environ.get("PACOMIND_AUTONOMY_TICK_INTERVAL_SECS")
        if legacy_tick and mode_source == "default":
            logger.warning(
                "PACOMIND_AUTONOMY_TICK_INTERVAL_SECS set without PACOMIND_AUTONOMY_MODE. "
                "Defaulting to PROACTIVE mode to preserve existing behavior. "
                "Add PACOMIND_AUTONOMY_MODE=proactive to make this explicit."
            )
        ...
        return cls(
            ...
            enabled_phases=_enabled_phases(os.environ.get("PACOMIND_AUTONOMY_PHASES")),
            proposals_only=_bool("PACOMIND_AUTONOMY_PROPOSALS_ONLY", False),
            timezone=timezone,
            ...
            bootstrap_check_interval_hours=_int(
                "PACOMIND_AUTONOMY_BOOTSTRAP_CHECK_INTERVAL_HOURS",
                defaults.bootstrap_check_interval_hours,
            ),
            self_reflection_interval_days=_int(
                "PACOMIND_AUTONOMY_SELF_REFLECTION_INTERVAL_DAYS",
                defaults.self_reflection_interval_days,
            ),
