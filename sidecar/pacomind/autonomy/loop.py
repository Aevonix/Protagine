"""AutonomyLoop — PacoMind's continuous operating cycle.
...
def _get_broadcast():
    ...
    if _broadcast is None:
        try:
            from pacomind.api.routers.host import broadcast_event
...
def _proactive_enforce_verdict_error(
    ...
    from pacomind.gate.surface_policy import ResponseGuardSurfacePolicyV1
...
class AutonomyLoop:
    """PacoMind's continuous operating cycle.
    ...
    Each tick:
      ...
      4. Run initiative engine — PacoMind decides whether to act
    ...
    def _start_governed_delivery_reconciler(self) -> None:
        ...
        if task is None or task.done():
            self._governed_reconcile_task = asyncio.create_task(
                ...
                name="pacomind-governed-delivery-reconciliation",
            )
    ...
    async def _tick_phases(self) -> None:
        ...
        # Phase 11d2: tom2 knowledge asymmetry (daily; PACOMIND_TOM2, default off)
    ...
    async def _phase_telemetry(self) -> None:
        try:
            from pacomind.api.routers.host import _telemetry
        ...
        try:
            ...
            initiatives = await engine.generate(
                ...
                cooldown_tasks=float(os.environ.get(
                    "PACOMIND_INITIATIVE_COOLDOWN_TASKS", "12",
                )),
                cooldown_contacts=float(os.environ.get(
                    "PACOMIND_INITIATIVE_COOLDOWN_CONTACTS", "72",
                )),
            )
    ...
    async def _phase_thinking(self) -> None:
        ...
        On a slow cadence (PACOMIND_THINKING_INTERVAL_SECS), hand the LLM a
        ...
        Disabled unless PACOMIND_ENABLE_INTERNAL_THINKING=true.
        """
        # Mode: off | shadow | live (PACOMIND_THINKING_MODE). Back-compat:
        # PACOMIND_ENABLE_INTERNAL_THINKING=true means "live". The autonomy
        # preset fills the unset case (explicit env always wins).
        mode = os.environ.get("PACOMIND_THINKING_MODE", "").strip().lower()
        if mode not in ("off", "shadow", "live"):
            if os.environ.get(
                    "PACOMIND_ENABLE_INTERNAL_THINKING", "false").lower() == "true":
                mode = "live"
            else:
                from pacomind.util.autonomy_preset import resolve
                mode = resolve("PACOMIND_THINKING_MODE",
                               ("off", "shadow", "live"), "off")
        if mode == "off":
            return
        ...
        try:
            ...
            from pacomind.proposals import build_from_thinker, proposal_to_payload
    ...
    def _build_thinking_situation(self) -> dict:
        ...
        try:
            from pacomind.api.routers.observations import (
    ...
    async def _recipient_is_owner(self, person_id: str, payload: dict) -> bool:
        ...
        if not person_id or person_id == "owner":
            return True
        try:
            from pacomind.identity.resolver import get_identity_resolver
    ...
    async def _route_reachout_delivery(self, payload: dict, delivery: Any) -> bool:
        ...
        """
        from pacomind.delivery.classification import is_reachout
        ...
        # logs, in enforce (PACOMIND_GUARD_MODE=enforce) it blocks a leaking
        ...
        # gate above.
        guard_enforcing = (
            os.environ.get("PACOMIND_GUARD_MODE", "").strip().lower()
            == "enforce"
        )
        try:
            from pacomind.api.routers.host import _response_guard
    ...
    def _get_rejection_store(self) -> Any:
        ...
        if not hasattr(self, "_rejection_store"):
            ...
            try:
                ...
                from pacomind.gate.rejection import RejectionStore
                state_dir = Path(os.environ.get(
                    "PACOMIND_STATE_DIR", os.path.expanduser("~/.pacomind")))
                self._rejection_store = RejectionStore(
                    str(state_dir / "pacomind-gate-rejections.db"))
            except Exception:
    ...
    async def _run_rejection_feedback(self, text: str, guard_result: Any, *,
        ...
        from pacomind.api.routers.host import _response_guard
        from pacomind.gate.rejection import RejectionFeedbackLoop
        if _response_guard is None:
            return None
    ...
    async def _phase_observation_sync(self) -> None:
        ...
        The agent is PacoMind's sensor array: when a domain's newest
        ...
        snapshots back to /v1/host/observations. PacoMind never calls
        ...
        if task_queue is None:
            return
        try:
            from pacomind.api.routers.observations import get_observation_store
            obs_store = get_observation_store()
        except Exception:
            obs_store = None
        ...
        from pacomind.observations.store import OBSERVATION_SYNC_INTERVALS

        enabled = os.environ.get(
            "PACOMIND_SYNC_DOMAINS",
                ...
                await task_queue.submit(
                    ...
                    params={
                        ...
                        "description": (
                            ...
                            f"connections and report snapshots to PacoMind"
                        ),
    ...
    async def _post_agent_action_to_queue(
        ...
        legacy_auto_approve_requested = (
            os.environ.get("PACOMIND_AGENT_AUTO_APPROVE", "false").lower()
            == "true"
        )
        ...
        if legacy_auto_approve_requested and is_gated:
            logger.warning(
                "Ignoring retired PACOMIND_AGENT_AUTO_APPROVE for effectful %s",
                action_hint,
            )
        ...
        try:
            ...
            if bounded_grant is not None and job_id:
                ...
                try:
                    from pacomind.events.broadcaster import emit as broadcast
            ...
            if gate_pending and job_id:
                ...
                if delivery and hasattr(delivery, "push_initiative"):
                    ...
                    # gates PacoMind's *own* proactive outreach, not the owner's need to
                    ...
                    await delivery.push_initiative({
                        ...
                        "description": getattr(initiative, "description", ""),
                        "rationale": getattr(initiative, "rationale", ""),
                        "suggested_action": "pacomind_approve_initiative",
                        "entity_id": getattr(initiative, "entity_id", None),
                        "channel_hint": "dm",
                        "context": {
                            "job_id": job_id,
    ...
    async def _feed_introduction_candidates(self, engine: Any) -> None:
        ...
        PACOMIND_INTROS_ENABLED=false; trust floor via PACOMIND_INTRO_TRUST_FLOOR.
        """
        if os.environ.get("PACOMIND_INTROS_ENABLED", "true").lower() != "true":
            return
        ...
        try:
            floor = os.environ.get("PACOMIND_INTRO_TRUST_FLOOR", "regular")
    ...
    async def _phase_job_writeback(self) -> None:
        ...
        was invisible to memory — PacoMind could act but never learn from
    ...
    async def _writeback_one_job(self, job: Any) -> None:
        ...
        try:
            from pacomind.task_queue.governor import job_declares_effect
        ...
        if graph is not None and hasattr(graph, "store_memory"):
            ...
            await graph.store_memory(
                ...
                source_uri=f"pacomind://jobs/{job.job_id}",
            )
        ...
        # 4. Skill capture (v0.17.0, PACOMIND_ENABLE_SKILL_SYNTHESIS) — feed
    ...
    def _get_skill_learning(self) -> Any:
        ...
        if os.environ.get("PACOMIND_ENABLE_SKILL_SYNTHESIS",
                          "false").lower() != "true":
            return None
        ...
        try:
            ...
            from pacomind.skills.learning import (
                NoveltyDetector,
                PatternExtractor,
                SkillLearningService,
            )
            from pacomind.skills.packager import SkillPackager

            library = pathlib.Path(
                os.environ.get("PACOMIND_SKILL_LIBRARY")
                or os.path.join(os.environ.get("PACOMIND_STATE_DIR", "."),
                                "skill_library"))
            packager = SkillPackager(
                registry=skills_registry,
                pacomind_id=os.environ.get("PACOMIND_NODE_ID", "pacomind"),
    ...
    async def _phase_project_result_reconciliation(self) -> None:
        ...
        try:
            raw_limit = int(os.environ.get(
                "PACOMIND_PROJECT_RECONCILIATION_LIMIT", "25",
            ))
        except (TypeError, ValueError):
            raw_limit = 25
        ...
        try:
            raw_budget = float(os.environ.get(
                "PACOMIND_PROJECT_RECONCILIATION_BUDGET_SECS", "5",
            ))
        except (TypeError, ValueError):
            raw_budget = 5.0
    ...
    async def _phase_trust_notices(self) -> None:
        ...
        if delivery is None:
            return
        try:
            from pacomind.proposals import Proposal, proposal_to_payload
        except Exception:
            return
    ...
    async def _phase_selfhood_benchmark(self) -> None:
        ...
        try:
            ...
            logger.error("Phase selfhood_benchmark error: %s", exc,
                         exc_info=True)
            return
        if os.environ.get("PACOMIND_BENCHMARK_REPORT",
    ...
    def _phase_budget_secs(self) -> float:
        try:
            return float(os.environ.get("PACOMIND_PHASE_BUDGET_SECS", "150"))
        except ValueError:
            return 150.0
    ...
    def _tick_budget_secs(self) -> float:
        ...
        try:
            v = float(os.environ.get("PACOMIND_TICK_BUDGET_SECS", "0") or 0)
        except ValueError:
            v = 0.0
        if v > 0:
            return v
    ...
    async def _toolsmith_body(self) -> None:
        ...
        try:
            from pacomind.toolsmith.registry import ToolStatus
    ...
    async def _phase_relationship_profiling(self) -> None:
        ...
        try:
            from pacomind.api.routers.host import _relationship_profiler
        except ImportError:
            return
    ...
    async def _phase_tom2_asymmetry(self) -> None:
        ...
        PACOMIND_TOM2 is set (default off; shadow computes counts only, live
    ...
    async def _phase_connectors(self) -> None:
        ...
        if manager is None:
            return
        try:
            from pacomind.connectors import connectors_mode
    ...
    async def _phase_condition_checks(self) -> None:
        ...
        if self._periodic_last.get("condition_checks") != key:
            ...
            from pacomind.autonomy.condition_worker import (
    ...
    async def _phase_memory_pruning(self) -> None:
        ...
        PACOMIND_MEMORY_PRUNE_MODE gates the phase: ``off`` disables it,
        ...
        """
        mode = os.environ.get(
            "PACOMIND_MEMORY_PRUNE_MODE", "shadow").strip().lower()
        if mode == "off":
            return
    ...
    async def _phase_memory_distillation(self) -> None:
        ...
        async def work(graph):
            try:
                from pacomind.intelligence.graph.distiller import MemoryDistiller
    ...
    async def _phase_approval_timeout(self) -> None:
        ...
        PACOMIND_APPROVAL_TIMEOUT_HOURS (default 72) are failed with reason
        ...
        if task_queue is None:
            return
        try:
            timeout_hours = float(os.environ.get("PACOMIND_APPROVAL_TIMEOUT_HOURS", "72"))
