...
async def _check_email_reply(params: dict) -> dict:
    ...
    Credentials are loaded from SecretsManager:
        PACOMIND_IMAP_HOST, PACOMIND_IMAP_PORT, PACOMIND_IMAP_USERNAME, PACOMIND_IMAP_PASSWORD
    ...
    try:
        ...
        from pacomind.email.providers import IMAPProvider
    except ImportError:
    ...
    sm = SecretsManager()
    host = sm.get_required("PACOMIND_IMAP_HOST")
    port = int(sm.get("PACOMIND_IMAP_PORT") or "993")
    username = sm.get_required("PACOMIND_IMAP_USERNAME")
    password = sm.get_required("PACOMIND_IMAP_PASSWORD")
...
async def _check_delivery_status(params: dict) -> dict:
    ...
    Carrier API keys are loaded from SecretsManager:
        PACOMIND_UPS_API_KEY, PACOMIND_FEDEX_API_KEY
    ...
    try:
        ...
        raise RuntimeError(
            "_check_delivery_status requires pacomind.secrets"
        ) from exc
...
async def _check_commitment_overdue(params: dict) -> dict:
    ...
    from pacomind.api.routers.host import _commitment_store, emit
...
async def _check_surprise_accumulation(params: dict) -> dict:
    ...
    from pacomind.api.routers.host import _surprise_store
