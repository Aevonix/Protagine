...
PacoMind-native tools are defined in tools/definitions.py and handlers
are in tools/handlers.py. These tools provide direct access to
PacoMind's intelligence systems (memory, goals, relationships, etc.)
...
from pacomind.tools.definitions import STATIC_TOOL_NAMES, get_tool_definitions
from pacomind.tools.handlers import TOOL_HANDLERS
from pacomind.reasoning.tool_policy import (
    ToolActorPolicy,
    ToolEffect,
    action_target,
    actor_allows_effect,
...
class ToolExecutor:
    ...
    handlers :
        ...
        a string result.
    registry :
        Optional SubsystemRegistry for PacoMind-native tools. When provided,
        PacoMind tools are automatically available.
    """
    ...
    def __init__(
        ...
        # Auto-register PacoMind-native tool handlers if registry is provided
    ...
    def register_native_tools(self, search_orchestrator=None, sandbox_dir: str = "") -> None:
        """Register PacoMind-native tools that run inside the sidecar.
        ...
        if sandbox_dir:
            try:
                from pacomind.reasoning.native_tools.file_ops import (
    ...
    def get_definitions(self, available_tools: list[str] | None = None) -> list[dict[str, Any]]:
        ...
        Returns PacoMind-native tool definitions that can be executed server-side.
        ...
        for tc in tool_calls:
            ...
            try:
                if handler is self._canonical_memory_handler:
                    result = await TOOL_HANDLERS['pacomind_memory_search'](
