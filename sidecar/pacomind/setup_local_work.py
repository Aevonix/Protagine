...
def verify_tools(endpoint, model, key, *, extra_body=None):
    ...
    if len(calls) != 1 or calls[0].get('function', {}).get('name') != 'pacomind_setup_echo':
    ...
    if json.loads(calls[0]['function']['arguments']) != {'token':'pacomind-ready'}:
...
def model_configuration(state, *, configuration_path=None):
    ...
    path = Path(configuration_path) if configuration_path else state/'.pacomind-llm-config.json'
    ...
    for index, entry in enumerate([options, *(options['fallback_model'] or [])]):
        name = 'pacomind-planning-'+str(index)
...
def worker_configuration(model, policy, plugin, lane):
    ...
    config = {**model, 'agent':{'max_turns':12},
        'toolsets':['pacomind_local_work','kanban'], 'platform_toolsets':{'cli':['pacomind_local_work','kanban']},
        'plugins':{'enabled':['pacomind'], 'pacomind':worker_plugin,
...
def install(state):
    ...
    if manifest.get('version') != 1 or manifest.get('profile') != 'local':
        raise ValueError('Expected a local PacoMind instance')
    adapter = manifest['adapter_binding']
    adapter_root = (state/'adapter/pacomind_hermes' if adapter['mode'] == 'private-directory'
                    else Path(adapter['sources'].get('pacomind_hermes') or adapter['sources']['pacomind_hermes']))
    ...
    if worker.exists() and not owned:
        raise ValueError('The pacomind-drafts profile already exists; retain or reconcile its binding')
    from dotenv import dotenv_values
    secret = dotenv_values(home/'.env').get('PACOMIND_NATIVE_API_KEY')
    if not secret:
    ...
    _atomic_hermes_config_write(env_path, env_before,
        (('PACOMIND_NATIVE_API_KEY' if plugin.get('api_key') == '${PACOMIND_NATIVE_API_KEY}' else 'PACOMIND_NATIVE_API_KEY')
    ...
    if manifest['adapter_binding']['mode'] == 'private-directory':
        (worker/'plugins/pacomind').mkdir(parents=True, exist_ok=True, mode=0o700)
        write_owned(worker/'plugins/pacomind/__init__.py', _forwarder(state/'adapter', 'pacomind_hermes'))
        write_owned(worker/'plugins/pacomind/plugin.yaml', (state/'adapter/pacomind_hermes/plugin.yaml').read_bytes())
    write_owned(worker/'SOUL.md', 'You execute one accepted local draft as part of the same PacoMind agent.\n'
    ...
    lines = [line for line in env_before.decode().splitlines() if not line.startswith(('PACOMIND_LOCAL_WORK_ENABLED=', 'PACOMIND_LOCAL_WORK_EXECUTOR=', 'PACOMIND_LOCAL_WORK_BOARD=', 'PACOMIND_LOCAL_WORK_PROFILE='))]
    lines += ['PACOMIND_LOCAL_WORK_ENABLED=true', 'PACOMIND_LOCAL_WORK_EXECUTOR=kanban',
              'PACOMIND_LOCAL_WORK_BOARD='+board, 'PACOMIND_LOCAL_WORK_PROFILE='+profile]
    _atomic_hermes_config_write(env_path, env_before, ('\n'.join(lines)+'\n').encode())
