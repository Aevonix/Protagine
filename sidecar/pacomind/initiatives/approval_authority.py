"""Durable, transport-neutral authority for PacoMind action approvals.

PacoMind owns approval *state* but deliberately knows nothing about WhatsApp,
...
SQLite transactions make decision and grant consumption atomic across PacoMind
workers.  The database lives in ``PACOMIND_STATE_DIR`` and is safe to copy with
the rest of PacoMind state for rollback.
"""
...
GRANT_MAX_TTL_ENV = "PACOMIND_GRANT_MAX_TTL_SECONDS"
GRANT_MAX_USES_ENV = "PACOMIND_GRANT_MAX_USES"
GRANT_UNLIMITED_SENTINEL = "unlimited"
PRESENTATION_SCHEMA = "PacoMindApprovalPresentationV1"
AUTHORIZATION_PROJECTION_SCHEMA = "PacoMindApprovalAuthorizationProjectionV1"
TYPED_APPROVAL_SUBJECT_SCHEMA = "PacoMindApprovalSubjectBindingV1"
...
def canonical_approval_timeout_seconds() -> int:
    ...
    raw = os.environ.get("PACOMIND_APPROVAL_TIMEOUT_HOURS")
    if raw in (None, ""):
        return DEFAULT_REQUEST_TTL_SECONDS
    try:
...
def build_action_binding(
    *,
    ...
    try:
        ...
        # than PacoMind's built-in registry. Their exact action/risk/type tuple is
...
def authority_mode() -> str:
    ...
    value = os.environ.get("PACOMIND_APPROVAL_AUTHORITY_MODE", "shadow").strip().lower()
