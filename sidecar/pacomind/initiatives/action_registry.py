...
in this registry, never a raw command string. PacoMind builds initiatives
...
Approval policy (``PACOMIND_APPROVAL_POLICY``, default ``strict``):
...
``PACOMIND_AGENT_AUTO_APPROVE=true`` collapses the gate for trusted
...
# to /v1/host/observations. PacoMind never calls external APIs itself.
...
_SPECS: List[ActionSpec] = [
    ...
    ActionSpec(
        ...
        tool="terminal",
        # The working checkout is deployment-specific: PACOMIND_WORK_REPO names
        # the repo the hygiene check inspects (default: the worker's own cwd).
        command=f"git -C {os.environ.get('PACOMIND_WORK_REPO', '.')} status --short",
        risk=RiskTier.READ_ONLY,
    ...
    ActionSpec(
        ...
        tool="terminal",
        command="pacomind doctor --subsystem $ID",
        risk=RiskTier.READ_ONLY,
        description="Investigate a degraded PacoMind subsystem",
        initiative_type="agent_action",
        required_params=["ID"],
    ),
    ActionSpec(
    ...
    ActionSpec(
        ...
        tool="terminal",
        command="pacomind graph cleanup-orphans $ID",
...
def get_approval_policy() -> str:
    ...
    ``PACOMIND_APPROVAL_POLICY=graduated`` opts a deployment into the
    graduated gate; anything else (including unset/unknown values) is
    ``strict`` — fail closed on typos.
    """
    mode = os.environ.get("PACOMIND_APPROVAL_POLICY", APPROVAL_POLICY_STRICT)
