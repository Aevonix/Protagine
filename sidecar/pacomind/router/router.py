...
Wraps LiteLLM's completion API. All PacoMind code that calls an LLM
...
# PacoMind loads its own explicit configuration.  LiteLLM otherwise asks
...
from pacomind.router.complexity_scorer import ComplexityScorer
from pacomind.router.fallback import FallbackHandler
from pacomind.router.self_learning import RouterSelfLearner
from pacomind.router.tiers import DEFAULT_TIERS, ModelTier, TierConfig
...
class LLMRouter:
    ...
    async def complete(
        ...
        *,
        ...
        stream:
            ...
            dispatch; PacoMind consumers currently request complete responses.
        """
    ...
    async def _call_function(self, snapshot, messages, context, tools, force_tier, stream, request_id):
        ...
        from pacomind.contextgate import estimate_tokens
