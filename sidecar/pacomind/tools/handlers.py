"""Tool handlers for PacoMind-native server-side execution.
...
for calling PacoMind's intelligence systems.
"""
...
async def handle_get_relationship(
    ...
    try:
        ...
        logger.error("pacomind_get_relationship failed: %s", e)
...
async def handle_list_goals(
    ...
    try:
        ...
        logger.error("pacomind_list_goals failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_get_briefing(
    ...
    try:
        ...
        logger.error("pacomind_get_briefing failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_record_insight(
    ...
    try:
        ...
        logger.error("pacomind_record_insight failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_query_entities(
    ...
    try:
        ...
        logger.error("pacomind_query_entities failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_start_research(
    ...
    try:
        ...
        logger.error("pacomind_start_research failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_discover_connections(
    ...
    try:
        ...
        logger.error("pacomind_discover_connections failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_list_boundaries(
    ...
    try:
        ...
        logger.error("pacomind_list_boundaries failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_recent_boundary_blocks(
    ...
    try:
        ...
        logger.error("pacomind_recent_boundary_blocks failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_flag_boundary_concern(
    ...
    try:
        ...
        logger.error("pacomind_flag_boundary_concern failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_task_complete(
    ...
    try:
        ...
        logger.error("pacomind_task_complete failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_task_snooze(
    ...
    try:
        ...
        logger.error("pacomind_task_snooze failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_task_dismiss(
    ...
    try:
        ...
        logger.error("pacomind_task_dismiss failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_initiative_feedback(
    ...
    try:
        ...
        logger.error("pacomind_initiative_feedback failed: %s", e)
        return json.dumps({"error": str(e), "status": "error"})
...
async def handle_recall_skills(
    ...
    try:
        ...
        from pacomind.skills_memory import relevant_skills
...
TOOL_HANDLERS: dict[str, callable] = {
    "pacomind_memory_search": handle_memory_search,
    "pacomind_get_relationship": handle_get_relationship,
    "pacomind_list_goals": handle_list_goals,
    "pacomind_get_briefing": handle_get_briefing,
    "pacomind_record_insight": handle_record_insight,
    "pacomind_query_entities": handle_query_entities,
    "pacomind_start_research": handle_start_research,
    "pacomind_discover_connections": handle_discover_connections,
    "pacomind_task_complete": handle_task_complete,
    "pacomind_task_snooze": handle_task_snooze,
    "pacomind_task_dismiss": handle_task_dismiss,
    "pacomind_initiative_feedback": handle_initiative_feedback,
    "pacomind_list_boundaries": handle_list_boundaries,
    "pacomind_recent_boundary_blocks": handle_recent_boundary_blocks,
    "repo_list_files": handle_repo_list_files,
    "repo_read_file": handle_repo_read_file,
    "repo_search": handle_repo_search,
    "pacomind_flag_boundary_concern": handle_flag_boundary_concern,
