"""PacoMind-native tool definitions for server-side execution.

These tools are advertised to the LLM via the ReasoningLoop and executed
by the ToolExecutor. They provide direct access to PacoMind's intelligence
...
# Core PacoMind tools — memory, relationships, goals
PACOMIND_CORE_TOOLS: list[dict[str, Any]] = [
    {
    ...
    {
        "name": "pacomind_get_relationship",
        "description": (
...
PACOMIND_EXTENDED_TOOLS: list[dict[str, Any]] = [
    {
        "name": "pacomind_query_entities",
        "description": (
            "Query PacoMind's world model for entities (people, places, organizations, concepts). "
    ...
    {
        "name": "pacomind_discover_connections",
        "description": (
    ...
    {
        "name": "pacomind_recent_boundary_blocks",
        "description": (
    ...
    {
        "name": "pacomind_flag_boundary_concern",
        "description": (
...
# All tools combined
PACOMIND_TOOLS: list[dict[str, Any]] = PACOMIND_CORE_TOOLS + PACOMIND_EXTENDED_TOOLS + NATIVE_TOOLS
...
STATIC_TOOL_NAMES = frozenset(str(tool["name"]) for tool in PACOMIND_TOOLS)
...
def get_tool_definitions(
    ...
    """
    tools = PACOMIND_CORE_TOOLS if not include_extended else PACOMIND_TOOLS
