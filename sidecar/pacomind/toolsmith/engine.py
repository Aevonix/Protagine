...
from pacomind.toolsmith.authority import GraduationAuthorityV1
from pacomind.toolsmith.integrity import (
    MAX_CAPTURE_BYTES,
    IntegrityError,
    candidate_digest,
    canonical_json,
...
from pacomind.toolsmith.miner import ToolCandidate, ToolsmithMiner
from pacomind.toolsmith.policy import validate_pure_tool
from pacomind.toolsmith.registry import Tool, ToolRegistry, ToolStatus
...
def toolsmith_enabled() -> bool:
    return os.environ.get(
        "PACOMIND_TOOLSMITH", "off").strip().lower() in ("shadow", "live")


def toolsmith_mode() -> str:
    m = os.environ.get("PACOMIND_TOOLSMITH", "off").strip().lower()
