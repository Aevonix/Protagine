...
from pacomind.contextgate.retrieve import EmbedFn, rank_chunks
...
class GateConfig:
    ...
    def from_env(cls) -> "GateConfig":
        cfg = cls()
        mode = os.environ.get("PACOMIND_CONTEXT_GATE", "").strip().lower()
        if mode in ("auto", "on", "off"):
            cfg.mode = mode
        for attr, env, cast in (
            ("headroom", "PACOMIND_CONTEXT_GATE_HEADROOM", float),
            ("default_budget_tokens", "PACOMIND_CONTEXT_GATE_BUDGET", int),
            ("chunk_tokens", "PACOMIND_CONTEXT_CHUNK_TOKENS", int),
            ("overlap_tokens", "PACOMIND_CONTEXT_OVERLAP_TOKENS", int),
        ):
