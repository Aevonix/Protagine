...
Usage::
    ...
    from pacomind.contextgate import prepare_context
...
- ``PACOMIND_CONTEXT_GATE``            — ``auto`` (default) | ``on`` | ``off``
- ``PACOMIND_CONTEXT_GATE_HEADROOM``   — fraction of budget that triggers
  gating (default 0.8)
- ``PACOMIND_CONTEXT_GATE_BUDGET``     — default token budget when neither
  the caller nor the model tier provides one (default 0 = no gating)
- ``PACOMIND_CONTEXT_CHUNK_TOKENS``    — target chunk size (default 1024)
- ``PACOMIND_CONTEXT_OVERLAP_TOKENS``  — chunk overlap (default 128)
- ``PACOMIND_CONTEXT_CHARS_PER_TOKEN`` — token estimator ratio (default 4.0)
"""

from pacomind.contextgate.estimate import estimate_tokens
from pacomind.contextgate.chunker import Chunk, chunk_text
from pacomind.contextgate.retrieve import rank_chunks
from pacomind.contextgate.gate import (
    GateConfig,
    GateDecision,
    PreparedContext,
    classify_task,
