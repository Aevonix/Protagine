...
def observed_boards():
    ...
    configured = os.environ.get('PACOMIND_HERMES_WORK_BOARDS')
...
def selected_board():
    ...
    if home is not None and home.parent.name == 'profiles':
        home = home.parent.parent
    board = os.environ.get('PACOMIND_LOCAL_WORK_BOARD', 'pacomind-drafts').strip()
    profile = os.environ.get('PACOMIND_LOCAL_WORK_PROFILE', 'pacomind-drafts').strip()
    ...
    with closing(sqlite3.connect(path.as_uri()+'?mode=ro', uri=True,
        ...
        if review:
            ...
            if task is not None and task['assignee'] == 'default' and db.execute(
                ...
                profile = 'default'
        creator = 'pacomind-followup' if followup else 'pacomind-initiative' if review else 'pacomind-local-work'
