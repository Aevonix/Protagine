...
class TurnIdempotencyLedger:
    ...
    def _initialize(self) -> None:
        ...
        with self._init_lock:
            ...
            with closing(self._connect()) as conn, conn:
                ...
                from pacomind.beliefs.source_projection import initialize
                initialize(conn)
                from pacomind.turns.media import initialize as initialize_media
                initialize_media(conn)
                from pacomind.turns.source_vectors import initialize as initialize_vectors
                initialize_vectors(conn)
                from pacomind.self_model.judgments import initialize as initialize_judgments
                initialize_judgments(conn)
                from pacomind.self_model.appraisals import initialize as initialize_appraisals
                initialize_appraisals(conn)
                from pacomind.turns.source_attribution import initialize as initialize_attribution
                initialize_attribution(conn)
                from pacomind.turns.source_annotations import initialize as initialize_annotations
    ...
    def record_source(
        ...
        with closing(self._connect()) as conn, conn:
            ...
            if row:
                ...
                return False
            from pacomind.turns.media import normalize_messages, SourceMedia
                ...
                for row in list(remaining_rows.values()):
                    ...
                    if row['turn_id'] not in erased_ids:
                        self._append_erasure(conn, turn_id=row['turn_id'], contact_id=contact_id,
                            session_id=row['session_id'], scope=row['scope'], messages=messages,
                            removed=[message for message in messages if message not in retained], whole=False)
                    from pacomind.beliefs.source_projection import erase_removed
                    erase_removed(conn, row["turn_id"], row["session_id"], retained)
                    from pacomind.turns.media import erase_removed as erase_media
                    erase_media(conn, row["turn_id"], row["session_id"], retained)
                    from pacomind.self_model.perspective import erase_removed as erase_preferences
                    erase_preferences(conn, row["turn_id"], row["session_id"], retained)
                    from pacomind.self_model.judgments import erase_removed as erase_judgments
                    erase_judgments(conn, row["turn_id"], row["session_id"], retained)
                    from pacomind.self_model.appraisals import erase_removed as erase_appraisals
    ...
    def search_sources(
        ...
        from pacomind.turns.audio import evidence_metadata
