...
def correct(ledger, *, operation_id, performed_by, old_contact_id, contact_id,
    ...
    """
    from pacomind.contacts.identity_links import _refs, _json
    ...
    with closing(ledger._connect()) as conn, conn:
        ...
        from pacomind.beliefs.source_projection import erase_removed as erase_claims, enqueue
        ...
        for sid in descendants:
            ...
            if row:
                ...
                erase_judgments(conn, sid, row['session_id'], [])
        try:
            from pacomind.self_model.appraisals import invalidate_source_attribution
        except ModuleNotFoundError as exc:
            if exc.name != 'pacomind.self_model.appraisals':
                raise
        else:
