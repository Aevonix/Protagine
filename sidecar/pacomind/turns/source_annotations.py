...
def expand(ledger, candidates, *, contact_id, session_id, covered=()):
    ...
    with closing(ledger._connect()) as conn:
        ...
        for original in candidates:
            ...
            # A conversation is indivisible: a surviving reply cannot carry an
            # erased or changed input under a newly rebound source revision.
            if original.get('kind') == 'conversation_pair' and any(
                    source(ref['source_id']) is None or canonical_turn_digest(
                        source(ref['source_id'])['messages']) != ref['source_version']
                    for ref in original.get('_conversation_source_refs', [])):
                continue
