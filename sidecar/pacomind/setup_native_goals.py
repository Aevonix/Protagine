...
def prepare(config, home, *, native_env, observer_env, local_work=False, draft_board=None):
    ...
    configured = observer_env.get('PACOMIND_HERMES_WORK_BOARDS')
    if 'PACOMIND_HERMES_WORK_BOARDS' in observer_env:
        try:
            boards = json.loads(configured)
        except (TypeError, ValueError):
            raise ValueError('Existing PACOMIND_HERMES_WORK_BOARDS must be a JSON board list') from None
        if (not isinstance(boards, list) or not 1 <= len(boards) <= 8
            ...
            raise ValueError('Existing PACOMIND_HERMES_WORK_BOARDS must select one to eight board slugs')
...
def enable(state):
    ...
    if 'PACOMIND_HERMES_WORK_BOARDS' not in observer_env:
        suffix = 'PACOMIND_HERMES_WORK_BOARDS='+json.dumps(details['boards'], separators=(',', ':'))+'\n'
