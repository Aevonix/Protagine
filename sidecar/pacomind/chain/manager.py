...
class ChainManager:
    """Main coordinator for PacoMindChain.
    ...
    async def submit_transaction(self, tx: Transaction) -> SubmitResult:
        ...
        if tx.from_pacomind_id != "system":
            persisted_nonce = self.store.get_nonce(tx.from_pacomind_id)
    ...
    async def handle_tx_submit(
        ...
        """Handle a TX_SUBMIT message from another pacomind."""
    ...
    async def get_chain_state_summary(
        ...
        return ChainStateResponse(
            ...
            n_colonies=len(state.pacomind_registry),
            n_sentinels=len(state.active_sentinels()),
            n_suspended=len(state.suspended_colonies),
            network_id=state.network_id,
            responder_id=responder_id or self.pacomind_id,
        )
    ...
    def get_instance(cls) -> "ChainManager":
        """Return the process-wide singleton backed by ~/.pacomind/data/chain.db."""
        global _chain_manager_instance
        if _chain_manager_instance is None:
            pacomind_home = get_state_dir()
            db_path = pacomind_home / "chain.db"
            # Derive pacomind_id from genesis block cryptographic identity
            pacomind_id = "local"
            if db_path.exists():
                try:
                    ...
                    if row:
                        ...
                        # Use the cryptographic pacomind_id (sha256 of Ed25519 pubkey)
                        # from genesis block metadata — not producer_id which is
                        # just a human-readable label and trivially spoofable.
                        pacomind_id = (
                            genesis_data.get("metadata", {}).get("genesis_pacomind_id")
                            or genesis_data.get("producer_id", "local")
                        )
                    conn.close()
                except Exception:
                    pass
            _chain_manager_instance = cls(db_path=db_path, pacomind_id=pacomind_id)
