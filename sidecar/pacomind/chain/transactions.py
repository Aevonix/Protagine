"""Transaction types and chain state dataclasses for PacoMindChain.
...
class TxType(str, Enum):
    PACOMIND_REGISTER = "pacomind_register"
    PACOMIND_ROTATE_KEY = "pacomind_rotate_key"
    PACOMIND_REVOKE_KEY = "pacomind_revoke_key"
    PACOMIND_RELEASE_NAME = "pacomind_release_name"
    ...
    PACOMIND_SUSPEND = "pacomind_suspend"
    PACOMIND_REINSTATE = "pacomind_reinstate"
...
class Transaction:
    ...
    def create(
        ...
        signing_payload = {
            "tx_id": tx_id,
            "type": tx_type.value,
            "from_pacomind_id": from_pacomind_id,
            "timestamp": ts,
            "nonce": nonce,
            "payload": payload,
        }
    ...
    def signing_bytes(self) -> bytes:
        ...
        payload = {
            ...
            "from_pacomind_id": self.from_pacomind_id,
    ...
    def from_dict(cls, d: dict[str, Any]) -> "Transaction":
        ...
        return cls(
            tx_id=d["tx_id"],
            type=tx_type,
            from_pacomind_id=d["from_pacomind_id"],
...
class ChainState:
    ...
    pacomind_registry: dict[str, PacoMindRecord] = field(default_factory=dict)
    name_registry: dict[str, str] = field(default_factory=dict)  # name -> pacomind_id
    ...
    # Track publish counts per pacomind for rate limiting
    plugin_publish_counts: dict[str, list[str]] = field(default_factory=dict)  # pacomind_id -> list of timestamps

    def pacomind_id_for_name(self, name: str) -> str | None:
        return self.name_registry.get(name.lower())

    def active_key_for_pacomind(self, pacomind_id: str) -> str | None:
        record = self.pacomind_registry.get(pacomind_id)
        if record:
            return record.active_public_key_hex
        return None

    def is_active(self, pacomind_id: str) -> bool:
        record = self.pacomind_registry.get(pacomind_id)
        return record is not None and record.status == "active"

    def is_suspended(self, pacomind_id: str) -> bool:
        return pacomind_id in self.suspended_colonies
