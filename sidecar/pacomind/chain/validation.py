...
class TransactionValidator:
    ...
    def validate(self, tx: Transaction) -> ValidationResult:
        ...
        if not tx.from_pacomind_id:
            return ValidationResult.fail("missing from_pacomind_id")

        # System transactions (auto-generated) skip signature check
        is_system = tx.from_pacomind_id == "system"
    ...
    def _verify_signature(self, tx: Transaction) -> ValidationResult:
        """Verify Ed25519 signature against the pacomind's active key."""
        pacomind = self.state.pacomind_registry.get(tx.from_pacomind_id)

        # For pacomind_register, use the public key from the payload itself
        if tx.type == TxType.PACOMIND_REGISTER:
            pub_key_hex = tx.payload.get("public_key_hex", "")
        elif pacomind is not None:
            pub_key_hex = pacomind.active_public_key_hex
        else:
            return ValidationResult.fail(
                f"pacomind {tx.from_pacomind_id!r} not registered"
            )

        if not pub_key_hex:
            return ValidationResult.fail("no public key for pacomind")
    ...
    def _check_nonce(self, tx: Transaction) -> ValidationResult:
        """Verify nonce is strictly greater than pacomind's last accepted nonce.

        The authoritative nonce check is enforced by ChainManager.submit_transaction()
        via ChainStore.get_nonce() + DB UNIQUE(pacomind_id, nonce). This method is a
        ...
        last_nonce = self._chain_store.get_nonce(tx.from_pacomind_id)
    ...
    def _validate_by_type(self, tx: Transaction) -> ValidationResult:
        dispatch = {
            TxType.PACOMIND_REGISTER: self._validate_pacomind_register,
            TxType.PACOMIND_ROTATE_KEY: self._validate_pacomind_rotate_key,
            TxType.PACOMIND_REVOKE_KEY: self._validate_pacomind_revoke_key,
            TxType.PACOMIND_RELEASE_NAME: self._validate_pacomind_release_name,
            ...
            TxType.PACOMIND_SUSPEND: self._validate_pacomind_suspend,
            TxType.PACOMIND_REINSTATE: self._validate_pacomind_reinstate,
    ...
    def _validate_pacomind_register(self, tx: Transaction) -> ValidationResult:
        p = tx.payload
        name = p.get("name", "").lower()
        pacomind_id = p.get("pacomind_id", "")
        public_key_hex = p.get("public_key_hex", "")

        if not name:
            return ValidationResult.fail("pacomind_register: missing name")
        if not _NAME_PATTERN.match(name):
            return ValidationResult.fail(
                f"pacomind_register: name {name!r} does not match required pattern"
            )
        if name in self.state.name_registry:
            return ValidationResult.fail(
                f"pacomind_register: name {name!r} already registered"
            )
        if pacomind_id in self.state.pacomind_registry:
            return ValidationResult.fail(
                f"pacomind_register: pacomind_id {pacomind_id!r} already registered"
            )
        # Verify pacomind_id = sha256(pubkey)
        try:
            expected_id = hashlib.sha256(bytes.fromhex(public_key_hex)).hexdigest()
        except ValueError:
            return ValidationResult.fail("pacomind_register: invalid public_key_hex")
        if expected_id != pacomind_id:
            return ValidationResult.fail(
                "pacomind_register: pacomind_id does not match sha256(public_key)"
            )
        # Genesis admin can only appear once (in genesis block)
        if p.get("genesis_admin", False) and self.state.genesis_admin_id:
            return ValidationResult.fail(
                "pacomind_register: genesis_admin=true only allowed in genesis block"
            )
        return ValidationResult.success()

    def _validate_pacomind_rotate_key(self, tx: Transaction) -> ValidationResult:
        pacomind = self.state.pacomind_registry.get(tx.from_pacomind_id)
        if pacomind is None:
            return ValidationResult.fail("pacomind_rotate_key: pacomind not registered")
        if pacomind.status == "suspended":
            return ValidationResult.fail("pacomind_rotate_key: pacomind is suspended")
        p = tx.payload
        new_key = p.get("new_public_key_hex", "")
        if not new_key:
            return ValidationResult.fail("pacomind_rotate_key: missing new_public_key_hex")
        # New key must not be registered to another pacomind
        for cid, record in self.state.pacomind_registry.items():
            if cid != tx.from_pacomind_id and record.active_public_key_hex == new_key:
                return ValidationResult.fail(
                    "pacomind_rotate_key: new key already registered to another pacomind"
                )
        return ValidationResult.success()

    def _validate_pacomind_revoke_key(self, tx: Transaction) -> ValidationResult:
        p = tx.payload
        revoked_key = p.get("revoked_key_hex", "")
        if not revoked_key:
            return ValidationResult.fail("pacomind_revoke_key: missing revoked_key_hex")
        # Key must be in history of the target pacomind
        # from_pacomind_id is either the pacomind itself, or "system"/"genesis"
        target_id = tx.from_pacomind_id
        history = self.state.key_history.get(target_id, [])
        if not any(e.public_key_hex == revoked_key for e in history):
            return ValidationResult.fail(
                "pacomind_revoke_key: key not found in pacomind key history"
            )
        return ValidationResult.success()

    def _validate_pacomind_release_name(self, tx: Transaction) -> ValidationResult:
        p = tx.payload
        name = p.get("name", "").lower()
        owner_id = self.state.name_registry.get(name)
        if owner_id is None:
            return ValidationResult.fail(
                f"pacomind_release_name: name {name!r} not registered"
            )
        is_genesis = tx.from_pacomind_id == self.state.genesis_admin_id
        if owner_id != tx.from_pacomind_id and not is_genesis:
            return ValidationResult.fail(
                "pacomind_release_name: not authorized (not owner or genesis admin)"
            )
        return ValidationResult.success()

    def _validate_trust_attest(self, tx: Transaction) -> ValidationResult:
        p = tx.payload
        target_id = p.get("target_pacomind_id", "")
        if not self.state.is_active(tx.from_pacomind_id):
            return ValidationResult.fail("trust_attest: attesting pacomind not active")
        if not self.state.is_active(target_id):
            return ValidationResult.fail("trust_attest: target pacomind not active")
    ...
    def _validate_untrust_attest(self, tx: Transaction) -> ValidationResult:
        p = tx.payload
        target_id = p.get("target_pacomind_id", "")
        if not self.state.pacomind_registry.get(tx.from_pacomind_id):
            return ValidationResult.fail("untrust_attest: attesting pacomind not registered")
        if not self.state.pacomind_registry.get(target_id):
            return ValidationResult.fail("untrust_attest: target pacomind not registered")
        return ValidationResult.success()

    def _validate_sentinel_register(self, tx: Transaction) -> ValidationResult:
        pacomind = self.state.pacomind_registry.get(tx.from_pacomind_id)
        if pacomind is None:
            return ValidationResult.fail("sentinel_register: pacomind not registered")
        if pacomind.status == "suspended":
            return ValidationResult.fail("sentinel_register: pacomind is suspended")
        p = tx.payload
    ...
    def _validate_sentinel_deregister(self, tx: Transaction) -> ValidationResult:
        ...
        is_genesis = tx.from_pacomind_id == self.state.genesis_admin_id
        is_system = tx.from_pacomind_id == "system"
        if record.pacomind_id != tx.from_pacomind_id and not is_genesis and not is_system:
            return ValidationResult.fail(
                "sentinel_deregister: not authorized (not owner or genesis admin)"
            )
        return ValidationResult.success()

    def _validate_pacomind_suspend(self, tx: Transaction) -> ValidationResult:
        p = tx.payload
        target_id = p.get("target_pacomind_id", "")
        if not target_id:
            return ValidationResult.fail("pacomind_suspend: missing target_pacomind_id")
        if target_id not in self.state.pacomind_registry:
            return ValidationResult.fail("pacomind_suspend: target pacomind not registered")
        if self.state.is_suspended(target_id):
            return ValidationResult.fail("pacomind_suspend: pacomind already suspended")
        is_genesis = tx.from_pacomind_id == self.state.genesis_admin_id
        is_system = tx.from_pacomind_id == "system"
        if not is_genesis and not is_system:
            return ValidationResult.fail(
                "pacomind_suspend: only genesis admin or system may suspend"
            )
        return ValidationResult.success()

    def _validate_pacomind_reinstate(self, tx: Transaction) -> ValidationResult:
        p = tx.payload
        target_id = p.get("target_pacomind_id", "")
        if not target_id:
            return ValidationResult.fail("pacomind_reinstate: missing target_pacomind_id")
        if not self.state.is_suspended(target_id):
            return ValidationResult.fail("pacomind_reinstate: pacomind is not suspended")
        is_genesis = tx.from_pacomind_id == self.state.genesis_admin_id
        is_system = tx.from_pacomind_id == "system"
        if not is_genesis and not is_system:
            return ValidationResult.fail(
                "pacomind_reinstate: only genesis admin or system may reinstate"
            )
        return ValidationResult.success()
