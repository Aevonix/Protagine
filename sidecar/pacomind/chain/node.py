"""PacoMind Node Identity — per-device identity within a PacoMind.

Each physical device running a PacoMind instance is a Node. Nodes have:
    node_id   — unique UUID per device, generated on first `pacomind start`
    node keypair — Ed25519, independent from PacoMind keypair
    node cert — signed by PacoMind private key, proving membership

The PacoMind is the logical identity. Nodes are the physical instances.
One PacoMind can run on many devices — each gets its own node_id.
"""
...
def ensure_node_keypair(state_dir: str | Path) -> "LocalKeyManager":
    ...
    if (keys_dir / "private.pem").exists():
        return LocalKeyManager(keys_dir=keys_dir, pacomind_id=node_id)

    km = LocalKeyManager.generate(keys_dir=keys_dir, pacomind_id=node_id)
...
def create_node_certificate(
    state_dir: str | Path,
    pacomind_key_manager: Optional["LocalKeyManager"] = None,
) -> dict:
    """Create a node certificate signed by the PacoMind's private key.

    The certificate binds: pacomind_id, node_id, node_public_key, issued_at.
    It proves this device belongs to the PacoMind.
    """
    from pacomind.chain.local_keys import LocalKeyManager
    ...
    # Sign with PacoMind private key
    if pacomind_key_manager is None:
        pacomind_keys_dir = state_dir / "pacomind-keys"
        passphrase = None  # Caller should handle passphrase
        if (pacomind_keys_dir / "private.pem").exists():
            pacomind_key_manager = LocalKeyManager(keys_dir=pacomind_keys_dir, pacomind_id=pacomind_id, passphrase=passphrase)

    if pacomind_key_manager is not None:
        # Deterministic payload for signing (exclude signature field)
        payload = json.dumps(cert, sort_keys=True, separators=(",", ":")).encode("utf-8")
        cert["signature"] = pacomind_key_manager.sign(payload)
        logger.info("Node certificate signed by PacoMind key")
    else:
        logger.warning("No PacoMind key manager available — node certificate is unsigned")
...
def verify_node_certificate(cert: dict, pacomind_public_key_hex: str) -> bool:
    """Verify a node certificate's signature against a PacoMind public key."""
    from pacomind.chain.identity import _verify_ed25519_signature
    ...
    return _verify_ed25519_signature(pacomind_public_key_hex, payload_bytes, signature)
...
def get_node_info(state_dir: str | Path) -> dict:
    ...
    if (node_keys_dir / "private.pem").exists():
        try:
            from pacomind.chain.local_keys import LocalKeyManager
            km = LocalKeyManager(keys_dir=node_keys_dir, pacomind_id=node_id or "unknown")
