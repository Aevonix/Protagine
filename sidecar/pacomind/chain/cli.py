"""CLI commands for PacoMindChain management.

Usage:
    pacomind chain status                     # Show chain state summary
    pacomind chain genesis --name NAME        # Initialize genesis block
    pacomind chain info --block N             # Show block details
    pacomind chain tx --tx-id ID              # Show transaction details
    pacomind chain list-colonies              # List registered colonies
    pacomind chain list-sentinels             # List active sentinels
    pacomind chain verify                     # Verify chain integrity
    pacomind chain mempool                    # Show pending mempool transactions
"""
...
_DEFAULT_DB = Path.home() / ".pacomind" / "chain.db"


def _load_store(db_path: str | None = None) -> "ChainStore":
    from pacomind.chain.storage import ChainStore
    path = Path(db_path) if db_path else _DEFAULT_DB
    return ChainStore(path)


def _load_state(db_path: str | None = None) -> "ChainState":
    from pacomind.chain.state_machine import ChainStateMachine
...
def cmd_chain_status(args) -> int:
    ...
    try:
        ...
        print(f"  Colonies:         {len(state.pacomind_registry)}")
...
def cmd_chain_genesis(args) -> int:
    """Initialize a new PacoMindChain with a genesis block."""
    from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
    from cryptography.hazmat.primitives.serialization import (
        Encoding, PublicFormat, PrivateFormat, NoEncryption
    )
    from pacomind.chain.genesis import GenesisConfig, create_genesis_block
    from pacomind.chain.storage import ChainStore
    ...
    pacomind_id = hashlib.sha256(bytes.fromhex(pub_hex)).hexdigest()

    def sign_fn(data: bytes) -> str:
        return priv.sign(data).hex()

    cfg = GenesisConfig(
        genesis_pacomind_name=name,
        genesis_pacomind_id=pacomind_id,
...
def cmd_chain_tx(args) -> int:
    ...
    try:
        store = _load_store(getattr(args, "db", None))
        txs = store.get_transactions_for_pacomind("")
...
def cmd_chain_list_colonies(args) -> int:
    ...
    try:
        ...
        if not state.pacomind_registry:
            print("No colonies registered.")
            return 0
        fmt = "{:<20} {:<12} {:<16} {}"
        print(fmt.format("NAME", "STATUS", "PACOMIND_ID (prefix)", "ENDPOINT"))
        print("-" * 80)
        for pacomind_id, record in sorted(state.pacomind_registry.items(), key=lambda x: x[1].name):
            flags = ""
...
def cmd_chain_list_sentinels(args) -> int:
    ...
    try:
        ...
        if not state.sentinel_roster:
            ...
            return 0
        fmt = "{:<36} {:<12} {:<20} {}"
        print(fmt.format("SENTINEL_ID", "STATUS", "HOST:PORT", "PACOMIND_ID (prefix)"))
        print("-" * 80)
...
def cmd_chain_verify(args) -> int:
    ...
    try:
        ...
        for idx in range(height + 1):
            ...
            from pacomind.chain.block import build_merkle_root
...
def cmd_chain_state(args) -> int:
    ...
    try:
        ...
        summary = {
            ...
            "pacomind_count": len(state.pacomind_registry),
...
def cmd_chain_register(args) -> int:
    """Register this pacomind on-chain (pacomind_register transaction)."""
    from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
    from cryptography.hazmat.primitives.serialization import Encoding, PublicFormat, PrivateFormat, NoEncryption
    from pacomind.chain.transactions import TxType, Transaction
    import hashlib
    ...
    try:
        ...
        nonce_map = {}
        for cid, rec in state.pacomind_registry.items():
            pass
        last_nonce = 0
        for block_idx in range(store.get_height() + 1):
            ...
            if block:
                for tx in block.transactions:
                    if tx.from_pacomind_id == pacomind_id and tx.nonce > last_nonce:
                        last_nonce = tx.nonce

        tx = Transaction.create(
            tx_type=TxType.PACOMIND_REGISTER,
            from_pacomind_id=pacomind_id,
            nonce=last_nonce + 1,
            payload={
                "name": name,
        ...
        print(f"Submitted pacomind_register to mempool:")
        print(f"  tx_id:      {tx.tx_id}")
        print(f"  pacomind_id:  {pacomind_id}")
        print(f"  name:       {name}")
        return 0
    except Exception as exc:
        print(f"Error: {exc}", file=sys.stderr)
...
def cmd_chain_rotate_key(args) -> int:
    """Rotate this pacomind's Ed25519 key on-chain."""
    from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
    from cryptography.hazmat.primitives.serialization import Encoding, PublicFormat, PrivateFormat, NoEncryption
    from pacomind.chain.transactions import TxType, Transaction
    import hashlib
    ...
    try:
        ...
        pacomind_id = hashlib.sha256(bytes.fromhex(old_pub_hex)).hexdigest()
        ...
        tx = Transaction.create(
            tx_type=TxType.PACOMIND_ROTATE_KEY,
            from_pacomind_id=pacomind_id,
            nonce=last_nonce + 1,
            payload={
        ...
        store.add_to_mempool(tx)
        print(f"Submitted pacomind_rotate_key to mempool:")
        print(f"  tx_id:      {tx.tx_id}")
        print(f"  pacomind_id:  {pacomind_id}")
...
def cmd_chain_trust(args) -> int:
    """Submit trust_attest for a peer pacomind."""
    from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
    ...
    key_file = getattr(args, "key_file", None)
    target = getattr(args, "target", None) or getattr(args, "pacomind", None)
    level_raw = getattr(args, "level", None) or getattr(args, "trust_level", "1")
    db_path = getattr(args, "db", None)

    if not key_file or not target:
        print("Error: --key-file and --target (or positional pacomind name) required", file=sys.stderr)
        return 1
    ...
    try:
        ...
        # Resolve target: may be a name or pacomind_id
        target_id = state.pacomind_id_for_name(target) or (target if target in state.pacomind_registry else None)
        if not target_id:
            print(f"Error: pacomind {target!r} not found in chain state", file=sys.stderr)
            return 1
...
def cmd_chain_untrust(args) -> int:
    """Submit untrust_attest for a peer pacomind."""
    from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
    from cryptography.hazmat.primitives.serialization import Encoding, PublicFormat
    from pacomind.chain.transactions import TxType, Transaction
    import hashlib
    ...
    try:
        ...
        target_id = state.pacomind_id_for_name(target) or (target if target in state.pacomind_registry else None)
        if not target_id:
            print(f"Error: pacomind {target!r} not found", file=sys.stderr)
            return 1
...
def run_chain_command(args: list) -> int:
    """Dispatch 'pacomind chain <subcommand>'."""
    if not args:
        _print_chain_help()
        return 1
...
def _print_chain_help() -> None:
    print("""Usage: pacomind chain <subcommand> [options]
...
Subcommands:
  ...
  register            Register this pacomind on-chain
  rotate-key          Rotate pacomind Ed25519 key on-chain
  trust               Submit trust_attest (--target NAME --level 0-4)
  untrust             Submit untrust_attest (--target NAME)

Options:
  --db PATH              SQLite database path (default: ~/.pacomind/chain.db)
  --name NAME            PacoMind name
  ...
  --target NAME          Target pacomind name or ID (trust/untrust)
  --level N              Trust level 0-4 (trust command)
  --endpoint URL         PacoMind endpoint URL (register)
  --description TEXT     PacoMind description (register)
""")
