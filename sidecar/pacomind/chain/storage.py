"""SQLite-backed persistent storage for PacoMindChain.
...
CREATE TABLE IF NOT EXISTS chain_transactions (
    ...
    from_pacomind_id  TEXT    NOT NULL,
    timestamp       TEXT    NOT NULL,
    nonce           INTEGER NOT NULL,
    payload_json    TEXT    NOT NULL,
    signature       TEXT    NOT NULL,
    UNIQUE(from_pacomind_id, nonce)
);
...
CREATE INDEX IF NOT EXISTS idx_tx_pacomind    ON chain_transactions(from_pacomind_id);
...
class ChainStore:
            ...
            for tx in block.transactions:
                conn.execute(
                    """INSERT OR IGNORE INTO chain_transactions
                       (tx_id, block_index, tx_type, from_pacomind_id, timestamp,
        ...
        with self._tx() as conn:
            conn.execute(
                """INSERT OR IGNORE INTO chain_mempool
                   (tx_id, tx_type, from_pacomind_id, received_at, priority,
    ...
    def get_nonce(self, pacomind_id: str) -> int:
        """Return the highest accepted nonce for pacomind_id (0 if none)."""
        with self._get_conn() as conn:
            row = conn.execute(
                "SELECT MAX(nonce) as n FROM chain_transactions WHERE from_pacomind_id = ?",
                (pacomind_id,),
            ).fetchone()
    ...
    def get_transactions_for_pacomind(self, pacomind_id: str) -> list[Transaction]:
        with self._get_conn() as conn:
            rows = conn.execute(
                "SELECT tx_id, tx_type, from_pacomind_id, timestamp, nonce, "
                "payload_json, signature FROM chain_transactions "
                "WHERE from_pacomind_id = ? ORDER BY nonce ASC",
                (pacomind_id,),
            ).fetchall()
        result = []
        for r in rows:
            result.append(
                Transaction(
                    ...
                    from_pacomind_id=r["from_pacomind_id"],
...
def _serialize_chain_state(state: ChainState) -> str:
    ...
    def _te(e: TrustEdge) -> dict:
        return {
            "from_pacomind_id": e.from_pacomind_id,
            "to_pacomind_id": e.to_pacomind_id,
    ...
    def _ue(e: UntrustEvent) -> dict:
        return {
            "from_pacomind_id": e.from_pacomind_id,
            "target_pacomind_id": e.target_pacomind_id,
    ...
    data: dict[str, Any] = {
        ...
        "pacomind_registry": {k: _cr(v) for k, v in state.pacomind_registry.items()},
...
def _deserialize_chain_state(state_json: str) -> ChainState:
    ...
    for k, v in data["pacomind_registry"].items():
        state.pacomind_registry[k] = PacoMindRecord(**v)
