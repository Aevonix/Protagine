"""PacoMind Local Key Manager — PEM-based Ed25519 signing for Phase 1/2.

Provides the same interface as PacoMindKeyManager (sign, public_key_hex, rotate)
but stores the key as a simple PEM file. Designed to be swapped out for the
Shamir-backed PacoMindKeyManager when Phase 3/4/5 ship.

Key storage:
    {state_dir}/pacomind-keys/
...
Migration path:
    ...
    Phase 3:   LocalKeyManager with passphrase (pacomind key set-passphrase)
    Phase 4/5: PacoMindKeyManager (Shamir-split, pacomind key migrate-to-shamir)
"""
...
class LocalKeyManager:
    """PEM-based Ed25519 key manager for single-node PacoMind deployments."""
    ...
    # ------------------------------------------------------------------
    # Public interface (matches PacoMindKeyManager)
    # ------------------------------------------------------------------
    ...
    def rotate(self, new_passphrase: Optional[bytes] = None) -> None:
        ...
        The pacomind_id does NOT change — only the signing key rotates.
        """
        from cryptography.hazmat.primitives import serialization
        from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
        ...
        logger.info("Key rotated for pacomind %s", self.pacomind_id)
    ...
    def set_passphrase(self, new_passphrase: bytes) -> None:
        ...
        logger.info("Passphrase updated for pacomind %s", self.pacomind_id)
    ...
    def _load_private_key(self):
        ...
        if not priv_path.exists():
            raise FileNotFoundError(
                f"No private key found at {priv_path}. Run 'pacomind init' or 'pacomind key generate'."
            )
    ...
    def generate(
        ...
        mgr = cls(keys_dir=keys_dir, pacomind_id=pacomind_id, passphrase=passphrase)
        private_key = Ed25519PrivateKey.generate()
        mgr._save_keypair(private_key, passphrase)
        logger.info("Generated new Ed25519 keypair for pacomind %s", pacomind_id)
        return mgr
