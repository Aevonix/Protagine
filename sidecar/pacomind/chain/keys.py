"""PacoMind key resilience: Shamir Secret Sharing, encrypted share persistence, key rotation.
...
class ShareBackend(Protocol):
    ...
    def retrieve_share(self, pacomind_id: str, share_index: int) -> Optional[KeyShare]:
        """Retrieve a share. Returns None if not present."""
        ...

    def delete_share(self, pacomind_id: str, share_index: int) -> None:
        """Delete a share (used during rotation)."""
        ...

    def list_shares(self, pacomind_id: str) -> List[int]:
...
class KeyShareStore:
    ...
    - MUST bind share to its pacomind_id and share_index via AAD.
    - MUST support multiple backends (local file, remote node, backup file).
    - MUST detect and reject share transplantation (wrong pacomind_id or index).
    """
    ...
    def _aad(self, share_index: int) -> bytes:
        ...
        return (
            bytes.fromhex(self.pacomind_id)
            + bytes([share_index])
    ...
    def decrypt_share(self, share: KeyShare, passphrase: bytes) -> bytes:
        ...
        Raises:
            ValueError: If pacomind_id mismatch (transplantation attack).
            cryptography.exceptions.InvalidTag: If decryption fails.
        """
        if share.pacomind_id != self.pacomind_id:
            raise ValueError(
                f"Share pacomind_id mismatch: expected {self.pacomind_id}, "
                f"got {share.pacomind_id}"
            )
    ...
    def collect(
        ...
        for backend in self.backends:
            indices = backend.list_shares(self.pacomind_id)
            for idx in indices:
                if idx in seen_indices:
                    continue
                share = backend.retrieve_share(self.pacomind_id, idx)
                if share is None:
                    continue
...
class PacoMindKeyManager:
    """High-level manager for pacomind signing operations.
    ...
    def sign(self, payload: bytes) -> str:
        ...
        self._audit(f"key_reconstruct pacomind={self.pacomind_id}")
        ...
        try:
            ...
            self._audit(f"key_zeroed pacomind={self.pacomind_id}")
    ...
    def rotate(
        ...
        Signs a pacomind_rotate_key payload with the OLD key first,
        ...
        for backend in self.store.backends:
            old_indices.extend(backend.list_shares(self.pacomind_id))
        for backend in self.store.backends:
            for idx in set(old_indices):
                backend.delete_share(self.pacomind_id, idx)
        ...
        logger.info("Key rotation complete for pacomind %s", self.pacomind_id)
...
class RemoteNodeShareBackend:
    ...
    authenticated share transport across PacoMind mesh nodes.

    API contract (served by the remote node at BASE_PATH):
      PUT    /{pacomind_id}/{share_index}  — store or replace a share
      GET    /{pacomind_id}/{share_index}  — retrieve a share (404 → None)
      DELETE /{pacomind_id}/{share_index}  — delete a share (404 → silent)
      GET    /{pacomind_id}               — list indices {"indices": [...]}
    ...
    def store_share(self, share: KeyShare) -> None:
        ...
        try:
            self._request(
                "PUT",
                f"{self.BASE_PATH}/{share.pacomind_id}/{share.share_index}",
                json=share.to_dict(),
            )
        except Exception as exc:
            logger.error(
                "store_share failed [node=%s pacomind=%s index=%d]: %s",
                self.node_id, share.pacomind_id, share.share_index, exc,
            )
            raise
    ...
    def retrieve_share(self, pacomind_id: str, share_index: int) -> Optional[KeyShare]:
        ...
        try:
            resp = self._request(
                "GET",
                f"{self.BASE_PATH}/{pacomind_id}/{share_index}",
            )
            ...
            if exc.response.status_code == 404:
                return None
            logger.error(
                "retrieve_share failed [node=%s pacomind=%s index=%d]: HTTP %d",
                self.node_id, pacomind_id, share_index, exc.response.status_code,
            )
            return None
        except Exception as exc:
            logger.error(
                "retrieve_share failed [node=%s pacomind=%s index=%d]: %s",
                self.node_id, pacomind_id, share_index, exc,
            )
            return None
    ...
    def delete_share(self, pacomind_id: str, share_index: int) -> None:
        ...
        try:
            ...
            logger.error(
                "delete_share failed [node=%s pacomind=%s index=%d]: HTTP %d",
                self.node_id, pacomind_id, share_index, exc.response.status_code,
            )
            raise
        except Exception as exc:
            logger.error(
                "delete_share failed [node=%s pacomind=%s index=%d]: %s",
                self.node_id, pacomind_id, share_index, exc,
            )
            raise
    ...
    def list_shares(self, pacomind_id: str) -> List[int]:
        ...
        """
        try:
            resp = self._request("GET", f"{self.BASE_PATH}/{pacomind_id}")
            return sorted(resp.json().get("indices", []))
        except Exception as exc:
            logger.error(
                "list_shares failed [node=%s pacomind=%s]: %s",
                self.node_id, pacomind_id, exc,
            )
            return []


class BackupFileShareBackend:
    """Read/write a portable .pacomindshare backup file.
    ...
    def _load(self) -> None:
        if self._share is None and self.file_path.exists():
            ...
            # Populate pacomind/network IDs from file if not set at ctor.
            if self._pacomind_id is None:
                self._pacomind_id = data.get("pacomind_id")
    ...
    def store_share(self, share: KeyShare) -> None:
        ...
        if self._pacomind_id is None or self._network_id is None:
            self._load()
        if self._pacomind_id is None or self._network_id is None:
            raise ValueError(
                "pacomind_id and network_id are required to write a backup file"
            )
...
def export_share_file(
    ...
    """Write a .pacomindshare backup file."""
    data = {
        "file_type": "pacomind_key_share",
        "file_version": 1,
...
def import_share_file(file_path: Path) -> tuple:
    """Read a .pacomindshare backup file.

    Returns:
        (pacomind_id, network_id, KeyShare) tuple.
    """
    data = json.loads(file_path.read_text())
    if data.get("file_type") != "pacomind_key_share":
        raise ValueError(f"Not a pacomind_key_share file: {file_path}")
    pacomind_id = data["pacomind_id"]
    network_id = data["network_id"]
    share = KeyShare.from_dict(data["share"])
    return pacomind_id, network_id, share
