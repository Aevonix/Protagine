...
class GenesisConfig:
    ...
    network_name: str = "pacomind-federation"
    genesis_pacomind_name: str = "genesis"
    genesis_pacomind_id: str = ""         # sha256(genesis_pubkey)
    genesis_pubkey_hex: str = ""        # 32-byte Ed25519 pubkey, hex
    genesis_endpoint: str = ""
    genesis_description: str = "Genesis pacomind — network founder"
...
def _derive_pacomind_id(public_key_hex: str) -> str:
    """Compute pacomind_id = sha256(public_key_bytes)."""
...
def create_genesis_block(
    ...
    """Create the genesis block with the founding pacomind's registration.
    ...
    # Derive pacomind_id if not provided
    if not config.genesis_pacomind_id and config.genesis_pubkey_hex:
        config.genesis_pacomind_id = _derive_pacomind_id(config.genesis_pubkey_hex)

    # Build the genesis pacomind_register transaction
    register_payload = {
        "name": config.genesis_pacomind_name,
        "public_key_hex": config.genesis_pubkey_hex,
        "pacomind_id": config.genesis_pacomind_id,
    ...
    register_tx = Transaction.create(
        ...
        from_pacomind_id=config.genesis_pacomind_id,
        nonce=1,
    ...
    metadata: dict[str, Any] = {
        ...
        "genesis_pacomind_id": config.genesis_pacomind_id,
...
class GenesisAdmin:
    ...
    def __init__(
        self,
        genesis_pacomind_id: str,
        sign_fn: Callable[[bytes], str],
        get_nonce: Callable[[str], int],
    ) -> None:
        self.genesis_pacomind_id = genesis_pacomind_id
        self.sign_fn = sign_fn
        self.get_nonce = get_nonce

    def _next_nonce(self) -> int:
        return self.get_nonce(self.genesis_pacomind_id) + 1

    def suspend_pacomind(
        self, target_pacomind_id: str, reason: str = "admin_action"
    ) -> Transaction:
        """Create a pacomind_suspend transaction as Genesis admin."""
        return Transaction.create(
            tx_type=TxType.PACOMIND_SUSPEND,
            from_pacomind_id=self.genesis_pacomind_id,
            nonce=self._next_nonce(),
            payload={
                "target_pacomind_id": target_pacomind_id,
                "reason": reason,
    ...
    def reinstate_pacomind(
        self,
        target_pacomind_id: str,
        conditions: str = "",
        pacomind_acknowledgment: str = "",
    ) -> Transaction:
        """Create a pacomind_reinstate transaction as Genesis admin."""
        return Transaction.create(
            tx_type=TxType.PACOMIND_REINSTATE,
            from_pacomind_id=self.genesis_pacomind_id,
            nonce=self._next_nonce(),
            payload={
                "target_pacomind_id": target_pacomind_id,
                ...
                "pacomind_acknowledgment": pacomind_acknowledgment,
            },
            sign_fn=self.sign_fn,
        )
    ...
    def force_release_name(self, name: str) -> Transaction:
        ...
        return Transaction.create(
            tx_type=TxType.PACOMIND_RELEASE_NAME,
            from_pacomind_id=self.genesis_pacomind_id,
            nonce=self._next_nonce(),
    ...
    def transfer_admin(
        self, new_admin_pacomind_id: str, new_admin_pubkey_hex: str
    ) -> Transaction:
        """Transfer Genesis admin role to another pacomind.
        ...
        return Transaction.create(
            ...
            payload={
                ...
                "description": f"Transfer genesis admin to {new_admin_pacomind_id}",
                "spec_url": "",
                "changes": {
                    "new_genesis_admin_id": new_admin_pacomind_id,
