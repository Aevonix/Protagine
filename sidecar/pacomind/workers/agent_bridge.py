"""PacoMind Agent Bridge -- unified daemon for the agent-side autonomy circuit.
...
skills-sync) with one long-running process that:
  ...
  3. Periodically syncs the agent's skill index to PacoMind
...
Run as a daemon (``pacomind-agent-bridge``) or one-shot (``--once``).
...
Environment:
  PACOMIND_URL                   sidecar URL          (default http://127.0.0.1:7777)
  PACOMIND_API_KEY               API key              (default dev-mode-no-key)
  PACOMIND_INITIATIVE_WEBHOOK    initiative webhook    (default http://127.0.0.1:8644/webhooks/pacomind-initiatives)
  PACOMIND_JOBS_WEBHOOK_URL      jobs webhook          (default http://127.0.0.1:8644/webhooks/pacomind-jobs)
  PACOMIND_AGENT_NAME            agent identity        (default hermes)
  PACOMIND_WORKER_MAX_JOBS       jobs per cycle        (default 1)
  PACOMIND_BRIDGE_POLL_SECS      main loop interval    (default 60)
  PACOMIND_BRIDGE_SKILLS_HOURS   skills sync interval  (default 24)
  PACOMIND_BRIDGE_LOG_CHANNEL    alert routing channel (optional)
  PACOMIND_BRIDGE_PLATFORM       platform for alerts   (default unset; set to your messaging platform)
  HERMES_SKILLS_DIR            skills directory      (default ~/.hermes/skills)
  PACOMIND_BRIDGE_STATE_DIR      state file directory  (default ~/.pacomind/bridge)
"""
...
from pacomind.workers.queue_worker import (
    agent_action_capabilities,
    encode_hermes_webhook,
    work_order_webhook_fields,
)

logger = logging.getLogger("pacomind.bridge")
...
def _cfg() -> Dict[str, Any]:
    state_dir = Path(
        os.environ.get("PACOMIND_BRIDGE_STATE_DIR", "~/.pacomind/bridge")
    ).expanduser()
    state_dir.mkdir(parents=True, exist_ok=True)

    node_id = os.environ.get("PACOMIND_WORKER_NODE_ID") or (
        os.environ.get("PACOMIND_AGENT_NAME", "hermes").strip().lower().replace(" ", "-")
        + "-agent"
    )
    return {
        "pacomind_url": os.environ.get("PACOMIND_URL", "http://127.0.0.1:7777"),
        "api_key": os.environ.get("PACOMIND_API_KEY", "dev-mode-no-key"),
        "initiative_webhook": os.environ.get(
            "PACOMIND_INITIATIVE_WEBHOOK",
            "http://127.0.0.1:8644/webhooks/pacomind-initiatives",
        ),
        "jobs_webhook": os.environ.get(
            "PACOMIND_JOBS_WEBHOOK_URL",
            "http://127.0.0.1:8644/webhooks/pacomind-jobs",
        ),
        "node_id": node_id,
        "max_jobs": int(os.environ.get("PACOMIND_WORKER_MAX_JOBS", "1")),
        "poll_secs": int(os.environ.get("PACOMIND_BRIDGE_POLL_SECS", "60")),
        "skills_hours": float(os.environ.get("PACOMIND_BRIDGE_SKILLS_HOURS", "24")),
        "log_channel": os.environ.get("PACOMIND_BRIDGE_LOG_CHANNEL", ""),
        "platform": os.environ.get("PACOMIND_BRIDGE_PLATFORM", ""),
        "state_dir": state_dir,
    }
    ...
    def check(self, cfg: Dict[str, Any]) -> dict:
        ...
        if not health:  # unreachable OR an empty 200 body — both are failures
            ...
            self._alert("sidecar_unreachable",
                        f"PacoMind sidecar at {cfg['pacomind_url']} is unreachable "
...
class QueueWorker:
    ...
    def claim_and_dispatch(self, cfg: Dict[str, Any]) -> int:
        ...
        try:
            ...
            return 0
        try:
            _post(cfg, f"{cfg['pacomind_url']}/v1/host/queue/workers/register", {
                "node_id": cfg["node_id"],
        ...
        for _ in range(cfg["max_jobs"]):
            job = _post(cfg, f"{cfg['pacomind_url']}/v1/host/queue/jobs/claim", {
                "node_id": cfg["node_id"],
                "capabilities": capabilities,
                "job_types": ["agent_action"],
                "max_concurrent": 1,
            ...
            pacomind_url = cfg["pacomind_url"]

            try:
                started = _post(
                    cfg,
                    f"{pacomind_url}/v1/host/queue/jobs/{job_id}/start",
                ...
                try:
                    _post(
                        cfg,
                        f"{pacomind_url}/v1/host/queue/jobs/{job_id}/release",
                        {"claim_attempt_id": claim_attempt_id},
                    )
                except Exception:
                    pass
            ...
            payload = {
                ...
                "payload": {
                    ...
                    **work_order_webhook_fields(params),
                    "pacomind_url": pacomind_url,
                    "observations_url": f"{pacomind_url}/v1/host/observations",
                    "heartbeat_url": f"{pacomind_url}/v1/host/queue/jobs/{job_id}/heartbeat",
                    "complete_url": f"{pacomind_url}/v1/host/queue/jobs/{job_id}/complete",
                    "fail_url": f"{pacomind_url}/v1/host/queue/jobs/{job_id}/fail",
...
class SkillsSyncer:
    """Periodically reports the agent's skill index to PacoMind."""
    ...
    def sync_if_due(self, cfg: Dict[str, Any]) -> bool:
        ...
        try:
            from pacomind.workers.skills_sync import scan, report
            observations = scan()
            if observations:
            ...
            logger.debug("Skills sync: pacomind.workers.skills_sync not importable, skipping")
            return False
        except Exception as exc:
...
class AgentBridge:
    """Unified daemon that keeps the PacoMind-to-agent circuit alive."""
    ...
    def run(self, once: bool = False) -> None:
        logger.info(
            "PacoMind Agent Bridge starting (poll=%ds, skills_sync=%dh, url=%s)",
            self.cfg["poll_secs"],
            int(self.cfg["skills_hours"]),
            self.cfg["pacomind_url"],
        )
        ...
        logger.info("PacoMind Agent Bridge stopped")
...
def main(argv: Optional[list] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="pacomind-agent-bridge",
        description=(
            "Unified daemon that bridges PacoMind's autonomy loop to the agent. "
    ...
    if args.dry_run:
        print("pacomind-agent-bridge (dry run):")
