"""Report the agent's skill index to PacoMind.

Installed as the ``pacomind-skills-sync`` console command.

Scans the Hermes skills directory for SKILL.md files, parses the
frontmatter (name / description / tags), and POSTs the index to
PacoMind's push-only "skills" observation domain. PacoMind's self-directed
thinking and capability-gap machinery read from there, so PacoMind
...
def report(observations: list) -> int:
    """POST the skill index to PacoMind's skills observation domain.
    ...
    req = urllib.request.Request(
        f"{pacomind_url}/v1/host/observations",
...
def main(argv: Optional[list] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="pacomind-skills-sync",
        description="Report the agent's installed skill index to PacoMind.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="scan and print what would be reported without POSTing to PacoMind",
    )
    ...
    if not observations:
        ...
        return 0
    if args.dry_run:
        print("pacomind-skills-sync (dry run — nothing reported):")
