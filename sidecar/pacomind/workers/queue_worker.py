"""Claim PacoMind agent_action jobs and hand them to the agent.

Installed as the ``pacomind-queue-worker`` console command.

This is the execution half of the agent-as-sensor loop:

  PacoMind notices a domain is stale → posts a read-only agent_sync_<domain>
  ...
  the agent POSTs observations back to PacoMind and completes the job →
  PacoMind generates initiatives from what the agent saw.
...
close the loop with plain curl — no special tooling needed:
  ...
  - report observations:  POST {PACOMIND_URL}/v1/host/observations
  - complete the job:     POST {PACOMIND_URL}/v1/host/queue/jobs/{id}/complete
  - fail the job:         POST {PACOMIND_URL}/v1/host/queue/jobs/{id}/fail

Environment:
  PACOMIND_URL                sidecar URL       (default http://127.0.0.1:7777)
  PACOMIND_API_KEY            API key           (default dev-mode-no-key)
  PACOMIND_JOBS_WEBHOOK_URL   agent webhook     (default http://127.0.0.1:8644/webhooks/pacomind-jobs)
  PACOMIND_WORKER_NODE_ID     worker identity   (default derived from PACOMIND_AGENT_NAME)
  PACOMIND_WORKER_MAX_JOBS    jobs per run      (default 1 — each is a full agent invocation)
  PACOMIND_AGENT_WORKER_ROUTES comma-separated exact no-effect lanes:
...
def agent_action_capabilities(routes: Optional[str] = None) -> list[str]:
    ...
    configured = (
        os.environ.get("PACOMIND_AGENT_WORKER_ROUTES", "agent_sync,hermes_run")
    ...
    if not selected or unknown:
        ...
        raise ValueError(
            "PACOMIND_AGENT_WORKER_ROUTES must select agent_sync and/or "
...
def encode_hermes_webhook(
    ...
    *,
    ...
    configured = (
        os.environ.get("PACOMIND_HERMES_WEBHOOK_SECRET", "")
...
def work_order_webhook_fields(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "result_contract": {
            ...
            "verification_note": (
                "Executor verification claims are observational; PacoMind "
...
def load_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "webhook_url": os.environ.get(
            "PACOMIND_JOBS_WEBHOOK_URL", "http://127.0.0.1:8644/webhooks/pacomind-jobs"
        ),
        "node_id": node_id,
...
def register_worker(cfg: Dict[str, Any]) -> None:
    ...
    _post(
        cfg,
        f"{cfg['pacomind_url']}/v1/host/queue/workers/register",
        {
            "node_id": cfg["node_id"],
...
def claim_job(cfg: Dict[str, Any]) -> Optional[dict]:
    ...
    return _post(
        cfg,
        f"{cfg['pacomind_url']}/v1/host/queue/jobs/claim",
        {
            "node_id": cfg["node_id"],
            "capabilities": capabilities,
            "job_types": ["agent_action"],
...
def main(argv: Optional[list] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="pacomind-queue-worker",
        description="Claim PacoMind agent_action jobs and hand them to the agent webhook.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    cfg = load_config()
    if args.dry_run:
        print("pacomind-queue-worker (dry run — no network calls):")
        print(f"  pacomind_url:  {cfg['pacomind_url']}")
