...
SKILL_NAME = 'pacomind-deep-research'
BUNDLE_PREFIX = 'pacomind_hermes/bundled_skills/'
...
def prepare(home, resources, *, refresh=False):
    ...
    for path, content in sorted(resources.items()):
        ...
        if not re.fullmatch(r'pacomind-[a-z0-9-]{1,56}', name):
            raise ValueError('Bundled skill must have a canonical pacomind- name')
...
def _prepare_skill(home, name, content, *, refresh):
    ...
    skill, marker = directory/'SKILL.md', directory/'.pacomind-owned.json'
    ...
    if recorded is None:
        ...
        try:
            ...
            owned = (record['owner'] == 'pacomind-hermes' and record['version'] == 1
        ...
        if previous != content and not refresh:
            raise ValueError('Bundled skill update requires explicit pacomind init --skills-only')
    after = (json.dumps({'owner': 'pacomind-hermes', 'version': 1, 'sha256': digest},
...
def install(updates):
    ...
    print('Bundled skills available: '+', '.join(names)+'. With this PacoMind adapter version active, ongoing conversations refresh skill discovery on their next request.')
