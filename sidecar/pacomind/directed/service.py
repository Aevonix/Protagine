...
Pipeline (option A: delegation only, PacoMind never mutates anything itself):
  ...
  dispatch-> POST the ScopedTask contract to the EXISTING env-configured
             delegate endpoint (PACOMIND_DIRECTED_TASK_URL, falling back to the
             agent-bridge jobs webhook). Dry-run mode logs the exact would-be
             dispatch and sends nothing (PACOMIND_DIRECTED_MODE, default dry_run).
...
from pacomind.directed.models import ScopedTask, ScopedTaskStore
from pacomind.directed.intake import scope_from_directive
from pacomind.directed.audit import audit_completion
...
def directed_mode() -> str:
    ...
    return resolve("PACOMIND_DIRECTED_MODE", ("off", "dry_run", "live"),
                   "dry_run")


def _dispatch_url() -> str:
    return (os.environ.get("PACOMIND_DIRECTED_TASK_URL", "")
            or os.environ.get("PACOMIND_BRIDGE_JOBS_WEBHOOK_URL", "")
            or os.environ.get("PACOMIND_JOBS_WEBHOOK_URL", ""))


def _strict_reports() -> bool:
    """Strict report-back (default): a completion report is accepted only for
    a task that was actually dispatched. PACOMIND_DIRECTED_STRICT_REPORTS=0
    restores the legacy accept-in-any-status behavior."""
    return os.environ.get("PACOMIND_DIRECTED_STRICT_REPORTS", "1").strip() != "0"


def _max_dispatch_per_day() -> int:
    """Daily dispatch cap (rolling 24h). <=0 disables the cap (legacy)."""
    try:
        return int(os.environ.get("PACOMIND_DIRECTED_MAX_DISPATCH_PER_DAY", "10"))
...
def _hmac_key() -> bytes:
    ...
    return os.environ.get("PACOMIND_DIRECTED_HMAC_KEY", "").encode("utf-8")
...
class DirectedActionService:
    ...
    async def dispatch(self, task_id: str) -> Dict[str, Any]:
        ...
        if cap > 0:
            ...
            if recent >= cap:
                logger.warning(
                    ...
                    "in 24h; PACOMIND_DIRECTED_MAX_DISPATCH_PER_DAY)",
                    task.id, recent, cap)
        ...
        # only when PACOMIND_DIRECTED_HMAC_KEY is set (unset = legacy unsigned).
        ...
        try:
            ...
            if key:
                headers["X-PacoMind-Directed-Signature"] = envelope["signature"]
    ...
    def _reject_report(self, task: ScopedTask, reason: str) -> Dict[str, Any]:
        ...
        logger.warning(
            ...
            "(strict reports; PACOMIND_DIRECTED_STRICT_REPORTS=0 for legacy)",
