...
The legacy ``PACOMIND_API_KEY`` remains a migration credential. New scoped
...
def _audience_person_ids() -> dict[str, str]:
    ...
    return {
        "owner": owner,
        "shared": os.environ.get("PACOMIND_SHARED_PERSON_ID", "shared").strip() or "shared",
        "global": os.environ.get("PACOMIND_GLOBAL_PERSON_ID", "global").strip() or "global",
    }
...
def request_authority(request: Request | None) -> RequestAuthority:
    ...
    value = getattr(request.state, "pacomind_authority", None)
...
def resolve_request_person(
    ...
    *,
    ...
    if authority.anonymous:
        target = target or (
            os.environ.get("PACOMIND_DEV_PERSON_ID", "dev-anonymous").strip()
            or "dev-anonymous"
        )
...
def resolve_turn_person(
    ...
    *,
    ...
    sender envelope PacoMind then discards the body contact claim, starts from the
...
def worker_authority_mode() -> str:
    ...
    value = os.environ.get("PACOMIND_WORKER_AUTHORITY_MODE", "shadow").strip().lower()
...
def required_scope(method: str, path: str) -> str:
    ...
    if (method.upper() == 'POST' and path in {
            '/v1/host/transport/ingress/admit', '/v1/host/transport/ingress/handoff',
            '/v1/host/transport/ingress/coverage'} or
            method.upper() == 'GET' and path == '/v1/host/transport/ingress/receipts'):
    ...
    if (method.upper() == 'POST'
            and re.fullmatch(r'/v1/host/commitments/(?:local-draft|[^/]+/local-draft|local-work/next|local-work/[^/]+/(?:finish|native-task|native-run))', path)):
        return 'turns:write'
    ...
    exact = {
        ...
        ("POST", "/v1/host/memory/sources/deadline"): "memory:read",
        ("GET", "/v1/host/memory/sources/erasures"): "turns:write",
        ("POST", "/v1/host/memory/sources/erasures"): "turns:write",
        ...
        ("POST", "/v1/host/executions/assess"): "turns:write",
        ("POST", "/v1/host/executions/assessments/read"): "context:read",
