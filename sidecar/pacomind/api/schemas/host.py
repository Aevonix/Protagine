...
Mirrors the TypeScript types in pacomind's src/types.ts.
...
from pacomind.gate.surface_policy import ResponseGuardSurfaceName
...
class RecordOutreachRequest(BaseModel):
    # Agent identity is deployment-specific — configure PACOMIND_AGENT_NAME
    # rather than relying on this generic fallback.
    agent_id: str = Field(
        default_factory=lambda: _os.environ.get("PACOMIND_AGENT_NAME", "agent")
    )
