...
from pacomind.task_queue.queue_manager import (
    QueueExecutionUnavailable,
    TaskQueueManager,
)
from pacomind.util.session_safety import (
...
class BoundedGrantRequest(BaseModel):
    ...
    def within_configured_envelope(self):
        ...
        if (
            ...
            raise ValueError(
                "expires_in_seconds exceeds PACOMIND_GRANT_MAX_TTL_SECONDS"
            )
        if (
            envelope.max_uses is not None
            and self.max_uses > envelope.max_uses
        ):
            raise ValueError("max_uses exceeds PACOMIND_GRANT_MAX_USES")
        return self
...
async def queue_contract(request: Request = None) -> Dict[str, Any]:
    ...
    from pacomind.task_queue.contract import (
    ...
    *,
    ...
    if mode == "invalid":
        raise _worker_authority_error(
            ...
            "PACOMIND_WORKER_AUTHORITY_MODE must be shadow or enforce",
            status=503,
        )
    if request is None:
    ...
    if mode == "invalid":
        raise HTTPException(
            ...
            detail={
                ...
                "message": (
                    "PACOMIND_APPROVAL_AUTHORITY_MODE must be shadow or enforce"
                ),
            },
        )
    if request is None:
        # Direct in-process calls are an explicit trusted integration surface,
        # not an HTTP body claim. They remain for embedded PacoMind deployments.
...
async def create_job(body: JobPostRequest) -> Dict[str, Any]:
    ...
    if reserved:
        raise HTTPException(status_code=400, detail={
            ...
            "message": "authority tags may only be set by PacoMind control planes",
            "tags": reserved,
        })
    ...
    if (
        ...
        raise HTTPException(status_code=400, detail={
            ...
            "message": (
                "ApprovalRelayCanaryV1 may only be issued by PacoMind's "
    ...
    if body.job_type == JobType.THOUGHT.value or (
        ...
        raise HTTPException(status_code=400, detail={
            ...
            "message": (
                "ThoughtJobV1 may only be issued by PacoMind's cognition spine"
            ),
        })
    ...
    if caller_route_capabilities:
        raise HTTPException(status_code=400, detail={
            ...
            "message": (
                "agent_action executor routes are derived by PacoMind and "
    ...
    if job_type is JobType.AGENT_ACTION:
        from pacomind.initiatives.action_registry import get_action
        ...
        if action_spec is None:
            raise HTTPException(status_code=400, detail={
                ...
                "message": (
                    ...
                    "from PacoMind's server-owned action registry"
                ),
            })
...
async def list_blocked_jobs(
    ...
    if response is not None:
        response.headers["X-PacoMind-Blocked-Legacy-Count"] = str(legacy_count)
