"""PacoMind sidecar host router — ``/v1/host`` API surface.

This is the contract used by external agent harnesses (OpenClaw and any
future shim) to mount PacoMind's intelligence as a plugin.
"""
...
from pacomind.goals.store import GoalNotFoundError
...
from pacomind.turns.tool_observations import ToolObservation
...
def _event_subscriber_queue_size() -> int:
    ...
    raw = os.environ.get("PACOMIND_EVENT_SUBSCRIBER_QUEUE_SIZE", "256")
    try:
        size = int(raw)
    except ValueError:
        logger.warning(
            "Invalid PACOMIND_EVENT_SUBSCRIBER_QUEUE_SIZE=%r; using 256", raw
        )
        return 256
    if size < 1:
        logger.warning(
            "PACOMIND_EVENT_SUBSCRIBER_QUEUE_SIZE must be positive; using 256"
        )
        return 256
...
def broadcast_event(event: dict) -> Optional[dict]:
    ...
    """
    from pacomind.events.journal import append_event_record
...
async def configure_host(body: HostConfigureRequest) -> HostConfigureResponse:
    ...
    its LLM provider credentials and model assignments. PacoMind does not
...
async def list_models() -> ModelListResponse:
    ...
    from pacomind.router.tiers import discover_local_models
...
def _temporal_health_policy() -> str:
    ...
    configured = os.environ.get(
        "PACOMIND_TEMPORAL_HEALTH_POLICY", "enforce"
    ).strip().lower()
...
async def health() -> HostHealthResponse:
    ...
    if _embedder is not None:
        ...
        try:
            from pacomind.vector import get_store
            store = get_store()
    ...
    try:
        if _telemetry is not None:
            thresholds = {
                "sync": float(os.environ.get("PACOMIND_STALE_SYNC_HOURS", "2.0")),
                "tick": float(os.environ.get("PACOMIND_STALE_TICK_HOURS", "24.0")),
                "initiative": float(os.environ.get("PACOMIND_STALE_INITIATIVE_HOURS", "48.0")),
                ...
                "prefetch": float(os.environ.get("PACOMIND_STALE_PREFETCH_HOURS", "24.0")),
            }
            ...
            if (
                ...
                health_status = "degraded"
            from pacomind.api.schemas.host import TemporalMetrics
...
async def auth_migration_status(request: Request) -> dict:
    ...
    telemetry = getattr(request.state, "pacomind_auth_telemetry", None)
    grants = getattr(request.state, "pacomind_contact_grants", None)
    keyring_status = getattr(request.state, "pacomind_keyring_status", None) or {
        "configured": False,
        "available": False,
    ...
    auth_configuration = getattr(request.state, "pacomind_auth_configuration", None) or {
...
async def memory_rerank(body: RerankRequest) -> RerankResponse:
    ...
    Requires the reranker to be initialized (see PACOMIND_RERANKER_MODEL env var).
    """
    if _reranker is None:
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail="Reranker not initialized. Set PACOMIND_RERANKER_MODEL to enable.",
        )
    try:
...
async def memory_backfill(body: BackfillRequest) -> BackfillResponse:
    ...
    async def _run():
        from pacomind.vector.backfill import backfill
        try:
...
async def memory_migrate(body: MigrateRequest) -> MigrateResponse:
    ...
    async def _run():
        ...
        try:
            ...
            from pacomind.vector.migrate import MigrationResult
...
async def _build_temporal_section(
    ...
    *, include_global_heads_up: bool = True,
    ...
    pacomind-memory plugin makes ("prefer the real current time provided by the
    ...
    """
    from pacomind.util import temporal as _temporal
    ...
    if include_global_heads_up:
        try:
            from pacomind.util.session_safety import load_last_user_message_at
...
async def context_assemble(
    ...
    if not _canonical_only:
        ...
        try:
            ...
            state_dir = Path(os.environ.get("PACOMIND_STATE_DIR", os.path.expanduser("~/.pacomind")))
            pacomind_id = get_or_create_pacomind_id(state_dir)
            identity_lines.append(f"PacoMind ID: {pacomind_id}")
    ...
    if _canonical_person_allowed and query_text:
        ...
        try:
            ...
            if packet.content:
                sections.append(ContextSection(
                    id="pacomind-memory", title="Relevant Memories", body=packet.content,
    ...
    if _canonical_person_allowed and _commitment_store is not None:
        try:
            ...
            if all_comms:
                ...
                lines = ["Claim the commitment ID with pacomind_commitment_work before undertaking it. Another session's live reservation means do not duplicate its work. Claims authorize no external effect."]
                for c in all_comms:
    ...
    if _canonical_person_allowed and contact_id:
        try:
            from pacomind.api.routers.social_state import appraisal_context
            ...
            if social_brief:
                sections.append(ContextSection(id='pacomind-appraisals',
    ...
    if _canonical_person_allowed and contact_id:
        try:
            ...
            if owner:
                from pacomind.api.routers.social_state import waiting_context
                waiting_brief = waiting_context(person)
                if waiting_brief:
                    sections.append(ContextSection(id='pacomind-waiting', title='Expected replies',
                ...
                if _situation_store is not None and re.search(r'\b(hardware|machine|server|model|endpoint|cluster|offline|online|running|doing|status)\b', query_text, re.I):
                    ...
                    if current['facts'] or current['stale']:
                        sections.append(ContextSection(id='pacomind-current-state',
        ...
        try:
            ...
            if get_owner_contact_id() == contact_id:
                ...
                if perspective is not None:
                    ...
                    if working_brief:
                        sections.append(ContextSection(id='pacomind-self-perspective',
    ...
    if _tom2_store is not None and _tom_context_facts is not None and _facts_store is not None:
        try:
            from pacomind.tom.facts import InferenceFactsView
    ...
    # Double-keyed: PACOMIND_TOM2_CONTEXT (default off) turns the section on,
    ...
    # default-inert: PACOMIND_TOM2_LEVEL=0 (shipped) skips it entirely — the
            ...
            if configured_level() >= 1 and contact_id != _lvl_owner:
                ...
                if _lres.level >= 2:
                    from pacomind.tom.eligibility import (
                        eligible_inferences)
                    from pacomind.tom.leveled import render_level2
    ...
    if _exact_person_allowed and _engagement_store is not None and contact_id:
        try:
            from pacomind.tom.engagement import build_guidance
            _guid = build_guidance(_engagement_store.get_profile(contact_id))
            if _guid:
                sections.append(ContextSection(
                    id="pacomind-engagement",
    ...
    if _tom2_facts is not None and not _tom2_facts.current():
        sections = [s for s in sections if s.id not in {
            'pacomind-tom2', 'pacomind-tom2-l1', 'pacomind-tom2-l2'}]
...
async def reasoning_turn(
    ...
    actor_policy = None
    if _p8_runtime is not None:
        from pacomind.reasoning.executor import ToolRegistryError
        from pacomind.reasoning.tool_policy import filter_tools_for_actor
...
async def tools_invoke(
    ...
    Used by the OpenClaw plugin to expose PacoMind's native tools
    ...
    if _p8_runtime is not None:
        from pacomind.reasoning.executor import ToolRegistryError
        from pacomind.reasoning.tool_policy import actor_allows_tool
    ...
    if hasattr(_tool_executor, "execute_batch"):
        ...
        result = (await _tool_executor.execute_batch(
            ...
            actor_policy=actor_policy,
            **({'memory_search': _reasoning_memory_search(request, body.identity, body.context)}
               if body.name == 'pacomind_memory_search' and body.context is not None else {}),
        ))[0]
...
async def _attribute_signal_contact(body: SignalIngestRequest) -> None:
    """Attribution for /signals/ingest (PACOMIND_SIGNALS_ATTRIBUTION=legacy/strict).
    ...
    """
    mode = os.environ.get("PACOMIND_SIGNALS_ATTRIBUTION", "legacy").strip().lower()
    try:
        ...
        if mode == "strict":
            ...
            logger.warning(
                ...
                "an unverified identity (set PACOMIND_SIGNALS_ATTRIBUTION=strict "
...
async def signals_ingest(body: SignalIngestRequest) -> SignalIngestResponse:
    ...
    if recorded > 0:
        try:
            from pacomind.cognition.trigger import trigger_cognition, _cognition_enabled
...
def _conversation_turn_concern_metadata(
    ...
    *,
    ...
    from pacomind.self_model.event_concerns import turn_concerns_enabled
...
async def source_freshness_feed(body: SourceFreshnessRequest, request: Request):
    ...
    if body.annotation_checks:
        from pacomind.turns.source_annotations import current_candidates
...
async def forget_turn_sources(body: SourceForgetRequest, request: Request = None):
    ...
    vector_cleanup = ('disabled_not_checked' if os.environ.get('PACOMIND_EMBED_PROVIDER') == 'skip'
                      else 'unavailable')
    from pacomind.vector import get_store
    ...
    graph_cleanup = ('disabled_not_checked' if os.environ.get('PACOMIND_GRAPH_ENABLED', 'true').lower()
    ...
    try:
        from pacomind.api.routers.transport_ingress_api import forget_sources
...
async def _resolve_scoped_sender_contact(authority, gateway: str, address: str):
    ...
    from pacomind.contacts.identity_links import normalized_handle
    try:
...
async def _ingest_turn_idempotently(
    ...
    if body.assistant_input_refs:
        from pacomind.turns.idempotency import SourceInputPending
        try:
...
async def turns_sync(
    ...
    from pacomind.api.routers.transport_ingress_api import reconcile_source
...
def source_observation_sync(turn_id: str, body: ToolObservationEnvelope, response: Response, request: Request):
    ...
    from pacomind.turns import get_turn_idempotency_ledger
    from pacomind.turns.idempotency import SourceErased
    from pacomind.turns.tool_observations import record
...
async def _process_turn_sync(
    ...
    *,
    ...
    if source_messages:
        from pacomind.turns import canonical_turn_digest, get_turn_idempotency_ledger
        ledger = get_turn_idempotency_ledger(get_state_dir())
    ...
    try:
        from pacomind.cognition.introspection import introspect_enabled, run_turn_introspection
    ...
    if body.user_message is not None:
        try:
            from pacomind.util.session_safety import save_last_user_message_at
    ...
    if _preference_learner is not None and body.user_message is not None:
        try:
            ...
            if owner_id and body.context.contact_id == owner_id:
                ...
                if hit is not None:
                    try:
                        from pacomind.events.broadcaster import emit as _emit
    ...
    # miner mode gates everything internally, see pacomind/mining/).
    try:
        from pacomind.api.routers.mining import get_mining_engine as _get_miner
        _miner = _get_miner()
...
async def events_ws(ws: WebSocket) -> None:
    ...
    try:
        ...
        if msg.get("type") != "auth":
            ...
            return
        token = msg.get("token", "")
        expected = os.environ.get("PACOMIND_API_KEY", "")
        ...
        if keyring_path:
            from pacomind.api.authority import KeyringLoader, scoped_authority
    ...
    from pacomind.events.journal import current_sequence, replay_events
    with _event_broadcast_lock:
...
async def create_goal(body: GoalCreateRequest) -> GoalResponse:
    ...
    """
    from pacomind.cognition.goal_spine import cognition_spine_exclusive
...
async def list_contacts(
    ...
    try:
        ...
        from pacomind.contacts.models import _TIER_RANK
        def _sort_key(c):
...
def _contact_policy_page_failure(
    *,
    ...
    return {
        "schema": "PacoMindContactPolicySourceV1",
        "version": 1,
        "available": False,
        "complete": False,
        "reason": reason,
...
async def contact_policy_source(
    ...
    fresh evaluation of PacoMind's existing policy.  The exact-person posture is
    ...
    registry = getattr(request.state, "pacomind_contact_grants", None)
    ...
    for contact in contacts:
        ...
        if outreach_dependencies_available:
            from pacomind.contacts.comms import evaluate_outreach
...
async def provision_contact_policy_identity(
    ...
    return {
        "schema": "PacoMindContactProvisionResultV1",
        "version": 1,
        "mode": body.mode,
...
async def add_contact_handle(contact_id: str, body: dict) -> dict:
    ...
    Owner curation surface behind pacomind_link_contact."""
    if _contacts_store is None:
        raise HTTPException(status_code=501, detail="Contact store not initialized")
...
async def resolve_contact_by_handle(gateway: str, address: str, request: Request, create: bool = False) -> ContactResponse:
    ...
    (platform + address) to the real PacoMind contact, so per-contact
...
def guard_derive_context_enabled() -> bool:
    """PACOMIND_GUARD_DERIVE_CONTEXT (default on): complete a guard-check
    ...
    return os.environ.get("PACOMIND_GUARD_DERIVE_CONTEXT", "1").strip().lower()\
...
async def response_guard_check(body: ResponseGuardCheckRequest) -> Dict[str, Any]:
    ...
    derived server-side (PACOMIND_GUARD_DERIVE_CONTEXT, default on) so the
    ...
    from pacomind.gate.surface_policy import EXCLUDED_SPEECH_SURFACES

    if _response_guard is None:
        configured_mode = (
            GuardMode.ENFORCE
            if os.environ.get("PACOMIND_GUARD_MODE", "").strip().lower()
...
async def env_risk(conversation_key: str, contact_id: str) -> Dict[str, Any]:
    ...
    from pacomind.gate.env_risk import classify, env_risk_window_hours
...
async def get_timeline(
    ...
    """Chronological timeline across all PacoMind subsystems (v0.21.0).
    ...
    """
    from pacomind.events.journal import replay_events
    from pacomind.util import temporal as _t
...
async def cognition_cycle(body: CognitionCycleRequest) -> dict:
    ...
    """
    from pacomind.self_model.benchmark import legacy_cpi_payload
...
async def tom2_status() -> dict:
    ...
    from pacomind.tom.asymmetry import tom2_mode
    from pacomind.tom.levels import (
...
async def _world_context_entities(query_text: str, limit: int = 5) -> list:
    ...
    PACOMIND_WORLD_CONTEXT_QUERY governs how the query hits the store:
    ...
    """
    mode = os.environ.get("PACOMIND_WORLD_CONTEXT_QUERY", "message").strip().lower()
    if mode == "entities":
        extractor = _get_conversation_extractor()
        if extractor is not None:
            try:
...
async def list_relationship_briefs() -> dict:
    """Profiled relationships: who PacoMind has real standing knowledge of."""
...
async def directed_approve(
    ...
    if _directed_service is None:
        return {"ok": False, "reason": "directed_not_wired"}
    from pacomind.api.routers.task_queue import _decision_authority
...
def _scope_from_authority(request: Request):
    ...
    from pacomind.cognition.drive_governance import ScopeV1
...
async def propose_cognition_drive(
    ...
    if _drive_governance is None:
        return {"available": False}
    from pacomind.cognition.drive_governance import DriveV1
...
async def propose_cognition_drive_signal(
    ...
    if _drive_governance is None or _drive_project_store is None:
        return {"available": False}
    from pacomind.cognition.drive_governance import DriveSignalV1
...
async def charter_transition_approval_readiness(request: Request) -> dict:
    ...
    if _drive_governance is None:
        return {
            "schema": "PacoMindCharterApprovalReadinessV1",
            "version": 1,
            "available": False,
            "ready": False,
            "route_ready": False,
    ...
    from pacomind.initiatives.approval_authority import authority_mode
...
async def resolve_concern(
    ...
    from pacomind.self_model.workspace import ConcernResolutionConflict
    try:
    ...
    if cascade_state["status"] == "pending":
        ...
        if created:
            try:
                from pacomind.self_model.settlement import settle_sources
            ...
            from pacomind.self_model.settlement import (
...
async def list_tools(status: str = "") -> dict:
    ...
    try:
        ...
        return {"available": True, "mode": os.environ.get("PACOMIND_TOOLSMITH", "off"),
...
async def get_tool(tool_id: str) -> dict:
    ...
    try:
        from pacomind.toolsmith.engine import _shadow_min
...
async def get_autonomy_posture(request: Request) -> dict:
    """Effective autonomy posture: the active PACOMIND_AUTONOMY_PRESET (if any)
    and the resolved value of every preset-managed mode flag, as the RUNNING
    process sees them. This is what `pacomind doctor` reads so plist/unit-pinned
    env is never invisible to diagnostics."""
    try:
        from pacomind.util.autonomy_preset import snapshot
        posture = snapshot()
        ...
        try:
            if _autonomy_loop is not None:
                posture["PACOMIND_AUTONOMY_MODE"] = _autonomy_loop.config.mode.value
                posture["PACOMIND_AUTONOMY_MODE_SOURCE"] = getattr(
                ...
                raw = (os.environ.get("PACOMIND_AUTONOMY_MODE") or "").strip().lower()
                if raw:
                    posture["PACOMIND_AUTONOMY_MODE"] = (
                        raw if raw in ("reactive", "proactive") else "reactive")
                    posture["PACOMIND_AUTONOMY_MODE_SOURCE"] = "env"
                else:
                    from pacomind.util.autonomy_preset import coupled_loop_mode
                    coupled = coupled_loop_mode()
                    if coupled:
                        posture["PACOMIND_AUTONOMY_MODE"] = coupled
                        posture["PACOMIND_AUTONOMY_MODE_SOURCE"] = "preset"
                    elif os.environ.get("PACOMIND_AUTONOMY_TICK_INTERVAL_SECS"):
                        posture["PACOMIND_AUTONOMY_MODE"] = "proactive"
                        posture["PACOMIND_AUTONOMY_MODE_SOURCE"] = "legacy_tick"
                    else:
                        posture["PACOMIND_AUTONOMY_MODE"] = "reactive"
                        posture["PACOMIND_AUTONOMY_MODE_SOURCE"] = "default"
        except Exception:
            pass
...
async def get_world_llm_extract_status() -> dict:
    if _world_llm_extractor is None:
        return {"available": False}
    from pacomind.world_model.llm_extract import llm_extract_mode
...
async def list_proposals(
    ...
    """List proposals PacoMind has generated (observability)."""
    if _proposal_store is None:
        return {"available": False, "proposals": []}
    try:
...
async def list_skills() -> SkillsListResponse:
    ...
    try:
        ...
        for s in skills:
            ...
            for skip in ("created_at", "updated_at", "author_pacomind_id", "status", "input_schema", "tags", "trigger_patterns"):
                d.pop(skip, None)
...
async def list_skill_drafts() -> dict:
    ...
    if _skills_registry is None:
        return {"drafts": []}
    try:
        from pacomind.skills.models import SkillStatus
...
async def approve_skill(skill_id: str) -> dict:
    ...
    try:
        ...
        # by PACOMIND_EMIT_HERMES_SKILLS (off by default) and a procedural
        # heuristic; a failure here must never block activation.
        try:
            from pacomind.skills.hermes_export import export_approved_skill
...
async def identity_status() -> IdentityStatusResponse:
    ...
    try:
        ...
        pacomind_id = _chain_manager.pacomind_id
        pubkey = None
        ...
        if key_mgr is not None:
            try:
                ...
                from pacomind.chain.identity import is_genesis as check_genesis
                is_genesis_flag = check_genesis(pacomind_id, pubkey)
            except Exception:
                pass
        ...
        try:
            from pacomind.chain.node import get_node_info, load_node_certificate
        ...
        from pacomind.chain.identity import get_genesis_manifest
        manifest = get_genesis_manifest()
        trust_anchor_verified = manifest is not None
        if is_genesis_flag:
            trust_tier = "GENESIS"
        elif keys_configured and trust_anchor_verified:
            # A properly-keyed pacomind sitting under a verified Genesis anchor
        ...
        return IdentityStatusResponse(
            ...
            initialized=pacomind_id is not None,
...
async def chain_verify(body: ChainVerifyRequest) -> ChainVerifyResponse:
    ...
    The attestation is ``sign(pacomind_id || ':' || data || ':' || now)``
    using the pacomind's Ed25519 private key. Callers verify it with
        ...
        if is_valid:
            ...
            if key_mgr is not None:
                try:
                    ...
                    payload = (
                        f"{pacomind_id}:{body.data}:{attested_at}".encode("utf-8")
                    )
...
async def cognition_trigger(body: CognitionTriggerRequest) -> CognitionTriggerResponse:
    ...
    The PacoMind plugin picks this up and calls sessions_spawn with the
...
async def create_world_entity(body: WorldEntityCreateRequest) -> WorldEntityDetailResponse:
    ...
    try:
        from pacomind.world_model.entities import BaseEntity, ENTITY_CLASS_MAP
        from pacomind.world_model.sqlite.backend import _generate_id
...
async def world_causal_chain(
    ...
    if _world_store is None:
        raise HTTPException(status_code=501, detail="World model not initialized")
    try:
        from pacomind.world_model.causal_query import causal_chain
...
async def world_causal_edges(min_confidence: float = 0.0,
    ...
    if _world_store is None:
        raise HTTPException(status_code=501, detail="World model not initialized")
    try:
        from pacomind.world_model.causal_query import causal_edges
...
async def create_agent_invite(body: AgentInviteRequest) -> AgentInviteResponse:
    ...
    pacomind_id = os.environ.get("PACOMIND_ID", str(uuid.uuid4()))
    ...
    setup_command = f"pacomind agent connect --setup-code {invite['setup_code']} --pacomind-url {pacomind_url}"
...
async def connect_remote_agent(body: AgentConnectRequest) -> AgentConnectResponse:
    ...
    # cert is signed by the PacoMind's Ed25519 key over the canonical payload
    ...
    # Build websocket URL
    pacomind_url = os.environ.get("PACOMIND_URL", "ws://localhost:7777")
    ws_url = f"{pacomind_url.replace('http', 'ws')}/v1/host/agents/{agent_id}/stream"
...
async def refresh_initiative_context(initiative_id: str) -> InitiativeResponse:
    ...
    from pacomind.initiatives.context_freshness import DURABLE, durability_for
...
def _initiative_to_response(initiative) -> InitiativeResponse:
    ...
    from pacomind.initiatives.context_freshness import durability_for
...
async def respond_to_initiative(
    ...
    if job_id and action in {
        ...
        try:
            from pacomind.api.routers import task_queue as approval_router
...
async def agent_snapshot() -> AgentSnapshotResponse:
    """Return a comprehensive snapshot of PacoMind state for agent evaluation."""
    now = datetime.now(timezone.utc)
...
async def session_report(body: SessionReportRequest) -> SessionReportResponse:
    ...
    from pacomind.sessions.reports import SessionReport
