...
def authorized_viewer(request: Request, contact_id: str, *, scope: str) -> tuple[str, bool]:
    ...
    owner = os.environ.get("PACOMIND_OWNER_PERSON_ID", "").strip() or os.environ.get("PACOMIND_OWNER_CONTACT_ID", "").strip()
...
async def with_queue_work(view, *, owner, limit=8):
    ...
    from pacomind.turns.executions import work_source_coverage
    import time
