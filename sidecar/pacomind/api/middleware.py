"""API key authentication + request-size middleware for PacoMind sidecar."""
...
# PACOMIND_MAX_BODY_BYTES in the environment.
_DEFAULT_MAX_BODY_BYTES = 10 * 1024 * 1024


# Endpoints reachable without a key, even in "dev mode" (no PACOMIND_API_KEY).
...
# PACOMIND_API_KEY presence — these accept or return credential-grade state
...
class ApiKeyMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        request.state.pacomind_authority = anonymous_authority()
        request.state.pacomind_auth_telemetry = self._telemetry
        request.state.pacomind_contact_grants = self._contact_grants
        request.state.pacomind_keyring_status = self._keyring.status()
        request.state.pacomind_auth_configuration = {
        ...
        if exact_approval_scope and approval_mode not in {"shadow", "enforce"}:
            self._record(
                request, authority=request.state.pacomind_authority,
        ...
        if not auth_configured:
            ...
            if (
                ...
                return JSONResponse(
                    ...
                    content={
                        "detail": (
                            "Set PACOMIND_API_KEY or PACOMIND_API_KEYRING_PATH in "
                            "the sidecar environment to enable this endpoint."
                        )
                    },
                )
            query_error = self._query_scope_error(
                request, request.state.pacomind_authority
            )
            ...
            return await self._call_authorized(
                request, call_next, authority=request.state.pacomind_authority,
                scope=scope, route=route,
            )
        ...
        if token:
            ...
            if authority is not None:
                claimed_principal = request.headers.get("X-PacoMind-Principal", "").strip()
