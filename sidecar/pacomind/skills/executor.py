"""PacoMind Skills — skill executor with capability-gated sandbox."""
...
from pacomind.skills.base import (
...
def _resolve_sandbox_mode() -> str:
    ...
    override = (os.environ.get("PACOMIND_SKILL_SANDBOX") or "").strip().lower()
    if override in ("subprocess", "inprocess"):
        if override == "inprocess" and _DEFAULT_SANDBOX_MODE == "subprocess":
            logger.warning(
                "PACOMIND_SKILL_SANDBOX=inprocess overrides the default subprocess "
...
class SkillExecutor:
    """Executes PacoMind skills inside a capability-gated sandbox.
    ...
    Execution pipeline:
      ...
      4. Execute either in a hardened subprocess (default on Linux) or
         in-process (rollback path, controlled by PACOMIND_SKILL_SANDBOX).
    ...
    async def _run_subprocess(
        ...
        proc = await asyncio.create_subprocess_exec(
            sys.executable,
            "-m",
            "pacomind.skills.sandbox_runner",
    ...
    async def _run_skill_inprocess(
        ...
        """Legacy in-process execution. Used only when PACOMIND_SKILL_SANDBOX=inprocess."""
        ...
        # Inject pacomind runtime if the skill signature expects it
        import inspect
        sig = inspect.signature(run_fn)
        if "pacomind" in sig.parameters:
            from pacomind.skills.runtime import PacoMindRuntime
            pacomind_rt = PacoMindRuntime(base_url=os.environ.get("PACOMIND_URL", "http://127.0.0.1:7777"))
            inputs = {"pacomind": pacomind_rt, **inputs}
