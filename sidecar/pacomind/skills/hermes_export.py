"""PacoMind Skills — render approved skills as Hermes SKILL.md documents (v0.18.0).

The Hermes↔PacoMind skills bridge, outbound half: when a captured DRAFT
skill is approved (DRAFT→ACTIVE), PacoMind can also publish it as an
*instructional* Hermes skill — a ``SKILL.md`` with YAML frontmatter
under ``~/.hermes/skills/pacomind/<skill-slug>/`` — so the agent host can
load the procedure as guidance, independent of PacoMind's sandboxed
executable skill.
...
* Skills whose pattern/source carries trace-derived steps — a
  non-empty ``step_sequence``, ``pacomind.tools.invoke`` replay calls in
...
* Anything ambiguous (no source, unreadable skill dir, bare manifest)
  ...
  ``PACOMIND_EMIT_HERMES_SKILLS`` env switch and human approval.
...
HERMES_AUTHOR = "PacoMind (captured from agent work)"
...
PROVENANCE_MARKER = "pacomind:provenance"

#: Env switch — exports are off unless this is truthy.
EMIT_ENV = "PACOMIND_EMIT_HERMES_SKILLS"

#: Env override for the export base directory.
BASE_DIR_ENV = "PACOMIND_HERMES_SKILLS_DIR"

_DEFAULT_BASE = Path("~/.hermes/skills/pacomind")

_SLUG_RE = re.compile(r"[^a-z0-9-]+")
_TOOL_INVOKE_RE = re.compile(r"pacomind\.tools\.invoke\(\s*['\"]([^'\"]+)['\"]")
...
def hermes_base_dir(base_dir: Optional[Path] = None) -> Path:
    ...
    if sync_root:
        return (Path(sync_root) / "pacomind").expanduser()
...
def load_body_source(manifest: Any) -> Dict[str, Any]:
    ...
    ``pacomind.tools.invoke``). Returns a dict shaped like the pattern
...
def is_procedural(manifest: Any, pattern_or_source: Optional[Dict[str, Any]]) -> bool:
    ...
    if steps:
        return True
    if "pacomind.tools.invoke" in source:
        return True
...
def _render_frontmatter(manifest: Any, slug: str) -> str:
    description = str(_get(manifest, "description") or "").strip() or (
        f"PacoMind-captured skill {slug}"
    )
    ...
    lines.append(f"#   pacomind_skill_id: {_yaml_str(_get(manifest, 'skill_id') or slug)}")
...
def render_skill_md(manifest: Any, pattern_or_source: Dict[str, Any]) -> str:
    """Render a Hermes-faithful SKILL.md for an approved PacoMind skill.
    ...
    body.append("")
    body.append(description or f"Procedure captured by PacoMind from agent task work ({skill_id}).")
    ...
    body.append("---")
    body.append(
        f"*Captured by PacoMind from agent work (pacomind skill `{skill_id}`"
        + (
            f", origin task `{_get(manifest, 'origin_task_id')}`"
            if _get(manifest, "origin_task_id")
            else ""
        )
        + "). The executable form runs sandboxed inside PacoMind; this "
...
def export_to_hermes(
    ...
    (``PACOMIND_EMIT_HERMES_SKILLS`` defaults to off) or the target file
    exists and was not authored by PacoMind (no provenance marker in its
    ...
    if target.exists():
        ...
        if PROVENANCE_MARKER not in _frontmatter_block(existing):
            logger.warning(
                "Hermes export: %s exists and is not pacomind-authored — refusing to overwrite",
                target,
            )
            return None
