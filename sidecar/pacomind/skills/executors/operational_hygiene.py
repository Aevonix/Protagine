...
class OperationalHygieneSkill(InitiativeExecutorSkill):
    ...
    async def _handle_backup(
        ...
        stores = ["pacomind.db", "pacomind-contacts.db"]
        backed_up = []
        for store_name in stores:
            source = Path(os.path.expanduser(f"~/.pacomind/data/{store_name}"))
            if source.exists():
    ...
    async def _handle_log_rotation(
        ...
        log_dir = Path(os.path.expanduser("~/.pacomind/logs"))
    ...
    async def _handle_disk_cleanup(
        ...
        cleaned_bytes = 0
        paths_to_clean = [
            Path(os.path.expanduser("~/.pacomind/cache")),
            Path(os.path.expanduser("~/.pacomind/tmp")),
        ]
