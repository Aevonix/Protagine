"""PacoMind Skills — pattern extraction from task solutions."""
...
class PatternExtractor:
    ...
    def _synthesize_source(
        ...
        each call via ``pacomind.tools.invoke`` so the skill actually runs
        ...
        # ``pacomind.tools.invoke(name, args)`` for dynamic dispatch; skills
        ...
        for i, entry in enumerate(tool_calls[:10]):
            ...
            replay_lines.append(
                f"    _r{i} = await pacomind.tools.invoke({tool_name!r}, {args_repr})"
            )
        replay_block = "\n".join(replay_lines)
        ...
        return textwrap.dedent(f"""\
            ...
            async def run(pacomind, {params}) -> Any:
