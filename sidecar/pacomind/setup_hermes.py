"""The guided Hermes path of ``pacomind init``.
...
def _select_state_environment(state):
    os.environ['PACOMIND_STATE_DIR'] = str(state)
...
def _owner_handles(args, ask, noninteractive):
    ...
    for value in values:
        ...
        if (not separator or not re.fullmatch(r'[a-z0-9][a-z0-9_.-]{0,63}', platform)
                or platform in {'cli', 'internal', 'system', 'background_review', 'pacomind_task'}
...
def _adapter_resources(wheel=None):
    packages = ('pacomind_hermes/', 'pacomind_memory/')
    if wheel:
        ...
        try:
            distribution = importlib.metadata.distribution('pacomind-hermes')
        except importlib.metadata.PackageNotFoundError:
            raise ValueError('Install the canonical pacomind-hermes package alongside pacomind, or supply --adapter-wheel') from None
    ...
    for name in ('pacomind_hermes/__init__.py', 'pacomind_hermes/evidence.py', 'pacomind_hermes/client.py',
                 'pacomind_hermes/commitment_work.py', 'pacomind_memory/__init__.py', 'pacomind_memory/provider.py'):
...
def _preflight_outbox(home, resources):
    ...
    """
    name = '_pacomind_setup_pathcheck'
    module = types.ModuleType(name)
    sys.modules[name] = module
    selected = home/'state'/'pacomind-turn-outbox.sqlite3'
    inspected = selected
    try:
        exec(compile(resources['pacomind_hermes/client.py'], '<canonical PacoMind client>', 'exec'), module.__dict__)
...
selected = []
for group, name, module in [('hermes_agent.plugins', 'pacomind', 'pacomind_hermes'),
                            ('hermes_agent.memory_providers', 'pacomind-memory', 'pacomind_memory')]:
    matches = [ep for ep in entries.select(group=group) if ep.name == name]
    if len(matches) > 1 or (matches and matches[0].value != module):
        raise ValueError('Conflicting PacoMind entry point')
    selected.append(matches[0] if matches else None)
if not any(selected):
    print(json.dumps({'mode': 'private-directory'}))
    sys.exit(0)
if not all(selected):
    raise ValueError('Both canonical native PacoMind entry points are required')
...
'''], input=json.dumps(expected), capture_output=True, text=True, timeout=30)
    if probe.returncode:
        raise ValueError('Hermes has an incomplete or different installed PacoMind adapter; select its matching artifact, upgrade that package explicitly, or use a separate Hermes interpreter')
...
def _forwarder(site, module, memory=False):
    ...
    hint = '# PacoMindMemoryProvider native provider entry point.\n' if memory else ''
...
def refresh_adapter(state, args):
    ...
    if binding['mode'] == 'private-directory':
        ...
        def retain_forwarder(profile, *, memory=False):
            names = ('pacomind-memory',) if memory else ('pacomind',)
            ...
            modules = ('pacomind_memory',) if memory else ('pacomind_hermes',)
            ...
            canonical = 'pacomind_memory' if memory else 'pacomind_hermes'
    ...
    try:
        if copied_change:
            staged = Path(tempfile.mkdtemp(prefix='.pacomind-adapter-', dir=state))
            for name, content in resources.items():
                _private_write(staged/name, content)
            # Hermes and PacoMind are stopped by the operator's existing lifecycle.
    ...
    print('Start this PacoMind instance and new Hermes processes through their existing lifecycle.')
...
def run(root_dir=None, args=None):
    ...
    try:
        ...
        env = dict(os.environ)
        state = Path(root_dir or env.get('PACOMIND_STATE_DIR') or
                     plugin_settings(current_config).get('instance_dir') or home/'pacomind').expanduser().resolve()
        if state == home or home.is_relative_to(state):
            raise ValueError('The private PacoMind directory must not contain the Hermes home')
        ...
        if any(name in config.get('plugins', {}).get('disabled', []) for name in ('pacomind',)):
            raise ValueError('PacoMind is explicitly disabled in this home; resolve that setting before attachment')
        ...
        if (state/'instance.json').exists():
            ...
            print(f'Use pacomind --instance {str(state)!r} start, then status.')
            return 0
        if getattr(args, 'refresh_adapter', False):
        ...
        for name in ('pacomind', 'pacomind-memory'):
            if (home/'plugins'/name).exists():
                raise ValueError('An existing PacoMind directory adapter needs an explicit upgrade; choose another home for this installer')
        selected_provider = config.get('memory', {}).get('provider')
        replace_provider = bool(getattr(args, 'replace_memory_provider', False))
        if selected_provider not in (None, '', 'pacomind', 'pacomind-memory') and not replace_provider:
        ...
        model_key = env.get('PACOMIND_MODEL_API_KEY', '')
        ...
        env_updates = {'PACOMIND_NATIVE_API_KEY': key, 'PACOMIND_GENERAL_PLUGIN_ACTIVE': '1',
        ...
        staged = Path(tempfile.mkdtemp(prefix='.pacomind-init-', dir=state.parent))
        try:
            ...
            values = {**PRESETS['passive'], 'PACOMIND_INSTALL_PROFILE': 'local', 'PACOMIND_STATE_DIR': str(state),
                'PACOMIND_SIDECAR_HOST': '127.0.0.1', 'PACOMIND_SIDECAR_PORT': str(port),
                'PACOMIND_OWNER_CONTACT_ID': owner_id, 'PACOMIND_OWNER_NAME': owner_name,
                'PACOMIND_PERSONA_NAME': agent_name, 'PACOMIND_CONTACTS_DB': str(state/'contacts.db'),
                'PACOMIND_API_KEYRING_PATH': str(state/'api-keyring.json'), 'PACOMIND_API_KEY': '',
                'PACOMIND_CLIENT_API_KEY': key, 'PACOMIND_GRAPH_ENABLED': 'false',
                'PACOMIND_EMBED_PROVIDER': 'skip',
                'PACOMIND_AUTONOMY_PRESET': 'passive', 'PACOMIND_EMBEDDED_WORKER_ENABLED': 'false',
                'PACOMIND_SOURCE_CLAIMS': 'on'}
            values.update({
                'PACOMIND_AGENT_VALUES': json.dumps(agent_preferences['values'], ensure_ascii=True),
                'PACOMIND_AGENT_TIMEZONE': agent_preferences['timezone'],
                'PACOMIND_AGENT_QUIET_HOURS': agent_preferences['quiet_hours'],
            })
            if goal_details is not None:
                values['PACOMIND_HERMES_WORK_BOARDS'] = json.dumps(goal_details['boards'], separators=(',', ':'))
            ...
            if replace_provider:
                ...
                candidate['memory']['provider'] = 'pacomind-memory'
            ...
            plugin = candidate['plugins']['pacomind']
            plugin.update(instance_dir=str(state), owner_contact_id=owner_id,
                api_key='${PACOMIND_NATIVE_API_KEY}', execution_registry_enabled=True,
                attested_system_platforms=['cli'], enabled_action_tools=[], enabled_message_tools=[],
                turn_outbox_path=str(home/'state'/'pacomind-turn-outbox.sqlite3'))
            candidate['memory']['config'].update(api_key='${PACOMIND_NATIVE_API_KEY}', turn_writer='disabled')
        ...
        try:
            ...
            if binding['mode'] == 'private-directory':
                for directory, module in (('pacomind', 'pacomind_hermes'), ('pacomind-memory', 'pacomind_memory')):
                    create(home/'plugins'/directory/'__init__.py', _forwarder(state/'adapter', module, directory == 'pacomind-memory'))
                    create(home/'plugins'/directory/'plugin.yaml', resources[module+'/plugin.yaml'])
                create(home/'plugins'/'pacomind-memory'/'cli.py', _forwarder(state/'adapter', 'pacomind_memory.cli'))
        ...
        print(f'Start: pacomind --instance {str(state)!r} start --detach')
        print(f'Status: pacomind --instance {str(state)!r} status')
        ...
        if getattr(args, 'start', False) or (not noninteractive and ask('Start this sidecar now? [Y/n]', 'Y').lower() in {'y','yes'}):
            result = subprocess.run([sys.executable, '-m', 'pacomind', '--instance', str(state), 'start', '--detach'], timeout=60)
