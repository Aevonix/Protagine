...
from pacomind.beliefs.contradictions import (
    claims_from_text, detect_conflicts, property_claims,
)
from pacomind.beliefs.decay import decay_entity, stale_entities
from pacomind.beliefs.models import Claim, beliefs_mode
from pacomind.beliefs.resolve import pick_winner
...
class BeliefEngine:
    ...
    def note_property_update(self, entity_id: str, key: str,
        ...
        from pacomind.beliefs.models import norm_value
    ...
    def _effective_mode(self) -> str:
        ...
        could never graduate. PACOMIND_BELIEFS_SUPERVISED_LIVE=1 adds a
        ...
        (generic across domains; enable via PACOMIND_SUPERVISED_LIVE_DOMAINS=
        beliefs or the legacy alias PACOMIND_BELIEFS_SUPERVISED_LIVE=1). The
        reversible operations named above are pinned in its
        REVERSIBLE_CONTRACT under ("beliefs", "supersede"/"decay")."""
        from pacomind.self_model.supervised import effective_mode
    ...
    def _record_outcome(self, mode: str, report: Dict[str, Any]) -> None:
        ...
        Strict (PACOMIND_TRUST_STRICT_OUTCOMES=1, the default): a run where
    ...
    async def _causal_pass(self, report: Dict[str, Any], mode: str) -> None:
        ...
        STALENESS: edges unsupported past PACOMIND_CAUSAL_TTL_DAYS lose 0.05
