...
def beliefs_mode() -> str:
    ...
    return resolve("PACOMIND_BELIEFS_MODE", ("off", "shadow", "live"), "shadow")
...
def beliefs_supervised_live() -> bool:
    ...
    PACOMIND_SUPERVISED_LIVE_DOMAINS=beliefs and the legacy
    PACOMIND_BELIEFS_SUPERVISED_LIVE=1 are equivalent."""
...
def stale_ttl_days() -> float:
    try:
        return max(1.0, float(os.environ.get("PACOMIND_BELIEFS_STALE_DAYS", "90")))
