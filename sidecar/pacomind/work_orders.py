...
PacoMind owns the goal/work ledger; it does not execute external effects.  A
...
from pacomind.execution_results import (
...
from pacomind.scope_bounds import VIEWER_SCOPE_MAX_CHARS
from pacomind.task_queue.models import Job, JobPriority, JobStatus, JobType
...
def load_receipt_verifier_from_env(environ: Optional[Mapping[str, str]] = None) -> Any:
    ...
    The verifier remains an operator-selected plugin.  PacoMind knows only a
    ...
    spec = str(env.get("PACOMIND_WORK_ORDER_RECEIPT_VERIFIER", "") or "").strip()
    if not spec:
        return None
    if len(spec) > 512 or not _IMPORT_PATH.fullmatch(spec):
        raise ReceiptVerifierConfigurationError(
            "PACOMIND_WORK_ORDER_RECEIPT_VERIFIER must be module:attribute"
        )

    raw_config = str(
        env.get("PACOMIND_WORK_ORDER_RECEIPT_VERIFIER_CONFIG", "{}") or "{}"
    )
...
class QueueWorkOrderAdapter:
    """Idempotently post/poll WorkOrderV1 jobs on PacoMind's durable queue."""
