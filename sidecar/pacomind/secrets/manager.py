"""SecretsManager — unified secrets access layer for PacoMind.

All PacoMind code MUST use SecretsManager to access secrets.
...
The backend is loaded from ~/.pacomind/config.yaml on first instantiation
and cached for the process lifetime.  Override ``PACOMIND_SECRETS_BACKEND``
...
from pacomind.secrets.types import SecretType
...
class SecretsManager:
    """Unified secrets access layer for PacoMind.
    ...
    def _load_backend(cls) -> SecretsBackend:
        ...
        backend_name = os.environ.get("PACOMIND_SECRETS_BACKEND")
        ...
        if not backend.is_available():
            logger.warning(
                ...
                "falling back to 'env'. Run `pacomind secrets status` for details.",
                backend_name,
            )
    ...
    def _read_config_backend(cls) -> str | None:
        """Read secrets.backend from ~/.pacomind/config.yaml."""
        try:
            import yaml
            from pathlib import Path

            pacomind_home = Path(os.environ.get("PACOMIND_HOME", Path.home() / ".pacomind"))
            config_path = pacomind_home / "config.yaml"
    ...
    def get_required(self, key: str) -> str:
        ...
        if value is None:
            raise KeyError(
                ...
                f"Run `pacomind secrets set {key} <value>` to configure it."
            )
        return value
