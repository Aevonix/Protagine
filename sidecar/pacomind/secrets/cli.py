"""CLI command handlers for `pacomind secrets`."""
...
_SECTION_KEYS: dict[str, list[str]] = {
    ...
    "Infrastructure": ["NEO4J_URI", "NEO4J_USERNAME", "NEO4J_PASSWORD", "PACOMIND_TOKEN_SECRET"],
    "Messaging Gateways": [
...
def cmd_secrets_list(args: "argparse.Namespace") -> None:
    """pacomind secrets list — show configured secrets grouped by category."""
    sm = _get_manager()
    ...
    if unconfigured:
        ...
        print("Not configured (run `pacomind secrets set KEY VALUE`):")
        for k in shown:
            print(f"  \u2717 {k}")
        if remaining > 0:
...
def cmd_secrets_get(args: "argparse.Namespace") -> None:
    """pacomind secrets get KEY — retrieve a secret."""
    sm = _get_manager()
...
def cmd_secrets_set(args: "argparse.Namespace") -> None:
    """pacomind secrets set KEY VALUE — store a secret."""
    from pacomind.secrets.types import infer_secret_type
    sm = _get_manager()
...
def cmd_secrets_delete(args: "argparse.Namespace") -> None:
    """pacomind secrets delete KEY — remove a secret."""
    sm = _get_manager()
    sm.delete(args.key)
...
def cmd_secrets_backend(args: "argparse.Namespace") -> None:
    """pacomind secrets backend — show current backend."""
    sm = _get_manager()
    ...
    if sm.backend_name == "1password":
        ...
        print("  Storage: ~/.pacomind/.env (plaintext, mode 0600)")
    print("\nChange backend: pacomind setup  (re-run onboarding wizard)")


def cmd_secrets_status(args: "argparse.Namespace") -> None:
    """pacomind secrets status — check backend availability."""
...
def cmd_secrets_migrate(args: "argparse.Namespace") -> None:
    """pacomind secrets migrate — move secrets between backends."""
    from pacomind.secrets.backends.env import EnvBackend
    from pacomind.secrets.backends.keyring import KeyringBackend
    from pacomind.secrets.backends.onepassword import OnePasswordBackend
    from pacomind.secrets.migration import SecretsMigrator
