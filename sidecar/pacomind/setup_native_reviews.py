...
def configure(state, *, install=False):
    ...
    if worker.exists():
        ...
        subprocess.run([manifest['hermes_python'], '-B', '-c',
            ...
            'create_profile("pacomind-reviews",no_alias=True,no_skills=True)'],
    ...
    if install:
        ...
        adapter = (state/'adapter/pacomind_hermes' if binding_info['mode'] == 'private-directory'
        ...
        write(worker/'SOUL.md',
            ...
            'Retained tasks may name pacomind_read_work_source or pacomind_review_report; use the '
            'corresponding advertised pacomind_read_work_source or pacomind_review_report tools.\n'
            'Use pacomind_read_work_source(0) for the registered observation. For log-volume reviews, '
            ...
            'Report with pacomind_review_report. It is your interface to the existing native '
