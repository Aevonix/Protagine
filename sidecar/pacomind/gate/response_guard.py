...
from pacomind.gate.communication_policy import CommunicationPolicyContextV1
from pacomind.gate.layers.l2_pii import PIIScanner
from pacomind.gate.layers.l4_trust_tier import TrustTierChecker
from pacomind.gate.layers.l5_injection import InjectionDetector
...
_guard_audit = logging.getLogger("pacomind.gate.guard")
...
def enforce_allowlist() -> Optional[frozenset]:
    ...
    PACOMIND_GUARD_ENFORCE_CHECKS, default ``secret_leak,tom2_epistemic`` —
    ...
    """
    raw = os.environ.get("PACOMIND_GUARD_ENFORCE_CHECKS",
...
def breaker_enabled() -> bool:
    """PACOMIND_GUARD_BREAKER, default on. off/0/false disables the breaker."""
    return os.environ.get("PACOMIND_GUARD_BREAKER", "on").strip().lower() not in (
        "off", "0", "false", "no")
...
def breaker_trip_blocks() -> int:
    ...
    <=0 disables tripping."""
    try:
        return int(os.environ.get("PACOMIND_GUARD_TRIP_BLOCKS", "10"))
