...
* **Deployment-configured gateways.** PacoMind ships NO gateway names.
  ``PACOMIND_ENV_RISK_GATEWAY_CLASS`` (default empty) maps gateway families to
...
from pacomind.channels.presence import STRONG_METHODS
...
def env_risk_window_hours() -> float:
    """PACOMIND_ENV_RISK_WINDOW_HOURS (default 48): census recency window."""
    try:
        v = float(os.environ.get("PACOMIND_ENV_RISK_WINDOW_HOURS", "48"))
...
def gateway_class(gateway: str) -> str:
    ...
    PACOMIND_ENV_RISK_GATEWAY_CLASS is a comma list of ``gateway:class`` pairs
    ...
    if not g:
        return ""
    raw = os.environ.get("PACOMIND_ENV_RISK_GATEWAY_CLASS", "")
