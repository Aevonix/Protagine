"""Self-directed thinking — PacoMind's internal cognition phase (v0.17.0).
...
an LLM a snapshot of PacoMind's situation — goals, commitments, recent
...
- Runs at most once per ``PACOMIND_THINKING_INTERVAL_SECS`` (default 1h),
  not every loop tick.
- At most ``PACOMIND_THINKING_MAX_INITIATIVES`` (default 3) per cycle.
...
class SelfDirectedThinker:
    ...
    def __init__(self, router: Any,
        ...
        self._interval = interval_secs if interval_secs is not None else int(
            os.environ.get("PACOMIND_THINKING_INTERVAL_SECS", "3600"))
        self._max = max_per_cycle if max_per_cycle is not None else int(
            os.environ.get("PACOMIND_THINKING_MAX_INITIATIVES", "3"))
