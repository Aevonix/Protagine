"""PacoMind relationship scoring — evidence-based assessment of PacoMind's own social world.

PacoMind maintains its own relationship scores for every person it interacts with across
connected gateways (iMessage, Telegram, etc.). These scores represent PacoMind's independent
assessment of each relationship, built from interaction patterns PacoMind itself observes.

Two-layer scoring model
-----------------------
Owner layer (``owner_guidance`` weight = 0.18): the owner's explicit input informs
PacoMind's view but does not dominate it. Owner input is authoritative at onboarding
when PacoMind has no history; its relative weight naturally declines as PacoMind accumulates
its own evidence.

PacoMind observation layer (remaining 0.82): all signals PacoMind observes directly —
message frequency, sentiment, latency, initiative, recency — plus three PacoMind-specific
signals that capture the quality of PacoMind's own engagement: ``pacomind_initiative_response``
(do people respond to PacoMind's outreach?), ``pacomind_effectiveness`` (do PacoMind's actions
help?), and ``pacomind_style_fit`` (is PacoMind's adapted style working?).

A score of 80+ means PacoMind has observed strong, consistent engagement with this person
...
class ScoreWeights:
    ...
    Interaction signals are PacoMind's direct observations of the exchange.
    Owner guidance informs but does not dominate.
    PacoMind-specific signals capture the quality of PacoMind's own behavior.
    """
    # Interaction signals (PacoMind's observations of the exchange)
    ...
    # PacoMind-specific signals (new)
    pacomind_initiative_response: float = 0.08  # response rate to PacoMind's own outreach
    pacomind_effectiveness: float = 0.06        # are PacoMind's suggestions/actions valued?
    pacomind_style_fit: float = 0.04            # how well PacoMind's adapted style is received
    ...
    def total(self) -> float:
        return sum([
            ...
            self.recency,
            self.owner_guidance,
            self.pacomind_initiative_response,
            self.pacomind_effectiveness,
            self.pacomind_style_fit,
        ])


@dataclass
class ScoreBreakdown:
    """Breakdown of PacoMind's score into source layers.
    ...
    Attributes:
        ...
        pacomind_observation_contribution: Points from contact-behavior signals
        pacomind_action_contribution: Points from PacoMind's own behavior signals
        dominant_signal: Which single signal most influenced the score
    """
    person_id: str
    total_score: float
    owner_layer_contribution: float
    pacomind_observation_contribution: float
    pacomind_action_contribution: float
    dominant_signal: str
...
class TierChangeProposal:
    """A PacoMind-generated proposal to change a person's tier.

    PacoMind surfaces these via the briefing system. Owner approval is required
...
class RelationshipScorer:
    """PacoMind's evidence-based scoring of its own relationships with each person.

    Scores are PacoMind's independent assessment derived from interaction signals
    ...
    owner_guidance weight (0.18) but does not override PacoMind's evidence-based view.

    PacoMind-specific signals (pacomind_initiative_response, pacomind_effectiveness,
    pacomind_style_fit) capture the quality of PacoMind's own engagement behavior and
    carry a combined 0.18 weight.

    A score of 80+ means PacoMind has observed strong, consistent engagement with
    ...
    SIGNAL_RANGES = {
        ...
        # PacoMind-specific signals are already normalized 0-1
        "pacomind_initiative_response": (0, 1, False),
        "pacomind_effectiveness": (0, 1, False),
        "pacomind_style_fit": (0, 1, False),
        "contact_response_to_pacomind": (0, 1, False),
    }

    # PacoMind-specific signal types (direction="pacomind" or "bilateral")
    PACOMIND_SIGNAL_TYPES = frozenset({
        "pacomind_initiative",
        "pacomind_initiative_response",
        "pacomind_effectiveness",
        "contact_response_to_pacomind",
        "pacomind_style_fit",
        "pacomind_style_signal",
    })

    def __init__(self, graph: "PacoMindGraph", weights: Optional[ScoreWeights] = None, metrics=None):
        self._metrics = metrics  # Optional PacoMindMetricsCollector
        self.graph = graph
    ...
    async def compute_score(
        ...
        weight_map = {
            ...
            # PacoMind-specific signals
            "pacomind_initiative_response": self.weights.pacomind_initiative_response,
            "contact_response_to_pacomind": self.weights.pacomind_initiative_response,
            "pacomind_effectiveness": self.weights.pacomind_effectiveness,
            "pacomind_style_fit": self.weights.pacomind_style_fit,
            "pacomind_style_signal": self.weights.pacomind_style_fit,
        }
        ...
        # Informs PacoMind's assessment without overriding it.
    ...
    async def compute_score_with_breakdown(
        ...
        if not signals:
            bd = ScoreBreakdown(
                ...
                pacomind_observation_contribution=0.0,
                pacomind_action_contribution=0.0,
        ...
        pacomind_action_signals = {
            "pacomind_initiative_response", "contact_response_to_pacomind",
            "pacomind_effectiveness", "pacomind_style_fit", "pacomind_style_signal",
        }
        ...
        for sig_type, total in type_scores.items():
            if sig_type in weight_map and type_counts[sig_type] > 0:
                ...
                if sig_type in pacomind_action_signals:
        ...
        bd = ScoreBreakdown(
            ...
            pacomind_observation_contribution=observation_total,
            pacomind_action_contribution=action_total,
    ...
    def propose_tier_change(
        ...
        PacoMind MAY propose tier changes (upward or downward) based on its
        ...
        if evidence_idx > current_idx:
            ...
            direction = "upward"
            reasoning = (
                f"PacoMind has observed consistent engagement from this person "
            ...
            direction = "downward"
            reasoning = (
                f"PacoMind's engagement with this person has declined "
