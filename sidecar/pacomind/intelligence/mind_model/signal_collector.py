"""PacoMind's interaction signal collection — zero LLM, pure computation.

PacoMind extracts behavioral signals from every message it exchanges across connected
gateways. These signals build PacoMind's own picture of each person it interacts with:
how they communicate, when they're engaged, their emotional tone. PacoMind's relational
...
class SignalCollector:
    """Extract behavioral signals from PacoMind's own interactions — zero LLM, pure computation.

    Each message PacoMind sends or receives is an observation. SignalCollector turns
    those observations into quantified signals (frequency, sentiment, latency, etc.)
    that feed PacoMind's mind model and relationship scoring. PacoMind builds its social
    ...
    def __init__(self, baseline_store: BaselineStore, graph: PacoMindGraph, metrics=None):
    ...
    async def ingest_raw(self, signal_data: dict) -> None:
        ...
        try:
            from pacomind.models.signal import Signal, SignalType, SignalStrength
    ...
    async def collect(self, message: Message) -> List[Signal]:
        """Extract all signals from a single message in PacoMind's interaction stream."""
        t0 = time.monotonic()
        signals = []
    ...
    async def _compute_initiation_ratio(self, person_id: str, days: int = 30) -> float:
        ...
        from this person (not PacoMind's outbound messages), we approximate
        the initiation ratio as (person-initiated conversations) / total.
        A signal with ``source="inbound"`` means the person sent it;
        ``source="outbound"`` means PacoMind sent first.  If direction info
