"""PacoMind Graph Memory System — Neo4j async client.
...
def _recency_factor(days_old: float) -> float:
    ...
    Set PACOMIND_RECENCY_HALF_LIFE_DAYS<=0 to disable. The previous behaviour was a
    near-flat ~10%/year discount that barely affected ranking.
    """
    import os
    try:
        half_life = float(os.environ.get("PACOMIND_RECENCY_HALF_LIFE_DAYS", "90"))
    except (ValueError, TypeError):
        half_life = 90.0
    if half_life <= 0:
        return 1.0
    try:
        floor = float(os.environ.get("PACOMIND_RECENCY_FLOOR", "0.5"))
    except (ValueError, TypeError):
        floor = 0.5
...
def distill_turn_summary(summary: str) -> str:
    """The distilled form of a turn summary (what PACOMIND_DISTILL_TURNS=1 stores).
...
class PacoMindGraph:
    ...
    async def ensure_pacomind_self(self) -> None:
        """Ensure PacoMind's self-representation exists in the graph (v0.11.0).

        Creates an Agent node for PacoMind with DEPENDS_ON edges to
        ...
        try:
            async with self.driver.session(database=self.database) as session:
                # Create Agent node
                await session.run("""
                    MERGE (a:Agent {id: 'pacomind-sidecar'})
                    SET a.name = 'PacoMind',
                        a.version = '0.11.1',
                        a.status = 'active',
                ...
                for sub_id, sub_name in subsystems:
                    ...
                    await session.run("""
                        MATCH (a:Agent {id: 'pacomind-sidecar'}), (s:Subsystem {id: $id})
                        MERGE (a)-[r:DEPENDS_ON]->(s)
                        SET r.created_at = coalesce(r.created_at, datetime())
                    """, id=sub_id)

                logger.info("PacoMind self-representation verified in graph")
        except Exception as e:
            logger.warning("Failed to ensure PacoMind self-representation: %s", e)
        ...
        *,
        ...
        Args:
            fn: async callable ``fn(query, documents, top_k=N)`` returning a
                ...
                PACOMIND_RECALL_RERANK is ``shadow`` or ``on``.
    ...
    async def _embed_query(self, query: str) -> List[float]:
        ...
        PACOMIND_EMBED_QUERY_INSTRUCTION (set to empty for symmetric models).
        """
        ...
        instr = os.environ.get("PACOMIND_EMBED_QUERY_INSTRUCTION", default_instr)
    ...
    async def record_turn(
        ...
        # deployment adds its own via PACOMIND_MEMORY_SKIP_MARKERS ('|'-separated, case-insensitive).
        ...
        _env = os.environ.get("PACOMIND_MEMORY_SKIP_MARKERS", "")
        ...
        content = summary
        if os.environ.get("PACOMIND_DISTILL_TURNS", "0") not in ("0", "false", "no"):
    ...
    async def recall(
        ...
        *,
        ...
        PACOMIND_RECALL_MIN_STRENGTH (default 0.1, the historical floor).
        ...
        if min_strength is None:
            try:
                min_strength = float(os.environ.get(
                    "PACOMIND_RECALL_MIN_STRENGTH", "0.1"))
            except (TypeError, ValueError):
                min_strength = 0.1
        # Strength-blended ranking (PACOMIND_RECALL_STRENGTH_RANKING, default
        ...
        strength_ranking = os.environ.get(
            "PACOMIND_RECALL_STRENGTH_RANKING", "off").strip().lower() in (
            "on", "1", "true", "yes")
        person_scope = str(person_id).strip() if person_id else None
        ...
        hybrid = os.environ.get("PACOMIND_RECALL_HYBRID", "off").strip().lower() in (
            "on", "1", "true", "yes")
        ...
        if self._vector_store is not None and self._embed_fn is not None:
            try:
                ...
                from pacomind.vector.collections import Collection
                # Oversample the ANN fetch (PACOMIND_RECALL_OVERSAMPLE, default 1
                ...
                if person_scope:
                    try:
                        scope_oversample = max(1, int(os.environ.get(
                            "PACOMIND_RECALL_SCOPE_OVERSAMPLE", "20")))
                    except (TypeError, ValueError):
                        scope_oversample = 20
                    fetch_limit = max(
                        fetch_limit,
                ...
                if hybrid:
                    from pacomind.memory.recall import fuse_candidates
    ...
    async def decay_memories(
        ...
        - Fact/semantic memories use their own half-life (defaults to the
          ...
          episodes it came from by raising PACOMIND_DECAY_HALF_LIFE_SEMANTIC_DAYS).
        - Result is capped at 1.0.

        Half-lives resolve, in order: explicit argument, environment
        (PACOMIND_DECAY_HALF_LIFE_DAYS / PACOMIND_DECAY_HALF_LIFE_SEMANTIC_DAYS),
        ...
        if half_life_days is None:
            try:
                half_life_days = float(os.environ.get(
                    "PACOMIND_DECAY_HALF_LIFE_DAYS", "7"))
            except (TypeError, ValueError):
                half_life_days = 7.0
        if semantic_half_life_days is None:
            _sem_env = os.environ.get("PACOMIND_DECAY_HALF_LIFE_SEMANTIC_DAYS", "")
            try:
    ...
    _ALLOWED_CYPHER: frozenset = frozenset({
        ...
        """
            MERGE (b:BootstrapEvent {pacomind_id: $pacomind_id})
            SET b.pacomind_name      = $pacomind_name,
                b.pacomind_version   = $pacomind_version,
...
# ConnectionDiscoverer._find_entity_patterns
    ...
    async def store_signal(self, signal: Any) -> str:
        ...
        """
        from pacomind.intelligence.graph.queries import STORE_SIGNAL, GET_BASELINE, UPDATE_BASELINE
    ...
    async def get_recent_signals(
        ...
        """
        from pacomind.intelligence.mind_model.signal_collector import Signal
    ...
    async def record_score_change(
        ...
        from pacomind.intelligence.graph.queries import RECORD_SCORE_CHANGE
