"""PacoMind Graph Schema — Node and edge type definitions.

Every node and relationship type used in the PacoMind Neo4j graph is modelled
...
class Owner(BaseModel):
    """The PacoMind owner (singleton node — one per graph)."""
...
class Agent(BaseModel):
    """An autonomous entity (PacoMind itself, or other agents/bots)."""
...
class Subsystem(BaseModel):
    """A PacoMind component that can be monitored and restarted."""
...
class Capability(BaseModel):
    """A tool or skill that PacoMind has or needs."""
...
class Concept(BaseModel):
    """A knowledge domain or concept PacoMind has encountered."""
...
class EdgeType(str, Enum):
    """All relationship types in the PacoMind graph."""
