...
class MemoryDistiller:
    ...
    Args:
        graph_client: PacoMindGraph instance (or compatible mock with execute())
        metrics: Optional PacoMindMetricsCollector for instrumentation
        min_recalls: Minimum recall count for a memory to be a distillation
            candidate (None = PACOMIND_DISTILL_MIN_RECALLS, default 3)
        min_strength: Minimum strength for a candidate
            (None = PACOMIND_DISTILL_MIN_STRENGTH, default 0.5)
        min_age_days: Minimum age in days, only distill established memories
            (None = PACOMIND_DISTILL_MIN_AGE_DAYS, default 7)
    ...
    def __init__(
        ...
        self._min_recalls = (self._env_int("PACOMIND_DISTILL_MIN_RECALLS", 3)
                             if min_recalls is None else min_recalls)
        self._min_strength = (self._env_float("PACOMIND_DISTILL_MIN_STRENGTH", 0.5)
                              if min_strength is None else min_strength)
        self._min_age_days = (self._env_int("PACOMIND_DISTILL_MIN_AGE_DAYS", 7)
