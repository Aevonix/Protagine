...
something on her mind." Generic in PacoMind; the deployment feeds it events
...
from pacomind.scope_bounds import (
...
def _resolution_material(
    *,
    ...
    payload = {
        "schema": "PacoMindConcernResolutionReceiptV1",
        "version": 1,
...
def _cascade_intent_material(
    *,
    ...
    payload = {
        "schema": "PacoMindConcernCascadeIntentV1",
        "version": 1,
...
def _cascade_material(
    *,
    ...
    payload = {
        "schema": "PacoMindConcernCascadeReceiptV1",
        "version": 1,
        "intent_id": intent_id,
...
def workspace_mode() -> str:
    """Effective PACOMIND_WORKSPACE: explicit env > autonomy preset > off.
    ...
    return resolve("PACOMIND_WORKSPACE", ("off", "shadow", "live"), "off")
...
def _capacity() -> int:
    try:
        return int(os.environ.get("PACOMIND_WORKSPACE_CAPACITY", "24"))
    except ValueError:
        return 24
...
def _decay_half_life_hours() -> float:
    try:
        return float(os.environ.get("PACOMIND_WORKSPACE_HALFLIFE_HOURS", "12"))
    except ValueError:
        return 12.0
...
def _evict_floor() -> float:
    try:
        return float(os.environ.get("PACOMIND_WORKSPACE_EVICT_FLOOR", "0.05"))
    except ValueError:
        return 0.05
...
def _thought_budget() -> int:
    try:
        return int(os.environ.get("PACOMIND_WORKSPACE_THOUGHT_BUDGET", "8"))
    except ValueError:
        return 8
...
def _resolved_ttl_hours() -> float:
    ...
    try:
        return float(os.environ.get("PACOMIND_WORKSPACE_RESOLVED_TTL_HOURS", "24"))
    except ValueError:
        return 24.0


def in_sleep_window(now: Optional[datetime] = None) -> bool:
    """PACOMIND_SLEEP_WINDOW = 'HH:MM-HH:MM' in the deployment's local time
    (uses the process tz). Empty disables. Wrap-around (22:00-06:00) ok."""
    win = os.environ.get("PACOMIND_SLEEP_WINDOW", "").strip()
...
class ConcernStore:
    ...
    def _attach_cascade_evidence(
        ...
        receipt["cascade_evidence"] = {
            "schema": "PacoMindConcernCascadeStateV1",
            "version": 1,
            "intent_id": intent["intent_id"],
            "intent_digest": intent["record_digest"],
            "resolution_id": base["resolution_id"],
...
class WorkspaceEngine:
    ...
    this module so PacoMind stays model-agnostic and tests inject a fake.
    """
