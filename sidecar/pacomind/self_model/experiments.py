...
``PACOMIND_COGNITION_P4_MODE`` defaults to ``off``. In ``shadow`` the complete
...
def experiments_enabled() -> bool:
    return os.environ.get(
        "PACOMIND_EXPERIMENTS_ENABLED", "true").strip().lower() != "false"
...
def experiment_pregrants_from_env() -> Dict[str, Tuple[float, float]]:
    ...
    raw = os.environ.get("PACOMIND_EXPERIMENT_PREGRANTS_JSON", "").strip()
    if not raw:
        return {}
    try:
        decoded = json.loads(raw)
    except (TypeError, ValueError) as exc:
        raise ValueError("PACOMIND_EXPERIMENT_PREGRANTS_JSON is invalid JSON") from exc
...
class ExperimentEngine:
    ...
    def _binding(self, exp: Mapping[str, Any]) -> Any:
        from pacomind.initiatives.approval_authority import ActionBinding
