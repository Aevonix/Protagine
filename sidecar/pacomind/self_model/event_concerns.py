...
def event_concern_mode() -> str:
    value = os.environ.get("PACOMIND_EVENT_CONCERNS", "off").strip().lower()
...
def external_event_concern_mode() -> str:
    ...
    value = os.environ.get(
        "PACOMIND_EXTERNAL_EVENT_CONCERNS", "off",
    ).strip().lower()
    return value if value in {"off", "shadow", "live"} else "off"
...
def turn_concern_mode() -> str:
    ...
    value = os.environ.get("PACOMIND_TURN_CONCERNS", "off").strip().lower()
    return value if value in {"off", "shadow", "live"} else "off"
...
def _turn_channel_config() -> tuple[tuple[str, ...], bool]:
    ...
    raw = os.environ.get("PACOMIND_TURN_CONCERNS_CHANNELS", "")
    if not raw.strip():
        return (), False
    parts = raw.split(",")
...
def _turn_excluded_platform_config() -> tuple[tuple[str, ...], bool]:
    raw = os.environ.get(
        "PACOMIND_TURN_CONCERNS_EXCLUDED_PLATFORMS", "",
    )
    if not raw.strip():
        return (), True
    parts = raw.split(",")
...
def _turn_excluded_session_prefix_config() -> tuple[tuple[str, ...], bool]:
    raw = os.environ.get(
        "PACOMIND_TURN_CONCERNS_EXCLUDED_SESSION_PREFIXES", "",
    )
    if not raw.strip():
        return (), True
    parts = raw.split(",")
...
def _bootstrap_mode() -> str:
    value = os.environ.get("PACOMIND_EVENT_CONCERNS_BOOTSTRAP", "tail").strip().lower()
...
def _turn_bootstrap_mode() -> str:
    value = os.environ.get(
        "PACOMIND_TURN_CONCERNS_BOOTSTRAP", "tail",
    ).strip().lower()
    return value if value in {"tail", "replay"} else "tail"
...
class EventConcernReducer:
    ...
    def __init__(
        ...
        *,
        ...
        gap_policy_env: str = "PACOMIND_EVENT_CONCERNS_GAP_POLICY",
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            gap_policy_env="PACOMIND_EXTERNAL_EVENT_CONCERNS_GAP_POLICY",
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            gap_policy_env="PACOMIND_TURN_CONCERNS_GAP_POLICY",
