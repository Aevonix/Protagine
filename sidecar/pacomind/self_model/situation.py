...
understands PacoMind's durable host-journal envelope plus the structured event
families shared with the host.  Deployments may construct ``SituationObservationV1``
directly for attested probes/transports without patching PacoMind.
"""
...
_EVIDENCE_PREFIXES = frozenset({
    "journal", "event", "receipt", "host-event", "pacomind-event",
...
def situation_spine_mode() -> str:
    ...
    value = os.environ.get("PACOMIND_SITUATION_SPINE", "off").strip().lower()
...
class JournalSituationAdapter:
    ...
    def adapt(
        ...
        *,
        source_kind: str = "pacomind_event",
...
class SituationReducer:
    ...
    def _gap_policy() -> str:
        return os.environ.get(
            "PACOMIND_SITUATION_GAP_POLICY", "stop",
        ).strip().lower()
