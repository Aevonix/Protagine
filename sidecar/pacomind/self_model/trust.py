...
The env PACOMIND_<X>_MODE remains the owner override: "off" stays off and
...
def autograduate_enabled() -> bool:
    return os.environ.get(
        "PACOMIND_TRUST_AUTOGRADUATE", "true").strip().lower() != "false"
...
class TrustEngine:
    ...
    def snapshot(self) -> List[Dict[str, Any]]:
        from pacomind.self_model.supervised import supervised_enabled
        with self._lock:
            rows = self._conn.execute(
    ...
    def gate(self, domain: str, description: str, *,
        ...
        if floor:
            ...
            if conf >= _fenv("PACOMIND_TRUST_ACT_THRESHOLD", 0.8):
                decision = "act"
    ...
    def after_outcome(self, domain: str) -> None:
        ...
        try:
            ...
            pass
        window = _fenv("PACOMIND_TRUST_BREAKER_WINDOW_HOURS", 24.0) * 3600.0
        ...
        if stage == "act_first" and (
                violations > 0
                or failures >= _ienv("PACOMIND_TRUST_BREAKER_FAILURES", 3)):
            self.set_stage(
        ...
        if stage == "shadow":
            ...
            if n >= _ienv("PACOMIND_TRUST_ASK_MIN_N", 3) and bad == 0:
            ...
            conf = self.confidence(domain)
            if (len(real) >= _ienv("PACOMIND_TRUST_ACT_MIN_N", 5)
                    and conf >= _fenv("PACOMIND_TRUST_ACT_THRESHOLD", 0.8)
    ...
    def delivery_cap(self, base: int) -> int:
        ...
        record, bounded by PACOMIND_TRUST_DELIVERY_CAP_MAX."""
        cap_max = _ienv("PACOMIND_TRUST_DELIVERY_CAP_MAX", 6)
