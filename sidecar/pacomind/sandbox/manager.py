...
def sandbox_mode() -> str:
    ...
    return resolve("PACOMIND_SANDBOX_MODE", ("off", "dry_run", "live"), "off")
...
def resolve_limits() -> SandboxLimits:
    ...
    return SandboxLimits(
        image=os.environ.get("PACOMIND_SANDBOX_IMAGE", "python:3.12-slim"),
        cpus=_fenv("PACOMIND_SANDBOX_CPUS", 1.0),
        memory=os.environ.get("PACOMIND_SANDBOX_MEMORY", "512m"),
        timeout_secs=_ienv("PACOMIND_SANDBOX_TIMEOUT", 30),
        pids_limit=_ienv("PACOMIND_SANDBOX_PIDS", 128),
        egress=os.environ.get("PACOMIND_SANDBOX_EGRESS", "none").strip().lower(),
        max_artifact_bytes=_ienv("PACOMIND_SANDBOX_MAX_ARTIFACT_BYTES", 1_048_576),
    )
...
class SandboxManager:
    ...
    def run(self, script: str, lang: str = "python", *, purpose: str = "",
        ...
        if mode == "dry_run":
            from pacomind.sandbox.backend import DockerSandbox
