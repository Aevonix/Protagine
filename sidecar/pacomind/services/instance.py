...
class InstanceService:
    def __init__(self, state, hermes_home, *, python=None, platform=None, home=None, runner=None):
        ...
        self.label = 'ai.pacomind.instance.' + suffix
        if self.platform == 'darwin':
            ...
            self.name = 'pacomind-' + suffix + '.service'
    ...
    def selected(cls):
        ...
        if not os.environ.get('PACOMIND_STATE_DIR'):
            load_environment()
        if not os.environ.get('PACOMIND_STATE_DIR'):
    ...
    def render(self):
        arguments = [self.python, '-m', 'pacomind', '--instance', str(self.state), 'start']
        environment = {'HERMES_HOME': str(self.hermes_home), 'PACOMIND_INSTANCE_SERVICE': self.label,
                       'PYTHONUNBUFFERED': '1'}
        if self.platform == 'darwin':
        ...
        return ('[Unit]\nDescription=PacoMind private instance ' + self.label + '\n\n[Service]\nType=exec\n'
    ...
    def _require_installed(self):
        if not self._owned():
            raise ServiceError('Service is not installed for this instance; run pacomind service install')
        self._manager_ready()
    ...
    def healthy(self):
        ...
        if ':' in host:
            host = '[' + host + ']'
        key = os.environ.get('PACOMIND_CLIENT_API_KEY') or os.environ.get('PACOMIND_API_KEY', '')
        try:
            response = httpx.get(f'http://{host}:{os.environ.get("PACOMIND_SIDECAR_PORT", "7777")}/v1/host/health',
    ...
    def start(self, *, restart=False, timeout=30):
        ...
        if not status['running']:
            ...
            host = {'0.0.0.0': '127.0.0.1', '::': '::1'}.get(host, host)
            try:
                with socket.create_connection((host, int(os.environ.get('PACOMIND_SIDECAR_PORT', '7777'))), timeout=.2):
