...
Processes pending initiatives using PacoMind's own reasoning pipeline
...
What it does every cycle (default 30s):
  ...
  - Runs the ReasoningLoop (LLM + PacoMind tools) to determine and
    execute the appropriate action
  - Reports results back (complete/fail with retry)
  - Respects rate limits and concurrency caps

Auto-starts at sidecar boot when PACOMIND_EXECUTOR_ENABLED is "true"
and an LLM provider is configured. Stays dormant otherwise.

Environment / config:
  PACOMIND_EXECUTOR_ENABLED           "false" (default) / "true"
  PACOMIND_EXECUTOR_CYCLE_SECS        cycle interval (default 30)
  PACOMIND_EXECUTOR_MAX_PER_CYCLE     max initiatives per cycle (default 5)
  PACOMIND_EXECUTOR_MODEL_TIER        LLM tier: small/medium/large (default "small")
  PACOMIND_EXECUTOR_TYPES             comma-separated initiative types to handle
                                    (default: all non-self types)
  PACOMIND_EXECUTOR_AGENT_ID          agent identity (default "pacomind-executor")
  PACOMIND_EXECUTOR_MAX_TOOL_ITERS    max tool-continuation rounds per initiative
...
def _default_allowed_types() -> set[str]:
    ...
    deployment can still opt them back in via PACOMIND_EXECUTOR_TYPES.
    """
    try:
        from pacomind.delivery.classification import reachout_types
...
class InitiativeExecutorService:
    ...
    def __init__(
        ...
        agent_id: str = "pacomind-executor",
    ...
    def _build_system_prompt(self, initiative: Any, itype: str) -> str:
        ...
        try:
            from pacomind.cognition.charter import build_system_prompt
            ...
            logger.debug("charter compose failed; minimal fallback",
                         exc_info=True)
            return ("You execute one initiative for PacoMind using the "
    ...
    def _record_outcome(self, itype: str, outcome: str,
        ...
        try:
            ...
            self._self_model.record(itype, outcome, latency_secs=latency,
                ...
                evidence_status='observed', outcome_contract='pacomind.initiative-runtime-outcome/v1',
    ...
    async def _find_recent_completion(self, initiative: Any) -> Any:
        ...
        if not itype:
            return None
        window_h = float(os.environ.get("PACOMIND_EXECUTOR_REPEAT_WINDOW_HOURS", "48"))
...
def create_from_env(
    ...
    Returns None if PACOMIND_EXECUTOR_ENABLED is not "true" or required
    dependencies are missing.
    """
    from pacomind.util.autonomy_preset import resolve_bool
    if not resolve_bool("PACOMIND_EXECUTOR_ENABLED", False):
        logger.info(
            "Initiative executor disabled (PACOMIND_EXECUTOR_ENABLED != true). "
            "Set PACOMIND_EXECUTOR_ENABLED=true or PACOMIND_AUTONOMY_PRESET="
    ...
    types_env = os.environ.get("PACOMIND_EXECUTOR_TYPES", "")
    allowed_types = (
    ...
    return InitiativeExecutorService(
        ...
        cycle_secs=float(os.environ.get("PACOMIND_EXECUTOR_CYCLE_SECS", "30")),
        max_per_cycle=int(os.environ.get("PACOMIND_EXECUTOR_MAX_PER_CYCLE", "5")),
        model_tier=os.environ.get("PACOMIND_EXECUTOR_MODEL_TIER", "small"),
        allowed_types=allowed_types,
        agent_id=os.environ.get("PACOMIND_EXECUTOR_AGENT_ID", "pacomind-executor"),
        max_tool_iterations=int(os.environ.get("PACOMIND_EXECUTOR_MAX_TOOL_ITERS", "8")),
    )
