...
This is the internal counterpart to the ``pacomind-agent-bridge`` console
script. The console script polls PacoMind over HTTP and is for split
...
Auto-starts at sidecar boot when PACOMIND_BRIDGE_ENABLED is not "false"
and an agent webhook URL is configured. Disabled automatically when no
webhook is set (the agent hasn't connected yet).

Environment / config:
  PACOMIND_BRIDGE_ENABLED            "true" (default) / "false"
  PACOMIND_BRIDGE_WEBHOOK_URL        agent initiative webhook (required)
  PACOMIND_BRIDGE_JOBS_WEBHOOK_URL   agent jobs webhook (optional, same host)
  PACOMIND_BRIDGE_CYCLE_SECS         cycle interval (default 60)
...
class AgentBridgeService:
    ...
    def __init__(
        ...
        self._jobs_webhook = jobs_webhook_url or webhook_url.replace(
            "/pacomind-initiatives", "/pacomind-jobs"
        )
    ...
    async def _maybe_sync_skills(self) -> None:
        ...
        try:
            from pacomind.workers.skills_sync import scan
            observations = scan()
            if not observations:
                return
...
def create_from_env(
    ...
    Returns None if PACOMIND_BRIDGE_ENABLED is "false" or no webhook URL
    is configured.
    """
    if os.environ.get("PACOMIND_BRIDGE_ENABLED", "true").lower() == "false":
        logger.info("Agent bridge disabled (PACOMIND_BRIDGE_ENABLED=false)")
        return None

    webhook = os.environ.get("PACOMIND_BRIDGE_WEBHOOK_URL", "")
    if not webhook:
        logger.info(
            "Agent bridge: no PACOMIND_BRIDGE_WEBHOOK_URL set, will not start. "
    ...
    return AgentBridgeService(
        ...
        webhook_url=webhook,
        jobs_webhook_url=os.environ.get("PACOMIND_BRIDGE_JOBS_WEBHOOK_URL", ""),
        cycle_secs=float(os.environ.get("PACOMIND_BRIDGE_CYCLE_SECS", "60")),
        skills_sync_hours=float(os.environ.get("PACOMIND_BRIDGE_SKILLS_HOURS", "24")),
        api_key=os.environ.get("PACOMIND_API_KEY", ""),
        node_id=os.environ.get("PACOMIND_BRIDGE_NODE_ID", "sidecar-bridge"),
    )
