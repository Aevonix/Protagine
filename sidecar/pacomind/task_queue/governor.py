...
Mode (PACOMIND_WORKERS_MODE, default shadow):
...
def workers_mode() -> str:
    ...
    explicit = os.environ.get("PACOMIND_WORKERS_MODE")
    if explicit is not None and explicit.strip():
        value = explicit.strip().lower()
        if value not in {"off", "shadow", "live"}:
            raise RuntimeError(
                "PACOMIND_WORKERS_MODE must be off, shadow, or live"
            )
    return resolve("PACOMIND_WORKERS_MODE", ("off", "shadow", "live"), "shadow")
    ...
    def _read_only_live_trial_status(
        ...
        * every real attempt consumes one of a small, bounded trial budget.
        ...
        try:
            ...
            try:
                ask_min = int(os.environ.get("PACOMIND_TRUST_ASK_MIN_N", "3"))
            except (TypeError, ValueError):
                ask_min = 3
            ask_min = max(1, min(ask_min, 100))
            try:
                trial_max = int(os.environ.get(
                    "PACOMIND_TRUST_READ_ONLY_TRIAL_MAX",
                    os.environ.get("PACOMIND_TRUST_ACT_MIN_N", "5"),
                ))
            except (TypeError, ValueError):
                trial_max = 5
            ...
            for event in shadow_history:
                ...
                if (
                    ...
                    == "pacomind.worker-outcome/v1"
                    and evidence.get("schema") == "pacomind.worker-outcome/v1"
        ...
        if outcome != "neutral" and self._self_model is not None:
            try:
                recorded = self._self_model.record(
                    ...
                    evidence={
                        "schema": "pacomind.worker-outcome/v1",
