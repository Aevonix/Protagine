...
from pacomind.task_queue.models import CANONICAL_JOB_ID_PATTERN
...
QUEUE_CONTRACT_SCHEMA = "PacoMindQueueContractV1"
...
def _release_identity() -> Dict[str, str]:
    commit = os.environ.get("PACOMIND_RELEASE_COMMIT", "").strip().lower()
    manifest_sha256 = os.environ.get(
        "PACOMIND_RELEASE_ARTIFACT_MANIFEST_SHA256", ""
    ).strip().lower()
    if not _COMMIT_RE.fullmatch(commit) or set(commit) == {"0"}:
        raise QueueContractIdentityError(
            "PACOMIND_RELEASE_COMMIT must be the exact 40-character lowercase "
            "Git commit deployed"
        )
    if (
        ...
        raise QueueContractIdentityError(
            "PACOMIND_RELEASE_ARTIFACT_MANIFEST_SHA256 must be the exact "
...
def _deployment_posture() -> Dict[str, Any]:
    ...
    if raw_claims in {"1", "true", "yes", "on"}:
        ...
        raise QueueContractIdentityError(
            "PACOMIND_AGENT_JOB_CLAIMS_ENABLED must be true or false"
        )
...
def _work_control_contract() -> Dict[str, Any]:
    ...
    return {
        ...
        "pacomind_supplies_host_control_adapter": False,
    ...
    contract: Dict[str, Any] = {
        ...
        "approval_at_birth": {
            ...
            "blocked_discovery": {
                ...
                "legacy_count_header": (
                    "X-PacoMind-Blocked-Legacy-Count"
                ),
