...
from pacomind.task_queue.handlers.base import JobHandler, Job, run_subprocess
...
class SystemMaintenanceHandler(JobHandler):
    ...
    Job payload:
        ...
        target_path (str, optional): Path scoped to the pacomind data directory.
    ...
    def __init__(self, allowed_roots: List[Path] | None = None) -> None:
        if allowed_roots is None:
            configured = os.environ.get("PACOMIND_MAINTENANCE_ROOTS", "")
            roots = [
            ...
            if not roots:
                roots = [Path(
                    os.environ.get("PACOMIND_STATE_DIR")
                    or os.environ.get("PACOMIND_HOME", "~/.pacomind")
                ).expanduser()]
        else:
    ...
    def _target(self, value: str | None, *, default_name: str) -> Path:
        ...
        if not any(resolved.is_relative_to(base) for base in self._allowed_roots):
            raise ValueError(
                "maintenance target must stay inside PACOMIND_MAINTENANCE_ROOTS"
            )
