"""InferenceHandler — context-enriched LLM inference via the PacoMind router."""
...
from pacomind.task_queue.handlers.base import JobHandler, Job
...
if TYPE_CHECKING:
    ...
    from pacomind.contacts.models import Contact

logger = logging.getLogger(__name__)

_PACOMIND_IDENTITY = (
    "You are PacoMind, an intelligent personal AI assistant. "
...
async def _update_world_model_async(
    ...
    """
    try:
        from pacomind.world_model.extraction.conversation_extractor import ConversationExtractor
        from pacomind.world_model.entities import (
...
class InferenceHandler(JobHandler):
    """Run an LLM inference request via the PacoMind router.
    ...
    Job payload keys:
        ...
        system_prompt (str, optional): Additional system context appended after
            the enriched PacoMind identity + contact + world model prompt.
    ...
    async def _ensure_wm_connected(self) -> None:
        ...
        if self._wm is None:
            try:
                ...
                import os
                pacomind_home = os.environ.get("PACOMIND_HOME", os.path.expanduser("~/.pacomind"))
                db_path = os.path.join(pacomind_home, "world_model.db")
    ...
    async def _gate_context(
        ...
        *,
        ...
        Uses the context gate (:mod:`pacomind.contextgate`): when the
        ...
        try:
            ...
            from pacomind.contextgate import GateDecision
        ...
        if (not cognition_read_only
            ...
            try:
                ...
                owner_id = os.environ.get(
                    ...
                    os.environ.get("PACOMIND_OWNER_CONTACT_ID", "owner"),
                ).strip() or "owner"
