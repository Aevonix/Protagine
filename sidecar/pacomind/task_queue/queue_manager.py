...
PacoMind event bus on each state transition.
"""
...
from pacomind.task_queue.config import (
...
class QueueManager:
    ...
    Usage::
        ...
        mgr = QueueManager(db_path=Path("~/.pacomind/task_queue.db"))
        await mgr.start()
    ...
    def __init__(
        ...
        from pacomind.task_queue.work_control import WorkControlService
    ...
    async def _reconcile_persisted_thought_routes(self) -> None:
        ...
        assert self._db is not None
        from pacomind.task_queue.routing import bind_thought_route
    ...
    def _evaluate_claim_authority(
        ...
        from pacomind.task_queue.governor import (
            ClaimVerdict,
            workers_mode,
        )
    ...
    async def reconcile_governance_holds(self, *, force: bool = False) -> int:
        ...
        from pacomind.task_queue.governor import ClaimVerdict, workers_mode
        ...
        try:
            recheck_seconds = max(1.0, float(
                os.environ.get(
                    "PACOMIND_WORKER_HOLD_RECHECK_SECONDS", "30")
            ))
        except ValueError:
    ...
    async def _mark_pending_work_controls_terminal(
        ...
        *,
        ...
        if not operation_ids:
            return []
        from pacomind.task_queue.work_control import canonical_json
            ...
            for row in rows:
                ...
                try:
                    timeout = _positive_seconds(
                        os.environ.get(
                            "PACOMIND_WORKER_OUTCOME_DELIVERY_TIMEOUT_SECS",
                            "60",
                        ),
                        default=60.0,
                        field="PACOMIND_WORKER_OUTCOME_DELIVERY_TIMEOUT_SECS",
                    )
    ...
    async def complete_job(
        ...
        if ambiguous_semantic_effect:
            ...
            tags.update({
                "success_verifier_identity": "pacomind:embedded-worker",
        ...
        if self._event_bus is not None:
            try:
                if target_status is JobStatus.COMPLETED:
                    from pacomind.task_queue.events import JobCompletedEvent
                    ...
                    from pacomind.task_queue.events import JobFailedEvent
                    ...
                    from pacomind.task_queue.events import JobNeutralEvent
        ...
        *,
        ...
        try:
            clock_skew = float(os.environ.get(
                "PACOMIND_ACTION_RECEIPT_CLOCK_SKEW_SECS", "300",
            ))
        except (TypeError, ValueError):
            clock_skew = 300.0
    ...
    async def _work_control_target_row(self, job: Job) -> aiosqlite.Row:
        ...
        assert self._db is not None
        from pacomind.task_queue.work_control import digest_json
        ...
        if row is not None:
            if str(row["authority_digest"]) != authority_digest:
                from pacomind.task_queue.work_control import WorkControlError
        ...
        run_id = "run-" + _uuid_module.uuid5(
            ...
            f"pacomind:queue-job:{target_id}",
        ).hex
        now = _now_iso()
        await self._db.execute(
        ...
        *,
        ...
        if projection["mode"] != "live":
            raise WorkControlError(
                ...
                "WorkControl mutations require PACOMIND_WORK_CONTROL_MODE=live",
                status_code=503,
            )
    ...
    async def reconcile_inactive_work_controls(self) -> int:
        ...
        assert self._db is not None
        from pacomind.task_queue.work_control import work_control_mode
...
class TaskQueueManager:
    """Singleton facade over QueueManager for the PacoMind API layer.
    ...
    def get_instance(cls) -> "TaskQueueManager":
        if cls._instance is None:
            raise RuntimeError(
                ...
                "Ensure the PacoMind server lifespan has started."
            )
    ...
    async def initialize(
        ...
        if db_path is None:
            pacomind_home = Path(
                os.environ.get("PACOMIND_HOME", "~/.pacomind")
            ).expanduser()
            task_db_env = os.environ.get("PACOMIND_TASK_DB_PATH", "")
            db_path = Path(task_db_env) if task_db_env else pacomind_home / "task_queue.db"

        if claim_timeout_secs is None:
            claim_timeout_secs = _positive_seconds(
                os.environ.get("PACOMIND_QUEUE_CLAIM_TIMEOUT_SECS", "30"),
                default=30.0,
                field="PACOMIND_QUEUE_CLAIM_TIMEOUT_SECS",
            )
        if worker_heartbeat_ttl_secs is None:
            worker_heartbeat_ttl_secs = validate_worker_heartbeat_ttl(
                os.environ.get(
                    "PACOMIND_QUEUE_HEARTBEAT_TIMEOUT_SECS",
    ...
    async def list_tasks(
        ...
        """
        from pacomind.api.schemas.common import ListResponse
