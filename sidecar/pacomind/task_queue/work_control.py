"""Generic, durable control contract for PacoMind queue work.
...
def work_control_mode() -> str:
    ...
    value = os.environ.get("PACOMIND_WORK_CONTROL_MODE", "off").strip().lower()
    if value not in {"off", "shadow", "live"}:
        raise WorkControlError(
            "work_control_configuration_invalid",
            "PACOMIND_WORK_CONTROL_MODE must be off, shadow, or live",
            status_code=503,
        )
    return value
...
def work_control_ack_timeout_secs() -> float:
    ...
    raw = os.environ.get("PACOMIND_WORK_CONTROL_ACK_TIMEOUT_SECS", "30")
    try:
        value = float(raw)
    except (TypeError, ValueError) as exc:
        raise WorkControlError(
            "work_control_configuration_invalid",
            "PACOMIND_WORK_CONTROL_ACK_TIMEOUT_SECS must be numeric",
            status_code=503,
        ) from exc
    if not 1.0 <= value <= 300.0:
        raise WorkControlError(
            "work_control_configuration_invalid",
            "PACOMIND_WORK_CONTROL_ACK_TIMEOUT_SECS must be within 1..300",
            status_code=503,
        )
    return value
