...
#   [IMPORTANT: The user has invoked the "pacomind-operations" skill ...]
...
def max_age_days() -> float:
    try:
        return float(os.environ.get("PACOMIND_REACHOUT_MAX_AGE_DAYS", "7"))
...
def _urgency_cap() -> float:
    try:
        return float(os.environ.get("PACOMIND_REACHOUT_URGENCY_CAP", "0.85"))
    except (TypeError, ValueError):
        return 0.85
