...
from pacomind.workers.queue_worker import encode_hermes_webhook
...
class ProactiveDeliveryBridge:
    ...
    def __init__(
        ...
        if rate_limiter is None:
            ...
            # caps. Lives alongside other sidecar state under PACOMIND_STATE_DIR.
            state_dir = os.environ.get("PACOMIND_STATE_DIR", ".")
            db_path = Path(state_dir) / "pacomind-delivery-rate-limit.db"
        ...
        _port = int(os.environ.get("PACOMIND_GATEWAY_INTERNAL_PORT", _DEFAULT_GATEWAY_INTERNAL_PORT))
        self._gateway_url: str = (
            gateway_url
            or os.environ.get("PACOMIND_GATEWAY_INTERNAL_URL", "")
            or f"http://localhost:{_port}"
        )
        self._gateway_api_key: str = (
            gateway_api_key
            or os.environ.get("PACOMIND_GATEWAY_API_KEY", "")
            or os.environ.get("PACOMIND_API_KEY", "")
        )
        self._gateway_contract = str(
            gateway_contract
            or os.environ.get("PACOMIND_GATEWAY_CONTRACT", "legacy_delivery")
        ).strip().lower()
        ...
        if self._gateway_contract == "governed_admission_v1":
            state_dir = Path(os.environ.get("PACOMIND_STATE_DIR", "."))
            outcome_path = Path(gateway_outcome_db) if gateway_outcome_db else (
                state_dir / "pacomind-governed-gateway-outcomes.db"
            )
    ...
    def governed_gateway_admission_enabled(
        ...
        sidecar cannot silently exempt an RCS/SMS route from PacoMind's legacy
    ...
    async def push_to_gateway(
        ...
        # PacoMind anything about a deployment's authority model.
        if delivery_id:
    ...
    def _prepare_initiative_dispatch(self, initiative: Dict[str, Any]) -> Dict[str, Any]:
        ...
        hermes_webhook_url = os.environ.get(
            "PACOMIND_HERMES_WEBHOOK_URL",
            "http://127.0.0.1:8644/webhooks/pacomind-initiatives",
        )

        headers: Dict[str, str] = {"Content-Type": "application/json"}
        webhook_secret = os.environ.get("PACOMIND_HERMES_WEBHOOK_SECRET", "")

        # Resolve agent name from env var — never hardcode
        agent_name = os.environ.get("PACOMIND_AGENT_NAME", "the assistant")
        ...
        if not raw_entity_id or is_self_initiative:
            ...
            person_id = os.environ.get("PACOMIND_OWNER_CONTACT_ID", "owner")
            user_channel = None
    ...
    async def _prepare_initiative_dispatch_async(
        ...
        configured_owner = str(
            os.environ.get("PACOMIND_OWNER_CONTACT_ID", "owner") or "owner"
        ).strip()
        ...
        body_bytes, headers = encode_hermes_webhook(
            payload,
            secret=os.environ.get("PACOMIND_HERMES_WEBHOOK_SECRET", ""),
        )
