...
def main() -> None:
    ...
    if args.command == "init":
        ...
        if os.environ.get("COLONY_INSTALL_PROFILE") == "local" and args.service_command:
            from apsimo.services.instance import manage, ServiceError
            try:
        ...
        if not model:
            try:
                from apsimo.vector.tiers import TIER_TABLE
                from apsimo.vector.scanner import HardwareScanner
        ...
        _load_dotenv()
        from apsimo.secrets import cli as _secrets_cli
        _handler = getattr(_secrets_cli, f"cmd_secrets_{args.secrets_cmd}")
        _handler(args)

    elif args.command == "autonomy":
        _load_dotenv()
        from apsimo.autonomy.cli import run_autonomy_command
        sys.exit(run_autonomy_command(args.autonomy_args))

    elif args.command == "feeds":
        from apsimo.feeds.cli import main as feeds_main
...
def _cmd_backup(args) -> None:
    ...
    try:
        from apsimo.chain.identity import backup_colony
...
def _cmd_restore(args) -> None:
    ...
    if args.full or memory_only:
        ...
        from apsimo.backup import restore_full_backup, restore_source_memory
        try:
            if memory_only:
...
def _cmd_init(args) -> None:
    ...
    if args.claim_genesis:
        from apsimo.chain.identity import create_genesis_manifest
...
def _cmd_node(args) -> None:
    ...
    if args.node_command == "info":
        from apsimo.chain.node import get_node_info
        from apsimo.chain.identity import get_or_create_colony_id
        colony_id = get_or_create_colony_id(state_dir)
...
def _cmd_key(args) -> None:
    ...
    if args.key_command == "info":
        ...
        from apsimo.chain.identity import get_or_create_colony_id, create_colony_manifest
        colony_id = get_or_create_colony_id(state_dir)
        keys_dir = os.path.join(state_dir, "colony-keys")
        passphrase = os.environ.get("COLONY_KEY_PASSPHRASE", "")
        passphrase_bytes = passphrase.encode() if passphrase else None
        ...
        from apsimo.chain.identity import get_or_create_colony_id, create_genesis_manifest, get_genesis_manifest
        colony_id = get_or_create_colony_id(state_dir)
...
def _is_service_loaded() -> bool:
    ...
    if os.environ.get("COLONY_INSTALL_PROFILE") == "local":
        from apsimo.services.instance import InstanceService
...
def _cmd_start_daemon(host: str, port: int, force: bool) -> None:
    ...
    if local_instance:
        from apsimo.setup import _check_port
        if _check_port(port):
            print("Selected port is already in use; no process was stopped.")
            raise SystemExit(1)
...
def _cmd_mcp(args) -> None:
    ...
    if args.mcp_command == "run":
        ...
        if getattr(args, 'print_config', False):
            import json
            from apsimo.mcp.config import _mcp_config
...
def _cmd_doctor(args) -> None:
    ...
    if getattr(args, "fix", False):
        ...
        try:
            from apsimo.setup import repair_persisted_llm_config
