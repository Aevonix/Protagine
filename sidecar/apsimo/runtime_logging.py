...
_started = ContextVar('apsimo_http_log_started', default=None)
_handler = None
...
def runtime_log_directory():
    ...
    path = os.environ.get('COLONY_LOG_PATH') or os.environ.get('APSIMO_LOG_PATH')
...
def configure_runtime_logging(path=None, *, max_bytes=None, backups=None, redirect_stdio=False):
    ...
    global _handler
    from apsimo import get_state_dir
    path = Path(path or os.environ.get('COLONY_LOG_PATH') or os.environ.get('APSIMO_LOG_PATH')
                or get_state_dir()/'service/sidecar.log').expanduser().absolute()
    max_bytes = int(max_bytes if max_bytes is not None else os.environ.get('APSIMO_LOG_MAX_BYTES', DEFAULT_MAX_BYTES))
    backups = int(backups if backups is not None else os.environ.get('APSIMO_LOG_BACKUPS', DEFAULT_BACKUPS))
    ...
    _handler = handler
    if redirect_stdio:
        sys.stdout = _LoggedStream('apsimo.stdout', logging.INFO)
        sys.stderr = _LoggedStream('apsimo.stderr', logging.ERROR)
    policy = {'schema': 'ApsimoRuntimeLoggingV1', 'path': str(path),
