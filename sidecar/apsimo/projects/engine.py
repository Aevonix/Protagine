...
from apsimo.projects.models import (
...
def _step_boundary_action(project: Project, step: Step):
    ...
    from apsimo.work_orders import action_authority
...
class ProjectEngine:
    ...
    async def _complete_project(self, project: Project, steps: List[Step],
        ...
        if project.outcome == "succeeded" and len(steps) >= 3 and mode == "live":
            try:
                from apsimo.skills_memory import distill_from_completion
                transcript = "\n".join(
    ...
    def _skills_block(self, situation: str) -> str:
        if self._skills is None:
            return ""
        try:
            from apsimo.skills_memory import (
