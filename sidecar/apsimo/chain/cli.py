...
def _load_store(db_path: str | None = None) -> "ChainStore":
    from apsimo.chain.storage import ChainStore
    path = Path(db_path) if db_path else _DEFAULT_DB
    return ChainStore(path)


def _load_state(db_path: str | None = None) -> "ChainState":
    from apsimo.chain.state_machine import ChainStateMachine
...
def cmd_chain_genesis(args) -> int:
    ...
    from apsimo.chain.genesis import GenesisConfig, create_genesis_block
    from apsimo.chain.storage import ChainStore
...
def cmd_chain_verify(args) -> int:
    ...
    try:
        ...
        for idx in range(height + 1):
            ...
            from apsimo.chain.block import build_merkle_root
...
def cmd_chain_register(args) -> int:
    ...
    from apsimo.chain.transactions import TxType, Transaction
    import hashlib
