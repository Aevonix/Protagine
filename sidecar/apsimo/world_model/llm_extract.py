...
from apsimo.world_model.constants import CAUSAL_RELATIONSHIP_TYPES
from apsimo.world_model.populator import _is_low_quality, _looks_like_fragment
...
class WorldLLMExtractor:
    def __init__(self, store: Any, *, graph: Any = None,
        ...
        if store is not None:
            try:
                from apsimo.world_model.resolution.entity_resolver import (
                    EntityResolver,
                )
    ...
    async def _llm_batch(self, texts: List[str]) -> Optional[dict]:
        if self._router_provider is not None:
            ...
            import asyncio
            from apsimo.beliefs.source_claims import final_text, extraction_timeout_seconds
    ...
    def _record_outcome(self, mode: str, report: Dict[str, Any]) -> None:
        ...
        if self._self_model is None:
            return
        try:
            from apsimo.self_model.supervised import supervised_domains
            if "world_model" not in supervised_domains():
                return
    ...
    async def _upsert(self, name: str, etype: str, conf: float, mode: str,
        ...
        if not self._may_write(mode, "entity_upsert", report):
            return None
        try:
            from apsimo.world_model.entities import ENTITY_CLASS_MAP, BaseEntity
