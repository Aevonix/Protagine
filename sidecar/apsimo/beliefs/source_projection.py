...
def subject_basis(conn, claim, *, contact_id):
    ...
    if not identifier:
        return None
    from apsimo.turns.idempotency import source_message_hash, canonical_turn_digest
...
class SourceClaimProjection:
    ...
    def commit(self, source, message, claims, *, model, lease_token=None):
        from apsimo.turns.idempotency import source_message_hash, canonical_turn_digest
        from apsimo.turns.audio import claim_message, claim_basis
    ...
    try:
        while True:
            ...
            if time.monotonic() >= next_identity_check:
                ...
                try:
                    from apsimo.api.routers.social_state import reconcile_pending_identities
