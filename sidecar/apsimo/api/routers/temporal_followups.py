...
from apsimo.api.routers.initiative_work import ReviewBinding
...
from apsimo.turns.hermes_kanban import task_snapshot, observed_boards
...
async def register(body: ExpectedReply, request: Request):
    ...
    def create():
        ...
        fields['timezone_name'] = body.timezone_name or os.environ.get('COLONY_AGENT_TIMEZONE', 'UTC')
        if body.quiet_start is None and body.quiet_end is None:
            configured = os.environ.get('COLONY_AGENT_QUIET_HOURS', '').strip()
            if configured:
