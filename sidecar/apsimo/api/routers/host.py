...
from apsimo.goals.store import GoalNotFoundError
...
def broadcast_event(event: dict) -> Optional[dict]:
    ...
    """
    from apsimo.events.journal import append_event_record
...
async def list_models() -> ModelListResponse:
    ...
    from apsimo.router.tiers import discover_local_models
    ...
    try:
        if _telemetry is not None:
            ...
            if (
                ...
                health_status = "degraded"
            from apsimo.api.schemas.host import TemporalMetrics
...
async def memory_backfill(body: BackfillRequest) -> BackfillResponse:
    ...
    async def _run():
        from apsimo.vector.backfill import backfill
        try:
...
async def memory_migrate(body: MigrateRequest) -> MigrateResponse:
    ...
    async def _run():
        ...
        try:
            ...
            from apsimo.vector.migrate import MigrationResult
...
async def _build_temporal_section(
    ...
    *, include_global_heads_up: bool = True,
    ...
    """
    from apsimo.util import temporal as _temporal
    ...
    if include_global_heads_up:
        try:
            from apsimo.util.session_safety import load_last_user_message_at
...
async def context_assemble(
    ...
    try:
        from apsimo.api.routers.executions import authorized_viewer, with_queue_work
    ...
    if not _canonical_only:
        ...
        identity_lines = []
        try:
            from apsimo.chain.identity import get_or_create_colony_id, get_genesis_manifest
        ...
        try:
            from apsimo.identity_bootstrap.self_query import (
    ...
    if _canonical_person_allowed and contact_id:
        try:
            from apsimo.api.routers.social_state import appraisal_context
    ...
    if _canonical_person_allowed and contact_id:
        try:
            ...
            if owner:
                from apsimo.api.routers.social_state import waiting_context
    ...
    if _tom2_store is not None and _tom_context_facts is not None and _facts_store is not None:
        try:
            from apsimo.tom.facts import InferenceFactsView
            ...
            if configured_level() >= 1 and contact_id != _lvl_owner:
                ...
                if _lres.level >= 2:
                    from apsimo.tom.eligibility import (
                        eligible_inferences)
                    from apsimo.tom.leveled import render_level2
    ...
    if _exact_person_allowed and _engagement_store is not None and contact_id:
        try:
            from apsimo.tom.engagement import build_guidance
...
async def reasoning_turn(
    ...
    actor_policy = None
    if _p8_runtime is not None:
        from apsimo.reasoning.executor import ToolRegistryError
        from apsimo.reasoning.tool_policy import filter_tools_for_actor
...
async def tools_invoke(
    ...
    if _p8_runtime is not None:
        from apsimo.reasoning.executor import ToolRegistryError
        from apsimo.reasoning.tool_policy import actor_allows_tool
...
async def signals_ingest(body: SignalIngestRequest) -> SignalIngestResponse:
    ...
    if recorded > 0:
        try:
            from apsimo.cognition.trigger import trigger_cognition, _cognition_enabled
...
def _conversation_turn_concern_metadata(
    ...
    *,
    ...
    from apsimo.self_model.event_concerns import turn_concerns_enabled
...
async def forget_turn_sources(body: SourceForgetRequest, request: Request = None):
    ...
    try:
        from apsimo.api.routers.transport_ingress_api import forget_sources
...
async def _ingest_turn_idempotently(
    ...
    if body.assistant_input_refs:
        from apsimo.turns.idempotency import SourceInputPending
        try:
...
async def turns_sync(
    ...
    from apsimo.api.routers.transport_ingress_api import reconcile_source
...
async def _process_turn_sync(
    ...
    *,
    ...
    if source_messages:
        from apsimo.turns import canonical_turn_digest, get_turn_idempotency_ledger
        ledger = get_turn_idempotency_ledger(get_state_dir())
    ...
    try:
        from apsimo.cognition.introspection import introspect_enabled, run_turn_introspection
    ...
    if body.user_message is not None:
        try:
            from apsimo.util.session_safety import save_last_user_message_at
    ...
    if _preference_learner is not None and body.user_message is not None:
        try:
            ...
            if owner_id and body.context.contact_id == owner_id:
                ...
                if hit is not None:
                    try:
                        from apsimo.events.broadcaster import emit as _emit
...
async def events_ws(ws: WebSocket) -> None:
    ...
    try:
        ...
        if keyring_path:
            from apsimo.api.authority import KeyringLoader, scoped_authority
    ...
    from apsimo.events.journal import current_sequence, replay_events
    with _event_broadcast_lock:
...
async def contact_policy_source(
    ...
    for contact in contacts:
        ...
        if outreach_dependencies_available:
            from apsimo.contacts.comms import evaluate_outreach
...
async def response_guard_check(body: ResponseGuardCheckRequest) -> Dict[str, Any]:
    ...
    from apsimo.gate.surface_policy import EXCLUDED_SPEECH_SURFACES
...
async def env_risk(conversation_key: str, contact_id: str) -> Dict[str, Any]:
    ...
    from apsimo.gate.env_risk import classify, env_risk_window_hours
...
async def cognition_cycle(body: CognitionCycleRequest) -> dict:
    ...
    """
    from apsimo.self_model.benchmark import legacy_cpi_payload
...
async def directed_approve(
    ...
    if _directed_service is None:
        return {"ok": False, "reason": "directed_not_wired"}
    from apsimo.api.routers.task_queue import _decision_authority
...
async def post_benchmark_samples(
    ...
    from apsimo.self_model.benchmark import cognition_p4_mode
...
def _scope_from_authority(request: Request):
    ...
    from apsimo.cognition.drive_governance import ScopeV1
...
async def propose_cognition_drive(
    ...
    if _drive_governance is None:
        return {"available": False}
    from apsimo.cognition.drive_governance import DriveV1
...
async def propose_cognition_drive_signal(
    ...
    if _drive_governance is None or _drive_project_store is None:
        return {"available": False}
    from apsimo.cognition.drive_governance import DriveSignalV1
...
async def charter_transition_approval_readiness(request: Request) -> dict:
    ...
    from apsimo.initiatives.approval_authority import authority_mode
...
async def resolve_concern(
    ...
    from apsimo.self_model.workspace import ConcernResolutionConflict
    try:
    ...
    if cascade_state["status"] == "pending":
        ...
        if created:
            try:
                from apsimo.self_model.settlement import settle_sources
            ...
            from apsimo.self_model.settlement import (
...
async def get_tool(tool_id: str) -> dict:
    ...
    try:
        from apsimo.toolsmith.engine import _shadow_min
...
async def get_autonomy_posture(request: Request) -> dict:
    ...
    try:
        from apsimo.util.autonomy_preset import snapshot
        posture = snapshot()
        ...
        try:
            if _autonomy_loop is not None:
                ...
                if raw:
                    ...
                    from apsimo.util.autonomy_preset import coupled_loop_mode
...
async def approve_skill(skill_id: str) -> dict:
    ...
    try:
        ...
        try:
            from apsimo.skills.hermes_export import export_approved_skill
    ...
    try:
        ...
        if key_mgr is not None:
            try:
                ...
                from apsimo.chain.identity import is_genesis as check_genesis
        ...
        try:
            from apsimo.chain.node import get_node_info, load_node_certificate
        ...
        from apsimo.chain.identity import get_genesis_manifest
        manifest = get_genesis_manifest()
...
async def cognition_trigger(body: CognitionTriggerRequest) -> CognitionTriggerResponse:
    ...
    """
    from apsimo.cognition.trigger import trigger_cognition
...
async def world_causal_chain(
    ...
    if _world_store is None:
        raise HTTPException(status_code=501, detail="World model not initialized")
    try:
        from apsimo.world_model.causal_query import causal_chain
...
async def world_causal_edges(min_confidence: float = 0.0,
    ...
    if _world_store is None:
        raise HTTPException(status_code=501, detail="World model not initialized")
    try:
        from apsimo.world_model.causal_query import causal_edges
...
async def refresh_initiative_context(initiative_id: str) -> InitiativeResponse:
    ...
    from apsimo.initiatives.context_freshness import DURABLE, durability_for
...
def _initiative_to_response(initiative) -> InitiativeResponse:
    ...
    from apsimo.initiatives.context_freshness import durability_for
...
async def respond_to_initiative(
    ...
    if job_id and action in {
        ...
        try:
            from apsimo.api.routers import task_queue as approval_router
...
async def session_report(body: SessionReportRequest) -> SessionReportResponse:
    ...
    from apsimo.sessions.reports import SessionReport
