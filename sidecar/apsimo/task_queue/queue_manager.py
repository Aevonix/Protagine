...
from apsimo.task_queue.config import (
...
class QueueManager:
    ...
    def __init__(
        ...
        from apsimo.task_queue.work_control import WorkControlService
    ...
    async def _reconcile_persisted_thought_routes(self) -> None:
        ...
        assert self._db is not None
        from apsimo.task_queue.routing import bind_thought_route
    ...
    def _evaluate_claim_authority(
        ...
        from apsimo.task_queue.governor import (
            ClaimVerdict,
            workers_mode,
        )
    ...
    async def reconcile_governance_holds(self, *, force: bool = False) -> int:
        ...
        from apsimo.task_queue.governor import ClaimVerdict, workers_mode
    ...
    async def _mark_pending_work_controls_terminal(
        ...
        *,
        ...
        if not operation_ids:
            return []
        from apsimo.task_queue.work_control import canonical_json
    ...
    async def complete_job(
        ...
        if self._event_bus is not None:
            try:
                if target_status is JobStatus.COMPLETED:
                    from apsimo.task_queue.events import JobCompletedEvent
                    ...
                    from apsimo.task_queue.events import JobFailedEvent
                    ...
                    from apsimo.task_queue.events import JobNeutralEvent
    ...
    async def _work_control_target_row(self, job: Job) -> aiosqlite.Row:
        ...
        assert self._db is not None
        from apsimo.task_queue.work_control import digest_json
        ...
        if row is not None:
            if str(row["authority_digest"]) != authority_digest:
                from apsimo.task_queue.work_control import WorkControlError
    ...
    async def reconcile_inactive_work_controls(self) -> int:
        ...
        assert self._db is not None
        from apsimo.task_queue.work_control import work_control_mode
...
class TaskQueueManager:
    ...
    async def list_tasks(
        ...
        """
        from apsimo.api.schemas.common import ListResponse
