...
def test_off_never_runs(monkeypatch):
    monkeypatch.setenv("PROTAGINE_SANDBOX_MODE", "off")
    mgr = SandboxManager()
...
def test_dry_run_executes_nothing(monkeypatch):
    monkeypatch.setenv("PROTAGINE_SANDBOX_MODE", "dry_run")
    mgr = SandboxManager()
    backend = _MockBackend()
    mgr._backend = backend
    out = mgr.run("print(1)", purpose="test", owner_directed=True)
...
def test_limits_come_from_env_not_caller(monkeypatch):
    monkeypatch.setenv("PROTAGINE_SANDBOX_CPUS", "0.5")
    monkeypatch.setenv("PROTAGINE_SANDBOX_MEMORY", "128m")
    monkeypatch.setenv("PROTAGINE_SANDBOX_TIMEOUT", "5")
    lim = resolve_limits()
