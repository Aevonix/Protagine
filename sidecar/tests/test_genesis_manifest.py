...
from protagine.chain import identity


@pytest.fixture(autouse=True)
def _reset_loaded_manifest(monkeypatch):
    monkeypatch.delenv("PROTAGINE_GENESIS_TRUST_PUBLIC_KEY", raising=False)
...
def signed_manifest(tmp_path):
    ...
    local_id = identity.get_or_create_protagine_id(tmp_path)
    keys = LocalKeyManager.generate(keys_dir=tmp_path / "protagine-keys", protagine_id=local_id)
    path = tmp_path / identity.GENESIS_MANIFEST_FILE
    manifest = identity.create_genesis_manifest(local_id, keys.public_key_hex(), path,
        private_key_pem=(tmp_path / "protagine-keys/private.pem").read_bytes())
...
def test_generated_manifest_requires_explicit_key(signed_manifest, monkeypatch):
    ...
    assert identity.is_genesis(manifest['protagine_id'], manifest['public_key_ed25519']) is False
    monkeypatch.setenv("PROTAGINE_GENESIS_TRUST_PUBLIC_KEY", manifest["public_key_ed25519"])
    ...
    assert identity.is_genesis(manifest['protagine_id'], manifest['public_key_ed25519']) is True
...
def test_invalid_or_different_trust_key_rejects_manifest(signed_manifest, monkeypatch, key):
    path, manifest = signed_manifest
    monkeypatch.setenv("PROTAGINE_GENESIS_TRUST_PUBLIC_KEY", key)
    assert identity.load_genesis_manifest(path) is None
...
def test_tampered_or_unsigned_manifest_clears_prior_trust(signed_manifest, monkeypatch, change):
    ...
    if change == 'tampered':
        changed['protagine_id'] = 'changed-instance'
    else:
...
def test_removing_config_revokes_cached_trust(signed_manifest, monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_GENESIS_TRUST_PUBLIC_KEY", manifest['public_key_ed25519'])
    assert identity.load_genesis_manifest(path) == manifest
    monkeypatch.delenv("PROTAGINE_GENESIS_TRUST_PUBLIC_KEY")
    assert identity.get_genesis_manifest() is None
    assert identity.is_genesis(manifest['protagine_id'], manifest['public_key_ed25519']) is False
...
def test_restoring_identity_does_not_grant_unconfigured_trust(signed_manifest, tmp_path):
    path, manifest = signed_manifest
    backup = identity.backup_protagine(path.parent)
    restored = tmp_path / 'restored'
    assert identity.restore_protagine(restored, backup) == manifest['protagine_id']
...
def test_local_cli_identity_initializes_without_federation(tmp_path, monkeypatch, capsys, create_manifest):
    ...
    local_id = identity.get_or_create_protagine_id(tmp_path)
    keys = LocalKeyManager(tmp_path / 'protagine-keys', local_id)
    assert len(keys.public_key_hex()) == 64
    assert len(keys.sign(b'local identity remains usable')) == 128
    assert identity.get_or_create_protagine_id(tmp_path) == local_id
    ...
    if create_manifest:
        assert 'PROTAGINE_GENESIS_TRUST_PUBLIC_KEY' in output
