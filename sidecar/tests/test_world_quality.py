...
from protagine.world_model.entities import ProductEntity
from protagine.world_model.populator import _is_low_quality
from protagine.world_model.sqlite.backend import SQLiteBackend, _fts_escape
...
def test_operational_phrases_rejected_as_persons():
    for bad in ("Root Cause", "Orphan Messages",
                "Initiatives Spawn Fresh Sessions", "Protagine Operations"):
...
def test_fts_escape_preserves_token_boundaries():
    ...
    assert _fts_escape("protagine-operations") == "protagine operations"
