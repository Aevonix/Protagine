...
from protagine.chain.keys import BackupFileShareBackend, KeyShare
...
def test_backup_backend_roundtrip(tmp_path):
    path = tmp_path / "share.protagineshare"
    ...
    got = reader.retrieve_share(protagine_id="ab" * 16, share_index=2)
...
def test_backup_backend_overwrite_allowed_when_flag_set(tmp_path):
    ...
    BackupFileShareBackend(
        file_path=path, protagine_id="ab" * 16, network_id="cd" * 16
...
def test_backup_backend_checksum_detects_tampering(tmp_path):
    ...
    with pytest.raises(ValueError, match="Checksum mismatch"):
        reader.retrieve_share(protagine_id="ab" * 16, share_index=1)
...
def test_backup_backend_delete_removes_file(tmp_path):
    ...
    backend.delete_share(protagine_id="ab" * 16, share_index=1)
...
def test_backup_backend_requires_metadata_for_new_file(tmp_path):
    ...
    with pytest.raises(ValueError, match="protagine_id and network_id"):
        backend.store_share(_share(index=1))
