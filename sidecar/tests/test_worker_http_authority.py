...
from protagine.api.authority import KeyringError, load_keyring, required_scope
...
def _worker_principal(
    ...
    *,
    ...
    return {
        ...
        # scopes are selected by PROTAGINE_WORKER_AUTHORITY_MODE=enforce.
        "scopes": [
            "api:access",
            "workers:contract",
            "workers:register",
...
async def test_restricted_worker_uses_exact_routes_but_not_api_fallback(
    ...
    monkeypatch.setenv("PROTAGINE_WORKER_AUTHORITY_MODE", "enforce")
    monkeypatch.setenv("PROTAGINE_WORKERS_MODE", "off")
    monkeypatch.setenv("PROTAGINE_RELEASE_COMMIT", "1" * 40)
    monkeypatch.setenv(
        "PROTAGINE_RELEASE_ARTIFACT_MANIFEST_SHA256", "2" * 64,
    )
