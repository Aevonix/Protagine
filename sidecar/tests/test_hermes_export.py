"""Hermes↔Protagine skills bridge (v0.18.0) — SKILL.md export.
...
from protagine.skills.hermes_export import (
    HERMES_AUTHOR,
    PROVENANCE_MARKER,
...
from protagine.skills.models import SkillManifest, SkillStatus
...
def _manifest(**overrides) -> SkillManifest:
    ...
    base = dict(
        ...
        author_protagine_id="protagine-test",
        created_at=now,
        updated_at=now,
...
PROCEDURAL_PATTERN = {
    ...
    "source_code": (
        "async def run(protagine, city: string):\n"
        "    _r0 = await protagine.tools.invoke('web_search', {'query': city})\n"
        "    return _r0\n"
    ),
}
...
class TestRenderSkillMd:
    ...
    def test_provenance_comment_block_in_frontmatter(self):
        ...
        assert "protagine_skill_id" in fm_raw
...
class TestProceduralHeuristic:
    ...
    def test_tool_invoke_in_source_is_procedural(self):
        p = {"source_code": "async def run(protagine):\n    await protagine.tools.invoke('x', {})\n"}
    ...
    def test_load_body_source_recovers_steps_from_skill_dir(self, tmp_path):
        ...
        (skill_dir / "skill.py").write_text(
            'async def run(protagine, city: str):\n'
        ...
        assert "protagine.tools.invoke" in body["source_code"]
...
class TestExportToHermes:
    def test_disabled_by_default_returns_none(self, tmp_path, monkeypatch):
        monkeypatch.delenv("PROTAGINE_EMIT_HERMES_SKILLS", raising=False)
    ...
    def test_explicit_false_returns_none(self, tmp_path, monkeypatch):
        monkeypatch.setenv("PROTAGINE_EMIT_HERMES_SKILLS", "false")
    ...
    def test_base_dir_from_env(self, tmp_path, monkeypatch):
        monkeypatch.setenv("PROTAGINE_EMIT_HERMES_SKILLS", "1")
        monkeypatch.setenv("PROTAGINE_HERMES_SKILLS_DIR", str(tmp_path / "via-env"))
    ...
    def test_export_approved_skill_skips_mcp(self, tmp_path, monkeypatch):
        monkeypatch.setenv("PROTAGINE_EMIT_HERMES_SKILLS", "true")
        monkeypatch.setenv("PROTAGINE_HERMES_SKILLS_DIR", str(tmp_path))
        m = _manifest(origin="mcp", mcp_server="github", mcp_tool="list_prs")
...
class TestApprovalHook:
    ...
    async def test_export_failure_does_not_block_approval(self, tmp_path, monkeypatch):
        monkeypatch.setenv("PROTAGINE_EMIT_HERMES_SKILLS", "true")
        monkeypatch.setenv("PROTAGINE_HERMES_SKILLS_DIR", str(tmp_path))
        import protagine.skills.hermes_export as hx
            ...
            json={
                ...
                "observations": [
                    ...
                    },
                    {
                        ...
                        "payload": {"description": "Protagine export", "source": "hermes"},
                    },
                ],
            },
        )
