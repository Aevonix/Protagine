...
(unsupported past PROTAGINE_CAUSAL_TTL_DAYS, default 120) lose 0.05 confidence
...
class TestStaleness:
    def test_ttl_default_120(self, monkeypatch):
        monkeypatch.delenv("PROTAGINE_CAUSAL_TTL_DAYS", raising=False)
    ...
    def test_ttl_env_override(self, monkeypatch):
        monkeypatch.setenv("PROTAGINE_CAUSAL_TTL_DAYS", "10")
...
async def test_stale_causal_decay_live(monkeypatch):
    monkeypatch.setenv("PROTAGINE_BELIEFS_MODE", "live")
    monkeypatch.setenv("PROTAGINE_CAUSAL_TTL_DAYS", "30")
