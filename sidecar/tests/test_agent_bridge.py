"""Tests for the unified protagine-agent-bridge worker (v0.21.31).
...
from protagine.workers import agent_bridge, queue_worker
...
def _clean_env(monkeypatch):
    for var in (
        "PROTAGINE_URL", "PROTAGINE_API_KEY", "PROTAGINE_INITIATIVE_WEBHOOK",
        "PROTAGINE_JOBS_WEBHOOK_URL", "PROTAGINE_AGENT_NAME",
        "PROTAGINE_WORKER_NODE_ID", "PROTAGINE_WORKER_MAX_JOBS",
        "PROTAGINE_BRIDGE_POLL_SECS", "PROTAGINE_BRIDGE_SKILLS_HOURS",
        "PROTAGINE_BRIDGE_LOG_CHANNEL", "PROTAGINE_BRIDGE_PLATFORM",
        "PROTAGINE_BRIDGE_STATE_DIR", "HERMES_SKILLS_DIR",
        "PROTAGINE_AGENT_WORKER_ROUTES", "PROTAGINE_AGENT_JOB_CLAIMS_ENABLED",
        "PROTAGINE_HERMES_WEBHOOK_SECRET", "PROTAGINE_HERMES_WEBHOOK_V1_COMPAT",
    ):
...
def test_dry_run_no_network(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setenv("PROTAGINE_BRIDGE_STATE_DIR", str(tmp_path / "state"))
    assert agent_bridge.main(["--dry-run"]) == 0
    out = capsys.readouterr().out
    assert "dry run" in out
    assert "protagine_url" in out
...
def test_health_sidecar_unreachable(tmp_path, monkeypatch):
    monkeypatch.setenv("PROTAGINE_BRIDGE_STATE_DIR", str(tmp_path))
...
def test_external_bridge_register_and_claim_use_same_narrow_route(
    ...
    monkeypatch.setenv("PROTAGINE_BRIDGE_STATE_DIR", str(tmp_path))
    monkeypatch.setenv("PROTAGINE_AGENT_WORKER_ROUTES", "hermes_run")
    cfg = agent_bridge._cfg()
    bodies = []
...
def test_health_alert_delivery_uses_exact_v2_signed_bytes(
    ...
    monkeypatch.setenv("PROTAGINE_HERMES_WEBHOOK_SECRET", "alert-secret")
...
def test_hermes_webhook_encoder_uses_pinned_v2_exact_body_hmac(monkeypatch):
    monkeypatch.delenv("PROTAGINE_HERMES_WEBHOOK_V1_COMPAT", raising=False)
