...
The ``e2e`` and ``integration`` suites talk to a *running* PacoMind sidecar over
HTTP (httpx against ``PACOMIND_URL``). They are skipped unless a live sidecar is
configured (``PACOMIND_URL`` + ``PACOMIND_API_KEY``) and actually reachable, so the
...
*entire* session (not just integration tests) whenever ``PACOMIND_URL`` was unset —
...
# operator's ~/.pacomind/.env and opens state stores immediately.  Unit tests
...
_TEST_STATE_DIR = tempfile.mkdtemp(prefix="pacomind-pytest-")
...
os.environ["PACOMIND_TEST_HOME"] = _TEST_HOME_DIR
...
os.environ["PACOMIND_SKIP_DOTENV"] = "1"
os.environ["PACOMIND_STATE_DIR"] = _TEST_STATE_DIR
...
def _live_sidecar_status() -> tuple[bool, str]:
    ...
    url = os.environ.get("PACOMIND_URL")
    key = os.environ.get("PACOMIND_API_KEY")
    if not url or not key:
        return False, (
            "live sidecar tests skipped — set PACOMIND_URL and PACOMIND_API_KEY "
