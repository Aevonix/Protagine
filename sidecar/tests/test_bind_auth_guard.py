...
non-loopback interface (PACOMIND_API_KEY unset + bind 0.0.0.0/LAN = open to net)."""
import pytest

from pacomind.cli import _guard_bind_auth, _is_loopback_host
...
def test_non_loopback_without_key_fails_closed(monkeypatch):
    ...
    monkeypatch.delenv("PACOMIND_ALLOW_OPEN_BIND", raising=False)
...
def test_explicit_override_allows_open_bind(monkeypatch):
    monkeypatch.delenv("PACOMIND_API_KEY", raising=False)
    monkeypatch.setenv("PACOMIND_ALLOW_OPEN_BIND", "1")
