...
from protagine.turns.idempotency import get_turn_idempotency_ledger
import sys
...
@pytest.mark.parametrize('module', ['protagine', 'protagine.cli'])
...
''', module)
    assert result.returncode == 0, result.stdout + result.stderr
    assert 'usage: protagine ' in result.stdout
...
def test_container_defaults_preserve_selected_state(tmp_path):
    ...
    for selected in ({}, {'PROTAGINE_STATE_DIR': '/current'}):
        ...
        assert json.loads(result.stdout) == (selected or {'PROTAGINE_STATE_DIR': '/var/lib/protagine'})
...
def test_concurrent_state_reads_never_rewrite_process_environment(tmp_path, monkeypatch):
    ...
    with ThreadPoolExecutor(max_workers=8) as pool:
        results = list(pool.map(lambda _: protagine.get_state_dir(), range(128)))
