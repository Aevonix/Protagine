...
from protagine.intelligence.graph.client import _recency_factor


def test_recency_defaults(monkeypatch):
    monkeypatch.delenv("PROTAGINE_RECENCY_HALF_LIFE_DAYS", raising=False)
    monkeypatch.delenv("PROTAGINE_RECENCY_FLOOR", raising=False)
...
def test_recency_configurable(monkeypatch):
    monkeypatch.setenv("PROTAGINE_RECENCY_HALF_LIFE_DAYS", "30")
    monkeypatch.setenv("PROTAGINE_RECENCY_FLOOR", "0.2")
    assert abs(_recency_factor(30) - (0.2 + 0.8 * 0.5)) < 0.01   # 0.6
    assert _recency_factor(0) == 1.0


def test_recency_disabled(monkeypatch):
    monkeypatch.setenv("PROTAGINE_RECENCY_HALF_LIFE_DAYS", "0")
