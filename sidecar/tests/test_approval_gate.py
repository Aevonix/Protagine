...
def _isolated_approval_state(monkeypatch, tmp_path):
    """Approval tests must never touch a developer/live Protagine state dir."""

    monkeypatch.setenv("PROTAGINE_STATE_DIR", str(tmp_path / "state"))
    monkeypatch.delenv("PROTAGINE_APPROVAL_AUTHORITY_MODE", raising=False)
...
async def test_legacy_auto_approve_env_cannot_bypass_central_gate(
    ...
    monkeypatch.setenv("PROTAGINE_AGENT_AUTO_APPROVE", "true")
    mgr = await _make_mgr(tmp_path)
    try:
        stub = _loop_stub(mgr)
        await AutonomyLoop._post_agent_action_to_queue(
...
async def test_phase_approval_timeout_respects_env(tmp_path, monkeypatch):
    monkeypatch.setenv("PROTAGINE_APPROVAL_TIMEOUT_HOURS", "1")
    mgr = await _make_mgr(tmp_path)
    try:
        stale = Job(
            job_type=JobType.AGENT_ACTION,
