...
def test_verdict_rows_never_prove_applied_enforcement(monkeypatch):
    monkeypatch.delenv("PROTAGINE_GUARD_EVIDENCE_MIN", raising=False)
    a = GuardAuditStore(":memory:")
    assert a.enforce_evidence("rcs") is False
    _enforce_rows(a, 2)
    assert a.enforce_evidence("rcs") is False
...
def test_enforce_evidence_min_env(monkeypatch):
    from protagine.gate.guard_audit import evidence_min

    monkeypatch.setenv("PROTAGINE_GUARD_EVIDENCE_MIN", "5")
    assert evidence_min() == 5
    monkeypatch.setenv("PROTAGINE_GUARD_EVIDENCE_MIN", "banana")
    assert evidence_min() == 3                       # malformed => default
    monkeypatch.setenv("PROTAGINE_GUARD_EVIDENCE_MIN", "0")
    assert evidence_min() == 1                       # zero-proof is not a config
    monkeypatch.delenv("PROTAGINE_GUARD_EVIDENCE_MIN", raising=False)
    a = GuardAuditStore(":memory:")
    _enforce_rows(a, 1, gateway="sms")
    monkeypatch.setenv("PROTAGINE_GUARD_EVIDENCE_MIN", "1")
