...
Sends a real conversation through Protagine's turn_sync, then verifies
that ToM extraction, pattern extraction, and surprise scoring fired
and stored results.

Usage: PROTAGINE_API_KEY=test python3 test_turn_sync_extraction.py
"""
...
def get(path, **params):
    r = httpx.get(f"{PROTAGINE_URL}{path}", headers=HEADERS, params=params, timeout=10)
    return r

def post(path, data):
    r = httpx.post(f"{PROTAGINE_URL}{path}", headers={**HEADERS, **CT}, json=data, timeout=15)
    return r

def patch(path, data):
    r = httpx.patch(f"{PROTAGINE_URL}{path}", headers={**HEADERS, **CT}, json=data, timeout=10)
    return r
...
def test_turn_sync_extraction():
    ...
    if r.status_code == 200:
        ...
        if "protagine-commitments" in section_ids:
            log("  Commitments in context", "✅")
        if "protagine-affect" not in section_ids:
            log("  Legacy mood estimates omitted from ordinary context", "✅")
        if "protagine-shared-facts" in section_ids:
            log("  Shared facts in context", "✅")
        if "protagine-surprises" in section_ids:
