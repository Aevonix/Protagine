...
from protagine.delivery.channels import Channel, ChannelRegistry
...
def clean_env(monkeypatch):
    """Remove PROTAGINE_CHANNEL_* env vars before each test."""
    for key in list(os.environ.keys()):
        if key.startswith("PROTAGINE_CHANNEL_") or key.endswith("_HOME_CHANNEL"):
...
class TestChannelRegistryLoad:
    ...
    def test_env_dm_channel(self, temp_json_path, monkeypatch):
        monkeypatch.setenv("PROTAGINE_CHANNEL_DM_owner", "telegram:@owner")
        registry = ChannelRegistry.load(json_path=temp_json_path)
    ...
    def test_env_home_channel(self, temp_json_path, monkeypatch):
        monkeypatch.setenv("PROTAGINE_CHANNEL_HOME", "discord:#general")
        registry = ChannelRegistry.load(json_path=temp_json_path)
    ...
    def test_env_home_per_person(self, temp_json_path, monkeypatch):
        monkeypatch.setenv("PROTAGINE_CHANNEL_HOME_owner", "signal:+15551234567")
        registry = ChannelRegistry.load(json_path=temp_json_path)
    ...
    def test_priority_env_over_json(self, temp_json_path, write_json, monkeypatch):
        monkeypatch.setenv("PROTAGINE_CHANNEL_DM_owner", "signal:+1555ZZZZZZZ")
        write_json({
            "contacts": {
                "owner": {
                    "dm": {"platform": "telegram", "chat_id": "@username"},
    ...
    def test_custom_gateway_map(self, temp_json_path, mock_contacts_store, monkeypatch):
        monkeypatch.setenv("PROTAGINE_CHANNEL_GATEWAY_MAP", '{"imessage": "telegram"}')
        registry = ChannelRegistry.load(
            json_path=temp_json_path,
            handle_inference=True,
            contacts_store=mock_contacts_store,
    ...
    def test_owner_contact_id_alias(self, temp_json_path, monkeypatch):
        """PROTAGINE_CHANNEL_DM_owner resolves for the owner's contact UUID."""
        monkeypatch.setenv("PROTAGINE_OWNER_CONTACT_ID", "cid-test-12345-owner")
        monkeypatch.setenv("PROTAGINE_CHANNEL_DM_owner", "whatsapp:+1555ZZZZZZZ")
        registry = ChannelRegistry.load(json_path=temp_json_path)
...
class TestChannelRegistryReload:
    ...
    def test_reload_picks_up_new_env(self, temp_json_path, monkeypatch):
        monkeypatch.setenv("PROTAGINE_CHANNEL_DM_owner", "telegram:@old")
        registry = ChannelRegistry.load(json_path=temp_json_path)
        assert registry.resolve("owner", "dm").chat_id == "@old"

        monkeypatch.setenv("PROTAGINE_CHANNEL_DM_owner", "telegram:@new")
        registry.reload()
