...
from protagine.intelligence.components.initiative_engine import InitiativeConfig, InitiativeEngine, InitiativeType
...
async def test_actual_loader_replaces_legacy_check_and_keeps_other_operational_checks(tmp_path, monkeypatch):
    ...
    old = tmp_path/'.protagine/backups/old.bak'
    ...
    monkeypatch.setenv('PROTAGINE_INITIATIVE_BACKUP_RECEIPT', str(pointer))
    log = tmp_path/'.protagine/logs/large.log'
    monkeypatch.setenv('PROTAGINE_LOG_PATH', str(log))
