...
from protagine.initiatives import approval_authority as authority
...
def test_default_grant_envelope_preserves_30_day_100_use_caps(
    ...
    monkeypatch.delenv("PROTAGINE_GRANT_MAX_TTL_SECONDS", raising=False)
    monkeypatch.delenv("PROTAGINE_GRANT_MAX_USES", raising=False)
    ...
    from protagine.api.routers.task_queue import BoundedGrantRequest
...
def test_configured_finite_grant_envelope_is_honoured(tmp_path, monkeypatch):
    monkeypatch.setenv("PROTAGINE_GRANT_MAX_TTL_SECONDS", str(60 * 24 * 60 * 60))
    monkeypatch.setenv("PROTAGINE_GRANT_MAX_USES", "250")
    now = datetime(2026, 1, 1, tzinfo=timezone.utc)
    ...
    monkeypatch.setenv("PROTAGINE_GRANT_MAX_TTL_SECONDS", "60")
    monkeypatch.setenv("PROTAGINE_GRANT_MAX_USES", "1")
...
def test_standing_grant_neither_expires_nor_exhausts(tmp_path, monkeypatch):
    monkeypatch.setenv("PROTAGINE_GRANT_MAX_TTL_SECONDS", "unlimited")
    monkeypatch.setenv("PROTAGINE_GRANT_MAX_USES", "unlimited")
    now = datetime(2026, 1, 1, tzinfo=timezone.utc)
    store = authority.ApprovalAuthorityStore(tmp_path / "authority.db")
...
def test_non_grantable_tool_is_refused_under_standing_envelope(monkeypatch):
    ...
    from protagine_hostworker.catalog import GRANT_AUTHORIZABLE_TOOL_NAMES
    from protagine_hostworker.conformance.harness import sqlite_harness
    from protagine_hostworker.conformance.suite import (
        check_non_grantable_tool_with_grant_proof,
    )

    assert "protagine_autonomy_enable" not in GRANT_AUTHORIZABLE_TOOL_NAMES
    assert "protagine_autonomy_disable" not in GRANT_AUTHORIZABLE_TOOL_NAMES
