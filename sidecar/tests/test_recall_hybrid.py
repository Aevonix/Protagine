...
from protagine.intelligence.graph.queries import lexical_query
...
def calibrated(monkeypatch, fixture, reranker, threshold=.8):
    ...
    monkeypatch.setenv("PROTAGINE_RECALL_RERANK_MIN_SCORE", str(threshold))
...
async def test_hybrid_finds_new_source_before_vector_index_catches_up(monkeypatch):
    monkeypatch.setenv("PROTAGINE_RECALL_HYBRID", "on")
