...
from protagine.task_queue.handlers.monitoring import _validate_endpoint
from protagine.task_queue.handlers.system_maintenance import (
    SystemMaintenanceHandler,
)
from protagine.task_queue.models import Job, JobType
from protagine.task_queue.worker import _prepare_embedded_report
...
async def test_monitoring_rejects_private_targets_unless_explicitly_allowlisted(
    ...
    monkeypatch.delenv("PROTAGINE_MONITORING_HOST_ALLOWLIST", raising=False)
    with pytest.raises(ValueError, match="non-public"):
        await _validate_endpoint("http://127.0.0.1:7777/health")

    monkeypatch.setenv("PROTAGINE_MONITORING_HOST_ALLOWLIST", "127.0.0.1")
...
async def test_monitoring_rejects_non_global_ip_and_unpinned_dns(monkeypatch):
    ...
    monkeypatch.setenv(
        "PROTAGINE_MONITORING_HOST_ALLOWLIST", "status.example.test",
    )
    assert await _validate_endpoint(
