...
from protagine.self_model.thinker import _parse
...
def test_decay_reduces_and_evicts(tmp_path, monkeypatch):
    monkeypatch.setenv("PROTAGINE_WORKSPACE_HALFLIFE_HOURS", "12")
    monkeypatch.setenv("PROTAGINE_WORKSPACE_EVICT_FLOOR", "0.2")
    ws, store = make(tmp_path)
...
def test_capacity_evicts_lowest(tmp_path, monkeypatch):
    monkeypatch.setenv("PROTAGINE_WORKSPACE_CAPACITY", "3")
    monkeypatch.setenv("PROTAGINE_WORKSPACE_EVICT_FLOOR", "0.0")
    ws, store = make(tmp_path)
    for i in range(5):
...
def test_sleep_window(monkeypatch):
    monkeypatch.setenv("PROTAGINE_SLEEP_WINDOW", "02:00-06:00")
...
def test_sleep_window_wraps_midnight(monkeypatch):
    monkeypatch.setenv("PROTAGINE_SLEEP_WINDOW", "22:00-06:00")
...
def test_sleep_window_disabled(monkeypatch):
    monkeypatch.delenv("PROTAGINE_SLEEP_WINDOW", raising=False)
...
async def _client(ws, authority=None):
    ...
    async def _legacy_authority(request, call_next):
        request.state.protagine_authority = authority or legacy_authority()
...
async def test_api_snapshot(tmp_path, monkeypatch):
    monkeypatch.setenv("PROTAGINE_WORKSPACE", "shadow")
    ws, _ = make(tmp_path)
