...
from protagine.task_queue.models import JobResult, JobStatus
...
async def test_capture_disabled_by_default(monkeypatch):
    monkeypatch.delenv("PROTAGINE_ENABLE_SKILL_SYNTHESIS", raising=False)
    loop = _loop()
    service = MagicMock()
...
async def test_capture_creates_deferred_review_initiative(monkeypatch):
    monkeypatch.setenv("PROTAGINE_ENABLE_SKILL_SYNTHESIS", "true")
    loop = _loop()
    service = MagicMock()
