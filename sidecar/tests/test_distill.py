...
record_turn stores the VERBATIM summary (PROTAGINE_DISTILL_TURNS stays a
...
class TestDistillerKnobs:
    def test_defaults_unchanged_without_env(self, monkeypatch):
        for var in ("PROTAGINE_DISTILL_MIN_RECALLS", "PROTAGINE_DISTILL_MIN_STRENGTH",
                    "PROTAGINE_DISTILL_MIN_AGE_DAYS"):
    ...
    def test_env_knobs_apply(self, monkeypatch):
        monkeypatch.setenv("PROTAGINE_DISTILL_MIN_RECALLS", "2")
        monkeypatch.setenv("PROTAGINE_DISTILL_MIN_STRENGTH", "0.3")
        monkeypatch.setenv("PROTAGINE_DISTILL_MIN_AGE_DAYS", "1")
        d = MemoryDistiller(_FakeGraph())
    ...
    def test_invalid_env_falls_back_to_defaults(self, monkeypatch):
        monkeypatch.setenv("PROTAGINE_DISTILL_MIN_RECALLS", "banana")
        monkeypatch.setenv("PROTAGINE_DISTILL_MIN_STRENGTH", "")
        d = MemoryDistiller(_FakeGraph())
        assert d._min_recalls == 3
        assert d._min_strength == 0.5
    ...
    async def test_thresholds_reach_candidate_query(self, monkeypatch):
        monkeypatch.setenv("PROTAGINE_DISTILL_MIN_RECALLS", "4")
        monkeypatch.setenv("PROTAGINE_DISTILL_MIN_STRENGTH", "0.6")
        monkeypatch.setenv("PROTAGINE_DISTILL_MIN_AGE_DAYS", "10")
        graph = _FakeGraph()
...
async def test_default_stores_verbatim_summary(monkeypatch):
    """Regression lock: PROTAGINE_DISTILL_TURNS unset -> content == summary."""
    monkeypatch.delenv("PROTAGINE_DISTILL_TURNS", raising=False)
...
async def test_distill_on_joins_with_semicolon_and_keeps_both_speakers(monkeypatch):
    monkeypatch.setenv("PROTAGINE_DISTILL_TURNS", "1")
    fx = _RecordTurnFixture()
...
def test_agent_label_blocks_contact_claim_extraction():
    ...
    from protagine.beliefs.contradictions import claims_from_text
