...
def store(tmp_path: Path) -> MiningStore:
    return MiningStore(db_path=str(tmp_path / "protagine-mining.db"))


@pytest.fixture
def miner(store: MiningStore, monkeypatch) -> EscalationMiner:
    monkeypatch.setenv("PROTAGINE_ESCALATION_MINING", "shadow")
...
def test_mode_off_banks_nothing(store, monkeypatch):
    monkeypatch.setenv("PROTAGINE_ESCALATION_MINING", "off")
...
def test_provider_escalation(store, monkeypatch):
    monkeypatch.setenv("PROTAGINE_ESCALATION_MINING", "shadow")
    monkeypatch.setenv("PROTAGINE_ESCALATION_HEAVY_RE", r"glm-5|opus")
    m = EscalationMiner(store)
    ...
    monkeypatch.delenv("PROTAGINE_ESCALATION_HEAVY_RE")
...
def test_live_mode_feeds_distiller(store, monkeypatch):
    monkeypatch.setenv("PROTAGINE_ESCALATION_MINING", "live")
...
class TestMiningApi:
    ...
    def client(self, store, tmp_path, monkeypatch):
        ...
        import protagine.api.routers.mining as mining_router
    ...
    def test_export_endpoint_gate(self, client, monkeypatch):
        monkeypatch.setenv("PROTAGINE_CORPUS_EXPORT_ENABLED", "false")
...
def test_consultation_via_tool_activity_file(store, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_TOOL_ACTIVITY_FILE", str(activity))
    m = EscalationMiner(store)
