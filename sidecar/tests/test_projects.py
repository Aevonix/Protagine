...
from protagine.projects import (
...
def _engine(monkeypatch, router=None, dm=None, deliver=None,
    ...
    monkeypatch.setenv("PROTAGINE_PROJECTS_REVIEW_SECS", "30")
...
async def test_owner_goal_negated_boundary_constraint_does_not_self_block(
    ...
    from protagine.directives import Directive, Polarity
    ...
    objective = (
        "verify Protagine cognition is live with a read-only internal plan; "
...
async def test_replan_on_failure_bounded(monkeypatch):
    monkeypatch.setenv("PROTAGINE_PROJECTS_MAX_REPLANS", "1")
...
async def test_planning_failure_eventually_abandons(monkeypatch):
    monkeypatch.setenv("PROTAGINE_PROJECTS_MAX_REPLANS", "0")
...
async def test_trust_graduation_lifts_shadow(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_PROJECTS_MODE", "off")   # owner override wins
