...
async def context(client, query='archive digest'):
    ...
    return next((s for s in response.json()['sections'] if s['id'] == 'protagine-memory'), None)
...
async def test_correction_representation_invalidates_previous_cutoff(monkeypatch):
    ...
    monkeypatch.setenv('PROTAGINE_RECALL_RERANK_MIN_SCORE', '0.7')
    monkeypatch.setenv('PROTAGINE_RECALL_RERANK_CALIBRATION', calibration_fingerprint(old_metadata))
    calls = []
...
def test_append_failure_is_atomic_and_session_scope_cannot_widen(annotated_app, monkeypatch):
    _, ledger = annotated_app
    from protagine.turns import source_vectors
