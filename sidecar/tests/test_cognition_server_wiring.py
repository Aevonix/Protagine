...
from protagine.server import _attach_cognition_spine
...
def test_default_off_does_not_create_database(tmp_path, monkeypatch):
    monkeypatch.delenv("PROTAGINE_COGNITION_SPINE", raising=False)
    deps = _dependencies()
    ...
    assert not (tmp_path / "protagine-cognition.db").exists()


def test_enabled_spine_attaches_with_fixed_policy_config(tmp_path, monkeypatch):
    monkeypatch.setenv("PROTAGINE_COGNITION_SPINE", "shadow")
    monkeypatch.setenv("PROTAGINE_PROJECTS_MAX_CONCURRENT", "2")
    monkeypatch.setenv(
    ...
    assert (tmp_path / "protagine-cognition.db").exists()
...
def test_live_spine_rejects_non_live_project_engine(
    ...
    monkeypatch.setenv("PROTAGINE_PROJECTS_MODE", projects_mode)
...
def test_enabled_spine_rejects_invalid_project_limit(
    ...
    monkeypatch.setenv("PROTAGINE_COGNITION_SPINE", "shadow")
    monkeypatch.setenv("PROTAGINE_PROJECTS_MAX_CONCURRENT", value)

    with pytest.raises(RuntimeError, match="PROTAGINE_PROJECTS_MAX_CONCURRENT"):
        _attach_cognition_spine(state_dir=tmp_path, **_dependencies())
...
def _configure_thought_owner(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_THOUGHT_WORKER_NODE_ID", owner)
    return owner
...
def test_live_spine_rejects_missing_or_wrong_thought_owner(
    ...
    monkeypatch.setenv("PROTAGINE_PROJECTS_MODE", "live")
    if configured:
        monkeypatch.setenv("PROTAGINE_THOUGHT_WORKER_NODE_ID", configured)
    else:
        monkeypatch.delenv("PROTAGINE_THOUGHT_WORKER_NODE_ID", raising=False)
    with pytest.raises(RuntimeError, match="PROTAGINE_THOUGHT_WORKER_NODE_ID"):
        _attach_cognition_spine(
            state_dir=tmp_path,
            llm_router=object(),
            embedded_worker_enabled=True,
