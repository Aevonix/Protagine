...
async def test_accepted_local_work_and_result_are_visible_only_to_actual_owner(tmp_path,monkeypatch):
    ...
    async def identity(request,next_call):
        request.state.protagine_authority=authority[0];return await next_call(request)
    ...
    async with AsyncClient(transport=ASGITransport(app=app),base_url='http://test') as client:
        for person in ('guest','owner'):
            ...
            if person=='guest':
                ...
                context=await host.context_assemble(request,SimpleNamespace(state=SimpleNamespace(protagine_authority=authority[0])))
                section=next(s for s in context.sections if s.id=='protagine-executions')
