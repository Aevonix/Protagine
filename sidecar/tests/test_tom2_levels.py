...
from pacomind.tom import levels
...
def test_level_defaults(monkeypatch):
    ...
    monkeypatch.delenv("PACOMIND_TOM2_MAX_LEVEL", raising=False)
    assert configured_level() == 0
    assert configured_max_level() == 1


def test_malformed_level_fails_closed(monkeypatch):
    monkeypatch.setenv("PACOMIND_TOM2_LEVEL", "two")
    assert configured_level() == 0
    monkeypatch.setenv("PACOMIND_TOM2_MAX_LEVEL", "!!")
    assert configured_max_level() == 0
    monkeypatch.setenv("PACOMIND_TOM2_LEVEL", "9")
    assert configured_level() == 2          # clamped, not trusted


def test_risk_caps_default(monkeypatch):
    monkeypatch.delenv("PACOMIND_TOM2_RISK_CAPS", raising=False)
...
@pytest.mark.parametrize("brake, expected", [
    ({"PACOMIND_TOM2_LEVEL": "0"}, 0),
    ({"PACOMIND_TOM2_MAX_LEVEL": "1"}, 1),
    ({"PACOMIND_TOM2_CROSS_CONTEXT": "0"}, 1),
    ({"PACOMIND_TOM2_RISK_CAPS": "0:2,1:0,2:0,3:0"}, 0),
])
