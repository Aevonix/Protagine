...
from protagine.tom import levels
...
def test_level_defaults(monkeypatch):
    ...
    monkeypatch.delenv("PROTAGINE_TOM2_MAX_LEVEL", raising=False)
    assert configured_level() == 0
    assert configured_max_level() == 1


def test_malformed_level_fails_closed(monkeypatch):
    monkeypatch.setenv("PROTAGINE_TOM2_LEVEL", "two")
    assert configured_level() == 0
    monkeypatch.setenv("PROTAGINE_TOM2_MAX_LEVEL", "!!")
    assert configured_max_level() == 0
    monkeypatch.setenv("PROTAGINE_TOM2_LEVEL", "9")
...
def test_malformed_risk_caps_all_zero(monkeypatch, raw):
    monkeypatch.setenv("PROTAGINE_TOM2_RISK_CAPS", raw)
...
@pytest.mark.parametrize("brake, expected", [
    ({"PROTAGINE_TOM2_LEVEL": "0"}, 0),
    ({"PROTAGINE_TOM2_MAX_LEVEL": "1"}, 1),
    ({"PROTAGINE_TOM2_CROSS_CONTEXT": "0"}, 1),
    ({"PROTAGINE_TOM2_RISK_CAPS": "0:2,1:0,2:0,3:0"}, 0),
])
...
async def test_hostile_environment_resolves_zero(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_OWNER_CONTACT_ID", OWNER)
    monkeypatch.delenv("PROTAGINE_ENV_RISK_GATEWAY_CLASS", raising=False)
    monkeypatch.setenv("PROTAGINE_TOM2_LEVEL", "2")
    monkeypatch.setenv("PROTAGINE_TOM2_MAX_LEVEL", "2")
    monkeypatch.setenv("PROTAGINE_TOM2_CROSS_CONTEXT", "1")
    set_evidence_probe(lambda gw: True)
