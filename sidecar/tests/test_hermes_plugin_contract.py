"""Contract test: the Hermes protagine plugin's endpoints must exist in the host API.
...
registered route. It exists because a `protagine_task_*` tool quietly called a
...
from protagine.api.routers.initiative_work import router as initiative_work_router
...
def test_integration_dir_present():
    assert _INTEGRATION.is_dir(), f"protagine Hermes integration missing at {_INTEGRATION}"
...
def test_plugin_endpoints_all_exist_in_host_api():
    ...
    assert not missing, (
        "Hermes protagine plugin references endpoints that are NOT registered in the host "
