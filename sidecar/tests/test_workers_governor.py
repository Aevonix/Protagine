...
from protagine.task_queue.governor import WorkerGovernor
...
from protagine.workers import protagine_worker as cw
...
def test_live_refuses_worker_lacking_required_capability(monkeypatch):
    monkeypatch.setenv("PROTAGINE_WORKERS_MODE", "live")
...
def test_shadow_observes_but_never_blocks(monkeypatch):
    monkeypatch.setenv("PROTAGINE_WORKERS_MODE", "shadow")
...
def test_embedded_worker_helper_matches_release_attestation_shape():
    """Keep Protagine compatible with the host's reviewed AST release gate."""
...
def test_invalid_worker_governor_mode_fails_closed(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_WORKERS_MODE", "typo-live")
...
def test_audit_allows_mutation_on_authorized_job():
    ...
    audit = gov.audit_report(job, {"summary": "patched", "operations": ["commit"],
                                   "commits": 1, "branch": "protagine/fix"})
    assert audit["verdict"] == "clean"
...
async def test_read_only_worker_earns_bounded_live_trials_then_act_first(
    ...
    monkeypatch.setenv("PROTAGINE_TRUST_ASK_MIN_N", "3")
    monkeypatch.setenv("PROTAGINE_TRUST_ACT_MIN_N", "5")
    monkeypatch.setenv("PROTAGINE_TRUST_READ_ONLY_TRIAL_MAX", "5")
    sm = _self_model(earned=False)
    governor = WorkerGovernor(
        directive_manager=_FakeDirectives(True),
        self_model=sm,
...
async def test_read_only_live_trial_budget_counts_unverified_attempts(
    ...
    monkeypatch.setenv("PROTAGINE_TRUST_ASK_MIN_N", "1")
    monkeypatch.setenv("PROTAGINE_TRUST_ACT_MIN_N", "2")
    monkeypatch.setenv("PROTAGINE_TRUST_READ_ONLY_TRIAL_MAX", "1")
    sm = _self_model(earned=False)
    governor = WorkerGovernor(
        directive_manager=_FakeDirectives(True),
        self_model=sm,
...
async def test_duplicate_success_replay_does_not_redistill_skill(monkeypatch):
    from protagine.skills_memory import SkillStore
    ...
    monkeypatch.setenv("PROTAGINE_WORKERS_MODE", "live")
    monkeypatch.setenv("PROTAGINE_SKILLS_DISTILL", "live")
    sm = _self_model()
    skills = SkillStore()
    llm = _LLM()
    governor = WorkerGovernor(
...
async def test_explicit_failure_remains_negative_trust_evidence(monkeypatch):
    ...
    assert events[0]["evidence_status"] == "observed"
    assert events[0]["outcome_contract"] == "protagine.worker-outcome/v1"
...
def test_worker_config_defaults(monkeypatch):
    for k in ("PROTAGINE_WORKER_CAPABILITIES", "PROTAGINE_WORKER_JOB_TYPES",
              "PROTAGINE_WORKER_NODE_ID"):
        monkeypatch.delenv(k, raising=False)
    monkeypatch.setenv("PROTAGINE_AGENT_NAME", "Test Bot")
    cfg = cw.load_config()
    assert cfg["node_id"] == "test-bot-worker"
    assert "research" in cfg["capabilities"]


def test_external_protagine_worker_starts_before_llm_and_completion(monkeypatch):
    calls = []
    ...
    assert cw.execute_job({"protagine_url": "http://protagine"}, {"job_id": "job-1"})
