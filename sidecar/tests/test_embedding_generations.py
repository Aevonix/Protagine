...
def test_cli_migration_stops_truthfully(monkeypatch, capsys, status_code, status, wait, expected):
    ...
    monkeypatch.setattr(sys, 'argv', ['protagine', 'migrate-tier', '--wait-seconds', str(wait)])
    clock = [0.]
...
async def test_explicit_request_width_requires_its_own_generation(tmp_path, monkeypatch):
    ...
    client = httpx.AsyncClient
    monkeypatch.setattr(httpx, 'AsyncClient', lambda **kw: client(transport=httpx.MockTransport(response), **kw))
    monkeypatch.setenv('PROTAGINE_EMBED_QUERY_INSTRUCTION', 'Retrieve: ')
...
async def test_provider_order_identity_and_query_format_are_bound(monkeypatch):
    ...
    monkeypatch.setenv('PROTAGINE_EMBED_QUERY_INSTRUCTION', 'Different: ')
...
def test_vector_cli_request_matches_current_host_contract(monkeypatch, capsys, command, request_type, summary):
    ...
    import time
    from protagine import cli
    from protagine.api.schemas import host
    monkeypatch.setattr(cli, '_load_dotenv', lambda: None)
    monkeypatch.setattr(sys, 'argv', ['protagine', command, '--batch-size', '32'])
