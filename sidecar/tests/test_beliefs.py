...
def test_source_trust_env_override(monkeypatch):
    monkeypatch.setenv("PROTAGINE_SOURCE_TRUST", "connector:0.95")
...
async def test_live_resolution_supersedes_and_audits(monkeypatch):
    monkeypatch.setenv("PROTAGINE_BELIEFS_MODE", "live")
...
async def test_shadow_detects_but_never_mutates(monkeypatch):
    monkeypatch.setenv("PROTAGINE_BELIEFS_MODE", "shadow")
    old_ts = datetime.now(timezone.utc) - timedelta(days=30)
    graph = FakeGraph([
        _mem_row("m-old", "Jordan works at Initech.", ts=old_ts),
        _mem_row("m-new", "Jordan works at Globex."),
...
async def test_decay_lowers_confidence_past_ttl(monkeypatch):
    monkeypatch.setenv("PROTAGINE_BELIEFS_MODE", "live")
    monkeypatch.setenv("PROTAGINE_BELIEFS_STALE_DAYS", "10")
...
def test_beliefs_graduation_requires_act_first(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_BELIEFS_MODE", "off")
...
# ---------------------------------------------------------------------------
# U26: supervised-live rung (PROTAGINE_BELIEFS_SUPERVISED_LIVE)
# ---------------------------------------------------------------------------
...
def test_supervised_flag_off_is_todays_exact_behavior(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_BELIEFS_MODE", "shadow")
    monkeypatch.delenv("PROTAGINE_BELIEFS_SUPERVISED_LIVE", raising=False)
    sm, _ = _self_model_at("ask_first")
    engine = BeliefEngine(BeliefStore(), self_model=sm)
    assert engine._effective_mode() == "shadow"
    monkeypatch.setenv("PROTAGINE_BELIEFS_SUPERVISED_LIVE", "0")
    assert engine._effective_mode() == "shadow"


def test_supervised_mode_only_at_ask_first(monkeypatch):
    monkeypatch.setenv("PROTAGINE_BELIEFS_MODE", "shadow")
    monkeypatch.setenv("PROTAGINE_BELIEFS_SUPERVISED_LIVE", "1")
...
async def test_legacy_and_generic_flags_are_equivalent(monkeypatch):
    """PROTAGINE_BELIEFS_SUPERVISED_LIVE=1 and PROTAGINE_SUPERVISED_LIVE_DOMAINS=
    ...
    monkeypatch.delenv("PROTAGINE_BELIEFS_SUPERVISED_LIVE", raising=False)
    monkeypatch.setenv("PROTAGINE_SUPERVISED_LIVE_DOMAINS", "beliefs")
    generic = await _run()
...
# ---------------------------------------------------------------------------
# H1.3: trust outcome integrity (PROTAGINE_TRUST_STRICT_OUTCOMES, default ON)
# ---------------------------------------------------------------------------
#
...
async def test_strict_noop_supervised_run_records_nothing(monkeypatch):
    ...
    monkeypatch.delenv("PROTAGINE_TRUST_STRICT_OUTCOMES", raising=False)
    monkeypatch.setenv("PROTAGINE_BELIEFS_MODE", "shadow")
    monkeypatch.setenv("PROTAGINE_BELIEFS_SUPERVISED_LIVE", "1")
    sm, cstore = _self_model_at("ask_first")
...
async def test_legacy_flag_zero_restores_unconditional_success(monkeypatch):
    """Regression lock for the escape hatch: PROTAGINE_TRUST_STRICT_OUTCOMES=0
    is exactly the old behavior — one success per run, no-op or not."""
    monkeypatch.setenv("PROTAGINE_TRUST_STRICT_OUTCOMES", "0")
    monkeypatch.setenv("PROTAGINE_BELIEFS_MODE", "shadow")
    monkeypatch.setenv("PROTAGINE_BELIEFS_SUPERVISED_LIVE", "1")
