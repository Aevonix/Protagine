...
from protagine.tom.engagement import EngagementStore, build_guidance
...
async def engagement_brief(client):
    ...
    return '\n'.join(s['body'] for s in response.json()['sections'] if s['id'] == 'protagine-engagement')


@pytest.fixture
async def approach(relations, monkeypatch, tmp_path):
    from protagine.contacts import store as contact_module
...
async def test_cached_approach_forget_reopen_changes_next_ordinary_context(approach, monkeypatch, tmp_path):
    ...
    async with AsyncClient(transport=ASGITransport(app=r.app), base_url='http://test') as client:
        ...
        assert 'neutral-source-topic' not in before['protagine-approach']
        assert 'mood is negative' not in before['protagine-approach']
        assert 'Recent mood:' not in before['protagine-approach']
        ...
        assert 'mood is negative' not in after['protagine-approach']
        assert 'independent topic' not in after['protagine-approach']
        assert 'independent topic' in r.engagement.get_profile('contact-a')['legacy_profile']['qual']['topics']
        assert 'mostly via test:thread-a' in after['protagine-approach']
...
async def test_ordinary_ingress_no_longer_produces_legacy_numeric_observations(approach, monkeypatch):
    ...
    async with AsyncClient(transport=ASGITransport(app=r.app), base_url='http://test') as client:
        ...
        assert 'later-source-topic' not in (await relationship_context(client))['protagine-approach']
...
async def test_old_cached_advice_is_omitted_when_current_store_unavailable(approach, unavailable, monkeypatch):
    ...
    async with AsyncClient(transport=ASGITransport(app=r.app), base_url='http://test') as client:
        ...
        after = (await relationship_context(client))['protagine-approach']
...
async def test_badge_turn_keeps_source_learning_without_mood_inference_or_injection(approach, monkeypatch):
    ...
    async with AsyncClient(transport=ASGITransport(app=r.app), base_url='http://test') as client:
        ...
        assert 'cobalt-716' in sections['protagine-memory']
        assert 'protagine-affect' not in sections
