"""Tests for protagine.vector.scanner — hardware detection."""
import pytest
from unittest.mock import patch, MagicMock
from protagine.vector.scanner import scan, HardwareProfile, _ram_gb
...
class TestScan:
    @patch("protagine.vector.scanner._has_cuda", return_value=False)
    @patch("protagine.vector.scanner._has_mlx", return_value=False)
    @patch("protagine.vector.scanner._ram_gb", return_value=16)
    ...
    @patch("protagine.vector.scanner._has_cuda", return_value=True)
    @patch("protagine.vector.scanner._cuda_info", return_value=("NVIDIA RTX 4090", 24, "535.0"))
    @patch("protagine.vector.scanner._has_mlx", return_value=False)
    @patch("protagine.vector.scanner._ram_gb", return_value=64)
    ...
    @patch("protagine.vector.scanner._has_cuda", return_value=True)
    @patch("protagine.vector.scanner._cuda_info", return_value=("NVIDIA GB10", 0, "580.142"))
    @patch("protagine.vector.scanner._has_mlx", return_value=False)
    @patch("protagine.vector.scanner._ram_gb", return_value=130)
    ...
    @patch("protagine.vector.scanner._has_cuda", return_value=False)
    @patch("protagine.vector.scanner._has_mlx", return_value=True)
    @patch("protagine.vector.scanner._mlx_info", return_value=("Apple M2 Pro", 32))
    @patch("protagine.vector.scanner._ram_gb", return_value=32)
...
class TestCudaInfo:
    def test_na_vram_fallback(self):
        ...
        from protagine.vector.scanner import _cuda_info
        import subprocess
        ...
        with patch("subprocess.run", return_value=mock_result):
            with patch("protagine.vector.scanner._ram_gb", return_value=130):
