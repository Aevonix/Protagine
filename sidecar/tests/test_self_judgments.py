...
def test_erasing_a_premise_tombstones_the_chain_and_its_entries_without_reviving(judgments, action):
    ...
    else:   # the moved turn is read again under its new contact (integration map X14); nothing revives
        queued = job(state, 'first')
        assert queued['contact_id'] == 'contact-z' and queued['done_at'] is None
...
def test_a_merge_moves_views_about_the_dropped_contact_to_the_kept_one(judgments):
    """Integration map X14: ``reattribute_subject(drop, keep)`` is the merge's hook for person views. A view
    about the dropped contact becomes one about the kept contact, none stays under the dropped id, and a
    view the kept contact already holds on the same topic stays the head."""
    state, _ = judgments
    source(state, 'guest', 'Mira delivered the parts list on time twice.', contact_id='contact-b')
    moved = state.form(proposal(state.admitted_premises('guest'), subject_kind='person', subject='contact-b',
                                topic='delivery reliability', source_ref='turn:guest')).stance_id
    source(state, 'kept', 'Mira was late with the invoice once this month.', contact_id='contact-c')
    kept = state.form(proposal(state.admitted_premises('kept'), subject_kind='person', subject='contact-c',
                               topic='invoice timing', stance='Invoices from them can run late.',
                               source_ref='turn:kept')).stance_id
    source(state, 'same', 'Mira delivered the parts list late last week.', contact_id='contact-b')
    clash = state.form(proposal(state.admitted_premises('same'), subject_kind='person', subject='contact-b',
                                topic='invoice timing', stance='Invoices from them arrive on time.',
                                source_ref='turn:same')).stance_id
    assert moved and kept and clash
    assert state.reattribute_subject('contact-b', 'contact-c') == 1
    assert state.head(subject_kind='person', subject='contact-c', topic='delivery reliability')['id'] == moved
    assert state.head(subject_kind='person', subject='contact-c', topic='invoice timing')['id'] == kept
    assert state.head(subject_kind='person', subject='contact-b', topic='delivery reliability') is None
    assert all(r['subject'] == 'contact-c' for r in state.revisions(history=True, subject_kind='person'))
    assert {r['id'] for r in state.revisions(subject_kind='person')} == {moved, kept}
    assert state.reattribute_subject('contact-b', 'contact-c') == 0


def test_an_identity_correction_queues_the_moved_turns_opinion_pass_under_the_new_contact(judgments):
    """Integration map X14: ``source_attribution.correct`` erases the views resting on a moved turn and
    queues the turn's opinion job again under the new contact, so they are rebuilt, not lost."""
    from protagine.turns.source_attribution import correct
    state, _ = judgments
    source(state, 'guest', 'Mira delivered the parts list on time twice.', contact_id='contact-b')
    with state.ledger._connect() as conn, conn:
        conn.execute("UPDATE opinion_jobs SET done_at=?,disposition='formed' WHERE ref='guest'", (time.time(),))
    formed_id = state.form(proposal(state.admitted_premises('guest'), subject_kind='person', subject='contact-b',
                                    topic='delivery reliability', source_ref='turn:guest')).stance_id
    assert job(state, 'guest')['done_at'] is not None
    correct(state.ledger, operation_id='merge-1', performed_by='owner', old_contact_id='contact-b',
            contact_id='contact-c', source_ids=['guest'], evidence_refs=['source:owner-confirmation'])
    queued = job(state, 'guest')
    assert queued['contact_id'] == 'contact-c' and queued['done_at'] is None and queued['kind'] == 'turn'
    assert state.get(formed_id)['status'] != 'current'
