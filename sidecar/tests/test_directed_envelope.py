...
from protagine.directed.service import report_token_for
...
def test_envelope_unsigned_by_default(monkeypatch):
    monkeypatch.setenv("PROTAGINE_DIRECTED_MODE", "dry_run")
    monkeypatch.delenv("PROTAGINE_DIRECTED_HMAC_KEY", raising=False)
    svc = _service()
...
def test_envelope_signed_when_key_set(monkeypatch):
    monkeypatch.setenv("PROTAGINE_DIRECTED_MODE", "dry_run")
    monkeypatch.setenv("PROTAGINE_DIRECTED_HMAC_KEY", "test-secret")
    svc = _service()
...
def test_dispatch_cap_binds(monkeypatch):
    monkeypatch.setenv("PROTAGINE_DIRECTED_MODE", "dry_run")
    monkeypatch.setenv("PROTAGINE_DIRECTED_MAX_DISPATCH_PER_DAY", "2")
    svc = _service()
...
def test_dispatch_cap_zero_disables(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_DIRECTED_MODE", "dry_run")
    monkeypatch.setenv("PROTAGINE_DIRECTED_MAX_DISPATCH_PER_DAY", "0")
    svc = _service()
