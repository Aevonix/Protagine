...
from protagine.tom.approvals import (
...
def test_ttl_env(monkeypatch):
    monkeypatch.delenv("PROTAGINE_TOM2_APPROVAL_TTL_DAYS", raising=False)
    assert approval_ttl_days() == 30.0
    monkeypatch.setenv("PROTAGINE_TOM2_APPROVAL_TTL_DAYS", "junk")
    assert approval_ttl_days() == 30.0
    monkeypatch.setenv("PROTAGINE_TOM2_APPROVAL_TTL_DAYS", "-5")
