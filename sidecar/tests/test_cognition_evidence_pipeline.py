...
from protagine.events.journal import append_event_record, _checksum
from protagine.execution_results import ExecutionResultV1, bounded_refs
from protagine.projects.event_outbox import ProjectEventProjector
from protagine.projects.engine import ProjectEngine
...
def _isolate(monkeypatch, tmp_path, *, mode="live"):
    ...
    monkeypatch.setenv("PROTAGINE_COGNITION_EVIDENCE", mode)
...
def test_external_evidence_uses_exact_shared_envelope_and_projection_validator(
    ...
    _isolate(monkeypatch, tmp_path)
    monkeypatch.setenv("PROTAGINE_OWNER_PERSON_ID", "owner-person-1")
...
def test_second_replay_error_after_gap_acknowledgement_stays_unhealthy(
    ...
    _isolate(monkeypatch, tmp_path)
    monkeypatch.setenv("PROTAGINE_COGNITION_EVIDENCE_GAP_POLICY", "acknowledge")
    projects = ProjectStore(str(tmp_path / "projects.db"))
    evidence = CognitionEvidenceStore(str(tmp_path / "evidence.db"))
    evidence.initialize_cursor(
