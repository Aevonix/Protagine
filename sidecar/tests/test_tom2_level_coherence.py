...
def _clean(monkeypatch, tmp_path):
    for var in ("PROTAGINE_TOM2_LEVEL", "PROTAGINE_TOM2_MAX_LEVEL",
                "PROTAGINE_TOM2_CROSS_CONTEXT", "PROTAGINE_TOM2_RISK_CAPS",
                "PROTAGINE_GUARD_ENFORCE_CHECKS"):
...
def _seed_evidence(tmp_path, gateway="dm", n=3):
    from protagine.gate.surface_policy import POLICY_ID

    audit = GuardAuditStore(db_path=str(tmp_path / "protagine-guard-audit.db"))
    for _ in range(n):
...
def test_warn_level2_without_max_level(monkeypatch):
    ...
    assert "PROTAGINE_TOM2_MAX_LEVEL" in r.detail
...
def test_warn_level2_without_cross_context(monkeypatch):
    ...
    r = doctor.check_tom2_level_coherence()
    assert r.status == doctor.WARN
    assert "PROTAGINE_TOM2_CROSS_CONTEXT" in r.detail


def test_warn_level2_without_allowlisted_check(monkeypatch):
    _arm_l2_flags(monkeypatch)
    monkeypatch.setenv("PROTAGINE_GUARD_ENFORCE_CHECKS", "secret_leak")
    r = doctor.check_tom2_level_coherence()
    assert r.status == doctor.WARN
...
def test_evaluation_row_recency_never_changes_evidence_authority(
    ...
    _arm_l2_flags(monkeypatch)
    _seed_evidence(tmp_path)
    db = tmp_path / "protagine-guard-audit.db"
...
def test_warn_on_malformed_risk_caps_when_raised(monkeypatch):
    ...
    r = doctor.check_tom2_level_coherence()
    assert r.status == doctor.WARN
    assert "PROTAGINE_TOM2_RISK_CAPS" in r.detail
