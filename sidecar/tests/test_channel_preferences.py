...
def attached(tmp_path, config):
    ...
    config['plugins'] = {'protagine': {'instance_dir': str(state)}}
...
def test_existing_profile_explicit_preference_preserves_scope_and_state(tmp_path, monkeypatch, choice, expected):
    ...
    assert len(list(home.glob('.config.yaml.protagine-backup-*'))) == 1
...
def test_omitted_receipt_preference_retains_config_bytes(tmp_path, monkeypatch):
    ...
    assert not list(home.glob('.config.yaml.protagine-backup-*'))
    ...
    monkeypatch.setenv('PROTAGINE_STATE_DIR', str(following))
    assert dict(os.environ)['PROTAGINE_STATE_DIR'] == str(following)
    assert get_state_dir() == following


def test_receipt_cli_flag_uses_existing_init_parser(tmp_path, monkeypatch):
    import sys
    from protagine import cli, setup
    observed = []
    monkeypatch.setattr(sys, 'argv', ['protagine', 'init', '--whatsapp-read-receipts', 'on',
...
def test_preference_only_cli_never_sets_up_identity_models_or_instance(tmp_path, monkeypatch, preview):
    ...
    argv = ['protagine', 'init', '--hermes-home', str(home), '--preferences-only',
