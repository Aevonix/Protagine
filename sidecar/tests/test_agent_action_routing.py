...
def _isolated_authority(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_APPROVAL_POLICY", "strict")
    monkeypatch.delenv("PROTAGINE_AGENT_SYNC_WORKER_NODE_ID", raising=False)
...
@pytest.mark.parametrize(
    ...
    [
        ("sync", "PROTAGINE_AGENT_SYNC_WORKER_NODE_ID", "sync-node",
         {AGENT_SYNC_ROUTE}),
        ("effect", "PROTAGINE_ACTION_PLANE_WORKER_NODE_ID", "action-node",
         {ACTION_PLANE_ROUTE}),
        ("work_order", "PROTAGINE_ACTION_PLANE_WORKER_NODE_ID", "action-node",
         {WORK_ORDER_ROUTE, ACTION_PLANE_ROUTE}),
        ("hermes", "PROTAGINE_HERMES_RUN_WORKER_NODE_ID", "text-node",
         {HERMES_RUN_ROUTE}),
    ],
)
...
async def test_active_restart_never_rewrites_route_or_owner_drift(
    ...
    try:
        ...
        if variant == "missing_owner":
            ...
            monkeypatch.setenv(
                "PROTAGINE_AGENT_SYNC_WORKER_NODE_ID", "replacement-node"
            )
...
async def test_action_receipt_rejects_stale_or_future_chronology(
    ...
    monkeypatch.setenv("PROTAGINE_ACTION_PLANE_WORKER_NODE_ID", "action-node")
    monkeypatch.setenv("PROTAGINE_ACTION_RECEIPT_CLOCK_SKEW_SECS", "1")
    manager = await _manager(tmp_path)
    try:
        effect = Job(
            job_type=JobType.AGENT_ACTION,
