...
from protagine.task_queue.models import Job
...
def test_selected_process_snapshot_preserves_delivery_stages_and_hides_private_fields(tmp_path, monkeypatch):
    ...
    snapshot = {'schema':'ProtagineWorkSnapshotV1', 'available':True, 'observed_at':1000,
    ...
    monkeypatch.setenv('PROTAGINE_WORKER_STATUS_PATHS', json.dumps({'Delivery':str(path)}))
...
def test_missing_delivery_store_is_partial_and_terminal_download_never_becomes_live(tmp_path, monkeypatch):
    ...
    path.write_text(json.dumps({'updated_at':1000,'ready':True,'state':{'work_snapshot':{
        'schema':'ProtagineWorkSnapshotV1','available':False,'observed_at':1000,
    ...
    monkeypatch.setenv('PROTAGINE_WORKER_STATUS_PATHS', json.dumps({'Delivery':str(path),'Download':str(terminal)}))
