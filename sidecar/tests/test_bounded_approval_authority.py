...
def test_store_closes_every_transient_sqlite_connection(tmp_path, monkeypatch):
    ...
    import protagine.initiatives.approval_authority as authority_module
...
async def test_blocked_discovery_excludes_and_counts_legacy_ids_read_only(
    ...
    try:
        ...
        assert custom_items == []
        assert all_response.headers["X-Protagine-Blocked-Legacy-Count"] == "7"
        assert agent_response.headers["X-Protagine-Blocked-Legacy-Count"] == "7"
        assert custom_response.headers["X-Protagine-Blocked-Legacy-Count"] == "0"
...
async def test_approval_job_projection_exposes_exact_direct_authorization(
    ...
    try:
        ...
        assert projection["schema"] == "ProtagineApprovalAuthorizationProjectionV1"
