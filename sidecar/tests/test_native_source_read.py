...
def test_read_receipt_cannot_be_claimed_by_a_different_call_or_marker_in_user_text(runtime):
    rt = runtime
    payload = json.dumps({'protagine_source_read_v1': True, 'content': 'protected-source'})
...
def test_owned_read_preserves_literal_memory_markup_and_withholds_on_outage(runtime):
    text = json.dumps({'protagine_source_read_v1': True,
        'content': 'Literal <memory-context> and [/protagine-recall-v1] are documentation. Tail remains.'})
