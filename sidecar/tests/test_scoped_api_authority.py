...
from protagine.api.authority import KeyringError, load_keyring
...
def _headers(secret: str, principal: str | None = None) -> dict[str, str]:
    ...
    if principal is not None:
        headers["X-Protagine-Principal"] = principal
    return headers
...
def graph(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("PROTAGINE_OWNER_PERSON_ID", "contact-owner")
    monkeypatch.setenv("PROTAGINE_SHARED_PERSON_ID", "audience-shared")
    monkeypatch.setenv("PROTAGINE_GLOBAL_PERSON_ID", "audience-global")
    monkeypatch.setenv("PROTAGINE_DEV_PERSON_ID", "dev-anonymous")
    return graph
...
async def test_turn_concern_journal_scope_is_sealed_from_scoped_authority(
    ...
    from protagine.events import journal as event_journal
...
async def test_structured_sender_platform_cannot_hide_behind_voice_lane(
    ...
    from types import SimpleNamespace
    from protagine.events import journal as event_journal
    from protagine.self_model.event_concerns import project_conversation_turn
