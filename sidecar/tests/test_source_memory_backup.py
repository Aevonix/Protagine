...
def test_memory_salvage_recovers_owned_bytes_and_current_corrections_scope_only(evidence, tmp_path, lost_original):
    ...
    runtime_files = ('protagine-goals.db', 'protagine-expectations.db', 'contacts.db', 'protagine-action-journal.db')
...
def test_restore_cli_reports_scope_and_requires_explicit_memory_destination(evidence, tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, 'argv', ['protagine', 'restore', '--memory-only', '--input', str(archive),
    ...
    assert "Run 'protagine start'" not in output

    monkeypatch.setenv('PROTAGINE_STATE_DIR', str(tmp_path / 'full-state'))
