...
def test_contact_facing_dismissals_and_notices_and_notes_are_not_the_agents_feelings(world):
    ...
    world.store.transition(lapsed.id, "expired", action="x", outcome="expired", verdict="ignored",
                           cancelled_at=world.now)
    assert world.update()["applied"] == 0 and world.level("affect.dismissed") == 0.0, \
        "an ask the owner let lapse is silence, not a dismissal (integration map X5)"
    for kind, type_ in (("message", "contradiction"), ("message", "check_in"), ("message", "link_proposal")):
        unanswered, _ = world.store.create_intention(kind=kind, type=type_, title="a question", drive="social",
                                                     cls="owner", decision="ask", decision_reason="x", status="asked",
                                                     dedup_key=f"lapsed-{type_}", recipient=OWNER, created_at=world.now)
        world.store.transition(unanswered.id, "expired", action="x", outcome="expired", verdict="ignored",
                               cancelled_at=world.now)
    assert world.update()["applied"] == 0 and world.level("affect.dismissed") == 0.0, \
        "an unanswered contradiction question or check-in confirmation raises no satiation"
...
def test_load_counts_near_owed_obligations_running_work_failures_and_asks(world):
    ...
    # Only asks about work load the mind: owner questions about check-ins, links and contradictions are its
    # reporting, and counting them would postpone the very check-ins they ask about (integration map X5).
    for kind, type_ in (("message", "check_in"), ("message", "contradiction"), ("message", "link_proposal"),
                        ("message", "cadence_confirm")):
        world.store.create_intention(kind=kind, type=type_, title=type_, drive="social", cls="owner",
                                     decision="ask", decision_reason="x", status="asked", dedup_key=f"asked-{type_}",
                                     recipient=OWNER, created_at=world.now)
    assert world.update()["load"] == pytest.approx(0.5)
