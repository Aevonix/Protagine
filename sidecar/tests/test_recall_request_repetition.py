...
from protagine.turns.idempotency import TurnIdempotencyLedger, source_message_hash
...
async def test_exact_request_is_supplementary_across_rerank_paths(monkeypatch, mode):
    ...
    monkeypatch.setenv("PROTAGINE_RECALL_RERANK", "on" if mode == "failed" else mode)
