...
def _load_plugin():
    name = "protagine_hermes_plugin_guard_mode_test"
...
def test_mode_defaults_and_legacy_shadow_inheritance(monkeypatch):
    ...
    monkeypatch.delenv("PROTAGINE_GUARD_CHAT_SHADOW", raising=False)
    assert module._guard_chat_mode() == "off"
    monkeypatch.setenv("PROTAGINE_GUARD_CHAT_SHADOW", "1")
...
def test_invalid_explicit_mode_uses_legacy_default(monkeypatch):
    module = _load_plugin()
    monkeypatch.setenv("PROTAGINE_GUARD_CHAT_MODE", "invalid")
    monkeypatch.delenv("PROTAGINE_GUARD_CHAT_SHADOW", raising=False)
    assert module._guard_chat_mode() == "off"
    monkeypatch.setenv("PROTAGINE_GUARD_CHAT_SHADOW", "yes")
    assert module._guard_chat_mode() == "shadow"
