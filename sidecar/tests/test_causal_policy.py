...
causal subset; causal_edges_actionable() is OFF unless PROTAGINE_CAUSAL_ACT=1
(query-only invariant holds by default); is_causal() classifies exactly the
causal subset.
"""

from protagine.world_model.causal_policy import (
    causal_edges_actionable, is_causal,
)
from protagine.world_model.constants import (
...
def test_actionable_defaults_off(monkeypatch):
    ...
    monkeypatch.delenv("PROTAGINE_CAUSAL_ACT", raising=False)
    assert causal_edges_actionable() is False
    for v in ("0", "false", "no", "off", "banana", ""):
        monkeypatch.setenv("PROTAGINE_CAUSAL_ACT", v)
