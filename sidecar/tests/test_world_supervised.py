...
Only when "world_model" is enrolled in PROTAGINE_SUPERVISED_LIVE_DOMAINS does
...
at PROTAGINE_WORLD_SUPERVISED_MAX_WRITES per run, never creates edges, never
...
def test_effective_mode_untouched_when_domain_not_enrolled(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_WORLD_LLM_EXTRACT", "shadow")
...
def test_effective_mode_supervised_when_enrolled_at_ask_first(monkeypatch):
    monkeypatch.setenv("PROTAGINE_WORLD_LLM_EXTRACT", "shadow")
    monkeypatch.setenv("PROTAGINE_SUPERVISED_LIVE_DOMAINS", "world_model")
...
def test_env_live_and_off_remain_owner_overrides(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_WORLD_LLM_EXTRACT", "off")
...
async def test_supervised_entity_upsert_and_alias_merge(monkeypatch):
    monkeypatch.delenv("PROTAGINE_WORLD_SUPERVISED_MAX_WRITES", raising=False)
    world = FakeWorld()
...
async def test_supervised_write_cap(monkeypatch):
    monkeypatch.setenv("PROTAGINE_WORLD_SUPERVISED_MAX_WRITES", "2")
