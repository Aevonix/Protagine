...
from protagine.directives.extractor import extract_directives, is_revocation
...
def test_extract_prohibition_dont_touch_repo():
    d = extract_directives("From now on, don't touch the protagine-web repo, it is a side project")
    assert len(d) == 1
    assert d[0].polarity == Polarity.PROHIBIT
    assert "protagine-web" in d[0].match_terms
...
def test_read_only_adjective_does_not_escalate_boundary_to_observe():
    from protagine.directives.models import Level

    goal = extract_directives(
        "Goal: verify Protagine cognition is live with a read-only internal "
...
def test_extract_revocation():
    d = extract_directives("actually you can go ahead and work on protagine-web again")
...
def test_store_add_list_active_revoke():
    ...
    d = Directive(subject="repo protagine-web", polarity=Polarity.PROHIBIT)
    s.add(d)
    assert s.count_active() == 1
    assert s.get(d.id).subject == "repo protagine-web"
...
def test_guard_blocks_specific_subject():
    g = _guard_with(Directive(subject="protagine-web repo", polarity=Polarity.PROHIBIT,
                              raw_text="don't touch protagine-web"))
    v = g.check(Action(kind="directed_action", text="clone and refactor the protagine-web repo",
                       high_risk=True))
    assert v.allowed is False
    assert "protagine-web" in v.reason
...
def test_guard_records_recent_blocks():
    g = _guard_with(Directive(subject="protagine-web", polarity=Polarity.PROHIBIT,
                              raw_text="don't touch protagine-web"))
    assert g.check(Action(kind="directed_action", text="refactor protagine-web")).allowed is False
    blocks = g.recent_blocks()
    assert len(blocks) == 1
    assert "protagine-web" in blocks[0]["subjects"]
...
def test_guard_context_brief():
    g = _guard_with(
        Directive(subject="protagine-web", polarity=Polarity.PROHIBIT, raw_text="don't touch protagine-web"),
        Directive(subject="check before deploy", polarity=Polarity.REQUIRE, raw_text="always check before deploy"),
    )
    brief = g.context_brief()
    assert "MUST NOT" in brief and "protagine-web" in brief
...
def test_manager_capture_then_enforce():
    m = DirectiveManager(DirectiveStore(db_path=None))
    m.capture_from_message("From now on, Don't touch the protagine-web repo")
    assert m.store.count_active() == 1
    # an autonomous action on protagine-web is now refused
    v = m.check(Action(kind="directed_action", text="open a PR in protagine-web", high_risk=True))
    assert v.allowed is False
...
def _mgr():
    from protagine.directives.service import DirectiveManager
    from protagine.directives.store import DirectiveStore
...
def test_anaphora_fragment_subjects_are_refused():
    ...
    for msg in (
        "please stop that and wipe it from protagine",
        "don't attempt them",
...
def test_common_term_fragment_cannot_match_everything():
    from protagine.directives.guard import _terms_match
    ...
    # ("wipe", "protagine") must not bind an unrelated internal job text.
    directive_terms = normalize_terms("that and wipe it from protagine")
    action_terms = normalize_terms(
        "Observe the system domain through your own connections and report "
        "snapshots to Protagine")
