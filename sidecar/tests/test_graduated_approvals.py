"""Graduated approval policy v0.18.0 — PROTAGINE_APPROVAL_POLICY=graduated.
...
from protagine.initiatives.approval_policy import is_authorized_target
...
def _isolated_env(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("PROTAGINE_APPROVAL_POLICY", raising=False)
    monkeypatch.delenv("PROTAGINE_AGENT_AUTO_APPROVE", raising=False)
...
def test_default_policy_is_strict(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_APPROVAL_POLICY", "yolo")
    assert get_approval_policy() == "strict"
...
def test_standing_approvals_persist_across_reload(tmp_path):
    ...
    # On disk, under $PROTAGINE_STATE_DIR
