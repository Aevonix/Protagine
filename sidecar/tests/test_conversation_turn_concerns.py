...
def turn_env(monkeypatch):
    ...
    monkeypatch.delenv("PROTAGINE_TURN_CONCERNS", raising=False)
    monkeypatch.delenv("PROTAGINE_TURN_CONCERNS_CHANNELS", raising=False)
    monkeypatch.delenv(
        "PROTAGINE_TURN_CONCERNS_EXCLUDED_SESSION_PREFIXES", raising=False,
    )
    monkeypatch.delenv(
        "PROTAGINE_TURN_CONCERNS_EXCLUDED_PLATFORMS", raising=False,
    )
    monkeypatch.delenv("PROTAGINE_TURN_CONCERNS_BOOTSTRAP", raising=False)
    monkeypatch.delenv("PROTAGINE_TURN_CONCERNS_GAP_POLICY", raising=False)
...
def test_flag_defaults_and_invalid_values_off_with_exact_noop(
    ...
    monkeypatch.setenv("PROTAGINE_TURN_CONCERNS", "unexpected")
    assert turn_concern_mode() == "off"
    assert bridge.run_once() == {"enabled": False, "processed": 0}
    assert store.event_cursor(bridge.consumer_id) is None
...
def test_explicitly_allowed_owner_and_subject_channels_preserve_scope(
    ...
    monkeypatch.setenv("PROTAGINE_TURN_CONCERNS_CHANNELS", "voice,intercom")
    monkeypatch.setenv("PROTAGINE_TURN_CONCERNS_BOOTSTRAP", "replay")
    journal = FakeJournal([
        turn_event(1),
        turn_event(
            2,
...
def test_internal_lanes_are_structurally_excluded(monkeypatch, channel_id):
    monkeypatch.setenv("PROTAGINE_TURN_CONCERNS_CHANNELS", "internal,cron,api")
...
def test_generic_whatsapp_like_lane_is_allowed(monkeypatch):
    monkeypatch.setenv("PROTAGINE_TURN_CONCERNS_CHANNELS", "whatsapp-like")
...
def test_configured_source_platform_exclusion_is_separate_from_lane(
    ...
    monkeypatch.setenv("PROTAGINE_TURN_CONCERNS_CHANNELS", "voice")
    monkeypatch.setenv(
        "PROTAGINE_TURN_CONCERNS_EXCLUDED_PLATFORMS", "rcs,operator",
    )
...
def test_non_allowlisted_channel_is_not_projected(monkeypatch):
    monkeypatch.setenv("PROTAGINE_TURN_CONCERNS_CHANNELS", "intercom")
...
def test_empty_or_malformed_allowlist_fails_closed(monkeypatch, allowlist):
    monkeypatch.setenv("PROTAGINE_TURN_CONCERNS_CHANNELS", allowlist)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "PROTAGINE_TURN_CONCERNS_CHANNELS", "",
            "turn_concern_channels_config_invalid",
        ),
        (
            "PROTAGINE_TURN_CONCERNS_EXCLUDED_SESSION_PREFIXES", "bad prefix",
            "turn_concern_session_prefix_config_invalid",
        ),
        (
            "PROTAGINE_TURN_CONCERNS_EXCLUDED_PLATFORMS", "rcs,*",
...
def test_explicit_session_prefix_excludes_deck_duplicate(monkeypatch):
    monkeypatch.setenv("PROTAGINE_TURN_CONCERNS_CHANNELS", "voice")
    monkeypatch.setenv(
        "PROTAGINE_TURN_CONCERNS_EXCLUDED_SESSION_PREFIXES", "buildbox-",
    )
...
def test_malformed_session_prefix_config_fails_closed(monkeypatch, prefixes):
    monkeypatch.setenv("PROTAGINE_TURN_CONCERNS_CHANNELS", "voice")
    monkeypatch.setenv(
        "PROTAGINE_TURN_CONCERNS_EXCLUDED_SESSION_PREFIXES", prefixes,
    )
...
async def test_boundary_mode_flip_holds_before_shadow_mutation_and_resumes(
    ...
    monkeypatch.setenv("PROTAGINE_PROJECTS_MODE", mode)
    store = ProjectStore()
    turn_live = {"value": True}
...
async def test_initiative_capacity_excludes_only_exact_turn_mode_hold(
    ...
    monkeypatch.setenv("PROTAGINE_PROJECTS_MAX_CONCURRENT", "1")
...
def test_shadow_turns_do_not_consume_live_workspace_capacity(
    ...
    monkeypatch.setenv("PROTAGINE_TURN_CONCERNS_CHANNELS", "voice")
    monkeypatch.setenv("PROTAGINE_TURN_CONCERNS_BOOTSTRAP", "replay")
    monkeypatch.setenv("PROTAGINE_WORKSPACE_CAPACITY", "2")
    monkeypatch.setenv("PROTAGINE_WORKSPACE_EVICT_FLOOR", "0")
    journal = FakeJournal([
        turn_event(
            index,
