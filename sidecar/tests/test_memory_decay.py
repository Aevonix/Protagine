...
- fact/semantic memories get their own half-life ONLY when
  PROTAGINE_DECAY_HALF_LIFE_SEMANTIC_DAYS is set (defaults to episodic);
...
class TestComputeDecayFactor:
    def test_episodic_default_formula(self):
        got = client_mod.ProtagineGraph._compute_decay_factor(
...
async def test_default_lambdas_match_legacy_seven_days(monkeypatch):
    ...
    monkeypatch.delenv("PROTAGINE_DECAY_HALF_LIFE_DAYS", raising=False)
    monkeypatch.delenv("PROTAGINE_DECAY_HALF_LIFE_SEMANTIC_DAYS", raising=False)
    fx = _DecayFixture()
...
async def test_env_half_lives_respected(monkeypatch):
    monkeypatch.setenv("PROTAGINE_DECAY_HALF_LIFE_DAYS", "14")
    monkeypatch.setenv("PROTAGINE_DECAY_HALF_LIFE_SEMANTIC_DAYS", "60")
    fx = _DecayFixture()
    await fx.graph.decay_memories()
...
async def test_semantic_env_defaults_to_episodic_env(monkeypatch):
    monkeypatch.setenv("PROTAGINE_DECAY_HALF_LIFE_DAYS", "21")
    monkeypatch.delenv("PROTAGINE_DECAY_HALF_LIFE_SEMANTIC_DAYS", raising=False)
    fx = _DecayFixture()
    await fx.graph.decay_memories()
    _, params = fx.decay_params
...
async def test_cypher_and_unit_math_agree(monkeypatch):
    ...
    monkeypatch.delenv("PROTAGINE_DECAY_HALF_LIFE_DAYS", raising=False)
    monkeypatch.setenv("PROTAGINE_DECAY_HALF_LIFE_SEMANTIC_DAYS", "30")
    fx = _DecayFixture()
    await fx.graph.decay_memories()
    _, params = fx.decay_params
    unit = client_mod.ProtagineGraph._compute_decay_factor(
...
async def test_stale_data_gap_only_proposes_without_decaying_memories():
    ...
    from protagine.intelligence.cognition.strategy_adjuster import StrategyAdjuster, AdjustmentStatus
