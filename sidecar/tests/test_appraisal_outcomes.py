...
def test_schema_requires_a_bounded_outcomes_array_for_strict_routers():
    ...
    assert schema['required'] == ['observations', 'incident_decisions', 'outcomes', 'contact']
...
def test_prompt_counts_occurrences_not_restatements():
    ...
    assert 'observations, incident_decisions, outcomes and contact' in system
...
@pytest.mark.asyncio
async def test_a_merge_rebuilds_the_moved_sources_appraisals_under_the_kept_contact(state):
    """Integration map X14: the identity correction a merge or link runs deletes what was derived under
    the dropped contact and queues the moved sources again, so the kept contact ends up with the
    appraisals rebuilt and none remain under the dropped id."""
    from protagine.turns.source_attribution import correct
    source(state, 'first', 'The export has failed again.', contact='dropped')
    await state.process_one(Processor())
    assert [r['subject_id'] for r in state.view('dropped', viewer_contact_id='owner')['records']] == ['dropped']
    correct(state.ledger, operation_id='merge-1', performed_by='owner', old_contact_id='dropped',
            contact_id='kept', source_ids=['first'], evidence_refs=['source:owner-confirmation'])
    assert state.view('dropped', viewer_contact_id='owner')['records'] == []
    assert (run_of(state, 'first')['status'], run_of(state, 'first')['disposition']) == ('pending', 'reattributed')
    processor = Processor()
    assert await state.process_one(processor) is True
    assert len(processor.requests) == 1
    rebuilt = state.view('kept', viewer_contact_id='owner')['records']
    assert len(rebuilt) == 1 and rebuilt[0]['subject_id'] == 'kept'
    assert state.view('dropped', viewer_contact_id='owner')['records'] == []
    assert await state.process_one(Processor()) is False


@pytest.mark.asyncio
async def test_one_answer_carries_outcomes_for_the_owner_and_the_contact_signal_for_a_contact(state):
    """Integration map X2: M5's contact signal and M6's outcomes share one call. Outcomes are the owner's
    only; the contact signal is for a non-owner speaker only, written after the commit."""
    seen = []
    ...
    def both(payload):
        return {**answer(reported)(payload), 'contact': {'their_valence': -0.6, 'opt_out': False}}
    owner_turn(state, 'owner-report', 'The archive export gave stale figures again.')
    await state.process_one(Processor(both))
    assert [r['turn_id'] for r in outcomes_of(state)] == ['owner-report'] and seen == []
    source(state, 'contact-report', 'The archive export gave me stale figures too.', contact='person')
    await state.process_one(Processor(both))
    assert [r['turn_id'] for r in outcomes_of(state)] == ['owner-report']
    assert [(s['contact_id'], s['their_valence']) for s in seen] == [('person', -0.6)]
    assert run_of(state, 'contact-report')['status'] == 'complete'
