...
assume the action is permitted. PROTAGINE_BOUNDARY_FAIL_CLOSED defaults to
...
def test_flag_false_restores_legacy(monkeypatch):
    monkeypatch.setenv("PROTAGINE_BOUNDARY_FAIL_CLOSED", "false")
...
def test_delivery_boundary_error_refuses_by_default(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="protagine.autonomy.loop"):
...
def test_directed_boundary_error_refuses_by_default(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="protagine.directed.service"):
