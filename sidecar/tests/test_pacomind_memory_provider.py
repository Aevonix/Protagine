"""Unit harness for the PacoMind memory provider.
...
_PROVIDER_PATH = (pathlib.Path(__file__).resolve().parents[2]
                  / "plugins" / "pacomind-memory" / "provider.py")


def _load_provider_module():
    spec = importlib.util.spec_from_file_location(
        "pacomind_memory_provider_under_test", _PROVIDER_PATH)
...
def _privacy_posture(monkeypatch):
    monkeypatch.setenv("PACOMIND_PREFETCH_QUERY_CHECK", "1")
    monkeypatch.setenv("PACOMIND_PREFETCH_TURN_CONTACT", "1")
    monkeypatch.setenv(
...
def _make_provider(provider_mod, fake_httpx, monkeypatch):
    ...
    p = provider_mod.PacoMindMemoryProvider(config={
...
def test_native_setup_settings_survive_restart_and_profiles_stay_separate(
    ...
    monkeypatch.setattr(provider_mod, "_profile_env", lambda name, home:
                        f"test-key-{home.name}" if name == "PACOMIND_API_KEY" else "")
    ...
    writer = provider_mod.PacoMindMemoryProvider(config={})
    providers = []
    for index, home in enumerate(homes):
        ...
        assert "must-not-be-saved" not in (home / "pacomind-memory.json").read_text()
        assert (home / "pacomind-memory.json").stat().st_mode & 0o777 == 0o600
        active = home
        provider = provider_mod.PacoMindMemoryProvider()
...
def test_invalid_saved_profile_does_not_fall_back_to_another_instance(
    ...
    (tmp_path / "pacomind-memory.json").write_text('{"url": "private-broken-value"')
    with pytest.raises(ValueError, match="invalid PacoMind memory configuration") as error:
        provider_mod.PacoMindMemoryProvider()
...
def test_claim_tool_starts_job_before_returning_it_to_model(
    ...
    monkeypatch.setenv("PACOMIND_MEMORY_WORKER_NODE_ID", "fixed-tool-worker")
    monkeypatch.setenv(
    ...
    provider = _make_provider(provider_mod, fake, monkeypatch)
    result = provider._tool_pacomind_claim_task({
    ...
    schema = next(
        ...
        if item["name"] == "pacomind_claim_task"
    )
...
def test_standalone_queue_tools_require_explicit_opt_in(
        provider_mod, monkeypatch):
    monkeypatch.delenv("PACOMIND_GENERAL_PLUGIN_ACTIVE", raising=False)
    monkeypatch.delenv("PACOMIND_MEMORY_WORKER_TOOLS", raising=False)
    fake = _FakeHttpx(routes={
        _QUEUE_CLAIM: {"job_id": "job-tool", "payload": {}},
        _QUEUE_START: {"success": True},
    })
    ...
    assert "pacomind_claim_task" not in names
    denied = json.loads(provider._tool_pacomind_claim_task({
        "worker_id": "tool-worker",
    }))
    assert denied["error"] == "pacomind worker tools are disabled"
    assert fake.requests == []

    monkeypatch.setenv("PACOMIND_MEMORY_WORKER_TOOLS", "1")
    assert "pacomind_claim_task" in {
...
def test_global_claim_kill_switch_overrides_standalone_provider_opt_in(
    ...
    assert "pacomind_claim_task" not in {
        schema["name"] for schema in provider.get_tool_schemas()
    }
    denied = json.loads(provider._tool_pacomind_claim_task({}))
...
def test_general_plugin_reduces_memory_provider_to_read_context_tools(
    ...
    assert "pacomind_approve_initiative" not in names
    ...
    dispatched = json.loads(provider.handle_tool_call(
        "pacomind_claim_task", {"worker_id": "tool-worker"},
    ))
    direct = json.loads(provider._tool_pacomind_claim_task({
        "worker_id": "tool-worker",
    }))
    approval = json.loads(provider._tool_pacomind_approve_initiative({
        "initiative_id": "initiative-1",
    }))
    assert dispatched["error"] == "PacoMind tool is not available in this mode"
    assert direct["error"] == "pacomind worker tools are disabled"
...
def test_approval_tool_is_never_model_visible(provider_mod, monkeypatch):
    ...
    assert "pacomind_approve_initiative" not in {
        schema["name"] for schema in provider.get_tool_schemas()
    }
...
def test_selected_profile_owns_memory_without_launcher_flags(provider_mod, monkeypatch, tmp_path):
    ...
    (tmp_path / "config.yaml").write_text(json.dumps({
        "plugins": {"enabled": ["pacomind"]}, "memory": {"provider": "pacomind-memory"}}))
    # Inherited standalone flags must not reinstate a second writer or tools.
    monkeypatch.setenv("PACOMIND_GENERAL_PLUGIN_ACTIVE", "0")
    monkeypatch.setenv("PACOMIND_MEMORY_WORKER_TOOLS", "1")
    ...
    assert json.loads(p._tool_pacomind_claim_task({}))["error"] == "pacomind worker tools are disabled"
    assert fake.requests == []


@pytest.mark.parametrize("plugins", [{"enabled": []}, {"enabled": ["pacomind"], "disabled": ["pacomind"]}])
def test_profile_deselection_and_existing_instance_are_isolated(provider_mod, monkeypatch, tmp_path, plugins):
    selected = tmp_path / "selected"
    root = tmp_path / "root"
    for home, selection in ((selected, {"enabled": ["pacomind"]}), (root, plugins)):
        home.mkdir()
        (home / "config.yaml").write_text(json.dumps({
            "plugins": selection, "memory": {"provider": "pacomind-memory"}}))
    active = selected
    monkeypatch.setattr(provider_mod, "_active_hermes_home", lambda: active)
    monkeypatch.setenv("PACOMIND_GENERAL_PLUGIN_ACTIVE", "1")
    monkeypatch.delenv("PACOMIND_MEMORY_TURN_WRITER", raising=False)
    ...
    for provider in (owned, standalone):
        ...
        assert not {"pacomind_write_memory", "pacomind_search_memory"} & names
...
def test_profile_ownership_honors_native_json_and_explicit_writer_precedence(provider_mod, monkeypatch, tmp_path):
    ...
    with pytest.raises(ValueError, match="owns memory"):
        provider_mod.PacoMindMemoryProvider()
    native = tmp_path / "pacomind-memory.json"
    native.write_text(json.dumps({"turn_writer": "disabled"}))
    assert provider_mod.PacoMindMemoryProvider()._turn_writer_enabled() is False
    native.write_text(json.dumps({"turn_writer": "enabled"}))
    with pytest.raises(ValueError, match="owns memory"):
        provider_mod.PacoMindMemoryProvider()
    assert provider_mod.PacoMindMemoryProvider(config={"turn_writer": "disabled"})._turn_writer_enabled() is False
    with pytest.raises(ValueError, match="owns memory"):
        provider_mod.PacoMindMemoryProvider(config={"turn_writer": "enabled"})
...
def test_general_plugin_handoff_never_advertises_hidden_memory_write_tool(
    ...
    assert "canonical PacoMind turn writer" in block
    assert "pacomind_write_memory" not in block
    assert set(re.findall(r"\bpacomind_[a-z_]+\b", block)) <= visible
...
def test_standalone_handoff_does_not_advertise_retired_graph_tools(
    ...
    assert not {"pacomind_write_memory", "pacomind_search_memory"} & visible
    assert "pacomind_write_memory" not in block
...
def test_native_file_edits_never_become_new_owner_evidence(
    ...
    monkeypatch.setenv("PACOMIND_GENERAL_PLUGIN_ACTIVE", "1" if general_active else "0")
    monkeypatch.setenv("PACOMIND_MEMORY_TURN_WRITER", "enabled")
    fake = _FakeHttpx()
    provider = _make_provider(provider_mod, fake, monkeypatch)
    ...
    for name, args in (("pacomind_write_memory", {"content": "edited interpretation"}),
                       ("pacomind_search_memory", {"query": "interpretation"})):
...
def test_prefetch_always_rejects_cache_for_a_different_query(
    ...
    monkeypatch.delenv("PACOMIND_PREFETCH_QUERY_CHECK", raising=False)
    fake = _FakeHttpx(routes={
...
def test_prefetch_internal_owner_lane_requires_explicit_attestation(
        provider_mod, monkeypatch):
    monkeypatch.delenv("PACOMIND_PREFETCH_TURN_CONTACT", raising=False)
...
@pytest.mark.parametrize(
    "flag",
    ["PACOMIND_PREFETCH_QUERY_CHECK", "PACOMIND_PREFETCH_TURN_CONTACT"],
)
def test_mandatory_privacy_flags_cannot_be_disabled(
    ...
    with pytest.raises(RuntimeError, match=f"{flag} is mandatory"):
        provider_mod.PacoMindMemoryProvider(config={
...
def test_guest_read_tools_never_call_legacy_unprojected_endpoints(
    ...
    denied = json.loads(provider.handle_tool_call(
        "pacomind_check_commitments", {"contact_id": "cid-bob"},
    ))
    ...
    assert "pacomind_search_memory" not in names
    assert "pacomind_list_pending_tasks" not in names
...
def test_catalog_attests_read_only_prompt_and_provider_privacy(
    ...
    assert set(catalog["model_visible_tool_names"]) == {
        "pacomind_check_commitments", "pacomind_get_affect",
        "pacomind_get_facts", "pacomind_timeline",
    }
    assert all(name in prompt for name in catalog["model_visible_tool_names"])
    assert "pacomind_write_memory" not in prompt
...
def test_canonical_projection_never_weakens_exact_guest_boundary(provider_mod, change):
    ...
    assert provider_mod.PacoMindMemoryProvider._projection_attestation_valid(
        projection, contact_id="cid-guest", require_scoped=True)
    assert not provider_mod.PacoMindMemoryProvider._projection_attestation_valid(
...
def test_profile_aliases_override_inherited_names_without_mutating_environment(
    ...
    monkeypatch.setenv('PACOMIND_API_KEY', 'inherited-disposable-key')
    monkeypatch.setenv('PACOMIND_API_KEY', 'other-inherited-key')
    profile = tmp_path / 'profile'
    profile.mkdir()
    (profile / '.env').write_text('PACOMIND_API_KEY=profile-disposable-key\n')
    before = dict(provider_mod.os.environ)
    assert provider_mod._profile_env('PACOMIND_API_KEY', profile) == 'profile-disposable-key'
    assert provider_mod._profile_env('PACOMIND_API_KEY', profile) == 'profile-disposable-key'
    assert dict(provider_mod.os.environ) == before
    (profile / '.env').write_text('PACOMIND_API_KEY=legacy-profile-key\n')
    assert provider_mod._profile_env('PACOMIND_API_KEY', profile) == 'legacy-profile-key'
    assert dict(provider_mod.os.environ) == before
...
def test_native_setup_updates_existing_secret_name_without_adding_an_alias(
    ...
    pytest.importorskip('hermes_cli.config')
    monkeypatch.setattr(provider_mod, '_active_hermes_home', lambda: tmp_path)
    (tmp_path / '.env').write_text('PACOMIND_API_KEY=existing-disposable-key\n')
    provider = provider_mod.PacoMindMemoryProvider()
    secret = next(row for row in provider.get_config_schema() if row['key'] == 'api_key')
    assert secret['env_var'] == 'PACOMIND_API_KEY'
    (tmp_path / '.env').write_text('PACOMIND_API_KEY=current-disposable-key\n')
    provider = provider_mod.PacoMindMemoryProvider()
    secret = next(row for row in provider.get_config_schema() if row['key'] == 'api_key')
    assert secret['env_var'] == 'PACOMIND_API_KEY'
