...
def test_factory_disabled_by_default(monkeypatch):
    monkeypatch.delenv("PROTAGINE_EXECUTOR_ENABLED", raising=False)
...
def test_factory_custom_types(monkeypatch):
    monkeypatch.setenv("PROTAGINE_EXECUTOR_ENABLED", "true")
    monkeypatch.setenv("PROTAGINE_EXECUTOR_TYPES", "follow_up,commitment")
    svc = create_from_env(
        initiative_store=FakeStore(),
        reasoning_loop=FakeReasoningLoop(),
    )
...
def test_factory_custom_config(monkeypatch):
    monkeypatch.setenv("PROTAGINE_EXECUTOR_ENABLED", "true")
    monkeypatch.setenv("PROTAGINE_EXECUTOR_CYCLE_SECS", "15")
    monkeypatch.setenv("PROTAGINE_EXECUTOR_MAX_PER_CYCLE", "3")
    monkeypatch.setenv("PROTAGINE_EXECUTOR_MODEL_TIER", "medium")
    monkeypatch.setenv("PROTAGINE_EXECUTOR_AGENT_ID", "my-executor")
    svc = create_from_env(
        initiative_store=FakeStore(),
        reasoning_loop=FakeReasoningLoop(),
    )
...
async def test_execute_one_refuses_boundary_violation():
    ...
    dm.capture_from_message("From now on, Don't touch the protagine-web repo")
    store = FakeStore([FakeInitiative()])
    reasoning = FakeReasoningLoop()
    svc = InitiativeExecutorService(
        initiative_store=store,
    ...
    init = FakeInitiative(
        description="clone and refactor the protagine-web repo",
        entity_id="protagine-web",
    )
    await svc._execute_one(init)
...
async def test_execute_one_fails_closed_when_boundary_check_raises(monkeypatch):
    ...
    monkeypatch.delenv("PROTAGINE_BOUNDARY_FAIL_CLOSED", raising=False)
