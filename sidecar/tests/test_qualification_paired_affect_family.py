...
def test_gate_arms_are_built_in_one_faculty_flag_apart():
    """full and full-affect are built-in profiles; no mechanism arm is declared until M6 adds the
    rule table and its flag."""
    from protagine.qualification import native_memory_worker as worker, paired_worker
    profiles = paired.validate_profiles({})
    ...
    assert 'full-affect-plus-rules' not in profiles
