...
from protagine.cognition.prompt import build_cognition_prompt, COGNITION_SYSTEM_PROMPT
from protagine.cognition.trigger import trigger_cognition, _cognition_enabled, _cognition_model
...
class TestCognitionTrigger:
    ...
    async def test_disabled_returns_not_accepted(self):
        with patch.dict(os.environ, {"PROTAGINE_COGNITION_ENABLED": "false"}):
    ...
    async def test_no_model_returns_not_accepted(self):
        with patch.dict(os.environ, {"PROTAGINE_COGNITION_ENABLED": "true", "PROTAGINE_COGNITION_MODEL": ""}):
            result = await trigger_cognition("turn_sync", {"conversation_text": "test"})
            assert result["accepted"] is False
    ...
    async def test_enabled_with_model_accepts(self):
        with patch.dict(os.environ, {
            ...
            "PROTAGINE_COGNITION_MODEL": "gemma-4-31b",
            "PROTAGINE_COGNITION_THROTTLE_SECONDS": "0",
        }):
    ...
    async def test_high_priority_bypasses_throttle(self):
        import protagine.cognition.trigger as trig
        ...
        with patch.dict(os.environ, {
            "PROTAGINE_COGNITION_ENABLED": "true",
            "PROTAGINE_COGNITION_MODEL": "gemma-4-31b",
            "PROTAGINE_COGNITION_THROTTLE_SECONDS": "300",
        }):
...
class TestCognitionConfig:
    def test_enabled_default_false(self):
        with patch.dict(os.environ, {}, clear=True):
            ...
            os.environ.pop("PROTAGINE_COGNITION_ENABLED", None)
            assert _cognition_enabled() is False

    def test_enabled_true(self):
        with patch.dict(os.environ, {"PROTAGINE_COGNITION_ENABLED": "true"}):
            assert _cognition_enabled() is True

    def test_model_unset(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("PROTAGINE_COGNITION_MODEL", None)
            assert _cognition_model() is None

    def test_model_set(self):
        with patch.dict(os.environ, {"PROTAGINE_COGNITION_MODEL": "gpt-4o-mini"}):
