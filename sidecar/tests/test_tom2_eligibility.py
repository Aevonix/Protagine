...
from protagine.tom.tom2 import Tom2Store, render_inference_for_contact
...
def world(monkeypatch):
    ...
    monkeypatch.delenv("PROTAGINE_TOM2_L2_APPROVAL", raising=False)
    monkeypatch.delenv("PROTAGINE_TOM2_MUTUAL_WINDOW_DAYS", raising=False)
    return World()
...
def test_approval_mode_default_required(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_TOM2_L2_APPROVAL", "off")
    assert l2_approval_mode() == "off"
    monkeypatch.setenv("PROTAGINE_TOM2_L2_APPROVAL", "banana")
...
def test_mutual_window_default(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_TOM2_MUTUAL_WINDOW_DAYS", "junk")
    assert mutual_window_days() == 30.0
...
async def test_ref_visibility_delegates_to_h35(world, monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_TOM2_CROSS_CONTEXT", "0")
    d = await world.evaluate()
