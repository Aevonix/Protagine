...
def _request(
    *,
    ...
    depth: str = "quick",
    tool_name: str = "protagine_research",
) -> dict:
    args = {"topic": topic, "depth": depth} if tool_name == "protagine_research" else {}
    approval = {
...
async def test_research_topic_bound_is_lossless_at_1400_and_rejects_1401(
    ...
    service = GovernedActionService(
        ...
        ProtagineSubsystemActionExecutor(projects=engine),
        clock=lambda: NOW,
    )
...
async def test_governed_research_is_a_fast_durable_project_enqueue(tmp_path, monkeypatch):
    engine = _engine(tmp_path / "projects.db", monkeypatch)
    executor = ProtagineSubsystemActionExecutor(projects=engine)
...
async def test_governed_project_replay_restart_mismatch_and_new_action(
    ...
    restarted_executor = ProtagineSubsystemActionExecutor(projects=restarted)
...
async def test_exact_insert_refuses_every_same_row_authority_mismatch(
    ...
    engine = _engine(tmp_path / "projects.db", monkeypatch)
    effect = await ProtagineSubsystemActionExecutor(projects=engine).perform(
        _request(), OWNER
    )
    project = engine.store.get_project(effect["effect_id"])
...
async def test_governed_capabilities_narrow_initial_plan_replan_and_dispatch(
    ...
    monkeypatch.setattr(
        "protagine.projects.planner.plan_project", directed_plan
    )
...
async def test_autonomy_results_are_prompt_and_match_observed_state():
    ...
    disable_request = _request(tool_name="protagine_autonomy_disable")
    started = time.monotonic()
    ...
    enable_request = _request(tool_name="protagine_autonomy_enable")
