...
The regression lock here is that with PROTAGINE_RECALL_OVERSAMPLE unset (or 1)
...
class RecallFixture:
    """ProtagineGraph wired to a fake ANN store + fake Neo4j hydration."""
...
async def test_default_fetch_uses_exact_limit(monkeypatch):
    monkeypatch.delenv("PROTAGINE_RECALL_OVERSAMPLE", raising=False)
    fx = RecallFixture(
...
async def test_default_relevance_formula_and_ordering(monkeypatch):
    ...
    monkeypatch.delenv("PROTAGINE_RECALL_OVERSAMPLE", raising=False)
    monkeypatch.delenv("PROTAGINE_RECALL_STRENGTH_RANKING", raising=False)
    fx = RecallFixture(
...
async def test_oversample_widens_fetch_and_trims_to_limit(monkeypatch):
    monkeypatch.setenv("PROTAGINE_RECALL_OVERSAMPLE", "3")
...
async def test_oversample_fetch_capped_at_100(monkeypatch):
    monkeypatch.setenv("PROTAGINE_RECALL_OVERSAMPLE", "50")
...
async def test_oversample_never_shrinks_below_limit(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_RECALL_OVERSAMPLE", "2")
    fx = RecallFixture(hits=[], node_props=[])
...
async def test_oversample_invalid_value_falls_back_to_legacy(monkeypatch):
    monkeypatch.setenv("PROTAGINE_RECALL_OVERSAMPLE", "banana")
    fx = RecallFixture(hits=[], node_props=[])
...
async def test_strength_ranking_on_blends_strength(monkeypatch):
    monkeypatch.setenv("PROTAGINE_RECALL_STRENGTH_RANKING", "on")
    fx = RecallFixture(
...
async def test_strength_ranking_keeps_hard_floor(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_RECALL_STRENGTH_RANKING", "on")
    monkeypatch.delenv("PROTAGINE_RECALL_MIN_STRENGTH", raising=False)
    fx = RecallFixture(
...
async def test_min_strength_env_governs_default_floor(monkeypatch):
    monkeypatch.setenv("PROTAGINE_RECALL_MIN_STRENGTH", "0.5")
    fx = RecallFixture(
