...
plus the protagine-tom2 context section, injected ONLY when the assembling
contact IS the owner, behind PROTAGINE_TOM2_CONTEXT (default 0).
...
def _tom2_sections(resp):
    return [s for s in resp.sections if s.id == "protagine-tom2"]
...
def test_flag_defaults_off(monkeypatch):
    monkeypatch.delenv("PROTAGINE_TOM2_CONTEXT", raising=False)
...
async def test_section_absent_for_every_non_owner_even_with_flag_on(
        wired, monkeypatch):
    """THE lock: no non-owner contact ever sees protagine-tom2, whatever the
