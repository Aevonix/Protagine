...
def env(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_ENV_RISK_GATEWAY_CLASS",
                       "dm:private,kiosk:embodied,web:public")
    monkeypatch.delenv("PROTAGINE_ENV_RISK_WINDOW_HOURS", raising=False)
...
def test_window_hours_default_and_malformed(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_ENV_RISK_WINDOW_HOURS", "banana")
    assert env_risk_window_hours() == 48.0
    monkeypatch.setenv("PROTAGINE_ENV_RISK_WINDOW_HOURS", "-1")
    assert env_risk_window_hours() == 48.0
