...
from protagine.intelligence.components.initiative_engine import InitiativeEngine, InitiativeType
...
async def test_backup_proposal_describes_only_the_observed_legacy_file(tmp_path,monkeypatch):
    ...
    backup=tmp_path/'.protagine/backups/old.bak';backup.parent.mkdir(parents=True);backup.write_text('unverified checkpoint')
    ...
    from protagine.initiatives.action_registry import get_action, RiskTier
...
async def test_measured_log_volume_reaches_default_proposal_gate_without_mutating_logs(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv('HOME', str(tmp_path))
    logs = tmp_path/'.protagine/logs'
    monkeypatch.setenv('PROTAGINE_LOG_PATH', str(logs/'sidecar.log'))
    ...
    assert 'protagine_read_work_source' in review['body'] and 'sources 1 through 5' in review['body']
...
async def test_inferred_proposal_needs_acceptance_before_real_followup_generation(tmp_path, monkeypatch):
    from protagine.goals.models import Goal, GoalSource, GoalStatus
