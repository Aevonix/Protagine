...
Enforce ramps one check at a time (PROTAGINE_GUARD_ENFORCE_CHECKS, default
...
async def test_allowlist_all_restores_full_enforcement(monkeypatch):
    ...
    monkeypatch.setenv("PROTAGINE_GUARD_ENFORCE_CHECKS", "all")
    guard = _guard()
...
async def test_breaker_trips_after_n_blocks(monkeypatch):
    monkeypatch.delenv("PROTAGINE_GUARD_ENFORCE_CHECKS", raising=False)
    monkeypatch.setenv("PROTAGINE_GUARD_TRIP_BLOCKS", "3")
    guard = _guard()
    for _ in range(3):
...
async def test_breaker_off_flag_keeps_enforcing(monkeypatch):
    ...
    monkeypatch.delenv("PROTAGINE_GUARD_ENFORCE_CHECKS", raising=False)
    monkeypatch.setenv("PROTAGINE_GUARD_BREAKER", "off")
...
async def test_trip_blocks_zero_disables_tripping(monkeypatch):
    monkeypatch.delenv("PROTAGINE_GUARD_ENFORCE_CHECKS", raising=False)
    monkeypatch.setenv("PROTAGINE_GUARD_TRIP_BLOCKS", "0")
    guard = _guard()
    for _ in range(5):
        r = await guard.evaluate(surface="text_chat", response_text=SECRET, target_gateway="rcs")
        assert r.decision == "revise"
