...
from protagine.tom import leveled as leveled_mod
from protagine.tom import levels as levels_mod
...
from protagine.tom.levels import clear_level_cache, set_evidence_probe
...
async def test_defaults_byte_identical_to_neutralized_block(world,
    ...
    for var in ("PROTAGINE_TOM2_LEVEL", "PROTAGINE_TOM2_MAX_LEVEL",
                "PROTAGINE_TOM2_RISK_CAPS", "PROTAGINE_TOM2_CROSS_CONTEXT",
                "PROTAGINE_TOM2_CONTEXT"):
    ...
    assert all(sid not in ("protagine-tom2-l1", "protagine-tom2-l2")
...
async def test_render_level1_not_invoked_at_level_zero(world, monkeypatch):
    monkeypatch.delenv("PROTAGINE_TOM2_LEVEL", raising=False)
    calls = []
...
async def test_render_level2_not_invoked_below_level_2(world, monkeypatch):
    monkeypatch.setenv("PROTAGINE_TOM2_LEVEL", "1")
    calls = []
    monkeypatch.setattr(leveled_mod, "render_level2",
                        lambda *a, **k: calls.append(2))
    ...
    assert [s.id for s in _leveled(resp)] == ["protagine-tom2-l1"]
...
async def test_level2_renders_with_bookkeeping(world, monkeypatch):
    ...
    assert ids == ["protagine-tom2-l1", "protagine-tom2-l2"]
    l2 = next(s for s in resp.sections if s.id == "protagine-tom2-l2")
...
async def test_budget_exhaustion_stops_further_renders(world, monkeypatch):
    ...
    assert all(s.id != "protagine-tom2-l2" for s in resp2.sections)
...
async def test_owner_h33_section_unaffected_by_leveled_flags(world,
                                                             monkeypatch):
    """H3.3 verbatim: the owner section keys ONLY off PROTAGINE_TOM2_CONTEXT,
    exactly as before the leveled wiring existed."""
    _arm_level2(monkeypatch)
    resp = await host.context_assemble(_req(OWNER, channel="dm:owner"))
    assert all(s.id != "protagine-tom2" for s in resp.sections)
    monkeypatch.setenv("PROTAGINE_TOM2_CONTEXT", "1")
    resp2 = await host.context_assemble(_req(OWNER, channel="dm:owner"))
    tom2_secs = [s for s in resp2.sections if s.id == "protagine-tom2"]
...
async def test_subject_joining_vanishes_their_inferences(world, monkeypatch):
    ...
    assert all(s.id != "protagine-tom2-l2" for s in resp.sections)


@pytest.mark.asyncio
async def test_candidate_verdict_rows_never_unlock_level_2(world, monkeypatch):
    from protagine.gate.surface_policy import POLICY_DIGEST, POLICY_ID

    monkeypatch.delenv("PROTAGINE_GUARD_ENFORCE_CHECKS", raising=False)
    monkeypatch.setenv("PROTAGINE_GUARD_TRIP_BLOCKS", "1")
