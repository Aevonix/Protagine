...
class TestSurpriseCreate:
    def test_create_basic(self, surprise_store):
        result = surprise_store.create_surprise(
            ...
            expected="User usually discusses Protagine",
            surprise_score=0.8,
        )
        assert result["observation"] == "User mentioned a new project called BlueBio"
        assert result["expected"] == "User usually discusses Protagine"
    ...
    def test_high_frequency_match(self, pattern_store):
        ...
        for _ in range(6):
            pattern_store.create_pattern(
                ...
                description="User and Protagine appear together",
                pattern_key="cooc:User:Protagine",
            )
        result = compute_surprise("User Protagine discussion", pattern_store=pattern_store)
...
class TestAccumulationConsumer:
    ...
    def _isolate(self, monkeypatch):
        ...
        monkeypatch.delenv("PROTAGINE_WORKSPACE", raising=False)
...
class TestPatternsScheduleDefault:
    def test_default_is_off(self, monkeypatch):
        """Regression-lock: without PROTAGINE_PATTERNS_SCHEDULE=on the daily
        pattern_extract task is not registered (server gate condition)."""
        monkeypatch.delenv("PROTAGINE_PATTERNS_SCHEDULE", raising=False)
        assert os.environ.get(
            "PROTAGINE_PATTERNS_SCHEDULE", "off").strip().lower() != "on"
