# Contributing to Protagine

Develop against the current Protagine baseline and its declared Hermes target.

## Development setup

Protagine is Python-only.

```bash
git clone https://github.com/Aevonix/Protagine.git
cd Protagine/sidecar
pip install -e ".[dev]"
```
...
| `sidecar/protagine/intelligence/` | Mind model, cognition components, synthesis, owner corrections |
...
```
feat(autonomy): PROTAGINE_AUTONOMY_PRESET - one knob for the agency posture
